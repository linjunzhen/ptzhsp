	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZSGTSCYJHZH");
    		}
    	}
		//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZSGTSCYJHZH", function(form, valid) {
			if (valid) {
				getOpinion();
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZSGTSCYJHZH");
				var url = $("#SHTZSGTSCYJHZH").attr("action");
...
<body>
	<form id="SHTZSGTSCYJHZH" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				...
				<div class="eui-word-title">
					关于<input type="text" class="eui-ipt" data-validation-engine="validate[required]" readonly="readonly" style="width:273px"  name="VB_GC_NAME_ST" maxlength="200" value="${SQData.blxmmc}" />项目《一阶段施工图》
					（送审版）审查意见回执函
				</div>
