;(function($){
	var methods = {
		...
		pageData: function(_v, _current){
			...
			var methodNum=$("input[name='methodNum']").val();
			var settotalNum='settotal'+methodNum+'()';
			eval(settotalNum);
			var vtotal=$("input[name='totalData']").val();
			var t = vtotal, p = opts.pageData, ajaxOpts = null;
		...
		pageRender: function(_v, _current){
			// 页码操作
			var methodNum=$("input[name='methodNum']").val();
			var currentPageNum='currentPage'+methodNum+'('+_current+')';
			eval(currentPageNum);
