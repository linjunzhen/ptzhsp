...
<script type="text/javascript">
	...
	function previewFile(fileId,index){
		if('${exeId}'){	
			...
			+"&exeId=${exeId}&itemCode=${itemCode}"+"&index="+index, "_blank", "menubar=yes, status=yes, location=yes, scrollbars=yes, resizable=yes, alwaysRaised=yes, fullscreen=yes");
		}else{
			art.dialog({
				content : "无法下载",
				icon : "error",
...
<body style="margin:0px;background-color: #f7f7f7;" class="easyui-layout" fit="true">
	<form id="GdzrForm" method="post"
		...
		<div id="GdzrFormDiv" data-options="region:'center',split:true,border:false" >
			...
			<input type="hidden" name="stockFromJson" value="${stockFromJson}" />
