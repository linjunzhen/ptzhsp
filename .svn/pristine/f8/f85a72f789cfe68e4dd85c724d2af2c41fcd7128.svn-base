...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/general/js/bdcgeneral.js"></script>
...
$(function(){
    ...
    if(flowSubmitObj){
        ...
        $("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
         //初始化表单字段权限控制
        FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_GENERAL_FORM");
        
        //初始化权利人信息和义务人信息
		var sqfs = $("input[name='SQFS']").val();
		if(sqfs=="2"){//窗口收件
		   $("#qlrDiv").attr("style","display:none");
		   $("#ywrDiv").attr("style","display:none");
		   //受理信息隐藏
		   $("#slxx_div").attr("style","display:none");
		}else if(sqfs=="1"){//前台申报
		   $("#qlrDiv").attr("style","");
		   $("#ywrDiv").attr("style","");
		}
		...
		//证件类型初始化(权利人信息，义务人信息)
		$("select[name='POWERPEOPLEIDTYPE']").on("click", function(event) {
			setZjValidate(this.value,'POWERPEOPLEIDNUM');
		});
		
		$("select[name='POWLEGALIDTYPE']").on("click", function(event) {
			setZjValidate(this.value,'POWLEGALIDNUM');
		});	
		
		$("select[name='POWAGENTIDTYPE']").on("click", function(event) {
			setZjValidate(this.value,'POWAGENTIDNUM')
		});	
		
		$("select[name='YWPEOPLEIDTYPE']").on("click", function(event) {
			setZjValidate(this.value,'YWPEOPLEIDNUM');
		});
		
		$("select[name='YWFZLEGALIDTYPE']").on("click", function(event) {
			setZjValidate(this.value,'YWFZLEGALIDNUM');
		});	
		
		$("select[name='YWDLAGENTIDTYPE']").on("click", function(event) {
			setZjValidate(this.value,'YWDLAGENTIDNUM');
		});	
        ...
        if(flowSubmitObj.busRecord){
            FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDCQLC_GENERAL_FORM");
            //表单内打印按钮是否可以操作
			isButtonAvailable();
			//动态切换抵押信息
			queryTypeFn();
			...
			if (flowSubmitObj.busRecord.RUN_STATUS) {
				if (flowSubmitObj.busRecord.RUN_STATUS != 0) {
					...
					if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME != '开始' ) {
						$("#taxRelatedInfo").hide();
						//非开始环节，不可编辑
						$("#T_BDCQLC_GENERAL_FORM").find("input[type='text']").attr("disabled","disabled");
						$("#T_BDCQLC_GENERAL_FORM").find("input[type='button']").attr("disabled","disabled");
						$("#T_BDCQLC_GENERAL_FORM").find("input[type='checkbox']").attr("disabled","disabled");
						$("#T_BDCQLC_GENERAL_FORM").find("input[type='radio']").attr("disabled","disabled");
						$("#T_BDCQLC_GENERAL_FORM").find("input,select").attr("disabled","disabled");
					}
					...
					if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '收费' || flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '登簿') {
						 if (flowSubmitObj.busRecord.ZYDJ_TYPE == "4") { 
						    if(flowSubmitObj.busRecord.PTT_SQFS=="2" || flowSubmitObj.busRecord.ISQCWB == "0"){
						    	$("#sssb_sfxx").show();//收费环节-增加收费信息（全程网办的办件）
								$("#sssb_sfxx input").each(function (data) {
									$(this).attr("disabled", false);
								});
						    }
						}
				...
				/* 	if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '收费' || flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '办结') {
					...
					if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME != '开始') {
						...
						if (flowSubmitObj.HJMC != '开始') {
							...
							$("#svaTr input").attr("disabled", true);
						}
            ...
            if(BDC_OPTYPE != null && BDC_OPTYPE !=""){
                $("#T_BDCQLC_GENERAL_FORM input").each(function(index){
                    $(this).attr("disabled","disabled");
                });
                $("#T_BDCQLC_GENERAL_FORM select").each(function(index){
                    $(this).attr("disabled","disabled");
                });
                //填写权证标识码
                if(BDC_OPTYPE == "1"){
	        ...
	        $.post("executionController.do?getResultFiles&fileIds="+fileid,{fileIds:fileid}, function(resultJson) {
	            if(resultJson!="websessiontimeout"){
	                ...
	                for(var i=0; i<list.length; i++){
	                    ...
	                    newhtml+='<a style="color: blue;" href="javascript:AppUtil.downLoadFile(\''+list[i].FILE_ID+'\');">';
	                    newhtml+=list[i].FILE_NAME+'</a>';
	                    newhtml += "<a href=\"javascript:void(0);\"  onclick=\"delUploadFile2('"+list[i].FILE_ID+"');\" ><font color=\"red\">删除</font></a>";
	                    newhtml+='</p>';
...
<body>
	...
	<form id="T_BDCQLC_GENERAL_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%-- 义务人信息明细 --%>
		<input type="hidden" name="YWPEOPLEINFO_JSON" id="YWPEOPLEINFO_JSON" value="${busRecord.YWPEOPLEINFO_JSON}"/>
		<%-- 权属来源信息明细 --%>
		...
		<%--是否全程网办接口 0全程网办 1智能审批--%>
	    <input type="hidden" name="ISQCWB" id="ISQCWB" value="${busRecord.ISQCWB}"/>
		...
		<%--结束引入公用申报对象 --%>
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/general/T_BDCQLC_GENERALACCEPT.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始引入权利人信息--%>
		<div id="qlrDiv" style="display:none">
			<jsp:include page="./bdcqlc/general/bdcGeneralQlrxx.jsp" />
		</div>
		<%--结束引入权利人信息--%>
		
		<%--开始引入义务人信息 --%>
		<div id="ywrDiv" style="display:none">
			<jsp:include page="./bdcqlc/general/bdcGeneralYwrxx.jsp" />
		</div>
		<%--结束引入义务人信息 --%>
		
		<%--开始引入权属来源信息--%>
	<%-- 	<jsp:include page="./bdcqlc/gyjsjfwzydj/clfsflb/bdcQslyType1.jsp" /> --%>
		<%--结束引入权属来源信息--%>
		
		<%--开始引入权利限制信息--%>
		<%-- <jsp:include page="./bdcqlc/gyjsjfwzydj/clfsflb/bdcQlxz.jsp" /> --%>
		<%--结束引入权利限制信息--%>
		...
		<%--开始引入房屋基本信息--%>
		<%-- <jsp:include page="./bdcqlc/general/bdcFwjbxxGeneral.jsp" /> --%>
		<%--开始引入房屋基本信息--%>
		
		<%--开始引入宗地信息--%>
      <%--    <jsp:include page="./bdcqlc/bdcZdxxTy.jsp" /> --%>
         <%--结束引入宗地信息--%>
        ...
        <%--开始引入家庭唯一生活用房承诺书信息--%>
        <div id="wyyfcns" style="display:none;">	
        ...
        <%--开始引入缴费-收费信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/clfsflb/sssbSfxx.jsp" />
		<%--结束引入缴费-收费信息--%>
		
		<%--开始引入登记审核信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/clfsflb/bdcDjshxx.jsp" />
		<%--开始引入登记审核信息--%>
