...
<script type="text/javascript">
	...
	function restartFlowTaskExplain(){
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			if(!isAllowStart){
				...
				AppUtil.operateDataGridRecord("flowTaskController.do?reStart",{selectTaskIds:selectTaskIds},"ZzhyDwysGrid");
			}else{
	 ...
	 * 编辑流程实例列表记录
	 */
	function editZzhyDwysGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("ZzhyDwysGrid");
		if (entityId) {
	...
	$(document).ready(function() {
		var start1 = {
			elem : "#ZzhyDwysT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var end1 = {
			elem : "#ZzhyDwysT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
	...
	function showBjxxGridRecord() {
		var rows = $("#ZzhyDwysGrid").datagrid("getChecked");
        if(rows.length==0){
            art.dialog({
                content: "请至少选择一条记录!",
                icon:"warning",
...
<%--                                onclick="restartFlowTask();">重启流程任务</a>--%>
				...
				<table class="dddl-contentBorder dddl_table"
					...
					<tbody>
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;padding-left:4px;"><input type="text"
								...
								id="ZzhyDwysT.CREATE_TIME_BEGIN" name="Q_E.CREATE_TIME_>=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;padding-left:4px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="ZzhyDwysT.CREATE_TIME_END" name="Q_E.CREATE_TIME_<=" /></td>
					...
					url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=YesOrNo',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 74,panelHeight: 'auto' " />
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('ZzhyDwysToolbar','ZzhyDwysGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="ZzhyDwysGrid" fitcolumns="false" toolbar="#ZzhyDwysToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="flowTaskController.do?zzhyNeedMeHandle&Q_E.APPLY_STATUS_EQ=1&isHaveHandup=true">
			<thead>
				<tr>
