...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteTzxmNeedMeHandleGridRecord() {
		AppUtil.deleteDataGridRecord("executionController.do?multiDel",
				"TzxmNeedMeHandleGrid");
	};
	function restartFlowTaskExplain(){
		var $dataGrid = $("#TzxmNeedMeHandleGrid");
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			if(!isAllowStart){
				...
				AppUtil.operateDataGridRecord("flowTaskController.do?reStart",{selectTaskIds:selectTaskIds},"TzxmNeedMeHandleGrid");
			}else{
	 ...
	 * 编辑流程实例列表记录
	 */
	function editTzxmNeedMeHandleGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("TzxmNeedMeHandleGrid");
		if (entityId) {
	...
	function toPostUrl(url){
		...
		$("#TzxmNeedMeHandleToolbar").append(ssoForm);
		ssoForm.submit();		
	}
	function sqcb(exeId,taskId){
		$.dialog.open("executionController.do?goHandle&exeId="+exeId+"&taskId="+taskId, {
			...
			resize : false,
			close: function(){
				AppUtil.gridDoSearch('TzxmNeedMeHandleToolbar','TzxmNeedMeHandleGrid');
			}
		}, false);
	}
	...
	function showBjxxGridRecord() {
		var rows = $("#TzxmNeedMeHandleGrid").datagrid("getChecked");
        if(rows.length==0){
            art.dialog({
                content: "请至少选择一条记录!",
                icon:"warning",
...
<div class="easyui-layout eui-jh-box" fit="true">
	<div region="center">
		<!-- =========================查询面板开始========================= -->
		<div id="TzxmNeedMeHandleToolbar">
			<!--====================开始编写隐藏域============== -->
			<!--====================结束编写隐藏域============== -->
			<div class="right-con">
...
<%--                                onclick="restartFlowTask();">重启流程任务</a>--%>
					...
					<tbody>
						...
						<tr style="height:28px;">
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('TzxmNeedMeHandleToolbar','TzxmNeedMeHandleGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="false" striped="true"
			id="TzxmNeedMeHandleGrid" fitcolumns="false" toolbar="#TzxmNeedMeHandleToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="flowTaskController.do?tzxmNeedMeHandle&isHaveHandup=true">
			<thead>
				<tr>
					...
					<th data-options="field:'SQRMC',align:'left'" width="7%">申请人</th>
					<th data-options="field:'JBR_NAME',align:'left'" width="7%">经办人</th>
