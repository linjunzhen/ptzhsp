...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("BusRuleAddForm", function(form, valid) {
			if (valid) {
				//将提交按钮禁用,防止重复提交
				$("input[type='submit']").attr("disabled", "disabled");
				var formData = $("#BusRuleAddForm").serialize();
				var url = $("#BusRuleAddForm").attr("action");
				AppUtil.ajaxProgress({
					...
					callback : function(resultJson) {
						if (resultJson.success) {
							...
							window.open("busRuleController.do?toAddRule&entityId=" + busCode,"addRuleWin");
							AppUtil.closeLayer();
						} else {
							parent.art.dialog({
								content: resultJson.msg,
	...
	function toAddRuleWindow(){
		var unitCode = $("#BusRuleAddForm input[name='BUSSYS_UNITCODE']").val();
		var platCode = $("#BusRuleAddForm input[name='BUSSYS_PLATCODE']").val();
		var busCode = $("#BusRuleAddForm input[name='BUS_CODE']").val();
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="BusRuleAddForm" method="post"
