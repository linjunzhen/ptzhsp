...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/zjdsyqscdj/js/zjdsyqscdj.js"></script>
...
<script type="text/javascript">
	$(function() {
		...
		<%-- 登记缴费信息、发证、归档信息隐藏 --%>
	    $("#djjfxx_zjdsyqscdj").attr("style","display:none;");
    	$("#djfzxx_zjdsyqscdj").attr("style","display:none;");
    	$("#djgdxx_zjdsyqscdj").attr("style","display:none;");
		/*权力信息标题*/
		$("#qlxxTitle").text("宅基地使用权-权利信息");
		...
		if (flowSubmitObj) {
			...
			//初始化表单字段权限控制
			FlowUtil.initFormFieldRightControl(currentNodeFieldRights, "T_BDCQLC_ZJSYSCDJ_FORM");			
			if (flowSubmitObj.busRecord) {
				//初始化表单字段值
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord, "T_BDCQLC_ZJSYSCDJ_FORM");
				...
				if( flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME != '开始') {
					...
					$("#dfspb").attr("onclick","goPrintTemplate('ZJDSYQSCDJJSPB','3')");
					$("#sfspb").attr("onclick","goPrintTemplate('ZJDSYQSCDJJSPB','3')");
					//开始、受理环节之后数据均不可编辑
					if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME!="受理" || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
						$("#T_BDCQLC_ZJSYSCDJ_FORM").find("input,select,.Wdate").attr("disabled","disabled");
						//a标签按钮禁用
				...
				if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '初审') {
					...
					$("#powerSourceInfo input , #powerSourceInfo select").each(function () {
						$(this).attr("disabled", false);
					})
					$("#zdjbxx input , #zdjbxx select").each(function () {
						$(this).attr("disabled", false);
					})
					$("#gyqlxx input , #gyqlxx select").each(function () {
						$(this).attr("disabled", false);
					})
	                ...
	                if(BDC_OPTYPE == "1"){//缮证（权证打印）                    
	                    ...
	                    $("#qzbsmsavebtn").removeAttr("disabled");
	                  	...
	                  	$("#BDC_QZVIEW").attr("style","");
	                   	$("#BDC_QZVIEW").removeAttr("disabled");
	                    //登记缴费、发证、归档信息
	                    $("#djjfxx_zjdsyqscdj").attr("style","");
    					$("#djfzxx_zjdsyqscdj").attr("style","");
    					$("#djgdxx_zjdsyqscdj").attr("style","");	                   	    					
    					$("#djjfxx_zjdsyqscdj").find("input,select,.Wdate").removeAttr("disabled");
    					$("#djfzxx_zjdsyqscdj").find("input,select,.Wdate").removeAttr("disabled");
    					$("#djgdxx_zjdsyqscdj").find("input,select,.Wdate").removeAttr("disabled");	                  
...
<body>
	...
	<form id="T_BDCQLC_ZJSYSCDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/zjdsyqscdj/T_ESTATE_SCDJ_ACCEPTINFO.jsp" /> 
		<%--结束引入受理信息--%>
		...
		<%--开始引入权利人信息--%>
		<jsp:include page="./bdcqlc/zjdsyqscdj/bdcQlrxx.jsp" />
		<%--结束引入权利人信息--%>
		
		<%--开始引入权属来源信息--%>
		<jsp:include page="./bdcqlc/zjdsyqscdj/bdcQsly.jsp" />
		<%--结束引入权属来源信息--%>
	    ...
	    <jsp:include page="./bdcqlc/common/djauditinfo.jsp">
	        <jsp:param value="zjdsyqscdj" name="domId" />
