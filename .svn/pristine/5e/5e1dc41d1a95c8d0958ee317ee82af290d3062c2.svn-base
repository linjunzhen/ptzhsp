...
<form action="" id="FILE_FORM"  >
	<div class="syj-title1 ">
		<span>前置审批及其他文件
		<c:if test="${requestParams.ISNEEDSIGN == 1}">
			<span style="color: red;">财务负责人，当不在面签人员里面时，要上传身份证正反面</span>
		</c:if>
		</span>
	...
	<div class="syj-title1 ">
		<span>冠福建省名或无行政区划名称材料
		</span>
	</div>
	<table cellpadding="0" cellspacing="0" class="syj-table2 ">
		<input  name="FILE_ID2"  type="hidden"
				value="${busRecord.FILE_ID2 }">
		<tr>
			<th>附件：</th>
			<td colspan="3">
				<div style="width:100%;display: none;" id="UPDATE_FILE_DIV2"></div>
				<a href="javascript:void(0);" onclick="openResultFileUploadDialog2()">
					<img id="UPDATE_FILE_BTN2" src="webpage/bsdt/applyform/images/tab_btn_sc.png" />
				</a>
