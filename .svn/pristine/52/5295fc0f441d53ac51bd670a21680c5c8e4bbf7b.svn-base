...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/ybqlc/zgjbylbxtb/js/ylbxtb.js"></script>
	...
	$(function(){
		...
		if(flowSubmitObj){
			...
			$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
			 //初始化表单字段权限控制
			//FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_YBQLC_ZGTB_FORM");
			if(flowSubmitObj.busRecord){
				//初始化表单字段值
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_YBQLC_ZGTB_FORM");
				...
				if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES =='审查' || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
				 	$("#T_YBQLC_ZGTB_FORM").find("input,select").attr("disabled","disabled");
				}
				if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES !="开始" || flowSubmitObj.EFLOW_EXERUNSTATUS=='2'){//除开始环节外均需开放受理信息
	    			$("#zgtbinfo").attr("style","");
...
<body>
	...
	<form id="T_YBQLC_ZGTB_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--职工停保对象--%>
		<div id="zgtbinfo" style="display:none">
			<jsp:include page="./ybqlc/zgjbylbxtb/zgtbinfo.jsp" >
