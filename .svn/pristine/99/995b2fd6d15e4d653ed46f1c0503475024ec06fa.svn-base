...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/gyjsjfwzydj/clfsflb/js/clfsflb.js"></script>
...
$(function(){
    ...
    if(flowSubmitObj){
        ...
        $("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
         //初始化表单字段权限控制
        FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM");
        //初始化表单字段值
        if(flowSubmitObj.busRecord){
			...
			//初始化权利人信息的套次承诺书与权属来源中的提示
			var sqfs = $("input[name='SQFS']").val();
			if(sqfs=="1"){//前台申报
			   $("#tccns").attr("style","");
			   $("#ts").attr("style","");
			}else{
			   $("#tccns").attr("style","display:none");
			...
			//平潭通办件，承诺栏信息（买卖双方）
			if(flowSubmitObj.busRecord.PTT_SQFS=="2"){
			    var buyCnJson = flowSubmitObj.busRecord.BUYCN_JSON;
		    	if(buyCnJson !="" && buyCnJson !=null && buyCnJson !="[]"){
		    		//初始化买方承诺信息JSON
		    		$("#buyCnXx").attr("style","");
			        var buyCnItems = JSON.parse(buyCnJson);
			        initBuyCnInfo(buyCnItems);
		    	}
		    	var sellCnJson = flowSubmitObj.busRecord.SELLCN_JSON;
		    	if(sellCnJson !="" && sellCnJson !=null && sellCnJson !="[]"){
		    		//初始化卖方承诺信息JSON
		    		$("#sellCnXx").attr("style","");
			        var sellCnItems = JSON.parse(sellCnJson);
			        initSellCnInfo(sellCnItems);
		    	}
		    }
			...
			if (flowSubmitObj.busRecord.RUN_STATUS) {
				if (flowSubmitObj.busRecord.RUN_STATUS != 0) {
					...
					if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '网上预审' || flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '待受理') {
						$("#taxRelatedInfo").hide();
						//去掉表单必填验证
					    $("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM").find("input[type='text']").attr("disabled","disabled");
						$("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM").find("input[type='button']").attr("disabled","disabled");
						$("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM").find("input[type='checkbox']").attr("disabled","disabled");
						$("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM").find("input[type='radio']").attr("disabled","disabled");
						$("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM").find("input,select").attr("disabled","disabled"); 
						$("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM").find(".syj-closebtn").removeAttr("onclick"); 
					}
            ...
            if(BDC_OPTYPE != null && BDC_OPTYPE !=""){
                $("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM input").each(function(index){
                    $(this).attr("disabled","disabled");
                });
                $("#T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM select").each(function(index){
                    $(this).attr("disabled","disabled");
                });
                //填写权证标识码
                if(BDC_OPTYPE == "1"){
...
<body>
	...
	<form id="T_BDCQLC_GYJSJFWZYDJCLFSFLB_FORM" method="post">
        ...
        <%--开始引入平潭通买方承诺信息--%>
        <div id="buyCnXx" style="display:none" >	
        	<jsp:include page="./bdcqlc/gyjsjfwzydj/clfsflb/pttBuyCnInfo.jsp" /> 
        </div>
        <%--结束引入平潭通买方承诺信息--%>
        
        <%--开始引入平潭通卖方承诺信息--%>
        <div id="sellCnXx" style="display:none" >	
        	<jsp:include page="./bdcqlc/gyjsjfwzydj/clfsflb/pttSellCnInfo.jsp" /> 
        </div> 
        <%--结束引入平潭通卖方承诺信息--%>
