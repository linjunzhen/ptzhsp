...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/ybqlc/lhjycbdj/js/lhjycbdj.js"></script>
		...
		$(function(){    
			...
			if(flowSubmitObj){
				...
				$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
				 //初始化表单字段权限控制
				//FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_YBQLC_LHJYCBDJ_FORM");
				if(flowSubmitObj.busRecord){
					//初始化表单字段值
					FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_YBQLC_LHJYCBDJ_FORM");
					
					if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES =='审查' || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
					 	$("#T_YBQLC_LHJYCBDJ_FORM").find("input,select").attr("disabled","disabled");
					 }
		    		...
		    		if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES != "受理"){
		    			$('#cbxx').find(":radio,:checkbox,input,select").attr('disabled',"disabled");
						...
						xzConfGrid3.editor={};
						var xzConfGrid4 = $('#xzxxGrid').datagrid('getColumnOption', 'aac031');
						xzConfGrid4.editor={};
		    		}
...
<body>
	...
	<form id="T_YBQLC_LHJYCBDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入参保信息--%>
		<div id="cbxx" style="display:none">
			<jsp:include page="./ybqlc/lhjycbdj/cbxx.jsp">			
