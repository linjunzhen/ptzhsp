...
<script type="text/javascript">
	$(function(){
    	...
    	if(flowSubmitObj){
    		...
    		if(flowSubmitObj.busRecord){
    			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BSFW_XMBAQSLHSC_FORM");
    			
    			//获取表单字段权限控制信息
        		var currentNodeFieldRights = flowSubmitObj.currentNodeFieldRights;
        		 //初始化表单字段权限控制
        		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BSFW_XMBAQSLHSC_FORM");
    		}
    		if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME){
				...
				} else if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "拟批复《项目备案请示+》"
    			...
    			if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "填写公示材料"
						...
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "拟批复《项目备案请示+》"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区行政审批局项目投资处领导审批"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区审批局领导审签"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "批复《项目备案请示+》"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "结束") {
		...
		//	busTableName : "T_BSFW_XMBAQSLHSC"
		//});
	...
	function FLOW_SubmitFun(flowSubmitObj) {
		//先判断表单是否验证通过
		var validateResult = $("#T_BSFW_XMBAQSLHSC_FORM").validationEngine(
				"validate");
		...
		if (validateResult) {
			...
			if (submitMaterFileJson || submitMaterFileJson == "") {
				...
				var formData = FlowUtil
						.getFormEleData("T_BSFW_XMBAQSLHSC_FORM");
...
<body>
   ...
   <form id="T_BSFW_XMBAQSLHSC_FORM" method="post" >
    <%--===================重要的隐藏域内容=========== --%>
	...
	<table cellpadding="0" cellspacing="1" class="tab_tk_pro">
		...
		<tr>			
             <td colspan="4"><font size="3">　　《项目备案请示+》（编修版）已经于</font> <input type="text" name="ZJZHSCSJ" class="laydate-icon validate[required]"
	...
	<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="GS_TABLE" style="display: none;">
		...
		<tr>
			...
			<td colspan="3"><textarea rows="10" cols="140" name="JSGMJZYNR" maxlegth="1998" id="jsgm"></textarea> </td>
		</tr>
		<tr>
