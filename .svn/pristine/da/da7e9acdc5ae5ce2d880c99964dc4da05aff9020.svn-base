...
<script type="text/javascript">
	...
	function restartFlowTaskExplain(){
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			if(!isAllowStart){
				...
				AppUtil.operateDataGridRecord("flowTaskController.do?reStart",{selectTaskIds:selectTaskIds},"BdcDwysGrid");
			}else{
	 ...
	 * 编辑流程实例列表记录
	 */
	function editBdcDwysGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("BdcDwysGrid");
		if (entityId) {
	...
	$(document).ready(function() {
		var start1 = {
			elem : "#BdcDwysT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
	...
	function showBdcDwysBjxxGridRecord() {
		var rows = $("#BdcDwysGrid").datagrid("getChecked");
        if(rows.length==0){
            art.dialog({
                content: "请至少选择一条记录!",
                icon:"warning",
						...
						<div class="e-toolbar-overflow">
						    <div style="color:#005588;">
								<img src="plug-in/easyui-1.4/themes/icons/script.png"
									style="position: relative; top:7px; float:left;" />&nbsp;待预审列表
								<a href="#"
                                class="easyui-linkbutton" reskey="SHOW_Bjxx"
                                iconcls="eicon-eye" plain="true"
                                onclick="showBdcDwysBjxxGridRecord();">查看补件要求</a>
<%--                                <a href="#"--%>
...
<%--                                onclick="restartFlowTask();">重启流程任务</a>--%>
					...
					<tbody>
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;padding-left:4px;"><input type="text"
								...
								id="BdcDwysT.CREATE_TIME_BEGIN" name="Q_E.CREATE_TIME_>=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;padding-left:4px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="BdcDwysT.CREATE_TIME_END" name="Q_E.CREATE_TIME_<=" /></td>
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('BdcDwysToolbar','BdcDwysGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="BdcDwysGrid" fitcolumns="false" toolbar="#BdcDwysToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="bdcExecutionController.do?bdcNeedMeHandle&Q_T.TASK_NODENAME_LIKE=%E9%A2%84%E5%AE%A1&Q_E.APPLY_STATUS_EQ=1&isHaveHandup=true">
			<thead>
				<tr>
