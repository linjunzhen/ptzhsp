...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("tzxmDataRefreshWin", function(form, valid) {
			if (valid) {
				var formData = $("#tzxmDataRefreshWin").serialize();
				var url = $("#tzxmDataRefreshWin").attr("action");
				AppUtil.ajaxProgress({
					...
					callback : function(resultJson) {
						if (resultJson.success) {
							...
							parent.$("#tzxmbjmxStatisGrid").datagrid("reload");
							AppUtil.closeLayer();
						} else {
							parent.art.dialog({
								content: resultJson.msg,
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="tzxmDataRefreshWin" method="post"
		action="statisticsController.do?refreshTzxmBjxl">
		<div id="tzxmDataRefreshWinDiv">
			<!--==========隐藏域部分结束 ===========-->
