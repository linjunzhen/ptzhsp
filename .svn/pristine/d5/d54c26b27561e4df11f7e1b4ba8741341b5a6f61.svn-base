...
<script type="text/javascript">
    ...
    var  exportDate;//导出数据默认初始化时间
	initTime();

	function initTime() {
		var bjtjbDateYear = new Date().getFullYear() - 1;
		var bjtjbDateMonth = new Date().getMonth() + 1;
		var bjtjbDateMonth = bjtjbDateMonth.toString() < 10 ? '0' + bjtjbDateMonth : '' + bjtjbDateMonth;
		var bjtjbDateDate = new Date().getDate();
		var bjtjbDateDate = bjtjbDateDate.toString() < 10 ? '0' + bjtjbDateDate : '' + bjtjbDateDate;
		var bjtjbDate = bjtjbDateYear + '-' + bjtjbDateMonth + '-' + bjtjbDateDate;
		exportDate = bjtjbDate;
		$("#bjtjbStatisGrid").attr("url", "statisticsController.do?bjtjbData&Q_T.STATIST_DATE_GE=" + bjtjbDate);
		$("input[name='Q_T.STATIST_DATE_>=']").val(bjtjbDate)
	}
    ...
    function exportExcel(){
    	...
    	+"&Q_T.STATIST_DATE_GE="+exportDate;
    }
	...
	$('#bjtjbStatisGrid').datagrid({ pagination: false,
		onLoadSuccess: function (data) {
			if (data.rows.length > 0) {
				//调用mergeCellsByField()合并单元格
				mergeCellsByField("bjtjbStatisGrid", "PARENT_NAME");
			}
		}
	});		
	/**
...
<div class="easyui-layout eui-jh-box" fit="true">
	<div region="center">
				...
				<div class="e-toolbar" style="z-index: 1111;top: 0px;left: 0px;">
					<div class="e-toolbar-ct">
						<div class="e-toolbar-overflow">
					         ...
					         onclick="showDataRefresh();">数据同步</a> 
					...
					<tbody>
						<tr style="height:28px;">
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('bjtjbStatisToolBar','bjtjbStatisGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="false" striped="true"
			id="bjtjbStatisGrid" fitcolumns="false" toolbar="#bjtjbStatisToolBar"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'YDJ',align:'center'" width="10%" rowspan="2">预登记</th>
					<th data-options="field:'YDJ_NEW',align:'center'" width="10%" rowspan="2">预登记(新)</th>
					<th colspan="4">办件渠道统计</th>
					<th colspan="4">收件(A)</th>
					<th colspan="10">办结(B)</th>
				...
				<tr>
					<th data-options="field:'WSSQ_XJ',align:'center'" width="10%" >网上申请</th>
					<th data-options="field:'CKSJ_XJ',align:'center'" width="10%" >窗口收件</th>
					<th data-options="field:'SWFQ_XJ',align:'center'" width="10%" >省网办发起</th>
					<th data-options="field:'SSLD_XJ',align:'center'" width="10%" >省市联动</th>
					...
					<th data-options="field:'BJ_JBJAS',align:'center'" width="10%" >即办件(按时)</th>
					<th data-options="field:'BJ_JBJYQ',align:'center'" width="10%" >即办件(逾期)</th>
