...
<script type="text/javascript">
	$(function() {
		$('#RuleTime_TacheA').combobox('disable');
		$('#RuleTime_ProcessA').combobox('disable');
		
		AppUtil.initWindowForm("BusRuleTimeForm", function(form, valid) {
			if (valid) {
				var platCode = $("#ruleTime_platCode").combobox('getValue');
				...
				var formData = $("#BusRuleTimeForm").serialize();
				var url = $("#BusRuleTimeForm").attr("action");
	...
	function getRuleTime(){
		var $temp_1 = $("#RuleTime_ProcessA").combobox('getValue');
		var $temp_2 = $("#RuleTime_ProcessB").combobox('getValue');
		var $temp_3 = $("#RuleTime_JudgeA").combobox('getValue');
		var $temp_4 = $("#RuleTime_JudgeB").combobox('getText');
		var $temp_5 = $("#RuleTime_TimeValue").val();
		var $temp_1t = $("#RuleTime_ProcessA").combobox('getText');
		var $temp_2t = $("#RuleTime_ProcessB").combobox('getText');
		...
		$("#BusRuleTimeForm input[name='JUDGE_CONDITIONS']").val(value);
		$("#BusRuleTimeForm textarea[name='JUDGE_DESC']").val(texts);
	}
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="BusRuleTimeForm" method="post"
		...
		<table style="width:100%;border-collapse:collapse;"
			...
			<tr>
				...
				<td width="*"><span style="width: 90px;float:left;text-align:right;">所属系统：</span>
					<input class="easyui-combobox" style="width:150px;" name="BUSSYS_PLATCODE" id="ruleTime_platCode" value="${busRule.BUSSYS_PLATCODE}" 
			...
			<tr>
				<td colspan="4"><span style="width: 90px;float:left;text-align:right;">开始时间：</span>
					业务环节-
					<input class="easyui-combobox validate[required]" style="width:150px;" name="RuleTime_TacheB"
						data-options="url:'busRuleController.do?loadTache&defaultEmpty=true&amp;busCode=${busRule.BUS_CODE }',editable:false,
						method: 'post',valueField:'TACHE_CODE',textField:'TACHE_NAME',panelWidth: 150,panelHeight: 'auto', 
						onSelect:function(rec){
							   if(rec.TACHE_CODE){
							   	  $('#RuleTime_ProcessB').combobox('clear');
	        			          var url2 = 'busRuleController.do?loadProcess&defaultEmpty=true&tacheCode='+rec.TACHE_CODE;
	    			    		  $('#RuleTime_ProcessB').combobox('reload',url2); 						   	  
							   }						   
							}" />
					&nbsp;&nbsp;过程编码-
					<input class="easyui-combobox" style="width:230px;" name="RuleTime_ProcessB" id="RuleTime_ProcessB"
						data-options="url:'busRuleController.do?loadProcess&defaultEmpty=true',editable:false,
			...
			<tr>
				<td colspan="4"><span style="width: 90px;float:left;text-align:right;">结束时间：</span>
					业务环节-
					<input class="easyui-combobox validate[required]" style="width:150px;" name="RuleTime_TacheA" id="RuleTime_TacheA" value="${busRule.TACHE_CODE}"
						data-options="url:'busRuleController.do?loadTache&defaultEmpty=true&amp;busCode=${busRule.BUS_CODE }',editable:false,
						method: 'post',valueField:'TACHE_CODE',textField:'TACHE_NAME',panelWidth: 150,panelHeight: 'auto',
						onSelect:function(rec){
						   if(rec.TACHE_CODE){
						   	  $('#RuleTime_ProcessA').combobox('clear');
        			          var url1 = 'busRuleController.do?loadProcess&defaultEmpty=true&tacheCode='+rec.TACHE_CODE;
    			    		  $('#RuleTime_ProcessA').combobox('reload',url1); 						   	  
						   }						   
						}" />
					&nbsp;&nbsp;过程编码-
					<input class="easyui-combobox validate[required]" style="width:230px;" name="RuleTime_ProcessA" id="RuleTime_ProcessA"
						value="${busRule.PROCESS_CODE}" data-options="url:'busRuleController.do?loadProcess&defaultEmpty=true&tacheCode=${busRule.TACHE_CODE}',editable:false,
						method: 'post',valueField:'PROCESS_CODE',textField:'PROCESS_NAME',panelWidth: 230,panelHeight: 'auto' " />
				</td>
			</tr>
			<tr>
				<td colspan="4"><span style="width: 90px;float:left;text-align:right;">规则判断符：</span>
					<input class="easyui-combobox" style="width:70px;" name="RuleTime_JudgeA" id="RuleTime_JudgeA"
					...
					固定值：<input class="easyui-numberbox" style="width:55px;" id="RuleTime_TimeValue" maxlength="3"/>
					单位：<input class="easyui-combobox" style="width:70px;" name="RuleTime_JudgeB" id="RuleTime_JudgeB"  
