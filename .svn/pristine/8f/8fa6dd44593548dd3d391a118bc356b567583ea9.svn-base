...
//***************************卓正PageOffice组件的使用********************************
	...
	if(map.get("READ_DOC")!=null&& !map.get("READ_DOC").equals("")){
	    poCtrl1.setSaveFilePage(request.getContextPath() + "/ticketsController/savefile.do?filename="+map.get("READ_DOC")+"&type=readtemplate");
	}else{
	poCtrl1.setSaveFilePage(request.getContextPath() + "/ticketsController/savefile.do?alias="+map.get("ALIAS")+"&type=readtemplate");
	}
   ...
   if(filepath!=null&&!filepath.equals("")){
    ...
    poCtrl1.setJsFunction_AfterDocumentSaved("updateRead()");
...
function updateRead(){
    ...
    AppUtil.ajaxProgress({
        url : "readTemplateController.do?saveOrUpdate",
        params : {
            READ_ID:$("input[name='readid']").val(),
            READ_DOC:strCustomSaveResult
            },
...
<body style="margin:0px;background-color: #f7f7f7;">
  ...
  <div class="easyui-layout" fit="true">
    ...
    <div data-options="region:'center',split:false">
        <input type="hidden" name="readid" value="${readTemplate.READ_ID}">
