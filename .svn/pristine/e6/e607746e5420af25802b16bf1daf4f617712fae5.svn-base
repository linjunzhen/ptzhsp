...
<%
	...
	String defId = (String)request.getAttribute("EFLOW_DEFID");//(String)flowSubmitInfo.get("EFLOW_DEFID");
	String exeId =(String)request.getAttribute("EFLOW_EXEID");// (String)flowSubmitInfo.get("EFLOW_EXEID");
	String nodeName = (String)request.getAttribute("nodeName");//EFLOW_EXEID(String)flowSubmitInfo.get("EFLOW_CURUSEROPERNODENAME");
...
function showDocumentTemplateWindow(templateId,userCode,sfhzd,docName) {
...
function bindMater(userName){	
	...
	parent.$.dialog.open("applyMaterController.do?addMissItemSelector&userName="+userName+"&taskId="+taskId+"&exeId="+exeId, {
		title : "申请材料选择器",
		width:"1000px",
		lock: true,
		resize:false,
...
function saveSelectMaters(materIds,userName){		
	...
	var layload = layer.load("正在提交请求中…");
	$.post("applyMaterController.do?addMissMater",{
			materIds :materIds,
			userName :userName,
			exeId :exeId,
			nodeName :nodeName,
...
<input type="hidden" name="FLOW_EXEID" value="${EFLOW_EXEID}"/>
<input type="hidden" name="FLOW_TASKID" value="${curTaskId}"/>
...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro2" >
	...
	<c:forEach items="${publicDocList}" var="publicDoc">
		...
		<tr>
			...
			<!-- 
			<td width="260px">
			    <input type="hidden" sfhzd="-1" filepath="${mater.FILE_PATH}" value="${mater.FILE_PATH}" filename='${mater.FILE_NAME}' tplid="${mater.TPL_ID}" authusername="${publicDoc.ASSIGNER_NAME}" authusercode="${publicDoc.ASSIGNER_CODE}" name="${mater.TPL_ID}_${publicDoc.ASSIGNER_CODE}_FILE_PATH">
			    <a href="javascript:void(0);" <c:if test="${!empty mater.FILE_PATH}">style="display: none;" </c:if>
			    id="${mater.TPL_ID}_${publicDoc.ASSIGNER_CODE}_ZXBJ" onclick="showDocumentTemplateWindow('${mater.TPL_ID}','${publicDoc.ASSIGNER_CODE}','-1','${mater.DOCUMENT_NAME}');" class="tab_btn_tj1 tab_tk_pro2btn">
			           在线编辑
			    </a>
			    <span id="${mater.TPL_ID}_${publicDoc.ASSIGNER_CODE}_DIV">
                <img id="${mater.TPL_ID}_${publicDoc.ASSIGNER_CODE}_SC" src="webpage/bsdt/applyform/images/tab_btn_sc.png" />
                </span>
...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro2" >
	...
	<c:forEach items="${publicDocList}" var="mater" varStatus="varStatus">
		<tr>
			...
			<td>${mater.DOCUMENT_NAME}</td><!-- 
			<td width="260px">
			    <input type="hidden" sfhzd="-1" filepath="${mater.FILE_PATH}" value="${mater.FILE_PATH}" filename='${mater.FILE_NAME}' tplid="${mater.TPL_ID}" authusername="${mater.AUTH_USER_NAMES}" authusercode="${mater.AUTH_USER_CODES}" name="${mater.TPL_ID}__FILE_PATH">
			    <a href="javascript:void(0);" <c:if test="${!empty mater.FILE_PATH}">style="display: none;" </c:if>
			    id="${mater.TPL_ID}__ZXBJ" onclick="showDocumentTemplateWindow('${mater.TPL_ID}','','-1','${mater.DOCUMENT_NAME}');" class="tab_btn_tj1 tab_tk_pro2btn">
			           在线编辑
			    </a>
			    <span id="${mater.TPL_ID}__DIV">
                <img id="${mater.TPL_ID}__SC" src="webpage/bsdt/applyform/images/tab_btn_sc.png" />
                </span>
...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
	...
	<tr>
		...
		<td width="260px">
		    <input type="hidden" sfhzd="1" filepath="${backDoc.FILE_PATH}" value="${backDoc.FILE_PATH}" tplid="${backDoc.TPL_ID}" filename='${backDoc.FILE_NAME}' authusername="" authusercode="" name="${backDoc.TPL_ID}__FILE_PATH">
		    <a href="javascript:void(0);" <c:if test="${!empty backDoc.FILE_PATH}">style="display: none;" </c:if>
		    id="${backDoc.TPL_ID}__ZXBJ" onclick="showDocumentTemplateWindow('${backDoc.TPL_ID}','','1','${backDoc.DOCUMENT_NAME}');" class="tab_btn_tj1 tab_tk_pro2btn">在线编辑</a>
		    <span id="${backDoc.TPL_ID}__DIV">
		    <img id="${backDoc.TPL_ID}__SC"  src="webpage/bsdt/applyform/images/tab_btn_sc.png" />
		    </span>
