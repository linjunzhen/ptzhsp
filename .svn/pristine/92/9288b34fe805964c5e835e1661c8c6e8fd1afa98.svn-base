	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"YJDSGTSCQSD");
    		}
    	}
		...
		AppUtil.initWindowForm("YJDSGTSCQSD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("YJDSGTSCQSD");
				var url = $("#YJDSGTSCQSD").attr("action");
...
<body>
	<form id="YJDSGTSCQSD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《一阶段施工图》（送审稿）审查意见签收单
				</div>
				<br>
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目《一阶段施工图》联合审查已完成，现将项目《一阶段施工图》（送审稿）审查意见交给你单位。
				</div>	
				<br>
