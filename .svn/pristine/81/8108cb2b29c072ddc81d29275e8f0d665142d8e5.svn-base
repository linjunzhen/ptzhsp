...
<script type="text/javascript">
	$(function(){
		...
		if(EFLOW_FLOWOBJ){
    		...
    		if(eflowObj.busRecord){
    			FlowUtil.initFormFieldValue(eflowObj.busRecord,"T_BSFW_ZFTZXMTYJGYS_FORM");
	...
	function submitFlow(flowSubmitObj){
		 AppUtil.submitWebSiteFlowForm('T_BSFW_ZFTZXMTYJGYS_FORM');
	}
	function tempSaveFlow(flowSubmitObj){
		 AppUtil.tempSaveWebSiteFlowForm('T_BSFW_ZFTZXMTYJGYS_FORM');
	}
</script>
</head>
	...
	<div class="eui-main">
		...
		<form id="T_BSFW_ZFTZXMTYJGYS_FORM" method="post">
			...
			<!-- <c:forEach items="${applyMaters}" var="appmatter" varStatus="varStatus">
				<c:if test="${appmatter.MATER_CODE=='345071904QT00206_12'&&appmatter.SUPPORT_WORD=='1'}">
						<jsp:include page="./annexCompleteAccept.jsp" />
	            </c:if>
	            <c:if test="${appmatter.MATER_CODE=='345071904QT01801_12'&&appmatter.SUPPORT_WORD=='1'}">
