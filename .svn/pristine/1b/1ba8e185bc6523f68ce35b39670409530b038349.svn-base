...
<script type="text/javascript">
    ...
    function ckServiceItemXzywGridRecord() {
        var entityId = AppUtil.getEditDataGridRecord("kfsywslGrid");
        if (entityId) {
    ...
    function showServiceItemXzywWindow(entityId) {
        $.dialog.open("serviceItemController.do?detailedInfo&entityId=" + entityId, {
            ...
            resize : false,
            close: function () {
                //AppUtil.gridDoSearch("kfsywslToolbar","kfsywslGrid");
            }
        }, false);
    };

    function addNewBusiness() {
        var itemId = AppUtil.getEditDataGridRecord("kfsywslGrid");
        if (itemId) {
            var rowData = $("#kfsywslGrid").datagrid("getChecked")[0];
            var defKey = rowData.DEF_KEY;
            var itemCode = rowData.ITEM_CODE;
            toUrl("executionController.do?goStart&defKey="+defKey+"&itemCode="+itemCode+"&isKfsywsl=1");
        }
    }
    function toUrl(url){
        var ssoForm=$("<form action='"+url+"' method='post' target='_blank'></form>");
        $("#kfsywslToolbar").append(ssoForm);
        ssoForm.submit();
    }
    $(document).ready(function() {
        $("#kfsywslGrid").datagrid({
...
url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=ServiceItemType',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 128,panelHeight: 'auto' " />
                        ...
                        <td colspan="4"><input type="button" value="查询"
                                               class="eve-button"
                                               onclick="AppUtil.gridDoSearch('kfsywslToolbar','kfsywslGrid')" />
        ...
        <table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
               id="kfsywslGrid" fitcolumns="true" toolbar="#kfsywslToolbar"
               method="post" idfield="ITEM_ID" checkonselect="false"
               selectoncheck="false" fit="true" border="false" nowrap="false"
               url="bdcExecutionController.do?kfsywslListData&Q_T.FWSXMXLB_NEQ=2">
            <thead>
            <tr>
