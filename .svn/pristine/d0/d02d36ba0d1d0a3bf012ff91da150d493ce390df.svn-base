	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZHZHZB");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZHZHZB", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZHZHZB");
				var url = $("#SHTZHZHZB").attr("action");
...
<body>
	<form id="SHTZHZHZB" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					<input type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:273px" 
					 name="VB_CGLSUBJECT_ST" maxlength="200" value="${SQData.blxmmc}" />项目《项目申请报告+》（送审版）<br />综合(专家)评审建议意见汇总表
				</div>
				<br>
