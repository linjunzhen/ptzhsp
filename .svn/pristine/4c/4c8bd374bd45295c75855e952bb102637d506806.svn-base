...
(function() {
	...
	var Z = {
		...
		bdList : [ "mshare,\u4e00\u952e\u5206\u4eab,mshare",
				...
				"twi,Twitter,twitter", "deli,delicious,delicious",
				"s51,51\u6E38\u620F\u793e\u533a,51shequ",
		...
		_jump : function(AX, AW, B, AE, AD) {
			var AV = Z, AC = AV.bdPic, W = AV.bdText, AA = AV.weiboText, AM = AV.wbUid, e = G, AL = AV.bdDesc, v = AV.bdComment, p = AV.bdMiniWindow || 0;
			C = (AW.getAttribute("data") && AW.getAttribute("data") != "") ? J
					._parseJson(AW.getAttribute("data")) : C;
			if (C != "") {
				...
				if (AX == "tsina") {
					if (C.text && C.weibotext) {
						W = encodeURIComponent((C.text + C.weibotext).substr(0,
								300))
					} else {
						if (C.weibotext) {
							W = encodeURIComponent((W + C.weibotext).substr(0,
									300))
						} else {
							if (C.text) {
								W = encodeURIComponent((C.text + AA).substr(0,
										300))
							} else {
					...
					W = encodeURIComponent((C.text || "").substr(0, 300))
							|| encodeURIComponent(W.substr(0, 300))
				}
			} else {
				if (AX == "tsina") {
					W = encodeURIComponent((W + AA).substr(0, 300))
			...
			var AJ = +new Date;
			AJ = AJ - AW.time;
			var k = [ U, T, o, AI, AF, j, i, AG, AS, n, AJ, q, AB, u, AH ]
					.join(".");
			var D;
			switch (Z.bdType) {
			case "button":
			...
			var r = "?click=1&url=" + e + "&uid=" + AV.bdUid + "&to=" + AY
					+ "&type=text&relateUid=" + AM + "&pic=" + AC + "&title="
					+ W + "&key=" + (AV.snsKey[AX] || "") + "&sign="
					+ AV.review + "&desc=" + AL + "&comment=" + v
					+ "&searchPic=" + AV.searchPic + "&l=" + AP + "&linkid="
					+ H + "&sloc=" + k + "&apiType=0&buttonType=" + D
			...
			if (AX != "goWebsite") {
				...
				if (AX == "bdxc") {
					var AT = window, AU = document, AO = "_bdXC", AK;
					if (AT[AO]) {
						...
						AK.src = "http://xiangce.baidu.com/zt/collect/mark.js?"
								+ (new Date()).getTime();
					...
					if (AX == "bdysc") {
						...
						if (window.baiduSC_yaq4d3elabjnvmijccc1zuo3o4yeizck) {
							...
							AN.src = "http://s.wenzhang.baidu.com/js/pjt/content_ex/page/bookmark.js?s=baidu_fenxiang&_t="
									+ Math.random();
