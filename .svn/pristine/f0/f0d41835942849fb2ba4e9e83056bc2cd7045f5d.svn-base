	...
	$(function(){
		...
		if(EFLOW_FLOWOBJ){
    		...
    		if(eflowObj.busRecord){
    			FlowUtil.initFormFieldValue(eflowObj.busRecord,"T_BSFW_XMBAQSLHSC_FORM");
    		}
		}
	});
...
<body style="background: #f0f0f0;">
	...
	<div class="eui-main">
		...
		<form id="T_BSFW_XMBAQSLHSC_FORM" method="post">
			...
			<div class="bsbox clearfix">
				...
				<div class="bsboxC">
					<table cellpadding="0" cellspacing="1" class="bstable1">
						<tr>
							<td colspan="4"><font size="3">《项目备案请示+》（编修版）已经于</font> <input
								type="text" name="ZJZHSCSJ"
								class="laydate-icon validate[required]"
								onclick="laydate({istime: false, format: 'YYYY-MM-DD'})"
								style="width:130px; height:26px;" /><font class="tab_color">*</font>
								<font size="3">提交项目投资窗口进行综合审查,并已按照专家审查意见进一步优化。</font></td>
						</tr>
					</table>
				</div>
			</div>
		...
		<jsp:include page="./submitdiv.jsp" >
		     <jsp:param value="AppUtil.submitWebSiteFlowForm('T_BSFW_XMBAQSLHSC_FORM');" name="submitFn"/>
		     <jsp:param value="AppUtil.tempSaveWebSiteFlowForm('T_BSFW_XMBAQSLHSC_FORM');" name="tempSaveFn"/>
		</jsp:include>
		<%--结束引入提交DIV界面 --%>
