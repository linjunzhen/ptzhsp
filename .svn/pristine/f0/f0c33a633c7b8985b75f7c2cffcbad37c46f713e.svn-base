	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZSGTSCLXD");
    		}
    	}
		//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZSGTSCLXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZSGTSCLXD");
				var url = $("#SHTZSGTSCLXD").attr("action");
...
<body>
	<form id="SHTZSGTSCLXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt" readonly="readonly"
						...
						name="VB_GCSUBJECT_ST" maxlength="200" value="${SQData.blxmmc}" />项目的《施工图》（送审版）已完成，现转给你单位。请于<input
