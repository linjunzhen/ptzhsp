var mobileUtil = new Object({
	...
	selectInit:function(selectData,postUrl, typeCode, id, isinit,callback) {
			...
			mobileUtil.muiAjaxForData({
				...
				callback : function(resultJson) {
					if (resultJson.success) {
						var rows = resultJson.data;
						if (rows.length == 0) {
							mui.toast("获取字典失败，请重试");
							return;
						}
						for (var i in rows) {
							selectData.push({
						...
						if(isinit){
							...
							mobileUtil.selectByData(selectData, id + '_text', id, callback);
						}
					} else {
						mui.toast("获取字典失败，请重试");
					}
	...
	selectOtherInit:function(selectData,postUrl, data, id, isinit,callback) {
		if (selectData.length == 0) {
			mobileUtil.muiAjaxForData({
				mask : true,
				url : postUrl,
