...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
        	AppUtil.initWindowForm("AttachFileMoveForm", function(form, valid){
				if(valid){
					//将提交按钮禁用,防止重复提交
					$("input[type='submit']").attr("disabled", "disabled");
					var formData = $("#AttachFileMoveForm").serialize();
					var url = $("#AttachFileMoveForm").attr("action");
...
<body style="margin:0px;background-color: #f7f7f7;">
    <form id="AttachFileMoveForm" method="post" action="attachFileController.do?saveMove">
        <div id="AttachFileMoveFormDiv">
            <!--==========隐藏域部分开始 ===========-->
