...
public class SelectDelEhcacheList extends RegexpMethodPointcutAdvisor {
    ...
    */
    ...
    public void setPatterns(String[] patterns) {
        ...
        try{
            List<Map<String, Object>> elist = sysEhcacheService.findByStatue("0");
            if (elist != null & elist.size() > 0) {
                for (int i = 0; i < elist.size(); i++) {
                    String estr = elist.get(i).get("EHCACHE_CLASS_NAME").toString().trim();
                    list.add(estr);
                    log.debug("删除缓存" + i + ":" + estr);
                }
            }
        }catch(Exception e){
            log.error(e.getMessage());
        }
        list.add("net.evecom.platform.system.service.impl.SysUserServiceImpl.findBySqlFilter");
        if (list.size() > 0) {
