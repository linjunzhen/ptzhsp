...
<script type="text/javascript">
	...
	$(function(){
    	...
    	if(flowSubmitObj){
    		...
    		$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
    		 //初始化表单字段权限控制
    		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BSFW_GQTZQYZDJCBA_FORM");
    		//初始化表单字段值
    		if(flowSubmitObj.busRecord){
    			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BSFW_GQTZQYZDJCBA_FORM");
	...
	function FLOW_SubmitFun(flowSubmitObj){
		 //先判断表单是否验证通过
		 var validateResult =$("#T_BSFW_GQTZQYZDJCBA_FORM").validationEngine("validate");
		 if(validateResult){
			 ...
			 if(submitMaterFileJson||submitMaterFileJson==""){
				 ...
				 //先获取表单上的所有值
				 var formData = FlowUtil.getFormEleData("T_BSFW_GQTZQYZDJCBA_FORM");
...
<body>
	...
	<form id="T_BSFW_GQTZQYZDJCBA_FORM" method="post">
		...
		<table cellpadding="0" cellspacing="1" class="tab_tk_pro" style="display: none">
		<tr>
			<th colspan="4">公司架构</th>
		</tr>
			<tr>
				<td class="tab_width">基本架构：</td>
				<td>
					<input type="text" maxlength="256" name="GS_JBGJ"
					class="tab_text validate[]" value="${busRecord.GS_JBGJ}"/>
				</td>
				<td class="tab_width">股东关联关系:</td>
				<td>
