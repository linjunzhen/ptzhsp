...
$(function() {
    ...
    setSbgcSingleProMaintype('01',currentTime);
 });
 </script>
 <style type="text/css">
 .textbox{
...
<div class="addBox">
	...
	<table cellpadding="0" cellspacing="0" class="syj-table2">
		...
		<tr>
			...
			<td colspan="3">						
				<input name="${currentTime}SINGLEPROMAINTYPE" id="${currentTime}singlepromaintype_sbgc" class="easyui-combobox" 
					data-options="
						...
						onSelect:function(rec){
						   $('#${currentTime}singleprotype_sbgc').combobox('clear');
						   $('#${currentTime}singleprosubtype_sbgc').combobox('clear');
						   if(rec.DIC_CODE){
								/* var PROTYPE = $('#PROTYPE').val();
								if(rec.DIC_CODE!=PROTYPE && PROTYPE!=''){
									$('#${currentTime}singlepromaintype_sbgc').combobox('clear');
									art.dialog({
							   ...
							   $('#${currentTime}singleprotype_sbgc').combobox('reload',url); 
								setSbgcSingleProMaintype(rec.DIC_CODE,'${currentTime}');
						   }
						}
				" />	
					<script>
						$('#${currentTime}singlepromaintype_sbgc').combobox({
							url:'dictionaryController/loadData.do?typeCode=PROTYPE&orderType=asc',
							method:'post',
							valueField:'DIC_CODE',
							textField:'DIC_NAME',
				...
				<input name="${currentTime}SINGLEPROTYPE" id="${currentTime}singleprotype_sbgc" class="easyui-combobox" 
					data-options="
						...
						editable:false,
						onSelect:function(rec){
						   setSbgcSingleprosubtype(rec,'${currentTime}');
						}
				" />
					<script>
						$('#${currentTime}singleprotype_sbgc').combobox({
				...
				<input name="${currentTime}SINGLEPROSUBTYPE" id="${currentTime}singleprosubtype_sbgc" class="easyui-combobox"  
					data-options="			                
						method:'post',
						valueField:'VALUE',
						textField:'TEXT',
				...
				" />
					<script>
						$('#${currentTime}singleprosubtype_sbgc').combobox({
	...
	<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20" style="margin-top: -1px;">		
		<tr class="${currentTime}singleProMaintype_1_sbgc">	
		...
		<tr class="${currentTime}singleProMaintype_2_sbgc">	
		...
		<tr>
			...
			<td colspan="3">
				...
				fieldname="${currentTime}ISSUPERHIGHTBUILDING_SBGC"></eve:radiogroup>
			</td>
		</tr>	
		<tr class="${currentTime}singleProMaintype_1_sbgc">
		...
		<tr class="${currentTime}singleProMaintype_1_sbgc">
			...
			<td>
			  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="${currentTime}STRUCTUPFLOORAREA_DWGC"  maxlength="16" placeholder="请输入±0.000以上面积" onblur="onlyNumber2(this);setSbArcharea('${currentTime}');" onkeyup="onlyNumber2(this);" />M<sup style="vertical-align: super;font-size: smaller;">2</sup>
			</td>
			<th><span class="syj-color2">*</span> ±0.000以下面积</th>
			<td>
			  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="${currentTime}STRUCTDWFLOORAREA_DWGC"  maxlength="16"  placeholder="请输入±0.000以下面积" onblur="onlyNumber2(this);setSbArcharea('${currentTime}');" onkeyup="onlyNumber2(this);"/>M<sup style="vertical-align: super;font-size: smaller;">2</sup>
			</td>
		</tr>
		<tr>
			<th>基坑支护方法</th>
		...
		<tr>
			...
			<td>
				<eve:radiogroup typecode="YesOrNo" width="130px"
				fieldname="${currentTime}ISSHOCKISOLATIONBUILDING_SBGC" ></eve:radiogroup>
			</td>
			<th><span class="syj-color2">*</span> 是否为装配式建筑	</th>
			<td>
				<eve:radiogroup typecode="YesOrNo" width="130px"
				fieldname="${currentTime}ISEQUIPPEDARCHITECTURE_SBGC"></eve:radiogroup>
			</td>
		</tr>	
		<tr>
			<th><span class="syj-color2">*</span> 是否绿色建筑	</th>
			<td>
				<eve:radiogroup typecode="YesOrNo" width="130px" onclick="setSbgcGreenbuidinglevel(this.value,'${currentTime}');"
				fieldname="${currentTime}ISGREENBUILDING_SBGC" ></eve:radiogroup>
			</td>
		...
		<tr class="${currentTime}greenbuidinglevelTr_sbgc" style="display:none">
			<th><span class="syj-color2">*</span>绿色建筑等级</th>
			<td colspan="3">
				<eve:eveselect clazz="syj-input1 w280 field_width" dataParams="GREENBUIDINGLEVEL"
				dataInterface="dictionaryService.findDatasForSelect" 
				defaultEmptyText="请选择基础建设等级" name="${currentTime}GREENBUIDINGLEVEL_SBGC">
				</eve:eveselect>
			</td>
		</tr>
		<tr>
			...
			<td colspan="3">
				<span class="${currentTime}singleProMaintype_1_jglx_sbgc">
				<eve:excheckbox
					dataInterface="dictionaryService.findTwoDatasForSelect"  
					name="${currentTime}STRUCTQUALTYPE_SBGC" width="749px;" clazz="checkbox validate[required]"
					dataParams="STRUCTQUALTYPE:2" >
				</eve:excheckbox>
				</span>
				<span class="${currentTime}singleProMaintype_2_jglx_sbgc">
				<eve:excheckbox
					dataInterface="dictionaryService.findTwoDatasForSelect"  
					name="${currentTime}STRUCTQUALTYPE_SBGC" width="749px;" clazz="checkbox validate[required]"
		...
		<tr>
			...
			<td colspan="3">
				<eve:excheckbox
					dataInterface="dictionaryService.findDatasForSelect" 
					name="${currentTime}BASICQUALTYPE_SBGC" width="749px;" clazz="checkbox validate[required]"
		...
		<tr>
			...
			<td colspan="3">
				<eve:excheckbox
					dataInterface="dictionaryService.findDatasForSelect"
					name="${currentTime}REBELQUAKELEVEL_SBGC" width="749px;" clazz="checkbox validate[required]"
	...
	<div class="syj-title1" style="margin-top: 5px;margin-bottom: 5px;">
		<a href="javascript:void(0);" onclick="javascript:addSbgcChildrenDiv('${currentTime}');" class="syj-addbtn" id="addSbgc" >添加</a>
	</div>
	<table cellpadding="0" cellspacing="1" class="syj-table2 syj-table3" id="${currentTime}childrenSbgcTable">
		<tr>
