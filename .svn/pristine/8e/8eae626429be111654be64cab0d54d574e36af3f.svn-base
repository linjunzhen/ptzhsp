...
function FLOW_SubmitFun() {
	...
	if(companySetNature=='02'||companySetNature=='04'){//设董事会、监事设立;设董事会监事会设立
		var num = 0;
		var dsnum = 0;
	...
	//验证表单是否合法
	var valiateTabForm = AppUtil.validateWebsiteTabForm();//还原
	if (valiateTabForm) {
...
function showSelectJyfw(){
	...
	var url = "domesticControllerController/selector.do?noEcho=1";
	$.dialog.open(url, {
		title : "经营范围选择器",
		width:"100%",
		lock: true,
...
function addDsxxDiv(){
	...
	var ssdjzw = '04'
	if(companySetNature=='02'||companySetNature=='04'){
		ssdjzw = '01';
	}
 ...
 */
function addJsxxDiv(){
	$.post("domesticControllerController/refreshJsxxDiv.do",{
	}, function(responseText, status, xhr) {
