...
<head>
    ...
    <script type="text/javascript">
	    $(document).ready(function(){
	    	...
	    	//初始化进度条
	    	$('#imageFileProcessBar').progressbar({value: 0});
	    	...
	    	uploader.on('fileQueued', function(file){
	    		...
	    		$('#imageFileProcessBar').progressbar({value: 0});
	    		$("#ImageFileForm").find("input[name='IMAGE_NAME']").val(file.name);
	    		$("#ImageFileForm").find("input[name='IMAGE_SUFFIX']").val(file.ext);
	    		//格式化文件大小
	    		var fileSize = WebUploader.Base.formatSize(file.size);
	    		$("#ImageFileForm").find("input[name='IMAGE_SIZE']").val(fileSize);
	    	});
	    	//某个文件开始上传前触发，一个文件只会触发一次
	    	uploader.on('uploadStart', function(file){
	    		$('#imageFilePickBtn').hide();
	    		//编辑
	    		if('${imageFile.IMAGE_ID}'){
	    			uploader.options.formData.uid = '${imageFile.IMAGE_ID}';
	    		//新增
	    		}else{
	    	...
	    	uploader.on('uploadProgress', function(file, percentage){
	    	    ...
	    	    $('#imageFileProcessBar').progressbar({value: progress});
	    	});
	    	//当文件上传成功时触发
	    	uploader.on('uploadSuccess', function(file, response){
	    		if(response.success){
	    			...
	    			parent.$("#ImageFileViewGrid").datagrid("reload");
					AppUtil.closeLayer();
	    		}else{
	    			$('#imageFileProcessBar').progressbar({value: 0});
	    			parent.art.dialog({
	    	...
	    	AppUtil.initWindowForm("ImageFileForm", function(form, valid){
        		if(valid){
        			if(fileInQueque){
        				uploader.upload();
        			}else{
...
<body style="margin:0px;background-color: #f7f7f7;">
    <form id="ImageFileForm" method="post">
        <!--==========隐藏域部分开始 ===========-->
        <input type="hidden" name="TYPE_ID" value="${imageFile.TYPE_ID}"/>
        <!--==========隐藏域部分结束 ===========-->
			...
			<tr style="line-height:10px;">
			    <td>
			        <span style="width:110px;text-align:right;float:left;line-height:28px;">
			            <font class="dddl_platform_html_requiredFlag">*</font>图片名：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="IMAGE_NAME" style="width:240px;float:left;" value="${imageFile.IMAGE_NAME}" readonly/>
			        <div id="imageFilePickBtn" style="float:left;margin-left:3px;">选择</div>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;line-height:28px;">
			            <font class="dddl_platform_html_requiredFlag">*</font>图片类型：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="IMAGE_SUFFIX" style="width:240px;float:left;" value="${imageFile.IMAGE_SUFFIX}" readonly/>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;line-height:28px;">
			            <font class="dddl_platform_html_requiredFlag">*</font>图片大小：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="IMAGE_SIZE" style="width:240px;float:left;" value="${imageFile.IMAGE_SIZE}" readonly/>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;">上传进度：</span>
			        <div id="imageFileProcessBar" style="width:240px;"></div>
			    </td>
			</tr>
        </table>
