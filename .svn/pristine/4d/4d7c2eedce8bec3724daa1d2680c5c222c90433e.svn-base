...
<head>
    ...
    <script type="text/javascript">
	    $(document).ready(function(){
	    	...
	    	//初始化进度条
	    	$('#attachFileProcessBar').progressbar({value: 0});
	    	...
	    	uploader.on('fileQueued', function(file){
	    		...
	    		$('#attachFileProcessBar').progressbar({value: 0});
	    		$("#AttachFileForm").find("input[name='ATTACHMENT_NAME']").val(file.name);
	    		$("#AttachFileForm").find("input[name='ATTACHMENT_SUFFIX']").val(file.ext);
	    		//格式化文件大小
	    		var fileSize = WebUploader.Base.formatSize(file.size);
	    		$("#AttachFileForm").find("input[name='ATTACHMENT_SIZE']").val(fileSize);
	    	});
	    	//某个文件开始上传前触发，一个文件只会触发一次
	    	uploader.on('uploadStart', function(file){
	    		$('#attachFilePickBtn').hide();
	    		//编辑
	    		if('${attachFile.ATTACHMENT_ID}'){
	    			uploader.options.formData.uid = '${attachFile.ATTACHMENT_ID}';
	    		//新增
	    		}else{
	    	...
	    	uploader.on('uploadProgress', function(file, percentage){
	    	    ...
	    	    $('#attachFileProcessBar').progressbar({value: progress});
	    	});
	    	//当文件上传成功时触发
	    	uploader.on('uploadSuccess', function(file, response){
	    		if(response.success){
	    			...
	    			parent.$("#AttachFileViewGrid").datagrid("reload");
					AppUtil.closeLayer();
	    		}else{
	    			$('#attachFileProcessBar').progressbar({value: 0});
	    			parent.art.dialog({
	    	...
	    	AppUtil.initWindowForm("AttachFileForm", function(form, valid){
        		if(valid){
        			if(fileInQueque){
        				uploader.upload();
        			}else{
...
<body style="margin:0px;background-color: #f7f7f7;">
    <form id="AttachFileForm" method="post">
        <!--==========隐藏域部分开始 ===========-->
        <input type="hidden" name="TYPE_ID" value="${attachFile.TYPE_ID}"/>
        <!--==========隐藏域部分结束 ===========-->
        <table style="width:100%;border-collapse:collapse;"class="dddl-contentBorder dddl_table">
			...
			<tr>
			    <td>
					...
					${attachFile.TYPE_NAME}
			    </td>
			</tr>
			<tr style="line-height:9px;">
			    <td>
			        <span style="width:110px;text-align:right;float:left;line-height:28px;">
			            <font class="dddl_platform_html_requiredFlag">*</font>附件名：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="ATTACHMENT_NAME" style="width:240px;float:left;" value="${attachFile.ATTACHMENT_NAME}" readonly/>
			        <div id="attachFilePickBtn" style="float:left;margin-left:3px;">选择</div>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;">
			            <font class="dddl_platform_html_requiredFlag">*</font>附件类型：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="ATTACHMENT_SUFFIX" style="width:240px;float:left;" value="${attachFile.ATTACHMENT_SUFFIX}" readonly/>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;">
			            <font class="dddl_platform_html_requiredFlag">*</font>附件大小：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="ATTACHMENT_SIZE" style="width:240px;float:left;" value="${attachFile.ATTACHMENT_SIZE}" readonly/>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;">上传进度：</span>
			        <div id="attachFileProcessBar" style="width:240px;"></div>
			    </td>
			</tr>
        </table>
