	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZSGTLHSCSLD");
    		}
    	}
		...
		AppUtil.initWindowForm("SHTZSGTLHSCSLD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled", "disabled");
				var formData = WordUtil.getFormEleData("SHTZSGTLHSCSLD");
				var url = $("#SHTZSGTLHSCSLD").attr("action");
...
<body>
	<form id="SHTZSGTLHSCSLD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《施工图》(修编版)联合审查受理单
				</div>
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt" value="${SQData.xmsqsj}"
						...
						name="VB_GCSUBJECT_ST" maxlength="200" value="${SQData.blxmmc}" />项目《施工图》（修编版）审查申请材料
					（<input type="checkbox" name="VB_SQB_CK" value="1">  项目《施工图》（修编版）联合审查申请表
					  <input type="checkbox" name="VB_CNS_CK" value="1">  申请办理行政审批事项承诺书
					  <input type="checkbox" name="VB_CBSJGS_CK" value="1">  经图审中心审查或采取“双院制”审查的《施工图》（修编版）（含初步设计及概算、审查文件）
					  <input type="checkbox" name="VB_QT_CK" value="1">  其他
					  <input type="text" name="VB_QTDETAIL_ST" class="eui-ipt" style="width:60px">）
					收悉，经审核，要件齐全，符合申请条件。请于<input type="text" class="eui-ipt" value="${SQData.xmsqsj}"
						style="width:135px;" name="VB_GCCN_TIME_ST" readonly="readonly"/>(22个工作日)凭此单到区行政服务中心大厅我局项目投资窗口领取相关《<施工图>联合审查意见书》。
				</div>
