	...
	$(function(){
		...
		if(EFLOW_FLOWOBJ){
    		...
    		if(eflowObj.busRecord){
    			FlowUtil.initFormFieldValue(eflowObj.busRecord,"T_BSFW_ZFTZXMGH_FORM");
    		}
		}
	...
	<div class="eui-main">
		...
		<%--开始引入对接表信息部分:政府投资项目规划选址初步对接表 
		  	<c:forEach items="${applyMaters}" var="appmatter" varStatus="varStatus">
				<c:if test="${appmatter.MATER_CODE=='345071904QT00201_03'&&appmatter.SUPPORT_WORD=='1'}">
		...
		<jsp:include page="./submitdiv.jsp" >
		     <jsp:param value="AppUtil.submitWebSiteFlowForm('T_BSFW_ZFTZXMGH_FORM');" name="submitFn"/>
		     <jsp:param value="AppUtil.tempSaveWebSiteFlowForm('T_BSFW_ZFTZXMGH_FORM');" name="tempSaveFn"/>
		</jsp:include>
		<%--结束引入提交DIV界面 --%>
