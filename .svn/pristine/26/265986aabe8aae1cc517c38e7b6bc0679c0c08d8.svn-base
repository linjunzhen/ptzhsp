...
<body style="background: none; min-width:800px;">
	<table cellpadding="0" cellspacing="0" class="zxtable2">
		<tr>
			...
			<th >环节名称</th>
            <th >审核时间</th>
			<th width="200px" style="background: none;">审核内容</th>
		</tr>
		<c:forEach items="${opinionList}" var="opinion" varStatus="varStatus">
		<tr>
             <td>${varStatus.index+1}</td>
             <td>${opinion.TASK_NODENAME}</td>
            <td>${opinion.CREATE_TIME}</td>
            <td>${opinion.HANDLE_OPINION}</td>
         </tr>
         </c:forEach>
	</table>
