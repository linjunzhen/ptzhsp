...
<title>平潭综合实验区商事主体登记申报系统</title>
    ...
    <!---引入公共JS--->
    <script src="<%=path%>/webpage/website/applyforms/zzhy/js/idCardArea.js"></script>
    <script src="<%=path%>/webpage/website/applyforms/zzhy/js/commercial.js"></script>
    <!---面签公共JS--->
    <script src="<%=path%>/webpage/website/applyforms/zzhy/js/sign.js"></script>
    <!---秒批JS--->
    <script src="<%=path%>/webpage/website/applyforms/zzhy/js/mp.js"></script>
    ...
    /*将法人和股东自动填写到  控股股东或实际控制人名称(法人代表或持股比例最大股东)*/
    function getCompanyPerson() {
        var CONTROLLER = '${busRecord.CONTROLLER}';
        var LEGAL_NAME = $("input[name='LEGAL_NAME']").val();
        var SHAREHOLDER_NAME = $("input[name*='SHAREHOLDER_NAME']")
        var html = '';
        if (LEGAL_NAME != null && LEGAL_NAME.length > 0) {
            html += "&nbsp;<input type='radio' class='radio validate[required]' name='CONTROLLER'"
            if (CONTROLLER.toString() == LEGAL_NAME.toString()) {
                html += " checked='checked' "
            }else if(CONTROLLER.toString()==""){
                html += " checked='checked' "
            }
            html += " value='"+LEGAL_NAME+"'>"+LEGAL_NAME+"&nbsp;&nbsp;"
        }
        for (var i = 0; i < SHAREHOLDER_NAME.length; i++) {
            if (SHAREHOLDER_NAME[i].value != null && SHAREHOLDER_NAME[i].value.length > 0) {
                html += "&nbsp;<input type='radio' class='radio validate[required]' name='CONTROLLER'"
                if (CONTROLLER == SHAREHOLDER_NAME[i].value) {
                    html += " checked='checked' "
                }
                html += " value='" + SHAREHOLDER_NAME[i].value + "'>" + SHAREHOLDER_NAME[i].value + "&nbsp;&nbsp;";
            }
        }
        $("#CONTROLLERRatio").html(html);
    }
...
<div class="eui-main">
    ...
    <input type="hidden" value="${requestParams.ISMEDICALREGISTER}">
    <input type="hidden" value="${requestParams.ISFUNDSREGISTER}">
    <div class="container" >
            ...
            <div class="syj-tyys " style="z-index: 99;" id="formcontainer">
                <div class="hd syj-tabtitle" id="formtabtitle" style="z-index: 99;">
                    <ul>
                        ...
                        <li><a href="javascript:AppUtil.validateSingleForm('CAPITAL_FORM');"  id="CAPITAL_FORM_A">资金、股东信息</a></li>
                        <li><a href="javascript:AppUtil.validateSingleForm('PERSONNEL_FORM');"  id="PERSONNEL_FORM_A">人事信息</a></li>
                        <li><a href="javascript:AppUtil.validateSingleForm('LEGAL_FORM');"  id="LEGAL_FORM_A">人员信息</a></li>
                        
                        <li><a href="javascript:AppUtil.validateSingleForm('SEAL_FORM');"  id="SEAL_FORM_A">印章和邮寄</a></li>   
                        <li><a href="javascript:AppUtil.validateSingleForm('OTHER_FORM');"  id="OTHER_FORM_A" onclick="getCompanyPerson()">税务/医社保/公积金/银行开户</a></li>
                        ...
                        <li id="applyItemMater"><a style="font-size: 12px;" href="javascript:AppUtil.validateSingleForm('APPLYITEMMATER_FORM');"  id="APPLYITEMMATER_FORM_A" >1+N证合一</a></li>
                ...
                <div class="bd" style="padding: 25px 30px;">
                    ...
                    <div>                       
                        <%--开始引入企业基本信息界面 --%>
                        <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_COMPANY.jsp" />
                        <%--结束引入企业基本信息界面 --%>
                    </div>
                    <div>
                        <%--开始引入企业资金、股东信息界面 --%>
                        <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_CAPITAL.jsp" />
                        <%--结束引入企业资金、股东信息界面 --%>
                    </div>
                        <div>
                                <%--开始引入企业人事信息界面 --%>
                            <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_PERSONNEL.jsp" />
                                <%--结束引入企业人事信息界面 --%>
                        </div>
                        <div>
                                <%--开始引入法定代表人信息界面 --%>
                            <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_LEGAL.jsp" />
                                <%--结束引入法定代表人信息界面 --%>
                        </div>
                  ...
                  <c:if test="${requestParams.SSSBLX=='1'||busRecord.SSSBLX=='1'}">
                            <div>
                                    <%--开始引入企业基本信息界面 --%>
                                <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_COMPANY_MP.jsp" />
                                    <%--结束引入企业基本信息界面 --%>
                            </div>
                            <div>
                                    <%--开始引入企业资金、股东信息界面 --%>
                                <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_CAPITAL_MP.jsp" />
                                    <%--结束引入企业资金、股东信息界面 --%>
                            </div>
                      <div>
                              <%--开始引入企业人事信息界面 --%>
                          <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_PERSONNEL_MP.jsp" />
                              <%--结束引入企业人事信息界面 --%>
                      </div>

                      <div>
                              <%--开始引入法定代表人信息界面 --%>
                          <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_LEGAL_MP.jsp" />
                              <%--结束引入法定代表人信息界面 --%>
                      </div>
                    </c:if>
                    ...
                    <div>                   
                        <%--开始引入印章信息界面 --%>
                        <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_SEAL.jsp" />
                        <%--结束引入印章信息界面 --%>
                    </div>
                    <%--非秒批 --%>
                    <c:if test="${requestParams.SSSBLX!='1'&&busRecord.SSSBLX!='1'}">
                        <div>
                                <%--开始引入其他信息界面 --%>
                            <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_OTHER.jsp" />
                                <%--结束引入其他信息界面 --%>
                        </div>
                    </c:if>
                    <%--秒批 --%>
                    ...
                    <div>                   
                        <%--开始引入其他信息界面 --%>
                        <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_OTHER_MP.jsp" />
                        <%--结束引入其他信息界面 --%>
                    </div>
                    </c:if>
                    <div>                               
                    ...
                    <div>
                        <%--开始引入前台用户上传界面 --%>
                        <jsp:include page="/webpage/website/applyforms/zzhy/T_COMMERCIAL_FILE.jsp" />
                        <%--结束引入前台用户上传界面 --%>
                    </div>
                    <div>
                        <%--开始引入关联事项材料界面 --%>
        ...
        <c:if test="${isQueryDetail!=null && EFLOWOBJ.EFLOW_CUREXERUNNINGNODENAMES!='开始'&&!(EFLOWOBJ.EFLOW_CUREXERUNNINGNODENAMES=='窗口办理'&&busRecord.ISNEEDSIGN=='1')}">
        <div class="syj-btn" style="background:#fff;padding: 0 0 40px 0;">
            <div class="eui-FooterB eui-button">
