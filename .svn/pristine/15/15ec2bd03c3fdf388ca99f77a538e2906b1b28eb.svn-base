...
<%@ page language="java" import="net.evecom.platform.system.service.DictionaryService"%>
...
$(function(){
	...
	//if($("#TechnicalPersonnelDiv").children().length==0&&EFLOW_VARS.EFLOW_EXEID==""){
		addAqzdTr();
	//}
});
...
<form action="" id="AQZD_FORM"  >
	...
	<div id="Aqzd_BaseInfo_Div" >
		...
		<td>
		<input type="text" name="AQZD_MC" placeholder="请输入管理制度名称" maxlength="32"
		 class="syj-input1 validate[]" value="${a.DIC_NAME }"/></td>
	...
	<tr>
		<td>
		<input type="text" name="AQZD_MC" placeholder="请输入管理制度名称" maxlength="32"
		 class="syj-input1 validate[]" value="${aqzd.AQZD_MC }"/></td>
		<td><input type="text" name="AQZD_WBBH" placeholder="请输入文本编号" maxlength="32"
		class="syj-input1 validate[]" style="width:75%;" value="${aqzd.AQZD_WBBH }"/>
