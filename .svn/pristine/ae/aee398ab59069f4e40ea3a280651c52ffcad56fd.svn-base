...
<script type="text/javascript">
    ...
    function doSelectRows(){
    	var rows = $("#bdctjmxGrid").datagrid("getChecked");
		...
		if(ydydyh.length>0){
			parent.art.dialog.confirm("不动产单元号"+ydydyh+"已抵押!是否继续办理?",function() {
				art.dialog.data("bdctjmxInfo", datas);
				AppUtil.closeLayer();
			});
		}else{
			art.dialog.data("bdctjmxInfo", datas);
	...
	function bdctjmxSearch(){
	    var validateResult =$("#bdctjmxForm").validationEngine("validate");
	    if(validateResult){
		    var count = 0;
	        AppUtil.gridDoSearch('bdctjmxToolbar','bdctjmxGrid');
	        $('#bdctjmxGrid').datagrid({
	            onLoadSuccess:function(){
	                //确保初始化后只执行一次
	                if(count == 0){
	                    var rows = $("#bdctjmxGrid").datagrid("getRows");
	                    if(rows.length==0){
	                        parent.art.dialog({
...
<body class="eui-diabody" style="margin:0px;">
	...
	<div class="easyui-layout" fit="true" >		
		<div data-options="region:'center',split:false" >
			<div id="bdctjmxToolbar">
				<form action="#" name="bdctjmxForm" id="bdctjmxForm">
					<!--====================开始编写隐藏域============== -->
					...
					<table class="dddl-contentBorder dddl_table"
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;"><input class="eve-input" 
								...
								name="bdcdyh" /></td>
							<td style="width:120px;text-align:right;">户号：</td>
							<td style="width:135px;"><input class="eve-input"
							...
							<td style="width:135px;"><input class="eve-input"
								type="text" maxlength="50" style="width:180px;"
								name="zh" /></td>
							<td style="width:120px;text-align:right;">户标识码：</td>
							<td style="width:135px;"><input class="eve-input"
								type="text" maxlength="50" style="width:180px;"
								name="hh" /></td>
							<td style="width:120px;text-align:right;">房屋坐落(项目名称)：</td>
							<td style="width:135px;"><input class="eve-input"
								type="text" maxlength="50" style="width:180px;"
								name="xmmc" /></td>
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="bdctjmxSearch()" />
								<input type="button" value="重置" class="eve-button"
								onclick="AppUtil.gridSearchReset('bdctjmxForm');" /></td>
						</tr>
					</table>
				</form>
			</div>
		...
		bdctjmxdaxxcxGrid" fitColumns="true" toolbar="#bdctjmxToolbar"
				method="post" idField="YWH" checkOnSelect="false"
				selectOnCheck="false" fit="true" border="false" nowrap="false"
				url="bdcDyqscdjController.do?bdctjmxDatagrid&noAuth=${noAuth}">
				<thead>
					<tr>
						...
						<th data-options="field:'111',align:'left'" width="100">状态</th>
						<th data-options="field:'BDCDYH',align:'left'" width="100">不动产单元号</th>
						<th data-options="field:'ZH',align:'left'" width="100">幢号</th>
						<th data-options="field:'111',align:'left'" width="100">户号</th>
						<th data-options="field:'ZCS',align:'left'" width="100">总层数</th>
						<th data-options="field:'111',align:'left'" width="100">所在层</th>
						<th data-options="field:'111',align:'left'" width="100">房屋结构</th>
						<th data-options="field:'111',align:'left'" width="100">规划用途</th>
						<th data-options="field:'111',align:'left'" width="100">房屋坐落</th>
						<th data-options="field:'111',align:'left'" width="100">建筑面积</th>
						<th data-options="field:'111',align:'left'" width="100">专有建筑面积</th>
					</tr>
				</thead>
			</table>
