...
function FLOW_SubmitFun(flowSubmitObj){	
	//先判断表单是否验证通过
	 var validateResult =$("#T_BDCQLC_ZJSYSCDJ_FORM").validationEngine("validate");
	 if(validateResult){		
		 ...
		 if(submitMaterFileJson||submitMaterFileJson==""){
			 ...
			 getPowSourceJson();
			 // 获取发证明细数据
			 getDjfzxxJson_zjdsyqscdj();
			 // 获取缴费信息
			 getDjjfxxJson_zjdsyqscdj();			 
			 //先获取表单上的所有值
			 var formData = FlowUtil.getFormEleData("T_BDCQLC_ZJSYSCDJ_FORM");
...
function initAutoTable(flowSubmitObj){
	...
	if(null != djfzxx_json && '' != djfzxx_json){
		...
		initDjfzxx_zjdsyqscdj(djfzxx_jsonItems);
	}
	...
	if(null != djjfmx_json && '' != djjfmx_json){
		...
		initDjjfxx_zjdsyqscdj(djjfmx_jsonItems);
	}
}
		...
		close: function () {
			...
			if(bdcdaxxcxInfo && bdcdaxxcxInfo.length == 1){
				...
				if(result.isPass == true){
					FlowUtil.initFormFieldValue(info,"T_BDCQLC_ZJSYSCDJ_FORM");		
