...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="sbgcTable">
	<tr>
	...
	<c:forEach items="${sbgcList}" var="sbgc" varStatus="s">
		<tr class="sbgcTr" id="sbgcTr_${s.index}">
			<input type="hidden" name="ROW_JSON" value="${sbgc.ROW_JSON}"/>
			<td style="text-align: center;">${sbgc.SINGLEPRONAME}</td>
			<td style="text-align: center;">
				<eve:eveselect clazz="tab_text validate[required] field_width" dataParams="PROTYPE"
				dataInterface="dictionaryService.findDatasForSelect" 
				defaultEmptyText="请选择工程类别" name="SINGLEPROMAINTYPE" id="SINGLEPROMAINTYPE" value="${sbgc.SINGLEPROMAINTYPE}">
				</eve:eveselect>
			</td>
			<td style="text-align: center;">${sbgc.STRUCTQUALTYPE}</td>
			<td style="text-align: center;">${sbgc.ARCHHEIGHT} ${sbgc.MUNILENGTH}</td>
			<td style="text-align: center;">${sbgc.ARCHAREA} ${sbgc.FLOORAREA}</td>
			<td style="text-align: center;">${sbgc.ITEMINVEST}</td>
			<td style="text-align: center;">
			<a href="javascript:void(0);" class="eflowbutton" onclick="getSbgcInfo('sbgc','${s.index}');">查 看</a>
			</td>
		</tr>
	</c:forEach>
</table>
