...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteHandledByMeTsjGridRecord() {
	...
	function revokeFlowExe() {
		var exeId = AppUtil.getEditDataGridRecord("HandledByMeTsjGrid");
		if (exeId) {
			art.dialog.confirm("您确定要追回该流程吗?", function() {
				...
				$.post("executionController.do?revokeFlow",{
						...
						if(resultJson.success == true){
							...
							$("#HandledByMeTsjGrid").datagrid('reload');
	...
	$(document).ready(function() {
		var handleByMeTsjStart = {
			elem : "#handleByMeTsjT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var handleByMeTsjEnd = {
			elem : "#handleByMeTsjT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
		...
		laydate(handleByMeTsjStart);
		laydate(handleByMeTsjEnd);

		AppUtil.initAuthorityRes("HandledByMeTsjToolbar");

	});
	
	/*序号列宽度自适应-----开始-----*/
	function fixRownumber(){
		var grid = $('#HandledByMeTsjGrid');  
...
<div class="easyui-layout" fit="true">
	<div region="center">
		<!-- =========================查询面板开始========================= -->
		<div id="HandledByMeTsjToolbar">
			<!--====================开始编写隐藏域============== -->
			<!--====================结束编写隐藏域============== -->
			<form action="#" name="HandledByMeTsjForm">
...
url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=FlowRunStatus',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 128,panelHeight: 'auto' " />
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;"><input type="text"
								...
								id="handleByMeTsjT.CREATE_TIME_BEGIN" name="Q_T.CREATE_TIME_&gt;=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="handleByMeTsjT.CREATE_TIME_END" name="Q_T.CREATE_TIME_&lt;=" /></td>
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('HandledByMeTsjToolbar','HandledByMeTsjGrid')" />
								<input type="button" value="重置" class="eve-button"
								onclick="AppUtil.gridSearchReset('HandledByMeTsjForm')" /></td>
						</tr>
					</tbody>
				</table>
			</form>
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="HandledByMeTsjGrid" fitcolumns="true" toolbar="#HandledByMeTsjToolbar"
			method="post" idfield="EXE_ID" checkonselect="false" 
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="serviceHandleController.do?handledByMe&Q_TWS.SXLX_EQ=3">
			<thead>
				<tr>
