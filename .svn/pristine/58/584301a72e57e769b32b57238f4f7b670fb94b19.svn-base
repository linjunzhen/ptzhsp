...
$(document).ready(function() {
 ...
 function searchData(){
	 ...
	 $.post("statisticsController.do?bjxlData",formData, function(responseText) {
		 var resultJson = $.parseJSON(responseText);
         myChart.hideLoading();
         myChart.clear();
         var o ;
         if(resultJson.echartType=="1"){
           o = getOptionBar(resultJson.TIME_LIST,resultJson.DAY_XLLIST,resultJson.PJ_XL);
         }else if(resultJson.echartType=="2"){
           o = getOptionPie(resultJson.TIME_LIST,resultJson.nameAndValueList,resultJson.PJ_XL);
         }
         // 填入数据
         myChart.setOption(o);
					...
					<tbody>
						<tr style="height:28px;">
							...
							<td style="width:135px;"><input type="text"
								...
								id="Sjattj.CREATE_TIME_BEGIN" name="Q_T.CREATE_TIME_>=" 
								value="${sTime}" /></td>
							<td style="width:68px;text-align:right;">结束日期</td>
							<td style="width:135px;"><input type="text"
								style="width:128px;float:left;" class="laydate-icon"
								id="Sjattj.CREATE_TIME_END" name="Q_T.CREATE_TIME_<=" 
								value="${eTime}"  /></td>
