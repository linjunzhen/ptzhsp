...
<base href="<%=basePath%>">
	...
	<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/jtjs/js/jtjs.js"></script>
	...
	<script type="text/javascript">
		$(function() {
			...
			if (flowSubmitObj) {
				...
				//初始化表单字段权限控制
				FlowUtil.initFormFieldRightControl(currentNodeFieldRights, "T_BDCQLC_JTJS_FORM");
				//初始化表单字段值
				if (flowSubmitObj.busRecord) {
					FlowUtil.initFormFieldValue(flowSubmitObj.busRecord, "T_BDCQLC_JTJS_FORM");
					...
					if (flowSubmitObj.busRecord.RUN_STATUS) {
						if (flowSubmitObj.busRecord.RUN_STATUS != 0) {
							...
							$("#spbdf").attr("onclick","goPrintTemplate('JTJSYDSYQSCDJSPB','3');");
							$("#spbsf").attr("onclick","goPrintTemplate('JTJSYDSYQSCDJSPB','3');");
						}
					...
					if (isEndFlow) {
						...
						//登记缴费和发证信息
						$("#djjfxx_jtjs").attr("style","");
						$("#djfzxx_jtjs").attr("style","");

						$("#T_BDCQLC_JTJS_FORM").find("input[type='text']").attr("disabled", "disabled");
						$("#T_BDCQLC_JTJS_FORM").find("input[type='button']").attr("disabled", "disabled");
						$("#T_BDCQLC_JTJS_FORM").find("input[type='checkbox']").attr("disabled", "disabled");
						$("#T_BDCQLC_JTJS_FORM").find("input[type='radio']").attr("disabled", "disabled");
						$("#T_BDCQLC_JTJS_FORM").find("input,select").attr("disabled", "disabled");
					}
				}else{
...
<body>
	...
	<form id="T_BDCQLC_JTJS_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/jtjsydsyqscdj/T_JTJSSCDJ_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
        ...
        <jsp:include page="./bdcqlc/common/djauditinfo.jsp">
            <jsp:param value="jtjs" name="domId" />
