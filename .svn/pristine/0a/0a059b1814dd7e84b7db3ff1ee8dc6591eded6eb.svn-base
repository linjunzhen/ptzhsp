...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("BusEsuperChangeGoForm", function(form, valid) {
			if (valid) {
				//将提交按钮禁用,防止重复提交
				$("input[type='submit']").attr("disabled", "disabled");
				var formData = $("#BusEsuperChangeGoForm").serialize();
				var url = $("#BusEsuperChangeGoForm").attr("action");
	...
	function toAddChangeWindow(){
		var unitCode = $("#BusEsuperChangeGoForm input[name='BUSSYS_UNITCODE']").val();
		var platCode = $("#BusEsuperChangeGoForm input[name='BUSSYS_PLATCODE']").val();
		var busCode = $("#BusEsuperChangeGoForm input[name='BUS_CODE']").val();
		if(!unitCode){
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="BusEsuperChangeGoForm" method="post"
		action="busEsuperChangeController.do?saveToChange">
		<!--==========隐藏域部分开始 ===========-->
		<!--==========隐藏域部分结束 ===========-->
