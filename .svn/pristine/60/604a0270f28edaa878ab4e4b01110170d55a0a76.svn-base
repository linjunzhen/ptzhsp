	...
	$(function(){
    	...
    	if(flowSubmitObj){
    		...
    		if(flowSubmitObj.busRecord){
    			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BSFW_XMSQBGZHPS_FORM");
    			
    			//获取表单字段权限控制信息
        		var currentNodeFieldRights = flowSubmitObj.currentNodeFieldRights;
        		 //初始化表单字段权限控制
        		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BSFW_XMSQBGZHPS_FORM");
    		}
    	}
		//初始化材料列表
	...
	function FLOW_SubmitFun(flowSubmitObj){
		 ...
		 if(validateResult){
			 ...
			 if(submitMaterFileJson||submitMaterFileJson==""){
				 ...
				 //先获取表单上的所有值
				 var formData = FlowUtil.getFormEleData("T_BSFW_XMSQBGZHPS_FORM");
