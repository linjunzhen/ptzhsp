...
<script type="text/javascript">
	$(function() {
		...
		$('#DSBDJG').prop("disabled","disabled");//地税比对结果不可编辑
		...
		if (flowSubmitObj) {
			...
			//初始化表单字段权限控制
			FlowUtil.initFormFieldRightControl(currentNodeFieldRights, "T_YBQLC_YRDWBG_FORM");
			//初始化表单字段值
			if (flowSubmitObj.busRecord) {
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord, "T_YBQLC_YRDWBG_FORM");
				if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES =='审查' || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
					 $("#T_YBQLC_YRDWBG_FORM").find("input,select,.Wdate").attr("disabled","disabled");				 
				}
				...
				if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES != '开始' || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){//除退回至开始环节外开放受理信息
		      	...
		      	if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES != '受理'){
	    	 		...
	    	 		//设置可编辑表格不可编辑
	    	 		var xzConfGrid1 = $('#paramConfGrid').datagrid('getColumnOption', 'XZ_TYPE');
					xzConfGrid1.editor={};
					var xzConfGrid2 = $('#paramConfGrid').datagrid('getColumnOption', 'ZSFS');
					xzConfGrid2.editor={};
					var xzConfGrid3 = $('#paramConfGrid').datagrid('getColumnOption', 'BEGIN_TIME');
					xzConfGrid3.editor={};
	    	 		...
	    	 		//审查，结束环节动态加载险种信息
					var url = "ybYrdwbgController.do?paramjson&ywId="+'${busRecord.YW_ID}';
...
<body>
	...
	<form id="T_YBQLC_YRDWBG_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<jsp:include page="./ybqlc/yrdwbg/yrdwbg.jsp" />
		</di>	
		<%--结束引入用人单位信息变更登记页面--%>
