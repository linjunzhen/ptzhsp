	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"YJDSGTLHSCLXD");
    		}
    	}
		//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("YJDSGTLHSCLXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("YJDSGTLHSCLXD");
				var url = $("#YJDSGTLHSCLXD").attr("action");
...
<body>
	<form id="YJDSGTLHSCLXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br> 《一阶段施工图》(修编版)联合审查联系单
				</div>
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt"
						...
						name="VB_GCXM_NAME_ST" maxlength="200" value="${SQData.blxmmc}" />项目的《一阶段施工图》图审工作已完成，现转给你单位。
					请于<input type="text" class="eui-ipt1 laydate-icon"
						data-validation-engine="validate[required]"
						style="width:150px;height: 36px;" name="VB_GCXM_TIME_ST"
						maxlength="10" readonly="readonly"
						onclick="laydate({istime: false, format: 'YYYY年MM月DD日'})" />前（ <input
						type="checkbox" name="VB_GCCNSJTEN_CK" value="1"> 10个工作日
					<input type="checkbox" name="VB_GCCNSJTWENTY_CK" value="1">
					20日）将项目相关的审查意见（ <input type="checkbox" name="VB_CBSJ_CK" value="1">
					初步设计（含概算） <input type="checkbox" name="VB_CS_CK" value="1">
					财审（含招标控制价） <input type="checkbox" name="VB_RFSPYJ_CK" value="1">
					人防审批意见 <input type="checkbox" name="VB_XFSPYJ_CK" value="1">
					消防审批意见 <input type="checkbox" name="VB_HYZGYJ_CK" value="1">
					行业主管意见 <input type="checkbox" name="VB_QT_CK" value="1">
					其他 <input type="text" class="eui-ipt"
						style="width:60px"
						name="VB_QTDETAIL_ST" />）提交我局。
					缺席联合审查或超期不反馈意见的，视为默认同意。其中，招标控制价超期未审定，将直接以概算90%作为招标控制价。
				</div>
				...
				<div class="eui-word-notes clearfix">
					<label>附件：</label><span> 1. 项目《一阶段施工图》（修编版）联合审查申请表；<br />
						2. <input type="text" class="eui-ipt"
						data-validation-engine="validate[required]"
						style="width:100px;font-size: 14px;"
						maxlength="200" value="${SQData.blxmmc}" />项目《一阶段施工图》（修编版）；<br />
