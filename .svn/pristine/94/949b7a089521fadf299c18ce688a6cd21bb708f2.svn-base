	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GHXZYDSQLXD");
    		}
    	}
		...
		AppUtil.initWindowForm("GHXZYDSQLXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GHXZYDSQLXD");
				var url = $("#GHXZYDSQLXD").attr("action");
...
<body>
	<form id="GHXZYDSQLXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt"
						...
						style="width:200px" name="VB_CGLSUBJECT_ST" maxlength="200"
						value="${SQData.blxmmc}" />项目的《项目建设方案》已完成，并已（
						<input type="checkbox" name="VB_GCZDXM_CK" value="1" >  纳入区年度重点项目 
						<input type="checkbox" name="VB_GCJCWJ_CK" value="1" >  引进该项目的决策文件），现转给你单位。请于<input
						type="text" class="eui-ipt laydate-icon"
						data-validation-engine="validate[required]"
						style="width:150px;height: 36px;" name="VB_CGLCN_TIME_ST"
						maxlength="10" readonly="readonly"
						onclick="laydate({istime: false, format: 'YYYY年MM月DD日'})" />前（7个工作日）将项目相关的规划选址及用地意见（
						<input type="checkbox" name="VB_GCGHXZYJ_CK" value="1" >  规划选址意见 
						<input type="checkbox" name="VB_GCYDYJ_CK" value="1" >  用地意见  
						<input type="checkbox" name="VB_GCYLYJ_CK" value="1" >  用林意见  
						<input type="checkbox" name="VB_GCYHYJ_CK" value="1" >  用海意见  
						<input type="checkbox" name="VB_GCSHWDFXYJ_CK" value="1" >  社会稳定低风险意见 
						<input type="checkbox" name="VB_GCHPYJ_CK" value="1" >  环评意见 
						<input type="checkbox" name="VB_GCSBYJ_CK" value="1" >  水保意见 
						<input type="checkbox" name="VB_GCQT_CK" value="1" >  其他
						<input type="text" name="VB_GCQTDETAIL_ST" class="eui-ipt" style="width:50px;"> ）提交我局，超期视为默认同意。
				</div>
				...
				<div class="eui-word-notes clearfix">
					<label>附件：</label><span>
					1.政府投资项目规划选址及用地申请表；<br />
					2.<input type="text" class="eui-ipt" 
						data-validation-engine="validate[required]" readonly="readonly"
						style="width:150px;font-size: 14px" name="VB_CGLFAXMMC_ST"
						value="${SQData.blxmmc}" />投资项目建设方案；<br />
					3.区年度重点项目通知或区管委会专题会议纪要。
					</span>
				</div>
				...
				<div class="eui-word-notes clearfix">
					<label>征求对象：</label>
					<span>区规划局、区环土局、区经发局（涉海）、区交建局（涉林）、片区开发管理局</span>
				</div>
			</div>
