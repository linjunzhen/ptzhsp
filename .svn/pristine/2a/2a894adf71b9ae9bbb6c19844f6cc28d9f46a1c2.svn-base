...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/zjfwsybgdj/js/zjfwsybgdj.js"></script>
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/js/bdcUtil.js"></script>
<script type="text/javascript">
	$(function() {
		<%-- 登记缴费信息、发证、归档信息隐藏 --%>
	    $("#djjfxx_zjfwsybgdj").attr("style","display:none;");
    	$("#djfzxx_zjfwsybgdj").attr("style","display:none;");
    	$("#djgdxx_zjfwsybgdj").attr("style","display:none;");
		...
		if (flowSubmitObj) {
			...
			//初始化表单字段权限控制
			FlowUtil.initFormFieldRightControl(currentNodeFieldRights, "T_BDCQLC_ZJFWSYBGDJ_FORM");			
			if (flowSubmitObj.busRecord) {
				//初始化表单字段值
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord, "T_BDCQLC_ZJFWSYBGDJ_FORM");
				//初始JSON格式信息
				...
				if( flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME != '开始') {
					...
					$("#dfspb").attr("onclick","goPrintTemplate('ZJFWSYBGDJJSPB','3')");
					$("#sfspb").attr("onclick","goPrintTemplate('ZJFWSYBGDJJSPB','3')");
					//开始、受理环节之后数据均不可编辑
					if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME!="受理" || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
						$("#T_BDCQLC_ZJFWSYBGDJ_FORM").find("input,select,.Wdate").attr("disabled","disabled");
						//a标签按钮禁用
	                ...
	                if(BDC_OPTYPE == "1"){//缮证（权证打印）                    
	                    ...
	                    $("#qzbsmsavebtn").removeAttr("disabled");
	                  	...
	                  	$("#BDC_QZVIEW").attr("style","");
	                   	$("#BDC_QZVIEW").removeAttr("disabled");
	                    //登记缴费、发证、归档信息
	                    $("#djjfxx_zjfwsybgdj").attr("style","");
    					$("#djfzxx_zjfwsybgdj").attr("style","");
    					$("#djgdxx_zjfwsybgdj").attr("style","");	                   	    					
    					$("#djjfxx_zjfwsybgdj").find("input,select,.Wdate").removeAttr("disabled");
    					$("#djfzxx_zjfwsybgdj").find("input,select,.Wdate").removeAttr("disabled");
    					$("#djgdxx_zjfwsybgdj").find("input,select,.Wdate").removeAttr("disabled");	                  
...
<body>
	...
	<form id="T_BDCQLC_ZJFWSYBGDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/zjfwsybgdj/T_ESTATE_BGDJ_ACCEPTINFO.jsp" /> 
		<%--结束引入受理信息--%>
		...
		<%--开始引入权利人信息--%>
		<jsp:include page="./bdcqlc/zjfwsybgdj/bdcQlrxx.jsp" />
		<%--结束引入权利人信息--%>
		
		<%--开始引入权属来源信息--%>
		<jsp:include page="./bdcqlc/zjfwsybgdj/bdcQsly.jsp" />
		<%--结束引入权属来源信息--%>
		
		<%--开始引入宗地信息-国有权力人信息--%>
  		<jsp:include page="./bdcqlc/zjfwsybgdj/bdcZdqlxx.jsp" />
 	    <%--开始引入宗地信息-国有权力人信息--%>
	    ...
	    <jsp:include page="./bdcqlc/common/djauditinfo.jsp">
	        <jsp:param value="zjfwsybgdj" name="domId" />
