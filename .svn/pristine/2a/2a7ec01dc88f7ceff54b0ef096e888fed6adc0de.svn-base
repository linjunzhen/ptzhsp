...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/ybqlc/cbryxxbg/js/cbryxxbg.js"></script>
		...
		$(function(){
			...
			if(flowSubmitObj){
				...
				$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
				 //初始化表单字段权限控制
				FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_YBQLC_CBRYXXBG_FORM");
				//初始化表单字段值
				if(flowSubmitObj.busRecord){
					
					FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_YBQLC_CBRYXXBG_FORM");
				}else{
...
<body>
	...
	<form id="T_YBQLC_CBRYXXBG_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入参保信息--%>
		<jsp:include page="./ybqlc/cbryxxbg/xxbg.jsp" />
		<%--结束引入参保信息 --%>
	</form>
</body>
</html>
