...
//***************************卓正PageOffice组件的使用********************************
	...
	Map map = (Map) request.getAttribute("documentTemplate");
	if(map.get("DOCUMENT_DOC")!=null&& !map.get("DOCUMENT_DOC").equals("")){
	    filepath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + path
	            + "/" + "attachFiles/documenttemplate/files/"+map.get("DOCUMENT_DOC").toString(); 
	}
	...
	if(map.get("DOCUMENT_DOC")!=null&& !map.get("DOCUMENT_DOC").equals("")){
	    poCtrl1.setSaveFilePage(request.getContextPath() + "/ticketsController/savefile.do?filename="+map.get("DOCUMENT_DOC")+"&type=documenttemplate");
	}else{
	poCtrl1.setSaveFilePage(request.getContextPath() + "/ticketsController/savefile.do?type=documenttemplate");
	}
   ...
   if(filepath!=null&&!filepath.equals("")){
    ...
    poCtrl1.setJsFunction_AfterDocumentSaved("updateDocument()");
...
function updateDocument(){
    ...
    AppUtil.ajaxProgress({
        url : "documentTemplateController.do?saveOrUpdate",
        params : {
            DOCUMENT_ID:$("input[name='documentid']").val(),
            DOCUMENT_DOC:strCustomSaveResult
            },
...
<body style="margin:0px;background-color: #f7f7f7;">
  ...
  <div class="easyui-layout" fit="true">
    ...
    <div data-options="region:'center',split:false">
        <input type="hidden" name="documentid" value="${documentTemplate.DOCUMENT_ID}">
