...
function addAuditTreeHoverDom(treeId, treeNode) {
	...
	function onAuditMoudleTreeClick(event, treeId, treeNode, clickFlag) {
		if(event.target.tagName=="SPAN"&&!treeNode.isParent){
			if(treeNode.id=="0"){
				$("#auditedContentToolbar input[name='MODULE_ID']").val("");
				$("#auditedContentToolbar input[name='Q_T.MODULE_ID_=']").val("");
			}else{
				$("#auditedContentToolbar input[name='MODULE_ID']").val(treeNode.id);
				$("#auditedContentToolbar input[name='Q_T.MODULE_ID_=']").val(treeNode.id);
			}
			
			AppUtil.gridDoSearch("auditedContentToolbar","auditedContentGrid");
	...
	function deleteAuditContentDataGridRecord() {
		AppUtil.deleteDataGridRecord(
				"contentController.do?multiDel", "auditedContentGrid");
	};
	...
	function previewAuditContentDataGridRecord(){
		var TID = AppUtil.getEditDataGridRecord("auditedContentGrid");
		var $dataGrid = $("#auditedContentGrid");
	 ...
	 * 审核文章
	 */
	function auditedContentDataGridRecord() {
		var contentId = AppUtil.getEditDataGridRecord("auditedContentGrid");
		if (contentId) {
			showAuditedWindow(contentId);
		}
	}
	/**
	 * 显示审核文章对话框
	 */
	function showAuditedWindow(contentId) {
		$.dialog.open("auditContentController.do?viewinfo&contentId=" + contentId, {
			title : "审核信息",
			width : "800px",
			height : "460px",
			lock : true,
...
<div class="easyui-layout" fit="true">
	...
	<div data-options="region:'center',split:false" >
				...
				<div class="e-toolbar" style="z-index: 1111;top: 0px;left: 0px;">
					<div class="e-toolbar-ct">
						<div class="e-toolbar-overflow">	
							<a href="#" class="easyui-linkbutton" iconCls="eicon-list" plain="true"
								onclick="auditedContentDataGridRecord();">审核明细</a>
						</div>
					</div>
				</div>
			</div>
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="auditedContentGrid" fitColumns="true" toolbar="#auditedContentToolbar"
			method="post" idField="TID" checkOnSelect="true" nowrap="false"
			selectOnCheck="true"  fit="true" border="false"
			data-options="onLoadSuccess:function(){
			     var TID = $('#auditedContentToolbar input[name=TID]').val();
			     if(TID&&TID!='0'){
			          $(this).datagrid('enableDnd');
			     }
			}"
			url="moduleController.do?auditedDatagrid">
			<thead>
				<tr>
