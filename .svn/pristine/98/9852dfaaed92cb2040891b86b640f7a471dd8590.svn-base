...
<body class="bg-none" ondragstart="return false" oncontextmenu="return false" onselectstart="return false">
    ...
    <div class="scroll-left">
        <div class="bd">
           	<c:forEach items="${itemList}" var="itemInfo" varStatus="infoStatus">
           		<c:choose>
	           		<c:when test="${infoStatus.index==0}">
           				<ul class="scroll-list nocolor">
							<li class="color1"><a href="javascript:void(0);" onclick="chooseMaterType('${itemInfo.ITEM_CODE}','${itemInfo.ITEM_NAME}','${itemInfo.DEF_KEY}')" >${itemInfo.ITEM_NAME}</a></li>
						</c:when>
    ...
    <script type="text/javascript">
		...
		function chooseMaterType(itemCode,itemName,defKey){					
	    	location.href="newCallController.do?toMaterTypeChoose&itemCode="+itemCode+"&itemName="+itemName+"&defKey="+defKey+
	    	"&roomNo=${roomNo}&businessCode=${businessCode}&businessName=${businessName}&departId=${departId}";
		}
