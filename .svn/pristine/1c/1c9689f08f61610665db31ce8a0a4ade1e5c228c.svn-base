 ...
 * 描述
 * @author Danto Huang
 * @created 2018年8月7日 下午4:41:27
 */
@Repository("preApprovalDao")
public class PreApprovalDaoImpl extends BaseDaoImpl implements PreApprovalDao {

    /**
     * 
     * 描述
     * @author Danto Huang
     * @created 2018年8月8日 下午4:49:47
     * @return
     * @see net.evecom.platform.smoga.dao.PreApprovalDao#getMaxSn()
     */
    public int getMaxSn() {
        StringBuffer sql = new StringBuffer("select decode(max(C_SN),null,0,max(C_SN)) FROM T_SMOGA_PREAPPROVAL ");
    ...
    public int getCurVersion() {
        ...
        sql.append("select decode(max(C_VERSION),null,1,max(C_VERSION)) FROM T_SMOGA_PREAPPROVAL ");
        return this.jdbcTemplate.queryForInt(sql.toString());
    }
     ...
     * 描述
     * @author Danto Huang
     * @created 2018年8月14日 上午9:35:01
     * @param variables
     * @return
     * @see net.evecom.platform.smoga.dao.PreApprovalDao#saveOrUpdateRecord(java.util.Map)
     */
    public String saveOrUpdateRecord(Map<String, Object> variables){
        String rightId = variables.get("PRE_ID").toString();
        ...
        if(StringUtils.isNotEmpty(rightId)&&isExist(rightId,version)){
            ...
            Set<String> columns = this.getColumnNameByTableName("T_SMOGA_PREAPPROVAL_HIS");
            Map<String,Object> insertMap = new HashMap<String,Object>();
            rightId = UUIDGenerator.getUUID();
            insertMap.put("PRE_ID", rightId);
            StringBuffer sql = new StringBuffer("insert into T_SMOGA_PREAPPROVAL_HIS (");
    ...
    private boolean isExist(String rightId,String version){
        StringBuffer sql = new StringBuffer("select count(*) from T_SMOGA_PREAPPROVAL_HIS ");
        sql.append(" where PRE_ID=? and C_VERSION=? ");
    ...
    private void updateRecord(Map<String,Object> variables){
        StringBuffer sql = new StringBuffer("update T_SMOGA_PREAPPROVAL_HIS set ");
        // 获取业务表的列名称
        Set<String> columns = this.getColumnNameByTableName("T_SMOGA_PREAPPROVAL_HIS");
        //获取业务表大字段的列名称
        List<String> bigColumns=this.getBigFiled("T_SMOGA_PREAPPROVAL_HIS");
        // 设置更新目标列
        List<String> targetCols = new ArrayList<String>();
        // 定义更新的目标值
        ...
        while (it.hasNext()) {
            ...
            if (columns.contains(fieldName.toUpperCase())
                    && !fieldName.toUpperCase().equals("PRE_ID")&& !fieldName.toUpperCase().equals("C_VERSION")) {
        ...
        sql.deleteCharAt(sql.length() - 1);
        sql.append(" where PRE_ID=? and C_VERSION=? ");
        targetVals.add(variables.get("PRE_ID"));
