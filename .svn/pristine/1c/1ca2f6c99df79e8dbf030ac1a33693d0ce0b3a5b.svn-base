	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZBAQSD");
    		}
    	}
		
		AppUtil.initWindowForm("SHTZBAQSD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZBAQSD");
				var url = $("#SHTZBAQSD").attr("action");
...
<body>
	<form id="SHTZBAQSD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《项目备案请示+》（送审版）<br />综合（专家）评审建议意见签收单
				</div>
				...
				<div class="eio-word-content">	
						...
						项目《项目备案请示+》（送审版）综合（专家）评审已完成，现将《项目综合（专家）评审会建议意见汇总表》转交给你单位，请及时组织修编，并申请修编版批复。
				</div>
