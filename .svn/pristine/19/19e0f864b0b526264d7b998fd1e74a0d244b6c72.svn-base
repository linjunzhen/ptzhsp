...
<script>
    ...
    /*初始化涉税登记材料列表*/
    function initTaxRelatedFileList(fileIds) {
        $.post("bdcGyjsjfwzydjController.do?taxRelatedFileList", {fileIds: fileIds}, function (data) {
            if (data) {
                ...
                for (let i = 0; i < json.length; i++) {
                    ...
                    str += "<a href=\"javascript:void(0);\" id='myBtn' onclick=\"delUploadFile('" + fileId + "');\"><font color=\"red\">删除</font></a></p>";
                }
                $("#taxRelatedBuyFile").append(str);
            }
        });
    }
    ...
    function openTaxRelatedFileUploadDialog() {
        //上传框类型:video-视频 attach-附件 audio-音频 flash-flash image-图片
        art.dialog.open('fileTypeController.do?openUploadDialog&attachType=attachToImage&materType=image&busTableName=T_BDCQLC_GYJSJFWZYDJ', {
            title: "上传(附件)",
            width: "650px",
            height: "300px",
            lock: true,
                ...
                if(uploadedFileInfo){
                    if(uploadedFileInfo.fileId){
                        ...
                        if (fileid != "" && fileid != null) {
                            $("input[name='SSDJ_BUY_FILEID']").val(fileid + "," + uploadedFileInfo.fileId);
                        } else {
                            $("input[name='SSDJ_BUY_FILEID']").val(uploadedFileInfo.fileId);
                        }
                        ...
                        $("#taxRelatedBuyFile").append(spanHtml);
    ...
    function delUploadFile(fileId) {
        art.dialog.confirm("您确定要删除该文件吗?", function() {
            ...
            $("input[name='SSDJ_BUY_FILEID']").val(arrayIds.join(","));
        });
    }

    function initSsdjStatus(flowSubmitObj) {
        if (flowSubmitObj.busRecord.IS_TAX_SUCCESS) {
            if (flowSubmitObj.busRecord.IS_TAX_SUCCESS == '1') {
                $("#isTaxSuccess").text("（已申报）");
                $("#getSsdjInfoBtn").show();
            } else {
                $("#isTaxSuccess").text("（未申报）")
                $("#getSsdjInfoBtn").hide();
            }
        } else {
            $("#isTaxSuccess").text("（未申报）")
    ...
    * */
    ...
    function saveTaxInfo() {
        ...
        taxFormData.SSDJ_BUY_FILEID = $("input[name='SSDJ_BUY_FILEID']").val();
        AppUtil.ajaxProgress({
            ...
            callback:function (data) {
                if (data.success) {
                    ...
                    // if (data.fileSuccess) {
                    //     $("#IS_TAXFILE_SUCCESS").val('1');
                    //     $("#sendSsdjFileInfoBtn").hide();
                    // } else {
                    //     $("#IS_TAXFILE_SUCCESS").val('0');
                    //     $("#sendSsdjFileInfoBtn").show();
                    // }
                } else {
                    art.dialog({
                        content: data.msg,
    ...
    function sendSsdjFileInfo() {
        var taxFormData = {};
        var flowSubmitObj = FlowUtil.getFlowObj();
        var SSDJ_BUY_FILEID = $("input[name='SSDJ_BUY_FILEID']").val();
        if (!(SSDJ_BUY_FILEID && SSDJ_BUY_FILEID != '')) {
            art.dialog({
                content: "未上传附件，无法发送！",
                icon: "error",
                ok: true
        ...
        taxFormData.EXE_ID = flowSubmitObj.EFLOW_EXEID;
        taxFormData.YW_ID = flowSubmitObj.EFLOW_BUSRECORDID;
        taxFormData.IS_TAXFILE_SUCCESS = $("#IS_TAXFILE_SUCCESS").val();
        taxFormData.SSDJ_BUY_FILEID = SSDJ_BUY_FILEID;
        AppUtil.ajaxProgress({
            url:"bdcSsdjController/sendBdcSsdjFileInfo.do",
            params:taxFormData,
            callback:function (data) {
                if (data) {
                    if (data.fileSuccess) {
...
<%--涉税登记申报状态--%>
<input type="hidden" name="IS_TAXFILE_SUCCESS" id="IS_TAXFILE_SUCCESS" value="${busRecord.IS_TAXFILE_SUCCESS}"/>
<%--涉税登记买方附件ID--%>
<input type="hidden" name="SSDJ_BUY_FILEID" id="SSDJ_BUY_FILEID" value="${busRecord.SSDJ_BUY_FILEID}">
...
<div id="taxRelatedInfo" style="display:none;">
    ...
    <div id="ssdjTypeSelect">
        <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
            <tr>
                <th colspan="2">
                    <span>涉税登记<span id="isTaxSuccess" style="color: red;"></span>
                        ...
                        <input type="button" style="float:right;margin-right: 10px;margin-top: 2px;" id="sendSsdjFileInfoBtn"  class="eflowbutton" value="发送附件" onclick="sendSsdjFileInfo();">
                    </span>
            ...
            <tr id="upload">
                <td class="tab_width">符合减免条件的有关材料：</td>
                <td>
                    <div style="margin-top: 4px;">
                        <a href="javascript:void(0);" onclick="openTaxRelatedFileUploadDialog()">
                            <img src="webpage/bsdt/applyform/images/tab_btn_sc.png" style="width:60px;height:22px;">
                        </a>
                        <span id="taxRelatedBuyFile"></span>
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
            ...
            <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
                <tr>
                    ...
                    <td style="display: none;" class="tab_width"><font class="tab_color">*</font>房屋属地纳税机关：</td>
                    <td style="display: none;">
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJSWJG"
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJZLFQSZYDX"
                                       dataInterface="dictionaryService.findDatasForSelect"
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJQSZYYT"
                                       dataInterface="dictionaryService.findDatasForSelect"
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJJDXZDM"
                                       dataInterface="dictionaryService.findDatasForSelect"
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJZLFCX"
                ...
                <tr>
                    ...
                    <td class="tab_width"><font class="tab_color">*</font>交易价格（元）：</td>
                    <td>
                ...
                <tr>
                    <td class="tab_width"><font class="tab_color">*</font>单价（元）：</td>
                    <td>
                    ...
                    <td class="tab_width"><font class="tab_color">*</font>合同金额（元）：</td>
                    <td>
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJQSZYFS"
                                       dataInterface="dictionaryService.findDatasForSelect"
                                       defaultEmptyValue="21" name="SSDJ_FYXX_QSZYFS" id="SSDJ_FYXX_QSZYFS" value="${busRecord.SSDJ_FYXX_QSZYFS}">
                        </eve:eveselect>
                    </td>
                    <td class="tab_width"><font class="tab_color">*</font>当期应收款金额（元）：</td>
                    <td>
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJJZJGLX"
                                       dataInterface="dictionaryService.findDatasForSelect"
        ...
        <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
            ...
            <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
                <tr>
                    ...
                    <td style="display: none;" class="tab_width"><font class="tab_color">*</font>房屋属地纳税机关：</td>
                    <td style="display: none;">
                        <eve:eveselect clazz="tab_text seletWidth validate[required]" dataParams="SSDJSWJG"
                                       dataInterface="dictionaryService.findDatasForSelect"
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJFCXZ"
                                       dataInterface="dictionaryService.findDatasForSelect"
                ...
                <tr>
                    <td style="display: none;" class="tab_width"><font class="tab_color">*</font>朝向：</td>
                    <td style="display: none;">
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJCLFPGCX"
                ...
                <tr>
                    ...
                    <td class="tab_width"><font class="tab_color">*</font>本次交易金额（元）：</td>
                    <td>
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJFCLX"
                                       dataInterface="dictionaryService.findDatasForSelect"
                                       defaultEmptyValue="01" name="SSDJ_FWXX_FCLX" id="SSDJ_FWXX_FCLX" value="${busRecord.SSDJ_FWXX_FCLX}">
                        </eve:eveselect>
                    </td>
                    <td class="tab_width"><font class="tab_color">*</font>权属转移对象：</td>
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJCLFQSZYDX"
                                       dataInterface="dictionaryService.findDatasForSelect"
...
<%--                        <input type="text" class="tab_text" maxlength="32" id="SSDJ_FWXX_PQMC" name="SSDJ_FWXX_PQMC" value="${busRecord.SSDJ_FWXX_PQMC}">--%>
                ...
                <tr>
                    ...
                    <td>
                        <eve:eveselect clazz="tab_text validate[required]" dataParams="YORN"
                                       dataInterface="dictionaryService.findDatasForSelect"
                                       defaultEmptyValue="Y" name="SSDJ_FWXX_YWDT" id="SSDJ_FWXX_YWDT" value="${busRecord.SSDJ_FWXX_YWDT}">
                        </eve:eveselect>
                    </td>
                    <td class="tab_width">备注：</td>
                    <td>
            ...
            <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
                ...
                <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
                    <tr>
                        ...
                        <td>
                            <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJGJ"
                                           dataInterface="dictionaryService.findDatasForSelect"
                    ...
                    <tr>
                        ...
                        <td>
                            <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJZZLX"
                                           dataInterface="dictionaryService.findDatasForSelect"
                    ...
                    <tr>
                        ...
                        <td>
                            <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJBUYFWTC"
                                           dataInterface="dictionaryService.findDatasForSelect"
                        ...
                        <td>
                            <eve:eveselect clazz="tab_text validate[required]" dataParams="YORN"
                                           dataInterface="dictionaryService.findDatasForSelect"
                                           name="SSDJ_BUY_SFYWS" defaultEmptyValue="N" id="SSDJ_BUY_SFYWS" value="${busRecord.SSDJ_BUY_SFYWS}">
                            </eve:eveselect>
                        </td>
                    </tr>
                    <tr>
            ...
            <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
                ...
                <table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
                    ...
                    <tr>
                        <td class="tab_width"><font class="tab_color">*</font>房屋套次：</td>
                        <td>
                            <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJSELLFWTC"
                                           dataInterface="dictionaryService.findDatasForSelect"
                        ...
                        <td>
                            <eve:eveselect clazz="tab_text validate[required]" dataParams="YORN"
                                           dataInterface="dictionaryService.findDatasForSelect"
                                           defaultEmptyValue="N" name="SSDJ_SELL_SFYWS" id="SSDJ_SELL_SFYWS" value="${busRecord.SSDJ_SELL_SFYWS}">
                            </eve:eveselect>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="tab_width"><font class="tab_color">*</font>上次取得房屋成本（元）：</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            <eve:eveselect clazz="tab_text validate[required]" dataParams="SSDJSCQDFWFS"
                                           dataInterface="dictionaryService.findDatasForSelect"
