...
<script type="text/javascript">
	$(function(){
    	...
    	if(flowSubmitObj){
    		...
    		var currentNodeFieldRights = flowSubmitObj.currentNodeFieldRights;
    		 //初始化表单字段权限控制
    		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BSFW_SHTZXMGH_FORM");
    		//初始化表单字段值
    		if(flowSubmitObj.busRecord){
    			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BSFW_SHTZXMGH_FORM");
    		}
    	}
	...
	function FLOW_SubmitFun(flowSubmitObj){
		 //先判断表单是否验证通过
		 var validateResult =$("#T_BSFW_SHTZXMGH_FORM").validationEngine("validate");
		 if(validateResult){
			 ...
			 if(submitMaterFileJson||submitMaterFileJson==""){
				 ...
				 //先获取表单上的所有值
				 var formData = FlowUtil.getFormEleData("T_BSFW_SHTZXMGH_FORM");
...
<body>
   ...
   <form id="T_BSFW_SHTZXMGH_FORM" method="post" >
    <%--===================重要的隐藏域内容=========== --%>
