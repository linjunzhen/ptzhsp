...
<div class="addBox">
	...
	<table cellpadding="0" cellspacing="0" class="syj-table2">
		<tr>
			<th>原股东</th>
			<td><input type="text" class="syj-input1" readonly="readonly"
					name="${s.index}SHAREHOLDER_NAME" value="${holder.SHAREHOLDER_NAME}" />
			   <input type="button" value="展开"  onclick="displayOrShowOldGdxx(this);" class="addbtn"/>
			</td>
		</tr>
	</table>
	<table cellpadding="0" cellspacing="0" class="syj-table2 " style="display:none">	
		...
		<tr>
			...
			<td>
				...
				defaultEmptyText="请选择证件类型" name="${s.index}LICENCE_TYPE" value="${holder.LICENCE_TYPE}">
				</eve:eveselect>
			</td>
		...
		<tr>
			...
			<td colspan="3"><input type="text" class="syj-input1 w878 validate[required]"
				name="${s.index}ID_ADDRESS" placeholder="请输入证件地址，请按身份证上的地址申报" maxlength="64" value="${holder.ID_ADDRESS}"/></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td colspan="3"><input type="text" class="syj-input1 validate[],custom[JustNumber]"
				...
				onblur="onlyNumber(this);setInvestment();isEqualToCash(this.value);" onkeyup="onlyNumber(this);" style="width: 638px;"  value="${holder.INVESTMENT_CASH}"/>
		...
		<tr>
			...
			<td><input type="text" class="syj-input1 inputBackgroundCcc validate[required]"
				name="${s.index}CONTRIBUTIONS"  placeholder="请填写股东出资方式" readonly="readonly"  value="${holder.CONTRIBUTIONS}"/>
				<input type="hidden" name="${s.index}CONTRIBUTIONS_REMAIN" value="${holder.CONTRIBUTIONS_REMAIN}" />
				<input type="hidden" name="${s.index}CONTRIBUTIONS_PRO" value="${holder.CONTRIBUTIONS_PRO}" />
			</td>
		...
		<tr>
			...
			<td><input type="text"  class="syj-input1 validate[required,custom[JustNumber]]"  onblur="onlyNumber(this);isEqualToSjcze(this.value);setPayTime(this);" onkeyup="onlyNumber(this);"
		     ...
		     <th ><font class="syj-color2" id="jfrq" style="display:none">*</font>实缴时间：</th>
					  ...
					  name="${s.index}PAYMENT_TIME"  placeholder="请选择实缴时间"  value="${holder.PAYMENT_TIME}" /></td>
		</tr>
	</table>
</div>
</c:forEach>
