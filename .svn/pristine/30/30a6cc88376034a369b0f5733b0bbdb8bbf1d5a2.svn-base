	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZHZLHSCLXD");
    		}
    	}
		//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZHZLHSCLXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZHZLHSCLXD");
				var url = $("#SHTZHZLHSCLXD").attr("action");
...
<body>
	<form id="SHTZHZLHSCLXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br> 《项目申请报告+》（修编版）联合审查联系单
				</div>
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt"
						...
						name="VB_GCXM_NAME_ST" maxlength="200" value="${SQData.blxmmc}" />项目的《项目申请报告+》（修编版）已完成，现转给你单位。
				...
				<div class="eui-word-notes clearfix">
					<label>附件：</label><span> 1. 《项目申请报告+》（修编版）联合审查申请表；<br />
						2. <input type="text" class="eui-ipt"
						data-validation-engine="validate[required]"
						style="width:100px;font-size: 14px;"
						maxlength="200" value="${SQData.blxmmc}" />《项目申请报告+》（修编版）；<br />
