...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("BusSpecialChangeForm", function(form, valid) {
			if (valid) {
				//将提交按钮禁用,防止重复提交
				$("input[type='submit']").attr("disabled", "disabled");
				var formData = $("#BusSpecialChangeForm").serialize();
				var url = $("#BusSpecialChangeForm").attr("action");
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="BusSpecialChangeForm" method="post"
		action="busSpecialChangeController.do?saveEditSpecial">
		<!--==========隐藏域部分开始 ===========-->
		<input type="hidden" name="BUS_ID" value="${busSpecial.BUS_ID}">
		<input type="hidden" name="CHANGE_ID" value="${busSpecial.CHANGE_ID}">
		...
		<table style="width:100%;border-collapse:collapse;"
			...
			<tr>
				<td colspan="2"><span style="width: 150px;float:left;text-align:right;">业务单位：</span>
					<c:if test="${busSpecial.CHANGE_ID!=null}">
					...
					</c:if><c:if test="${busSpecial.CHANGE_ID==null}">
					<input class="easyui-combobox" style="width:250px;" name="UNIT_CODE" value="${busSpecial.UNIT_CODE}"
						data-options="url:'departmentController.do?load&defaultEmpty=true',editable:false,
			...
			<tr>
				<td><span style="width: 150px;float:left;text-align:right;">专项编码：</span>
					<c:if test="${busSpecial.CHANGE_ID!=null}">
					   <input type="text" class="eve-input validate[required]" style="width:250px;float:left;"  value="${busSpecial.BUS_CODE}" name="BUS_CODE" id="BUS_CODE" readonly="readonly" />
					</c:if> 
					<c:if test="${busSpecial.CHANGE_ID==null}">
