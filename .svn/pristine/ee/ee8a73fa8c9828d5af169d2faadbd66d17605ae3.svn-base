	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GCKXXYJBGPSHJYYJHZ");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("GCKXXYJBGPSHJYYJHZ", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GCKXXYJBGPSHJYYJHZ");
				var url = $("#GCKXXYJBGPSHJYYJHZ").attr("action");
...
<body>
	<form id="GCKXXYJBGPSHJYYJHZ" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目<br />《工程可行性研究报告+》（送审版）综合<br />(专家)评审建议意见汇总表
				</div>
				<br>
				...
				<br>
					...
					onpropertychange="this.style.height=this.scrollHeight + 'px'"
					oninput="this.style.height=this.scrollHeight + 'px'"
					name="VB_KXXBGXMYJ_TDC_ST" ></textarea><br>
					&nbsp;&nbsp;区重点办意见：<br>
					&nbsp;&nbsp;<textarea rows="6" 
					style="width: 95%;font-size:21px; vertical-align:top; overflow: hidden;line-height: 36px;"
					onpropertychange="this.style.height=this.scrollHeight + 'px'"
					...
					onpropertychange="this.style.height=this.scrollHeight + 'px'"
					oninput="this.style.height=this.scrollHeight + 'px'"
					name="VB_KXXBGXMYJ_QT_ST" ></textarea><br>
				</div>
