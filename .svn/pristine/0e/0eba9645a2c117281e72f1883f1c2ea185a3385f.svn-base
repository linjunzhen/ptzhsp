...
<script type="text/javascript"
	src="<%=basePath%>/webpage/bsdt/applyform/js/commercial.js"></script>
	...
	<form id="T_COMMERCIAL_DOMESTIC_FORM" method="post">
		...
		<table cellpadding="0" cellspacing="1" class="tab_tk_pro2" id="gslxtable">
			...
			<tr>
				...
				<td colspan="3">					
					...
					<input name="COMPANY_SETNATURE" id="sllx" class="easyui-combobox" style="width:182px; height:26px;" 
						data-options="			                
			...
			<tr>
				<td class="tab_width"><font class="tab_color">*</font>企业名称${busRecord.IS_FREE_ENGRAVE_SEAL}：</td>
				<td colspan="3">
		...
		<div id="jbxx" >
			...
			<c:if test="${busRecord.SSSBLX!='1'}">
	        	<jsp:include page="./T_COMMERCIAL_COMPANY.jsp" />
			</c:if>
			<!-- 秒批-->
			<c:if test="${busRecord.SSSBLX=='1'}">
				<jsp:include page="./T_COMMERCIAL_COMPANY_MP.jsp" />
			</c:if>
		</div>
		<%--结束引入企业基本信息界面 --%>
		...
		<%--开始引入企业资金信息界面 --%>
		<div id="zjxx" >
		<jsp:include page="./T_COMMERCIAL_CAPITAL.jsp" />
		</div>
		<%--结束引入企业资金信息界面 --%>

		<%--开始引入股东信息界面 --%>
		<div id="gdxx" >
		<jsp:include page="./T_COMMERCIAL_SHAREHOLDER.jsp" />
		</div>
		<%--结束引入股东信息界面 --%>
		...
		<%--开始引入法人信息界面 --%>
		<div id="frxx" >
		<jsp:include page="./T_COMMERCIAL_LEGAL.jsp" />
		</div>
		<%--结束引入法人信息界面 --%>
		...
		<%--开始引入社会保险信息界面 --%>
		<c:if test="${busRecord.SSSBLX!='1' && busRecord.ISSOCIALREGISTER == '1'}">
		<div id="sbxx" >
