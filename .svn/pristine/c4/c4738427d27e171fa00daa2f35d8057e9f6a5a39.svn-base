	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"TYYSSLDCWJS");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("TYYSSLDCWJS", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("TYYSSLDCWJS");
				var url = $("#TYYSSLDCWJS").attr("action");
...
<body>
	<form id="TYYSSLDCWJS" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt" data-validation-engine="validate[required]"
					 style="width:135px;" name="VB_CGXMSQSJ_ST" readonly="readonly" value="${SQData.xmsqsj}" />
					提请的	
					<input
						type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:273px"  name="VB_CGLSUBJECT_ST" maxlength="150" value="${SQData.blxmmc}" />
						项目竣工财务决算审核申请材料(
						<input type="checkbox" name="VB_JSSQB_CK" value="1" /> 项目统一竣工财务决算申请表
						<input type="checkbox" name="VB_CNS_CK" value="1" /> 申请办理行政审批事项承诺书
						<input type="checkbox" name="VB_GCJS_CK" value="1" /> 项目工程结算
						<input type="checkbox" name="VB_CWJS_CK" value="1" /> 项目工程竣工财务决算
						...
						(
						<input
						type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:30px"  name="VB_CGLDAYS_ST" maxlength="200" />
						个工作日)凭此单到区行政服务中心大厅我局项目投资窗口领取《联合竣工验收验收意见书》。
				</div>
