...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteHandleMonitorRecord() {
	...
	* 终结流程实例
	*/
	function endHandleMonitorRecord(){
	...
	*/
	function changeFlowBranch(){
		var entityId = AppUtil.getEditDataGridRecord("handleMonitorGrid");
		if (entityId) {
	...
	$(document).ready(function() {
		var handleMonitorStart = {
			elem : "#handleMonitor.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var handleMonitorEnd = {
			elem : "#handleMonitor.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
		...
		laydate(handleMonitorStart);
		laydate(handleMonitorEnd);
	});
					...
					<div class="e-toolbar-ct">
						<div class="e-toolbar-overflow">
						   <a href="#"
								...
								onclick="endHandleMonitorRecord();">终结流程实例</a> 
						   <a href="#"
								class="easyui-linkbutton" 
						   ...
						   <a href="#"
								...
								onclick="deleteHandleMonitorRecord();">删除流程实例</a>
						</div>
					</div>
				</div>
			</div> -->
			<form action="#" name="HandleMonitorForm">
...
url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=FlowRunStatus',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 128,panelHeight: 'auto' " />
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;"><input type="text"
								...
								id="handleMonitor.CREATE_TIME_BEGIN" name="Q_T.CREATE_TIME_&gt;=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="handleMonitor.CREATE_TIME_END" name="Q_T.CREATE_TIME_&lt;=" /></td>
						...
						<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('handleMonitorToolbar','handleMonitorGrid')" />
								<input type="button" value="重置" class="eve-button"
								onclick="$('#handleMonitorToolbar input[name=\'Q_tsd.DEPART_ID_=\']').val('');AppUtil.gridSearchReset('HandleMonitorForm')" /></td>
							</tr>
					</tbody>
				</table>
			</form>
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="handleMonitorGrid" fitcolumns="true" toolbar="#handleMonitorToolbar"
			method="post" idfield="EXE_ID" checkonselect="false" nowrap="false"
			selectoncheck="false" fit="true" border="false"
			url="serviceHandleController.do?handleMonitor&Q_T.RUN_STATUS_NEQ=0&Q_TWS.SXLX_EQ=1">
			<thead>
				<tr>
