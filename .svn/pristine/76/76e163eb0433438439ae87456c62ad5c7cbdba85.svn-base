...
<head>
	...
	<script type="text/javascript">
		$(function() {
			AppUtil.initWindowForm("ZrztxxForm", function(form, valid) {
				if (valid) {
					//将提交按钮禁用,防止重复提交
					$("input[type='submit']").attr("disabled","disabled");
					var formData = $("#ZrztxxForm").serialize();
					var url = $("#ZrztxxForm").attr("action");
					AppUtil.ajaxProgress({
						...
						callback : function(resultJson) {
							if(resultJson.success){
								...
								parent.$("#cjgfzrztGrid").datagrid("reload");
								AppUtil.closeLayer();
							}else{
								parent.art.dialog({
									content: resultJson.msg,
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="ZrztxxForm" method="post" action="projectFinishManageController.do?saveOrUpdateZrzt">
	    <div>
	     	<!--==========隐藏域部分开始 ===========-->
