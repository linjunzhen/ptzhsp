...
$(function(){
	...
	if(flowSubmitObj){
		...
		$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
		 //初始化表单字段权限控制
		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDC_ZJSYSCDJ_FORM");
		//初始化表单字段值
		if(flowSubmitObj.busRecord){
			initAutoTable(flowSubmitObj);//初始化权利信息
			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDC_ZJSYSCDJ_FORM");
			if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始'){
				$("#T_BDC_ZJSYSCDJ_FORM input").each(function(index){
					$(this).attr("disabled","disabled");
				});
				$("#T_BDC_ZJSYSCDJ_FORM select").each(function(index){
					$(this).attr("disabled","disabled");
				});
...
function FLOW_SubmitFun(flowSubmitObj){
	 //先判断表单是否验证通过
	 var validateResult =$("#T_BDC_ZJSYSCDJ_FORM").validationEngine("validate");
	 if(validateResult){
		 getPowerInfoJson();
	     ...
	     if(isAuditPass == "-1"){
			 ...
			 if(submitMaterFileJson||submitMaterFileJson==""){
				 ...
				 //先获取表单上的所有值
				 var formData = FlowUtil.getFormEleData("T_BDC_ZJSYSCDJ_FORM");
...
<body>
	...
	<form id="T_BDC_ZJSYSCDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
