...
<script type="text/javascript">
	...
	function doSelectRows(){
		...
		if(valAll != "" && valAll != "-1"){
			updateOrderStatus(exeIdAll,valAll,"1");
		}
	...
	function submitAjaxJson(){
		...
		if(sssbSfxxInfo && sssbSfxxInfo.length >= 1){
			...
			$.post("bdcDdjfxxController.do?pushOrderDetailInfo", {
				"ORDER_TOTALPRICE" : sssbSfTotalPrice,
				"EXE_ID":exeId,
				"UPDATEORDERSTATUS": valAll, 
	...
	//订单状态变更
	function updateOrderStatus(exeId,val,opType){
		exeIdAll = exeId;
	    valAll = val;
		if(opType == "0"){
			return;
		...
		$.post('bdcDdjfxxController.do?saveOrUpdate',{
			KeyName:"EXE_ID",
			EXE_ID:exeId,
			ORDER_STATUS:val,
		    }, function(responseText, status, xhr) {
...
<%-- 							<td>
								<select id="orderStatusSelect" class="tab_text " style="width:150px;" onchange='updateOrderStatus("${exeId}",this.value,"0")'>
									<option value="-1">订单状态选择</option>
									<option value="1" dicdesc="null">已通知待缴费</option>
									<option value="2" dicdesc="null">已缴费</option>
									<option value="3" dicdesc="null">已取消</option>
								</select>			   
							</td> --%>
						</div>
					</div>
