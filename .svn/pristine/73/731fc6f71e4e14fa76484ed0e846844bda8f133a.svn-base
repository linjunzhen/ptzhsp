...
<script type="text/javascript">
	function syncjgxx(){
		var rows = $("#jgxxSyncGrid").datagrid("getChecked");
		if(rows.length<=0){
			parent.art.dialog({
				content : "请选择需要同步的记录",
				icon : "warning",
		...
		AppUtil.ajaxProgress({
			url : "swbDataController.do?jgxxSync",
			params : {exeIds : exeIds},
			callback : function(resultJson) {
				$("#jgxxSyncGrid").datagrid("reload");
				parent.art.dialog({
...
<div class="easyui-layout" fit="true">
	<div region="center">
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true"  striped="true"
			id="jgxxSyncGrid" nowrap="false"
			toolbar="#jgxxSyncToolbar" method="post" idfield="EXE_ID"
			checkonselect="false" selectoncheck="false" fit="true" border="false"
			url="swbDataController.do?jgxxUnSyncData">
			<thead>
				<tr>
