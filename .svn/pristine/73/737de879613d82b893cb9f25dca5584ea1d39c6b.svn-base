...
<script type="text/javascript">
	$(function(){
    	...
    	if(flowSubmitObj){
    		...
    		if(flowSubmitObj.busRecord){
    			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BSFW_ZFTZXMTYJGYS_FORM");
    			
    			//获取表单字段权限控制信息
        		var currentNodeFieldRights = flowSubmitObj.currentNodeFieldRights;
        		 //初始化表单字段权限控制
        		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BSFW_ZFTZXMTYJGYS_FORM");
    		}
    		if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME){
    			if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME=="竣工财务决算意见"){
					...
					$("#GSJG_TR").attr("style", "");
					$("#GSXT_TR").attr("style", "");
				} else if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "拟批复竣工财务决算意见"
						...
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "批复竣工财务决算意见"
					...
					if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME=="批复竣工财务决算意见"){
	    				$("#PF_TR").attr("style","");
	    			}
					/* $("#NHZ_TR").attr("style", ""); */
				}
    			if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "填写公示材料"
						...
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "拟批复竣工财务决算意见"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区行政审批局项目投资处领导审批"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区审批局领导审签"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区管委会领导签发"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "批复竣工财务决算意见"
		...
		//	busTableName : "T_BSFW_ZFTZXMTYJGYS"
		//});
	...
	function FLOW_SubmitFun(flowSubmitObj) {
		//先判断表单是否验证通过
		var validateResult = $("#T_BSFW_ZFTZXMTYJGYS_FORM").validationEngine(
				"validate");
		if (validateResult) {
			...
			if (submitMaterFileJson || submitMaterFileJson == "") {
				...
				var formData = FlowUtil
						.getFormEleData("T_BSFW_ZFTZXMTYJGYS_FORM");
...
<body>
   ...
   <form id="T_BSFW_ZFTZXMTYJGYS_FORM" method="post" >
    <%--===================重要的隐藏域内容=========== --%>
	...
	<jsp:include page="./matterListZF.jsp" >
         <jsp:param value="T_BSFW_ZFTZXMTYJGYS_FORM" name="formName"/>
     </jsp:include>
	...
	<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="YWCZ_TABLE" style="display: none;">
		...
		<tr id="SFXYXT_TR" style="display: none;">
			<td class="tab_width"><font class="tab_color">*</font>竣工财务决算意见：</td>
			<td><eve:radiogroup typecode="SHSFTG" width="130px"
					fieldname="JGCWJSYJ" defaultvalue="1"></eve:radiogroup></td>
		</tr>
