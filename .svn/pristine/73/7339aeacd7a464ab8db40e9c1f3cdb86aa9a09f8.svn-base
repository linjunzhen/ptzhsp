...
<c:forEach items="${directorChangeList}" var="director" varStatus="s">
<div class="addBox">
	<table cellpadding="0" cellspacing="0" class="syj-table2">			
		<tr>
			<select name="setDsInfo" class="input-dropdown w280" onchange="setDsxxInfoMsg(this,this.value,'DIRECTOR')" >
		...
		<tr>
			...
			<td><input type="text" class="syj-input1 validate[required]" 
				name="${s.index}DIRECTOR_NAME"  placeholder="请输入姓名，须填写自然人姓名"  maxlength="32" value="${director.DIRECTOR_NAME}"/></td>
			<th><font class="syj-color2">*</font>职务：</th>
			<td>	
				<c:if test="${busRecord.COMPANY_NATURE=='01'||busRecord.COMPANY_NATURE=='03'}">
					...
					defaultEmptyText="请选择职务" name="DIRECTOR_JOB"  onchange="setDirectorJob(this.value,'${s.index}DIRECTOR_APPOINTOR')"
					value="${director.DIRECTOR_JOB}">
					</eve:eveselect>
				</c:if>
				<c:if test="${busRecord.COMPANY_NATURE=='02'||busRecord.COMPANY_NATURE=='07'||busRecord.COMPANY_NATURE=='04'}">
		...
		<tr>
			<th> 任期（年）：</th>
			<td colspan="3"><input type="text" class="syj-input1 w280 inputBackgroundCcc"  readonly="readonly"
