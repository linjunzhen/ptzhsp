	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZBALHSCYJHZ");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZBALHSCYJHZ", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZBALHSCYJHZ");
				var url = $("#SHTZBALHSCYJHZ").attr("action");
...
<body>
	<form id="SHTZBALHSCYJHZ" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					《<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目备案请示+》<br />联合审查情况汇总表
				</div>
				<br>
				<div class="eui-word-xmbh">
					项目编号：岚综实项目〔<input type="text"  data-validation-engine="validate[required]" name="VB_BHN_ST" class="eui-ipt1" style="width:60px"/>〕
					<input type="text" name="VB_BHH_ST"  data-validation-engine="validate[required]" class="eui-ipt1" style="width:40px"/>号
				</div>
				<br>
				...
				<br>
					...
					onpropertychange="this.style.height=this.scrollHeight + 'px'"
					oninput="this.style.height=this.scrollHeight + 'px'"
					name="VB_KFGLJ_ST" ></textarea><br>
					&nbsp;&nbsp;其他事项：<br>
					&nbsp;&nbsp;<textarea rows="6" 
					style="width: 95%;font-size:21px; vertical-align:top; overflow: hidden;line-height: 36px;"
					onpropertychange="this.style.height=this.scrollHeight + 'px'"
					oninput="this.style.height=this.scrollHeight + 'px'"
					name="VB_QTSX_ST" ></textarea><br>
					&nbsp;&nbsp;公示情况说明：<br>
					&nbsp;&nbsp;<textarea rows="6" 
					style="width: 95%;font-size:21px; vertical-align:top; overflow: hidden;line-height: 36px;"
					onpropertychange="this.style.height=this.scrollHeight + 'px'"
