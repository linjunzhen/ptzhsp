...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("BusColumnEditForm", function(form, valid) {
			if (valid) {
				//将提交按钮禁用,防止重复提交
				//$("input[type='submit']").attr("disabled", "disabled");
				//var formData = $("#BusColumnEditForm").serialize();
				var url = $("#BusColumnEditForm").attr("action");
				
				var data = $('#BusColumnEditGrid').datagrid('getData');
				for(var t=0;t<data.rows.length;t++){
					$('#BusColumnEditGrid').datagrid('endEdit', t);
					//$('#BusColumnEditGrid').datagrid('refreshRow', t);
				}
				
				var insertRows = $('#BusColumnEditGrid').datagrid('getChanges','inserted');
   				var updateRows = $('#BusColumnEditGrid').datagrid('getChanges','updated');
   				var deleteRows = $('#BusColumnEditGrid').datagrid('getChanges','deleted');
   				var changesRows = {
    				inserted : [],
    				updated : [],
    				deleted : [],
		...
		//
		$('#BusColumnEditGrid').datagrid({  //初始化datagrid
			...
			columns:[[
		        ...
		        {field:'STATUS',title:'操作',width:80,align:'center',
		    ...
		    onBeforeEdit:function(index,row){
		        row.editing = true;
		        $('#BusColumnEditGrid').datagrid('refreshRow', index);
		        editcount++;
		    },
		    ...
		    onClickRow:function (index, row) {
                $('#BusColumnEditGrid').datagrid('beginEdit', index);
            }
		});
		AppUtil.initAuthorityRes("BusColumnEditbar");
	});
	var editcount = 0;
	...
	function deleterow(serialId){
		$.messager.confirm('确认','您确定要删除掉该记录吗?',function(r){
			if (r){
				var $dataGrid = $("#BusColumnEditGrid");
				...
				$('#BusColumnEditGrid').datagrid('deleteRow', rowindex);
			}
		});
	}
	function saverow(index){
		$('#BusColumnEditGrid').datagrid('endEdit', index);
	}
	function cancelrow(index){
		$('#BusColumnEditGrid').datagrid('cancelEdit', index);
	}
	function addrow(){
		var $dataGrid = $("#BusColumnEditGrid");
		var length = $dataGrid.datagrid('getData').rows.length+1;
		$('#BusColumnEditGrid').datagrid('appendRow',{
			SERIAL_ID:'t_'+length,
			COLUMN_NAME:'',
			FIELD_TYPE:'0',
			BUSSYS_SN:length
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="BusColumnEditForm" method="post"
