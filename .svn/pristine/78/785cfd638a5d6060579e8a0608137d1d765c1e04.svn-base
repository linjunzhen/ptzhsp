function FLOW_SubmitFun(flowSubmitObj) {
    //先判断表单是否验证通过
    var validateResult = $("#T_BDCQLC_JTJSFW_FORM").validationEngine("validate");
    if (validateResult) {
        ...
        getPowSourceJson();
        // 获取发证明细数据
        getDjfzxxJson_jtjsfw();
        // 获取缴费信息
        getDjjfxxJson_jtjsfw();

        var submitMaterFileJson = AppUtil.getSubmitMaterFileJson("", 1);
        if (submitMaterFileJson || submitMaterFileJson == "") {
            $("input[name='EFLOW_SUBMITMATERFILEJSON']").val(submitMaterFileJson);
            //先获取表单上的所有值
            var formData = FlowUtil.getFormEleData("T_BDCQLC_JTJSFW_FORM");
        ...
        close : function() {
            ...
            if (bdcdaxxcxInfo && bdcdaxxcxInfo.length == 1) {
                ...
                if (result.isPass == true) {
                    FlowUtil.initFormFieldValue(info,"T_BDCQLC_JTJSFW_FORM");
                    //回填权属来源信息
...
function initAutoTable(flowSubmitObj){
    ...
    if(null != djfzxx_json && '' != djfzxx_json){
        ...
        initDjfzxx_jtjsfw(djfzxx_jsonItems);
    }
    ...
    if(null != djjfmx_json && '' != djjfmx_json){
        ...
        initDjjfxx_jtjsfw(djjfmx_jsonItems);
    }
...
function yjmbSelect() {
    $("#zddjyy").attr("onclick","bdcCyyjmbSelector('jtjjfwzd_yy','GYTD_DJYY','input')")
    $("#zdfj").attr("onclick","bdcCyyjmbSelector('jtjjfwzd_fj','GYTD_FJ','textarea')")
    $("#fwdjyy").attr("onclick","bdcCyyjmbSelector('jtjjfwfw_yy','FW_DJYY','input')")
    $("#fwfj").attr("onclick","bdcCyyjmbSelector('jtjjfwfw_fj','FW_FJ','textarea')")
}

function disabledForm() {
    $("#T_BDCQLC_JTJSFW_FORM select , #T_BDCQLC_JTJSFW_FORM input , #T_BDCQLC_JTJSFW_FORM textarea").each(function (index) {
