	...
	$(function(){		
		...
		if(flowSubmitObj){
			...
			if(flowSubmitObj.busRecord){
				//初始化表单字段值
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_SBQLC_QYZGBGDJ_FORM");
				if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES =='审核' || 
					...
					flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
				 	$("#T_SBQLC_QYZGBGDJ_FORM").find("input,select,.Wdate").attr("disabled","disabled");
				 	//a标签按钮禁用
				 	$("#qyzgbgdj").find(".easyui-linkbutton").attr("disabled",true).css("pointer-events","none");		 	
				 	$("#dzzz").attr("disabled",true).css("pointer-events","none");  
				 }				 				 		
...
<body>
	...
	<form id="T_SBQLC_QYZGBGDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--企业职工基本养老保险变更登记信息--%>
		<div id="qyzgbgdj" style="display:none">
			<jsp:include page="./sbqlc/qyzgbgdj/qyzgbgdj.jsp">
