...
<script type="text/javascript">
    ...
    function setUploadHidden(attachKey) {
        $("#" + attachKey + "__SC").hide();
        $("#" + attachKey + "__SCAN").hide();
        $("#" + attachKey + "__LICENSE").hide();
    }
...
<div class="syj-title1" style="height:30px;">
	<span>材料信息(<font color="red">上传图片时建议上传200dpi的图片。</font>)</span>
</div>
<input type="hidden" name="EFLOW_BUSTABLENAME" value="${serviceItem.FORM_KEY}" /> 
<table cellpadding="0" cellspacing="1" class="syj-table2 tmargin2">
...
<table cellpadding="0" cellspacing="1" class="syj-table2 tmargin2" id="materCheckList" style="margin-top: 15px;">
    <tr>
    ...
    <c:forEach items="${applyMaters}" var="applyMater" varStatus="varStatus">
        <tr <c:if test="${applyMater.MATER_PARENTCODE!=null&&applyMater.MATER_PARENTCODE!=''}"> class="${applyMater.MATER_PARENTCODE}"</c:if>>
            ...
            <td name="busClass"  style="text-align: center;" <c:if test="${applyMater.BUSCLASS_NAME!=null&&applyMater.BUSCLASS_NAME!=''}">class="busClass"</c:if>>${applyMater.BUSCLASS_NAME}<%--				<c:if test="${applyMater.BUSCLASS_NAME=='无'}"></c:if>--%> <%--				<c:if test="${applyMater.BUSCLASS_NAME!='无'}">${applyMater.BUSCLASS_NAME}</c:if>--%>
            </td>
...
<!-- 						<option value="2"<c:if test="${applyMater.SFSQ==2}">selected="true"</c:if>>未收取</option> -->
	...
	<div class="syj-title1" style="height:30px;">
		<span>办件申报时已收取材料（备查用）</span>
	</div>
	<table cellpadding="0" cellspacing="1" class="syj-table2 tmargin2" id="materCheckList">
		<tr>
			<th style="width:50px">序号</th>
			<th style="width:80px">事项子项</th>
			<th style="width:480px">材料名称</th>
			<th style="width:80px">材料说明</th>
			<th>附件</th>
			<c:if test="${empty nodeConfig||nodeConfig.UPLOAD_FILES=='1'}">
			<th style="width:100px">收取方式</th>
		...
		<c:forEach items="${ysqMaters}" var="ysqMater"
			...
			<tr <c:if test="${ysqMater.MATER_PARENTCODE!=null&&ysqMater.MATER_PARENTCODE!=''}"> class="${ysqMater.MATER_PARENTCODE}"</c:if> >
				...
				<td style="text-align: center;" >${applyMater.BUSCLASS_NAME}
	...
	<%--				<c:if test="${applyMater.BUSCLASS_NAME!='无'}">${applyMater.BUSCLASS_NAME}</c:if>--%>
				...
				<c:if test="${empty nodeConfig||nodeConfig.UPLOAD_FILES=='1'}">
				<td style="text-align: center;" ><select type="select" class="tab_text" style="width: 90px;"
				...
				<c:if test="${!empty nodeConfig&&nodeConfig.UPLOAD_FILES=='-1'}">
	                <td style="text-align: center;"   width="100px" id="materAudit_${ysqMater.MATER_CODE}">
	...
	<table cellpadding="0" cellspacing="1" class="syj-table2 tmargin2" id="materCheckList">
		<tr>
			...
			<th class="tab_width1" colspan="5">材料名称</th>
	        <th class="tab_width1" colspan="2">上传时间</th>
		</tr>
		<c:forEach items="${materList}" var="materListInfo" varStatus="materst">
			<tr>
				<td style="text-align: center;" >${materst.index+1}</td>
				<td style="text-align: center;"  colspan="5" title="${materListInfo.FILE_NAME}">
				...
				<td style="text-align: center;"  colspan="2">${materListInfo.CREATE_TIME}</td>
			</tr>
		</c:forEach>
	<table>
	</c:if>
