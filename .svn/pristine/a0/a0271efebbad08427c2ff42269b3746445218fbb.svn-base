...
<script type="text/javascript">
	$(function(){
		$.dialog.open("newCallController.do?winSelect", {
		    ...
		    close : function() {
    			...
    			if(selectWinInfo){
					...
					AppUtil.gridDoSearch('callingToolbar','callingNewGrid');
					AppUtil.gridDoSearch('QueuingDayToolbar','QueuingDayGrid');
		...
		var isYctb = '${isYctb}';
		if(isYctb=='true'){
			$("#yctbAgainButton").attr("style","");
		}else{
			$("#yctbAgainButton").attr("style","display:none;");
		}
		
		//定时刷新待办列表
		setInterval(function(){
			$("#callingNewGrid").datagrid("reload");
			setTimeout("setWaitNum();",2000);
		},60000);
...
// 			} 
			if(isBdcdjsly=='true'){			
 				 html += "<a href='#' onclick='bdcWaitAccept("+index+")'>不动产待受理</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
 			} 
			if(isYctb=='true'){			
				 html += "<a href='#' onclick='yctbAccept("+index+")'>一窗通办受理</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
			}
			if(isYctb=='true'
			&&row.ITEM_CODE != "" 
			&& row.ITEM_CODE != null 
			&& row.ITEM_CODE != undefined){			
				 html += "<a href='#' onclick='yctbItemAccept("+index+")'>一窗通办直接受理</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
			}
...
// 			"<a href='#' onclick='pass("+index+")'>过号</a>&nbsp;&nbsp;|&nbsp;&nbsp;"+
	...
	function evaluateformater(value,row,index){
		if(value){
			...
			if(row.CALL_STATUS=='2'&&index<=2){
				return "<button onclick=\"passAccept("+index+");\" style=\"cursor:pointer\"> 过号受理</button>";
			}else{
				return "未评价";
			}
		}
	...
	function setCompany(index) {
		...
		if (row) {
			$.dialog.open("newCallController.do?setCompany&recordId="
					+ row.RECORD_ID, {
				title : "企业设立登记",
				width : "450px",
				height : "260px",
...
// 	yctbItemAccept
	function yctbItemAccept(index) {
		var yctbsl = 1;
		var row = $('#callingNewGrid').datagrid('getData').rows[index];
		if (row) {
		    var url=encodeURI("executionController.do?goStart&defKey="
                + row.DEF_KEY + "&itemCode=" + row.ITEM_CODE
                + "&acceptway=" + yctbsl
					+ "&takenoway=" + row.TAKENOWAY
					+ "&lineId=" + row.RECORD_ID);
						toUrl(url);
	...
	function bdcAccept(index) {
		...
		if (row) {
			if(window.confirm("此操作将直接完成叫号受理，请确定是否已在业务系统完成业务受理？")){
				AppUtil.ajaxProgress({
					url : "newCallController.do?bdcAccept",
					params : {
	...
	function forward(index) {
		var row = $('#callingNewGrid').datagrid('getData').rows[index];
		var url = "newCallController.do?goQueuingForward&recordId="+row.RECORD_ID;
		if (row) {
			$.dialog.open(url, {
				title : "办件转发",
				width : "500px",
...
// 		$.dialog.open("webpage/callnew/queuing/VideoInputTakeNoCtlZT.jsp?", {
			title : "窗口取号",
			width:"810px",
			lock: true,
			resize:false,
	...
	//窗口取号
	function bindScanCtrlLT(){
		$.dialog.open("webpage/callnew/queuing/VideoInputTakeNoCtlLT.jsp?", {
			title : "窗口取号",
			width:"810px",
			lock: true,
			resize:false,
...
<div class="easyui-layout eui-in-box" fit="true">	
	<div data-options="region:'west'" style="width:85%;">
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			...
			url="newCallController.do?queuingGrid">
			<thead>
				<tr>
					...
					<th data-options="field:'ITEM_NAME',hidden:true">ITEM_NAME</th>
					<th data-options="field:'DEF_KEY',hidden:true">DEF_KEY</th>
					<th data-options="field:'MATER_TYPE_ID',hidden:true">MATER_TYPE_ID</th>
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			...
			toolbar="#QueuingDayToolbar" method="post" idfield="RECORD_ID"
			checkonselect="false" selectoncheck="false" fit="false" border="false"
	...
	<div data-options="region:'east'" style="width:15%;position:relative;">
		...
		<div style="position:absolute;text-align: center;">
			...
			<input type="button" value="一窗通办继续受理" id="yctbAgainButton" onclick="yctbAcceptAgain();" style="display: none;"
			class="callbutton" onmousemove="this.className='callbutton_in'" onmouseout="this.className='callbutton'">
		...
		<div style="position:absolute;bottom:0;text-align: center;border-top-style: solid;border-top-color: #d9d9d9;border-top-width: 1px;">
			<a href="javascript:;" class="easyui-linkbutton" reskey="Video_takeNo" onclick="bindScanCtrl2();" style="background-color:#2ba4d9;width: 135px;height:36px;margin-top: 5px;margin-bottom: 5px;">文件测试</a>
			...
			<a href="javascript:;" class="easyui-linkbutton" reskey="Video_takeNo" onclick="bindScanCtrlLT();" style="background-color:#2ba4d9;width: 135px;height:36px;margin-top: 5px;margin-bottom: 5px;">台胞柜台取号</a>
		...
		<div>
			...
			<div class="eui-rightbtn">
				...
				<button id="againButton" onclick="acceptAgain();"><i class="eui-jhicon-4"></i> 继续受理</button>
				<!-- 一窗通办继续受理 <button id="yctbAgainButton" onclick="yctbAcceptAgain();"><i class="eui-jhicon-5"></i> 继续受理</button>-->
			...
			<ul class="eui-rbottombtn">
				...
				<a href="javascript:;" onclick="bindScanCtrlLT();" ><li>台胞柜台取号</li></a>
