...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
	...
	<tr>
		...
		<td colspan="3">
			<input type="radio" name="TAKECARD_TYPE" value="1" <c:if test="${busRecord.TAKECARD_TYPE!='2'}">checked="checked"</c:if> >抵押权首次登记
			<input type="radio" name="TAKECARD_TYPE" value="2" <c:if test="${busRecord.TAKECARD_TYPE=='2'}">checked="checked"</c:if> >抵押权首次登记（转本）
		</td>
	</tr>
	<tr>
