...
<!--可信文件js开始-->
var electBusTableName = "${serviceItem.FORM_KEY}";
function electDocumentShow(selectid){
    var selectValue = $("#SQFS_" + selectid).val();
    var applyType="${applyType}";
    if(applyType=="1"){
    	var type="reception";    //前台
    	getElectDocumentInfo(selectValue, selectid, type);
    }else{
    	var type="backstage";    //后台
    	getElectDocumentInfo(selectValue, selectid, type);
    }
}

//获取电子文件信息
function getElectDocumentInfo(selectValue, attachKey, type) {
    var projectCode=$("input[name='PROJECTCODE']").val();
    if(typeof(projectCode)=="undefined" || projectCode==""){
    	projectCode=$("input[name='PRJ_CODE']").val();
    }
    if(typeof(projectCode)=="undefined" || projectCode==""){
    	projectCode=$("input[name='PROJECT_CODE']").val();
    }
    if(typeof(projectCode)=="undefined" || projectCode==""){
    	var projectCode="2018-350128-47-03-022986";
    }
    var itemName="${serviceItem.ITEM_NAME}";
    var itemCode="${serviceItem.ITEM_CODE}";
    var exeId="${serviceItem.EXE_ID}";
    if(projectCode==""){
        alert("请填写投资项目编号");
        return;
	}
    var url=encodeURI("creditController.do?electDocumentInfo&projectCode="+projectCode+"&itemName="+
    		itemName+"&itemCode="+itemCode+"&exeId="+exeId+"&attachKey="+attachKey+"&busTableName="+electBusTableName);
    $.dialog.open(url, {
        title : "可信文件信息列表",
        width:"880px",
        lock: true,
        ...
        close: function () {
            var resultJsonInfo = art.dialog.data("resultJsonInfo");
            if (resultJsonInfo) {
                initElectDocumentUploadMaters(resultJsonInfo);
                art.dialog.removeData("resultJsonInfo");
            }
        }
    }, false);
...
function initElectDocumentUploadMaters(resultJson){
    for (var i = 0; i < resultJson.length; i++) {
        //获取文件ID
        var fileId = resultJson[i].fileId;
        //获取文件名称
...
/* function downLoadElectFile(fileId){
	window.location.href="http://xzfwzx.pingtan.gov.cn:8888/DownLoadServlet?fileId="+fileId;
} */
...
<!--可信文件js结束-->
     ...
     function preViewFile(fileType,filePath){
         var checkFileType = ['pdf','doc','docx','xls','xlsx','ppt','pptx'];
         var checkImageType = ['jpg','png','bpm','jpeg','gif'];
         //判断文件类型
         if(checkFileType.indexOf(fileType)>=0){
            var url=encodeURI("certificateTemplateController.do?preViewFile&filePath="+filePath
         ...
         }else if(checkImageType.indexOf(fileType)>=0){//图片格式
               var url=encodeURI("certificateTemplateController.do?preViewFile&filePath="+filePath
               +"&type=image&fileType="+fileType);
			   $.dialog.open(url, {
					title : "附件预览",
					width: "90%",
					height: "80%",
...
<script type="text/javascript">
    ...
    setTimeout(function (){
        ...
        var oMaterssyw = $("input:radio[name='EXE_SUBBUSCLASS']:checked");
    	if(oMaterssyw&&oMaterssyw[0]&&oMaterssyw[0].id){
    	    checkHandleTypeNew(oMaterssyw[0].id)
    	}
	}, 2000);
    ...
    function hideSC() {
		...
		if(flowSubmitObj.busRecord){			
			if (flowSubmitObj.busRecord.RUN_STATUS != 0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始') {
				...
				$("div[id*='KXWJ']").attr("style","display:none;");
			}
		}
    }
    ...
    function setUploadHidden(attachKey) {
        ...
        $("#" + attachKey + "__KXWJ").attr("style", "display: none;");
    }
	...
	function checkHandleTypeNew(val){
/* 		var index = 1;
		var tds = $("#materCheckList").find(".MATER_SSYW");
		tds.each(function(){
			var tabtext = $(this).html();
			if((',' + tabtext + ',').indexOf(',' + val + ',')=='-1'){
				$(this).parent().attr("style","display:none");
...
.tab_tk_pro1 tr:nth-child(1) td,.tab_tk_pro1 tr td:first-child {
	font-size: 14px;
	color: #8f9399;
	font-weight: 400;
	line-height: 29px;
...
.tab_tk_pro2 tr td .tab_text {
	height: 30px;
	line-height: 30px;
}
.tab_tk_pro2 tr .tab_width {
	width: 170px;
	background: #fbfbfb;
}
.tab_height2 {
...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro" style="margin: 10px 30px;width: calc(100% - 60px);">
	<tr>
		...
		<tr>
			...
			<td>
				<c:forEach items="${handleTypes}" var="handleType">
					<input type="radio" name="EXE_SUBBUSCLASS" onclick="checkHandleTypeNew('${handleType.RECORD_ID}')"  id="${handleType.RECORD_ID}" value="${handleType.bus_handle_type }">${handleType.bus_handle_type }&nbsp;
				</c:forEach>
			</td>
		</tr>
	</c:if>
...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro1" id="materCheckList" style="margin: 10px 30px;width: calc(100% - 60px);">
    <!-- <tr>
        <th colspan="7">材料信息<font color="red">（上传图片时建议上传200dpi的图片。）</font></th>
    </tr> -->
    <tr>
        ...
        <td class="tab_width1" width="180px" 
						...
						注意事项</td>
        <td class="tab_width1" width="180px">材料说明</td>
    ...
    <c:forEach items="${applyMaters}" var="applyMater" varStatus="varStatus">
        <tr <c:if test="${applyMater.MATER_PARENTCODE!=null&&applyMater.MATER_PARENTCODE!=''}"> class="${applyMater.MATER_PARENTCODE}"</c:if>>
			<td style="display: none;" class="MATER_SSYW">${applyMater.MATER_SSYW}</td>
			<td class="materIndex">${varStatus.index+1}</td>
            <td name="busClass"  <c:if test="${applyMater.BUSCLASS_NAME!=null&&applyMater.BUSCLASS_NAME!=''}">class="busClass"</c:if>>${applyMater.BUSCLASS_NAME}</td>
            <td>
                            ...
                            <c:if test="${applyMater.UPLOADED_SIZE!=0}">
                                <p id="${uploadFile.FILE_ID}">
                                    <a href="javascript:void(0);" onclick="AppUtil.downLoadFile('${uploadFile.FILE_ID}');" style="cursor: pointer;"> 
										<font color="blue"> ${uploadFile.FILE_NAME}<c:if test="${uploadFile.CREADIT_FILEID!=null}">（电子证照收取）</c:if> </font>
                                    </a>
                                    <c:if test="${uploadFile.FILE_TYPE!='etc'}">
                                       <a href="javascript:void(0);"
										   onclick="preViewFile('${uploadFile.FILE_TYPE}','${uploadFile.FILE_PATH}');">
			...
			<td width="200px">
				<div id="div1_${applyMater.MATER_CODE}" <c:if test="${applyMater.SQFS==2}">style="display: none;"</c:if>>
	                ...
	                <div id="${applyMater.MATER_CODE}__SC" style="width: 200px; margin: auto;" >
...
<!-- 					</div> -->
					...
					<!--电子文件前台用户，当材料勾选了电子文件才显示-->
					<c:if test="${fn:contains(applyMater.collect_method,'05')}">
						<div id="${applyMater.MATER_CODE}__KXWJ"
							 >
							<a href="javascript:electDocumentShow('${applyMater.MATER_CODE}')">
								<img id="${applyMater.MATER_CODE}" src="webpage/bsdt/ptwgform/images/kxwj.png" style="width:73px;height:27px;"/>
							</a>
						</div>
					</c:if>
...
<!-- 						<option value="2"<c:if test="${applyMater.SFSQ==2}">selected="true"</c:if>>未收取</option> -->
	...
	<%--				<c:if test="${applyMater.BUSCLASS_NAME!='无'}">${applyMater.BUSCLASS_NAME}</c:if>--%>
								...
								<p id="${uploadFile.FILE_ID}" >
	                                ...
	                                onclick="AppUtil.downLoadFile('${uploadFile.FILE_ID}');"
	                                 style="cursor: pointer;">
						              <font color="blue">
						                    ${uploadFile.FILE_NAME}<c:if test="${uploadFile.CREADIT_FILEID!=null}">（电子证照收取）</c:if><c:if test="${ysqMater.IS_HIS==1}"><font style="color: red;">(历史已审核)</font></c:if>
						              </font>
						             </a>
						             	<c:if test="${uploadFile.FILE_TYPE!='etc'}">
	                                       <a href="javascript:void(0);"
				...
				<c:if test="${empty nodeConfig||nodeConfig.UPLOAD_FILES=='1'}">
				<td style="padding: 0 10px;"><select type="select" class="tab_text" style="width: 90px;"
