...
function FLOW_SubmitFun(flowSubmitObj){
	...
	//先判断表单是否验证通过
	var validateResult =$("#T_BDCQLC_GYJSJFWZYDJSDQLB_FORM").validationEngine("validate");
	if(validateResult){
		...
		if (POWERPEOPLEINFO_JSON && POWERPEOPLEINFO_JSON != "" && POWERPEOPLEINFO_JSON != "[]") {
		 ...
		 if(submitMaterFileJson||submitMaterFileJson==""){
			 ...
			 //先获取表单上的所有值
			 var formData = FlowUtil.getFormEleData("T_BDCQLC_GYJSJFWZYDJSDQLB_FORM");
...
function queryTypeFn(){
	...
	if(val == '5'){
		...
		$("#powerDY_div").attr("style", "display:none;");
		// 存量房税费联办
		if ((val == '4' || val=="7") && itemList.indexOf('不动产登记审批表三审') != -1) {
			$("#dfspb").attr("onclick", "goPrintTemplate('GYJSJFWZYDJTYSPB','3')");
			$("#sfspb").attr("onclick", "goPrintTemplate('GYJSJFWZYDJTYSPB','3')");
		} else {
			$("#dfspb").attr("onclick", "errorAction()");
	...
	$.dialog.open("bdcApplyController.do?bdcDyInfoSelector&allowCount=1", {
		...
		close: function () {
			...
			if(bdcdaxxcxInfo && bdcdaxxcxInfo.length == 1){
				...
				FlowUtil.initFormFieldValue(info,"T_BDCQLC_GYJSJFWZYDJSDQLB_FORM");
	...
	$.dialog.open(url, {
		...
		close: function () {
			...
			if(bdcdaxxcxInfo && bdcdaxxcxInfo.length == 1){
				...
				} else if (zydjType == '4' || zydjType == '6' || zydjType == '3' ||  zydjType == '7') {
