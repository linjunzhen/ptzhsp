	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"YCXGZD");
    		}
    	}
		...
		AppUtil.initWindowForm("YCXGZD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("YCXGZD");
				var url = $("#YCXGZD").attr("action");
...
<body>
	<form id="YCXGZD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eio-word-content">
					<textarea id="opinion" rows="17" data-validation-engine="validate[required]"
						style="width: 99%;font-size:21px; vertical-align:top; overflow: hidden;line-height: 36px;"
						onpropertychange="this.style.height=this.scrollHeight + 'px'"
						oninput="this.style.height=this.scrollHeight + 'px'"
						name="VB_GCOPINION_ST" ></textarea>
				</div>
