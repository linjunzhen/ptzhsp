...
function FLOW_SubmitFun(flowSubmitObj){
	...
	//先判断表单是否验证通过
	var validateResult =$("#T_BDCQLC_GYJSJFWZYDJ_FORM").validationEngine("validate");
	if(validateResult) {
		if (flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES == '收费') {
			art.dialog.confirm("请确认费用已收取， 可以进行登簿？",
			function() {
				FLOW_SubmitFun2(flowSubmitObj);
			},
			function() {
			});
			return;
...
function FLOW_SubmitFun2(flowSubmitObj){
	...
	if (POWERPEOPLEINFO_JSON && POWERPEOPLEINFO_JSON != "" && POWERPEOPLEINFO_JSON != "[]") {
	 ...
	 if(submitMaterFileJson||submitMaterFileJson==""){
		 ...
		 //先获取表单上的所有值
		 var formData = FlowUtil.getFormEleData("T_BDCQLC_GYJSJFWZYDJ_FORM");
