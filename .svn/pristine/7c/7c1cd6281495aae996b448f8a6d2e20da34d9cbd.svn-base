...
function FLOW_SubmitFun(flowSubmitObj){
	...
	//先判断表单是否验证通过
	var validateResult =$("#T_BDCQLC_GENERAL_FORM").validationEngine("validate");
	if(validateResult){
		...
		if (POWERPEOPLEINFO_JSON && POWERPEOPLEINFO_JSON != "" && POWERPEOPLEINFO_JSON != "[]") {
		 ...
		 if(submitMaterFileJson||submitMaterFileJson==""){
			 ...
			 //先获取表单上的所有值
			 var formData = FlowUtil.getFormEleData("T_BDCQLC_GENERAL_FORM");
...
function initAutoTable(flowSubmitObj){
	...
	//初始化义务人信息
	var ywpeopleinfoJson = flowSubmitObj.busRecord.YWPEOPLEINFO_JSON;
	if(null != ywpeopleinfoJson && '' != ywpeopleinfoJson && '[]' != ywpeopleinfoJson){
		var ywrItems = JSON.parse(ywpeopleinfoJson);
		initYwPeople(ywrItems);
	}
	...
	$.dialog.open("bdcApplyController.do?bdcDyInfoSelector&allowCount=1", {
		...
		close: function () {
			...
			if(bdcdaxxcxInfo && bdcdaxxcxInfo.length == 1){
				...
				FlowUtil.initFormFieldValue(info,"T_BDCQLC_GENERAL_FORM");
