...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteBdcJwspGridRecord() {
	...
	function revokeFlowExe() {
		var exeId = AppUtil.getEditDataGridRecord("BdcJwspGrid");
		if (exeId) {
	...
	$(document).ready(function() {
		...
		AppUtil.initAuthorityRes("BdcJwspToolbar");

	});

	//空数据，横向滚动
	$('#BdcJwspGrid').datagrid({
...
url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=FlowRunStatus',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 128,panelHeight: 'auto' " />
						...
						<tr style="height:28px;">
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('BdcJwspToolbar','BdcJwspGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="BdcJwspGrid" fitcolumns="true" toolbar="#BdcJwspToolbar"
			method="post" idfield="EXE_ID" checkonselect="false" 
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="bdcExecutionController.do?bdcHandledByMe">
			<thead>
				<tr>
