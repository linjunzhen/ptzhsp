...
function FLOW_SubmitFun(flowSubmitObj) {
	//先判断表单是否验证通过
	var validateResult = $("#T_BDCQLC_DYQSCDJ_FORM").validationEngine("validate");
	if (validateResult) {
		 /*
		 //涉会人员判断
		 ...
		 // 获取发证明细数据
	     getDjfzxxJson_dyqscdj();
		 // 获取缴费信息
		 getDjjfxxJson_dyqscdj();
		 //抵押明细
		 getDymxInfoJson();		 
		...
		if (submitMaterFileJson || submitMaterFileJson == "") {
			...
			//先获取表单上的所有值
			var formData = FlowUtil.getFormEleData("T_BDCQLC_DYQSCDJ_FORM");
...
//登簿环节保存抵押权登记信息
function saveDyqdjxx(){
	var DYQDJ_ZGZQQDSS = $('input[name="DYQDJ_ZGZQQDSS"]').val();
	var DYQDJ_DYFW = $('input[name="DYQDJ_DYFW"]').val();
	var DYQDJ_FJ = $('textarea[name="DYQDJ_FJ"]').val();
    var params = {};
	...
	if(flowSubmitObj){				
		...
		if((DYQDJ_ZGZQQDSS==null || DYQDJ_ZGZQQDSS=="" || DYQDJ_ZGZQQDSS==undefined) ||
		   (DYQDJ_DYFW==null || DYQDJ_DYFW=="" || DYQDJ_DYFW==undefined) ||
		   (DYQDJ_FJ==null || DYQDJ_FJ=="" || DYQDJ_FJ==undefined)){
			art.dialog({
				content: "最高债权确定事实、抵押范围、附记不允许为空，请填写并点击保存按钮！",
				icon: "error",
				time: 5,
				ok: true
			});
		...
		params.DYQDJ_ZGZQQDSS = DYQDJ_ZGZQQDSS;
		params.DYQDJ_DYFW = DYQDJ_DYFW;
		params.DYQDJ_FJ = DYQDJ_FJ;
		if (flowSubmitObj) {
			AppUtil.ajaxProgress({
				url: "bdcDyqscdjController.do?saveDyqdjxx",
				params: params,
...
function BDCQLC_bdcdbcz(){
	...
	var DYQDJ_DYFW = $('input[name="DYQDJ_DYFW"]').val();
	var DYQDJ_FJ = $('textarea[name="DYQDJ_FJ"]').val();
	var DJSHXX_HZYJ = $("textarea[name='DJSHXX_HZYJ']").val();
	if((DYQDJ_ZGZQQDSS==null || DYQDJ_ZGZQQDSS=="" || DYQDJ_ZGZQQDSS==undefined) ||
	   (DYQDJ_DYFW==null || DYQDJ_DYFW=="" || DYQDJ_DYFW==undefined) ||
	   (DYQDJ_FJ==null || DYQDJ_FJ=="" || DYQDJ_FJ==undefined)||
	   (DJSHXX_HZYJ==null || DJSHXX_HZYJ=="" || DJSHXX_HZYJ==undefined)){
		art.dialog({
			content: "最高债权确定事实、抵押范围、附记、核定意见不允许为空，请填写并点击相应的保存按钮！",
			icon: "error",
			time: 5,
			ok: true
		});
