...
<script type="text/javascript"
	src="<%=path%>/webpage/website/common/js/language.js"></script>
<script>
function mcxy(){
...
<%-- 
	<div class="eui-headCenter">
		<a href="https://zwfw.fujian.gov.cn/"><img
			src="<%=path%>/webpage/website/newzzhy/images/new/logo-main.png"></a>
		<span class="del"> <c:if test="${sname==null}">
				平潭综合实验区行政服务中心
			</c:if> <c:if test="${sname!=null}">
				${sname}
			</c:if>
		</span>
		<div class="divItem">
			<c:if test="${sessionScope.curLoginMember==null}">
				<span class="workHeight user-logo"> <img
					src="<%=path%>/webpage/website/newzzhy/images/new/user.png">
					<a href="<%=path%>/webSiteController/mztRegist.do"
					style="color:#FFFFFF">注册</a>
				</span>
				<span>&nbsp;|&nbsp;</span>
				<span class="workHeight"> <a href="javascript:mztLogin();"
					style="color:#FFFFFF">登录</a>
				</span>
			</c:if>
			<c:if test="${sessionScope.curLoginMember!=null}">
				<span class="workHeight user-logo"> <c:if
						test="${sessionScope.curLoginMember!=null}">
		    	您好!&nbsp;
		    	<c:choose>
							<c:when
								test="${fn:length(sessionScope.curLoginMember.YHMC) > 40}">
								<c:out
									value="${fn:substring(sessionScope.curLoginMember.YHMC, 0, 36)}..." />
							</c:when>
							<c:otherwise>
								<c:out value="${sessionScope.curLoginMember.YHMC}" />
							</c:otherwise>
						</c:choose> &nbsp;&nbsp;
			</c:if> <img src="<%=path%>/webpage/website/newzzhy/images/new/user.png">
...
--%>
	...
	<div class="menuDiv">
		<ul class="menu-wrap">
            ...
            <a href="https://xzfwzx.pingtan.gov.cn" style="color:#FFFFFF;display:block;">首页</a>
            </li>
			<li <c:if test="${currentNav=='zxgk'}">class="on"</c:if>>
            <a href="https://xzfwzx.pingtan.gov.cn/unitInfo" style="color:#FFFFFF;display:block;">中心概况</a>
            </li>
			<li <c:if test="${currentNav=='zxdt'}">class="on"</c:if>>
            <a href="https://xzfwzx.pingtan.gov.cn/centerNews" style="color:#FFFFFF;display:block;">中心动态</a>
            </li>
			<li <c:if test="${currentNav=='wssb'}">class="on"</c:if>>
            <a href="https://zwfw.fujian.gov.cn/indexMain?type=1&siteUnid=EC4BF9378FEEB8761D62BEDFF8EDE105" style="color:#FFFFFF;display:block;">网上申报</a>
            </li>
			<li <c:if test="${currentNav=='bmfw'}">class="on"</c:if>>
            <a href="https://xzfwzx.pingtan.gov.cn/convenient" style="color:#FFFFFF;display:block;">便民服务</a>
            </li>
			<li <c:if test="${currentNav=='xxgk'}">class="on"</c:if>>
            <a href="https://xzfwzx.pingtan.gov.cn/msgPublic" style="color:#FFFFFF;display:block;">信息公开</a>
            </li>
            <li><a href="<%=userCenter%>" target="_blank" style="color:#FFFFFF;display:block;">用户中心</a></li>
		</ul>
	</div>
</div>
