	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"TYYSCWJSQSD");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("TYYSCWJSQSD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("TYYSCWJSQSD");
				var url = $("#TYYSCWJSQSD").attr("action");
...
<body>
	<form id="TYYSCWJSQSD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				...
				<div class="eio-word-content">
					...
					提请的	
					<input
						type="text" class="eui-ipt" data-validation-engine="validate[required]" style="min-width: 200px;"  name="VB_CGLSUBJECT_ST" maxlength="200" value="${SQData.blxmmc}" />
						项目统一验收受理已完成，现将项目《联合竣工验收意见书》交给你单位。
				</div>
