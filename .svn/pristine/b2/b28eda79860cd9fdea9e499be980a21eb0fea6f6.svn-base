...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/js/bdcUtil.js"></script>
 ...
 <script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/bdcqlcygspfdyqygdjbg/js/ygspfdyqygdjbg.js"></script> 
...
$(function(){
	...
	if(flowSubmitObj){
		...
		$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
		 //初始化表单字段权限控制
		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_YGSPFDYQYGDJBG_FORM");
		//初始化表单字段值
		if(flowSubmitObj.busRecord){
			initAutoTable(flowSubmitObj);//初始化信息	
			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDCQLC_YGSPFDYQYGDJBG_FORM");

			if (flowSubmitObj.busRecord.RUN_STATUS && flowSubmitObj.busRecord.RUN_STATUS != 0) {
				/*其它环节审批表可以进行打印*/
				$("#SPBDF").attr("onclick","goPrintTemplate('YGSPFDYQYGDJBGSPB','3');");
				$("#SPBSF").attr("onclick","goPrintTemplate('YGSPFDYQYGDJBGSPB','3');");
			}

			if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始'){
				$("#T_BDCQLC_YGSPFDYQYGDJBG_FORM input").each(function(index){
					$(this).attr("disabled","disabled");
				});
				$("#T_BDCQLC_YGSPFDYQYGDJBG_FORM select").each(function(index){
					$(this).attr("disabled","disabled");
				});
			...
			//预告登记的变更（预购商品房抵押权预告登记变更）模块中，持证类型默认为共同持有，登记原因默认为"预告登记的变更（预购商品房抵押权预告登记变更）"。
			$("select[name='TAKECARD_TYPE']").val("1");
			$('#DJYY').val("预告登记的变更（预购商品房抵押权预告登记变更）");
	...
	// AppUtil.initNetUploadMaters({
		// busTableName:"T_BDCQLC_YGSPFDYQYGDJBG"
	// });
});
...
<body>
	...
	<form id="T_BDCQLC_YGSPFDYQYGDJBG_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/bdcqlcygspfdyqygdjbg/T_YGSPFDYQYGDJBG_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始登记审核信息--%>
		<%--开始引入预购预告信息 --%>
		<jsp:include page="./bdcqlc/bdcqlcygspfdyqygdjbg/T_YGSPFDYQYGDJBG_YGDYQYG.jsp" />
		<%--结束引入预购预告信息--%>
