	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"YJDSGTSCSLD");
    		}
    	}
		...
		AppUtil.initWindowForm("YJDSGTSCSLD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled", "disabled");
				var formData = WordUtil.getFormEleData("YJDSGTSCSLD");
				var url = $("#YJDSGTSCSLD").attr("action");
...
<body>
	<form id="YJDSGTSCSLD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《一阶段施工图》(送审版)审查受理单
				</div>
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt" value="${SQData.xmsqsj}"
						...
						name="VB_GCSUBJECT_ST" maxlength="200" value="${SQData.blxmmc}" />项目《一阶段施工图》（送审版）审查申请材料
					（<input type="checkbox" name="VB_SQB_CK" value="1">  项目《一阶段施工图》（送审版）审查申请表 
					  <input type="checkbox" name="VB_CNS_CK" value="1">  申请办理行政审批事项承诺书
					  <input type="checkbox" name="VB_CBSJGS_CK" value="1">  《一阶段施工图》（含初步设计及概算）（送审版）
				...
				<div class="eui-word-notes clearfix">
					<label>备注：</label>
					<span>
						1.该《一阶段施工图》（送审版）受理在《一阶段施工图》送审期间进行，不计入审批时限；
					</span>
					<span>
						2.该审查环节只征求行业主管部门意见。
					</span>
