...
<script type="text/javascript"
	src="<%=basePath%>/webpage/bsdt/applyform/gcjsxm/gcjssgxkfqzbl/js/gcjssgxkfqzbl.js"></script>
<style>
		...
		if(flowSubmitObj){
			...
			if(flowSubmitObj.busRecord){
				if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始'||flowSubmitObj.busRecord.RUN_STATUS==2){
					$('#T_BSFW_GCJSSGXKFQZBL_FORM').find('input,textarea').prop("readonly", true);
					$('#T_BSFW_GCJSSGXKFQZBL_FORM').find('select').prop("disabled", "disabled");
					$('#T_BSFW_GCJSSGXKFQZBL_FORM').find(":radio,:checkbox").attr('disabled',"disabled");
					$('#T_BSFW_GCJSSGXKFQZBL_FORM').find(".laydate-icon").attr('disabled',"disabled");	
					$("#loadData").remove();
				}
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BSFW_GCJSSGXKFQZBL_FORM");
				//获取表单字段权限控制信息
				var currentNodeFieldRights = flowSubmitObj.currentNodeFieldRights;
				 //初始化表单字段权限控制
				FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BSFW_GCJSSGXKFQZBL_FORM");
				
				if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME=='质安站变更施工单位和监理单位数据'){
					$(".afterUpdate").show();
				} else{
					$(".afterUpdate").remove();
				}
...
<body>
   ...
   <form id="T_BSFW_GCJSSGXKFQZBL_FORM" method="post" >
    <%--===================重要的隐藏域内容=========== --%>
    ...
    <input type="hidden" name="JSDW_JSON_AFTER" />
    <input type="hidden" name="SGDW_JSON_AFTER" />
    <input type="hidden" name="JLDW_JSON_AFTER" />
    <%--===================重要的隐藏域内容=========== --%>
	...
	<%--开始引入项目基本信息页面 --%>
	<jsp:include page="./gcjsxm/gcjssgxkfqzbl/info.jsp" />
	<%--结束引入项目基本信息页面 --%>
	<%--开始引入责任主体信息页面 --%>
	<jsp:include page="./gcjsxm/gcjssgxkfqzbl/zrztxx.jsp" />
	<%--结束引入责任主体信息页面 --%>
	
	<%--开始引入公用上传材料界面 --%>
	<jsp:include page="./matterListTZXM_ZBTB.jsp" >
    <jsp:param value="T_BSFW_GCJSSGXKFQZBL_FORM" name="formName"/>
    </jsp:include>
