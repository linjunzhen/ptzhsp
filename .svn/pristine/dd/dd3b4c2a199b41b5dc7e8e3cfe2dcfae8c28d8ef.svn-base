...
<div class="addBox">
	<table cellpadding="0" cellspacing="0" class="syj-table2 ">
		...
		<tr>
			...
			<td><input type="text" class="syj-input1 validate[required,custom[checkShareHolderName]]"   onchange="setSelectHolderName(this.value,'selectholder${s.index}SHAREHOLDER')"
			...
			<td>
				...
				setGdlxValidate(this.value,'${s.index}LEGAL_IDNO_PERSON');setGdType(this.value,'${s.index}SHAREHOLDER_TYPE','${s.index}LICENCE_TYPE','${s.index}LICENCE_NO');"
	...
	<table cellpadding="0" cellspacing="0" class="syj-table2 " style="margin-top: -1px;">
		<tr>
			...
			<td>
				...
				<c:choose>
						<c:when test="${requestParams.COMPANY_TYPECODE=='zrrkg'||busRecord.COMPANY_TYPECODE=='zrrkg'||requestParams.COMPANY_TYPECODE=='zrrdz'||busRecord.COMPANY_TYPECODE=='zrrdz'}">
					...
					<c:when test="${requestParams.COMPANY_TYPECODE=='frdz'||busRecord.COMPANY_TYPECODE=='frdz'}">
		...
		<tr>
			...
			<td colspan="3"><input type="text" class="syj-input1 validate[required] w878"
				name="${s.index}ID_ADDRESS" placeholder="请输入身份证件地址，请按身份证上的地址申报" maxlength="64" value="${holder.ID_ADDRESS}"/></td>
		</tr>
		<tr>
		...
		<tr>
			<th>  <font class="syj-color2 ${s.index}LEGAL_PERSON_CLASS" style="display: none" >*</font>投资企业或其他组织机构法定代表人姓名：</th>
			...
			<th>  <font class="syj-color2 ${s.index}LEGAL_PERSON_CLASS" style="display: none" >*</font>法定代表人身份证号码：</th>
		...
		<tr>
			<th> 现金</th>
			<td colspan="3"><input type="text" class="syj-input1 validate[required,custom[numMustBiggerZero]]" onchange="alertInvesment(this.value)"
		...
		<tr>
			...
			<td colspan="3"><input type="text" class="syj-input1 validate[],custom[JustNumber]"
				name="${s.index}INVESTMENT_MATERIAL"  placeholder="请输入实物(万元)" maxlength="16"  disabled="disabled"
		...
		<tr>
			<th> 技术出资</th>
			<td colspan="3"><input type="text" class="syj-input1 validate[],custom[JustNumber]"
				name="${s.index}INVESTMENT_TECHNOLOGY"  placeholder="请输入技术出资(万元)" maxlength="16" disabled="disabled"
		...
		<tr>
			<th> 土地使用权</th>
			<td colspan="3"><input type="text" class="syj-input1 validate[],custom[JustNumber]"
				name="${s.index}INVESTMENT_LAND" placeholder="请输入土地使用权(万元)" maxlength="16"  disabled="disabled"
		...
		<tr>
			<th> 股权</th>
			<td colspan="3"><input type="text" class="syj-input1 validate[],custom[JustNumber]"
				name="${s.index}INVESTMENT_STOCK"  placeholder="请输入股权(万元)" maxlength="16" disabled="disabled"
		...
		<tr>
			<th> 其他</th>
			<td colspan="3"><input type="text" class="syj-input1 validate[],custom[JustNumber]"
				name="${s.index}INVESTMENT_OTHER"  placeholder="请输入其他(万元)" maxlength="16" disabled="disabled"
		...
		<tr>
			...
			<td><input type="text"  class="syj-input1 validate[required,custom[JustNumber]]"  onblur="onlyNumber(this)" onkeyup="onlyNumber(this);" onchange="alertInvesment(this.value)"
					   name="${s.index}PAIDCAPITAL"  placeholder="请输入实缴出资额" maxlength="14" value="${holder.PAIDCAPITAL}"/></td>
		</tr>
		<script type="text/javascript">
			$(function() {
				...
				setZjValidate('${holder.LICENCE_TYPE}','${s.index}LICENCE_NO');
			});
		</script>
	</table>
	<c:if test="${s.index>1}">
