 ...
 * 描述 多媒体中心音频类管理Dao实现类
 * @author Bryce Zhang
 * @created 2016-10-19 下午4:51:35
 */
@Repository("audioFileDao")
public class AudioFileDaoImpl extends BaseDaoImpl implements AudioFileDao{
    ...
    public int countByFileType(String fileType){
        StringBuffer sql = new StringBuffer("select count(*) from T_FILES_AUDIO where TYPE_ID = ? ");
    ...
    public void saveMove(Map<String, Object> colValues){
        ...
        if(StringUtils.isNotEmpty(sourceFileIds) && StringUtils.isNotEmpty(targetTypeId)){
            ...
            StringBuffer sql = new StringBuffer("update T_FILES_AUDIO set TYPE_ID = ? where AUDIO_ID = ? ");
