...
$(function(){
	...
	$("input[name='FAT_TYPE']").click(function(){
		...
		var Value = $(this).val();
		if(Value=="1"){
			$("input[name='FAT_AREA']").attr("disabled",false);
		}else{
			$("input[name='FAT_AREA']").attr("disabled","disabled");
			$("input[name='FAT_AREA']").val('');
		}
	});
	...
	/*$("input[name='IS_ETDZ']").click(function(){
		...
		var Value = $(this).val();
		if(Value=="1"){
			$("input[name='ETDZ_AREA']").attr("disabled",false);
		}else{
			$("input[name='ETDZ_AREA']").attr("disabled","disabled");
			$("input[name='ETDZ_AREA']").val('');
		}
	});*/
	...
	$("input[name='COMPANY_NATURE']").click(function(){
		...
		if(Value=="04"){
			...
			$("input[name='JOINT_STOCK_TYPE']").removeAttr("checked");
			$("input[name='UNLISTED_TYPE']").removeAttr("checked");
			$("#stock").attr("style","display:none");
			$("#unlist").attr("style","display:none");
		}
	});
	...
	$("input[name='BUSINESS_TYPE']").click(function(){
		...
		if(Value=="02"){
			$("#research").attr("style","");
			$("input[name='FUNCTION_TYPE']").removeAttr("checked");
			$("input[name='FUNCTION_OTHER']").val('');
			$("#function").attr("style","display:none");
		}else if(Value=="03"){
			$("#function").attr("style","");
			$("input[name='RESEARCH_TYPE']").removeAttr("checked");
			$("#research").attr("style","display:none");
		}else{
			$("input[name='RESEARCH_TYPE']").removeAttr("checked");
			$("#research").attr("style","display:none");
			$("input[name='FUNCTION_TYPE']").removeAttr("checked");
	...
	$("input[name='FUNCTION_TYPE']").click(function(){
		...
		var Value = $(this).val();
		if(Value=="07"){
			$("input[name='FUNCTION_OTHER']").prop("disabled", false);
		}else{
			$("input[name='FUNCTION_OTHER']").val('');
			$("input[name='FUNCTION_OTHER']").prop("disabled", true);
		}
	});
	...
	$("input[name='IS_DUTY_FREE']").click(function(){
		...
		var Value = $(this).val();
		if(Value=="1"){
			$("input[name='DUTY_FREE_TYPE']").prop("disabled", false);
		}else{
			$("input[name='DUTY_FREE_TYPE']").prop("disabled", true);
			$("input[name='DUTY_FREE_TYPE']").removeAttr("checked");
			$("input[name='NATIOINEN_INDUSTRY']").prop("disabled", true);
			$("input[name='NATIOINEN_INDUSTRY']").val('');
			$("input[name='MIDWEST_INDUSTRY']").prop("disabled", true);
			$("input[name='MIDWEST_INDUSTRY']").val('');
		}
	});
	...
	$("input[name='DUTY_FREE_TYPE']").click(function(){
		...
		var Value = $(this).val();
		if(Value=="1"){
			$("input[name='NATIOINEN_INDUSTRY']").prop("disabled", false);
			$("input[name='MIDWEST_INDUSTRY']").prop("disabled", true);
			$("input[name='MIDWEST_INDUSTRY']").val('');
		}else{
			$("input[name='NATIOINEN_INDUSTRY']").prop("disabled", true);
			$("input[name='NATIOINEN_INDUSTRY']").val('');
			$("input[name='MIDWEST_INDUSTRY']").prop("disabled", false);
		}
	});
	...
	//----------------法人信息开始-----------------------
	$("#legalCountry").attr("style","");
	$("#legalJob option[value='20']").remove();
	$("#jlzw option[value='20']").remove();
	//----------------法人信息开始-----------------------
...
function initAutoTable(flowSubmitObj){
	...
	if(flowSubmitObj.busRecord.COMPANY_SETTYPE=='zwhz'){
		for(var i=0;i<directors.length;i++){
			...
			var fieldValueObj = directors[i];
			var text = fieldValueObj['DIRECTOR_APPOINTOR'];
			$("#director_"+(i+1)).find("[name='DIRECTOR_APPOINTOR']").append("<option selected='selected' value="+text+">"+text+"</option>");
		}
	}else{
	   		$('#director_zw').remove();
	   		$('#director_ws').attr('style','');
		FlowUtil.initFormFieldValue(directors[0],"director_ws");
	}
	var supervisorJson = flowSubmitObj.busRecord.SUPERVISOR_JSON;
	var supervisors = eval(supervisorJson);
	FlowUtil.initFormFieldValue(supervisors[0],"supervisor");
	var mangerJson = flowSubmitObj.busRecord.MANAGER_JSON;
	var mangers = eval(mangerJson);
	FlowUtil.initFormFieldValue(mangers[0],"manager");
	...
	var controllerJson = flowSubmitObj.busRecord.CONTROLLER_JSON;
	var controllers = eval(controllerJson);
	for(var i=0;i<controllers.length;i++){
		if(i==0){
			FlowUtil.initFormFieldValue(controllers[i],"controller_1");
		}else{
			addController();
			FlowUtil.initFormFieldValue(controllers[i],"controller_"+(i+1));
		}
		var CONTROLLER_TYPE = controllers[i].CONTROLLER_TYPE;
		var CONTROL_METHOD = controllers[i].CONTROL_METHOD;
		var fields = $("#controller_"+(i+1)).find("[name='CONTROLLER_TYPE_"+(i+1)+"']");
		fields.each(function(){
	        var radioValue = $(this).val();
	        if(radioValue==CONTROLLER_TYPE){
	        	$(this).attr("checked","checked");
	        }
	    });
		fields = $("#controller_"+(i+1)).find("[name='CONTROL_METHOD_"+(i+1)+"']");
		fields.each(function(){
	        var radioValue = $(this).val();
	        if(radioValue==CONTROL_METHOD){
	        	$(this).attr("checked","checked");
	        }
	    });
	}
...
function getInvestChinese(){
	if($("#gslx").combobox("getValue")=="zwhz"){
		var datas = [];
		var table = document.getElementById("ChineseInvest");
		for ( var i = 1; i <= table.rows.length-1; i++) {
			var haveOne = false;
...
/**==========董事、监事、经理信息开始================================*/
var directorSn = 3;
function addDirector(idSn){
	directorSn = directorSn+1;
	var trHtml = $("#director_"+idSn).html();
	var findexa = trHtml.indexOf("addDirector('");
	var findex = trHtml.indexOf("deleteDirector('");
	if(directorSn>10){
		var replacestra = trHtml.substring(findexa,findexa+17);
		var replacestr = trHtml.substring(findex,findex+20);
	}else{
		var replacestra = trHtml.substring(findexa,findexa+16);
		var replacestr = trHtml.substring(findex,findex+19);
	}
	trHtml = trHtml.replace(replacestra,"addDirector('"+directorSn+"')");
	trHtml = trHtml.replace(replacestr,"deleteDirector('"+directorSn+"')");
	trHtml = "<tr id=\"director_"+directorSn+"\">"+trHtml+"</tr>";
	$("#director_zw").append(trHtml);
	$("[id='appointor']").val($("input[name='AUTHORITY_NAME']").val());
}

function deleteDirector(idSn){
	var table = document.getElementById("director_zw");
...
function getDirectorJson(){
	...
	if($("#gslx").combobox("getValue")=="zwhz"){
		...
		var trData = {};
		$("#director_ws").find("*[name]").each(function(){
	          var inputName= $(this).attr("name");
	          var inputValue = $(this).val();
	          //获取元素的类型
			  var fieldType = $(this).attr("type");
...
function getSupervisorJson(){
	var datas = [];
	var trData = {};
	$("#supervisor").find("*[name]").each(function(){
          var inputName= $(this).attr("name");
          var inputValue = $(this).val();
          //获取元素的类型
		  var fieldType = $(this).attr("type");
...
function getManagerJson(){
	var datas = [];
	var trData = {};
	$("#manager").find("*[name]").each(function(){
          var inputName= $(this).attr("name");
          var inputValue = $(this).val();
          //获取元素的类型
		  var fieldType = $(this).attr("type");
...
function addController(){
	controllerSn = controllerSn+1;
	var table = document.getElementById("controller");
	var trContent = table.getElementsByTagName("tr")[1];
	var trHtml = trContent.innerHTML;
	var findex = trHtml.indexOf("deleteController('");
	if(controllerSn>10){
		var replacestr = trHtml.substring(findex,findex+22);
	}else{
		var replacestr = trHtml.substring(findex,findex+21);
	}
	trHtml = trHtml.replace(replacestr,"deleteController('"+controllerSn+"')");

	var findex1 = trHtml.indexOf("CONTROLLER_TYPE_");
	if(controllerSn>10){
		var replacestr1 = trHtml.substring(findex1,findex1+18);
	}else{
		var replacestr1 = trHtml.substring(findex1,findex1+17);
	}
	trHtml = trHtml.replaceAll(replacestr1,"CONTROLLER_TYPE_"+controllerSn);
	
	var findex2 = trHtml.indexOf("CONTROL_METHOD_");
	if(controllerSn>10){
		var replacestr2 = trHtml.substring(findex2,findex2+17);
	}else{
		var replacestr2 = trHtml.substring(findex2,findex2+16);
	}
	trHtml = trHtml.replaceAll(replacestr2,"CONTROL_METHOD_"+controllerSn);
	trHtml = "<tr id=\"controller_"+controllerSn+"\">"+trHtml+"</tr>";
	$("#controller").append(trHtml);
}

function deleteController(idSn){
	var table = document.getElementById("controller");
	if(table.rows.length==2){
		parent.art.dialog({
			content: "最少一个最终实际控制人！",
...
function getController(){
	...
	for ( var i = 1; i <= table.rows.length-1; i++) {
		var trData = {};
		$("#controller_"+i).find("*[name]").each(function(){
	          var inputName= $(this).attr("name");
	          var inputValue = $(this).val();
	          //获取元素的类型
			  var fieldType = $(this).attr("type");
			  if(fieldType=="radio"){
			  	  var isChecked = $(this).is(':checked');
			  	  if(isChecked){
			  		  if(inputName.indexOf("CONTROLLER_TYPE")>-1){
			  			inputName = "CONTROLLER_TYPE";
			  		  }else if(inputName.indexOf("CONTROL_METHOD")>-1){
			  			inputName = "CONTROL_METHOD";
			  		  }			  		  
			  		  trData[inputName] = inputValue;
			  	  }
			  }else if(fieldType=="checkbox"){
	...
	$("input[type='hidden'][name='CONTROLLER_JSON']").val(JSON.stringify(datas));
	return true;
}
/**==========外商投资企业最终实际控制人页面js结束=======================================*/
