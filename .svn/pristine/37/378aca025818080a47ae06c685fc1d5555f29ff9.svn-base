...
<form action="" id="PERSONNEL_FORM"  >
	...
	<div id="dsxxDiv">
				...
				<tr>
					...
					<td>
						<c:if test="${requestParams.COMPANY_SETNATURE=='01'||busRecord.COMPANY_SETNATURE=='01'
							...
							defaultEmptyText="请选择职务" name="DIRECTOR_JOB"  onchange="setGenerationModeByDirector(this.value,'DIRECTOR_GENERATION_MODE');setDirectorJob(this.value,'DIRECTOR_APPOINTOR')">
							</eve:eveselect>
						</c:if>
		...
		<jsp:include page="./initDsxxDiv_MP.jsp"></jsp:include>
	</div>
	...
	<div class="syj-title1 ">
		<a href="javascript:void(0);" class="syj-addbtn" id="addJlxx" style="display:none;"></a><span>经理信息</span>
		<select  name="setHolderInfo" style="margin-left: 82px;" class="input-dropdown " onchange="setInfoMsg(this.value,'MANAGER')">
			<option value="">请选择复用人员信息</option>
			<option value="SHAREHOLDER">股东信息1</option>
		</select>
	</div>
			...
			<table cellpadding="0" cellspacing="0" class="syj-table2 ">
				<tr>
					...
					<tr>
						<th><font class="syj-color2">*</font>产生方式：</th>
						<td>
							<select name="MANAGER_GENERATION_MODE" class="input-dropdown  w280 validate[required]" >
							<option value="03"  selected="selected" dicdesc="1">聘任</option>
								</select>
						</td>
						<th><font class="syj-color2">*</font>任命方：</th>
