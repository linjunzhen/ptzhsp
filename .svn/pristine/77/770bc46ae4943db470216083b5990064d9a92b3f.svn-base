	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZTYYSYJSFW");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZTYYSYJSFW", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZTYYSYJSFW");
				var url = $("#SHTZTYYSYJSFW").attr("action");
...
<body>
	<form id="SHTZTYYSYJSFW" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eio-word-content">
					...
					项目统一验收申请，按照《行政许可法》第二十六条的规定，经征求相关职能部门意见及区领导审批，决定如下：
							...
							<div style="text-indent: 0px;">
							<span style="font-weight: bold;">七、其他:</span><br/>
							<textarea rows="6" data-validation-engine="validate[required]" 
							style="width: 99%;font-size:21px; vertical-align:top; overflow: hidden;line-height: 36px;border: none;text-indent:42px;" 
					  		   onpropertychange="this.style.height=this.scrollHeight + 'px'" 
					  		   oninput="this.style.height=this.scrollHeight + 'px'"
					  			name="VB_QT_ST" maxlength="200" ></textarea>
							</div>
				</div>
