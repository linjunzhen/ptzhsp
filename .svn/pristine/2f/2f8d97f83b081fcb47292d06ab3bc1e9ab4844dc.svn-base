...
<script type="text/javascript">
    ...
    function backToParent(){
        window.parent.document.getElementById('takeFrame').src="callYqyzController/toYqyzBusinessItemChooseMacW.do?roomNo=${itemMap.roomNo}&ifMaterPrint=${itemMap.ifMaterPrint}&departId=${itemMap.departId}&zzType=${zzType}";
    }

    function readCard(){
        $.post("callYqyzController/countYqyzItemNum.do",{},function (data) {
            if (data) {
                var json = JSON.parse(data);
                if (json.flag) {
                    window.parent.document.getElementById('takeFrame').src = "callYqyzController/yqyzReadCardWhite.do?zzType=${zzType}";
                }else {
                    parent.art.dialog({
                        content: "<font style=\"font-size:24px;font-family:\"汉仪综艺体简\";\">只能选择一个事项进行取号</font>",
                        icon:"warning",
                        time:10,
                        width:"400px",
                        height:"150px",
