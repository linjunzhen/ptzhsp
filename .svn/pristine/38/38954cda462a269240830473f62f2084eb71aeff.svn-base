...
<div class="addBox">
	...
	<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20">
		...
		<tr>
			...
			<td colspan="3">						
				<input name="${currentTime}SINGLEPROMAINTYPE" id="${currentTime}singlepromaintype" class="easyui-combobox" 
					data-options="
						...
						onSelect:function(rec){
						   ...
						   if(rec.DIC_CODE){
							   ...
							   $('#${currentTime}singleprotype').combobox('reload',url); 
								setSingleProMaintype(rec.DIC_CODE,'${currentTime}');
						   }
						}
				" />	
					<script>
						$('#${currentTime}singlepromaintype').combobox({
							url:'dictionaryController/loadData.do?typeCode=PROTYPE&orderType=asc',
							method:'post',
							valueField:'DIC_CODE',
							textField:'DIC_NAME',
				...
				<input name="${currentTime}SINGLEPROTYPE" id="${currentTime}singleprotype" class="easyui-combobox" 
					data-options="
						...
						editable:false,
						onSelect:function(rec){
						   setSingleprosubtype(rec,'${currentTime}');
						}
				" />
					<script>
				...
				<input name="${currentTime}SINGLEPROSUBTYPE" id="${currentTime}singleprosubtype" class="easyui-combobox"  
					data-options="			                
						method:'post',
						valueField:'VALUE',
						textField:'TEXT',
	...
	<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20">		
		<tr class="${currentTime}singleProMaintype_1">	
			<th><font class="syj-color2">*</font>建筑高度</th>
			<td><input type="text" class="syj-input1 validate[required],custom[JustNumber]" onblur="onlyNumber3(this);setDwgcInfo('${currentTime}');" onkeyup="onlyNumber3(this);"
		...
		<tr class="${currentTime}singleProMaintype_2">	
			<th><font class="syj-color2">*</font>市政长度</th>
			<td>
				<input type="text" class="syj-input1 validate[required],custom[JustNumber]" onblur="onlyNumber3(this);setDwgcInfo('${currentTime}');" onkeyup="onlyNumber3(this);"
		...
		<tr>
			...
			<td colspan="3">
				...
				fieldname="${currentTime}ISSUPERHIGHTBUILDING"></eve:radiogroup>
			</td>
		</tr>	
		<tr class="${currentTime}singleProMaintype_1">
			<th><span class="syj-color2">*</span> ±0.000以上层数</th>
			<td>
			  <input type="text" class="syj-input1 validate[required],custom[numberplus]" name="${currentTime}STRUCTUPFLOORNUM"  maxlength="16"  placeholder="请输入±0.000以上层数" onblur="onlyNumber3(this);setDwgcInfo('${currentTime}');" onkeyup="onlyNumber3(this);" />
			</td>
			<th><span class="syj-color2">*</span> ±0.000以下层数</th>
			<td>
			  <input type="text" class="syj-input1 validate[required],custom[numberplus]" name="${currentTime}STRUCTDWFLOORNUM"  maxlength="16"  placeholder="请输入±0.000以下层数" onblur="onlyNumber3(this);setDwgcInfo('${currentTime}');" onkeyup="onlyNumber3(this);" />
			</td>
		</tr>
		<tr class="${currentTime}singleProMaintype_1">
			<th><span class="syj-color2">*</span> ±0.000以上面积</th>
			<td>
			  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="${currentTime}STRUCTUPFLOORAREA_DWGC"  maxlength="16" placeholder="请输入±0.000以上面积" onblur="onlyNumber2(this);setDwgcInfo('${currentTime}');" onkeyup="onlyNumber2(this);" />M<sup style="vertical-align: super;font-size: smaller;">2</sup>
			</td>
			<th><span class="syj-color2">*</span> ±0.000以下面积</th>
			<td>
			  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="${currentTime}STRUCTDWFLOORAREA_DWGC"  maxlength="16"  placeholder="请输入±0.000以下面积" onblur="onlyNumber2(this);setDwgcInfo('${currentTime}');" onkeyup="onlyNumber2(this);"/>M<sup style="vertical-align: super;font-size: smaller;">2</sup>
			</td>
		</tr>
		<tr>
			<th>基坑支护方法</th>
		...
		<tr>
			...
			<td colspan="3">
			  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="${currentTime}ITEMINVEST"  maxlength="16"  placeholder="请输入工程总造价（万元）"  onblur="onlyNumber6(this);setDwgcInfo('${currentTime}');" onkeyup="onlyNumber6(this);"/><div style="color:red;"> 指的是本单位工程的总造价，桩基工程先行办理施工许可时也要填写整个单位工程（含上部工程）的总造价。</div>
			</td>
		</tr>
		<tr>
			...
			<td>
				<eve:radiogroup typecode="YesOrNo" width="130px"
				fieldname="${currentTime}ISSHOCKISOLATIONBUILDING" ></eve:radiogroup>
			</td>
			<th><span class="syj-color2">*</span> 是否为装配式建筑	</th>
			<td>
				<eve:radiogroup typecode="YesOrNo" width="130px"
				fieldname="${currentTime}ISEQUIPPEDARCHITECTURE"></eve:radiogroup>
			</td>
		</tr>	
		<tr>
			<th><span class="syj-color2">*</span> 是否绿色建筑	</th>
			<td>
				<eve:radiogroup typecode="YesOrNo" width="130px" onclick="setGreenbuidinglevel(this.value,'${currentTime}');"
				fieldname="${currentTime}ISGREENBUILDING_DWGC" ></eve:radiogroup>
			</td>
		...
		<tr class="${currentTime}greenbuidinglevelTr" style="display:none">
			<th><span class="syj-color2">*</span>绿色建筑等级</th>
			<td colspan="3">
				<eve:eveselect clazz="syj-input1 w280 field_width" dataParams="GREENBUIDINGLEVEL"
				dataInterface="dictionaryService.findDatasForSelect" 
				defaultEmptyText="请选择基础建设等级" name="${currentTime}GREENBUIDINGLEVEL">
				</eve:eveselect>
			</td>
		</tr>
		<tr>
			...
			<td colspan="3">
				<span class="${currentTime}singleProMaintype_1_jglx">
				<eve:excheckbox
					dataInterface="dictionaryService.findTwoDatasForSelect" onclick="setDwgcInfo('${currentTime}')" 
					name="${currentTime}STRUCTQUALTYPE" width="749px;" clazz="checkbox validate[required]"
					dataParams="STRUCTQUALTYPE:2" >
				</eve:excheckbox>
				</span>
				<span class="${currentTime}singleProMaintype_2_jglx">
				<eve:excheckbox
					dataInterface="dictionaryService.findTwoDatasForSelect" onclick="setDwgcInfo('${currentTime}')" 
					name="${currentTime}STRUCTQUALTYPE" width="749px;" clazz="checkbox validate[required]"
		...
		<tr>
			...
			<td colspan="3">
				<eve:excheckbox
					dataInterface="dictionaryService.findDatasForSelect" onclick="setDwgcInfo('${currentTime}')" 
					name="${currentTime}BASICQUALTYPE" width="749px;" clazz="checkbox validate[required]"
		...
		<tr>
			...
			<td colspan="3">
				<eve:excheckbox
					dataInterface="dictionaryService.findDatasForSelect"
					name="${currentTime}REBELQUAKELEVEL" width="749px;" clazz="checkbox validate[required]"
	...
	<div class="syj-title1" style="margin-top: 5px;margin-bottom: 5px;">
		<a href="javascript:void(0);" onclick="javascript:addDwgcChildrenDiv('${currentTime}');" class="syj-addbtn" id="addDwgc" >添加</a>
	</div>
	<table cellpadding="0" cellspacing="1" class="syj-table2 syj-table3" id="${currentTime}childrenDwgcTable">
		<tr>
