...
function FLOW_SubmitFun(flowSubmitObj){	
	//先判断表单是否验证通过
	 var validateResult =$("#T_BDCQLC_ZJFWSYZYDJ_FORM").validationEngine("validate");
	 if(validateResult){		
		 ...
		 if(submitMaterFileJson||submitMaterFileJson==""){
			 ...
			 getPowSourceJson();
			 // 获取发证明细数据
			 getDjfzxxJson_zjfwsyzydj();
			 // 获取缴费信息
			 getDjjfxxJson_zjfwsyzydj();			 
			 //先获取表单上的所有值
			 var formData = FlowUtil.getFormEleData("T_BDCQLC_ZJFWSYZYDJ_FORM");
...
function bdcqtdbfun(qzinfos,dialog) {
	...
	if(qzinfos.length == qlrs.length){
		...
		if(iflag != qlrs.length){
			...
			$("input[name='FW_DBRMC']").val(currentUser);
			$("input[name='FW_DJRQ']").val(currentTime);
			if(dialog){
				art.dialog({
					content :"登簿成功",
					icon : "succeed",
...
function initAutoTable(flowSubmitObj){
	...
	if(null != djfzxx_json && '' != djfzxx_json){
		...
		initDjfzxx_zjfwsyzydj(djfzxx_jsonItems);
	}
	...
	if(null != djjfmx_json && '' != djjfmx_json){
		...
		initDjjfxx_zjfwsyzydj(djjfmx_jsonItems);
	}
}
		...
		close: function () {
			...
			if(bdcdaxxcxInfo && bdcdaxxcxInfo.length == 1){
				...
				var info = {
					...
					"POWERSOURCE_CLOSE_TIME2": bdcdaxxcxInfo[0].QLJSSJ1,//权利结束时间1
				...
				if(result.isPass == true){
					FlowUtil.initFormFieldValue(info,"T_BDCQLC_ZJFWSYZYDJ_FORM");		
					...
					//回填权利类型
					$("#QLLX option").each(function(){
						var text = $(this).text();
						if(text == info.QLLX.trim()){
							$(this).attr("selected",true);
						}
					});
