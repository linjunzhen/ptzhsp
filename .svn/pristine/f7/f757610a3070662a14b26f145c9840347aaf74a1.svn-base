	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GCKXXYJBGXBLHSCLXD");
    			//如果其它选项选中，其它后面的内容为必填
		...
		AppUtil.initWindowForm("GCKXXYJBGXBLHSCLXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GCKXXYJBGXBLHSCLXD");
				var url = $("#GCKXXYJBGXBLHSCLXD").attr("action");
...
<body>
	<form id="GCKXXYJBGXBLHSCLXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《工程可行性研究报告+》（修编版）<br />联合审查联系单
				</div>
				<br>
				<div class="eui-word-xmbh">
					项目编号：岚综实项目〔<input type="text" name="VB_KXXBGXMBHN_ST" class="eui-ipt1" style="width:60px"/>〕<input type="text" name="VB_KXXBGXMBHH_ST"  class="eui-ipt1" style="width:40px"/>号
				</div>
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目的《工程可行性研究报告+》（修编版）已完成，现转给你单位。请于<input
						...
						提交我局。超期不反馈意见的，视为默认同意。在综合(专家)评审阶段未提出的意见，在本审查阶段提出的，如无充足理由，不予采纳。
				</div>
				...
				<div class="eui-word-time">
					<input type="text" class="eui-ipt1 laydate-icon" data-validation-engine="validate[required]"
					 style="width:150px;height: 36px;" name="VB_KXXBGDQ_TIME_ST" maxlength="10" value="${dataMap.DQ_TIME}"
					 readonly="readonly" onclick="laydate({istime: false, format: 'YYYY年MM月DD日'})"/>
				</div>
				<div class="eui-word-notes clearfix">
					<label>附件：</label><span>
					1.《工程可行性研究报告+》（修编版）联合审查申请表；<br />
					2.《工程可行性研究报告+》（修编版）.
					</span>
				</div>
