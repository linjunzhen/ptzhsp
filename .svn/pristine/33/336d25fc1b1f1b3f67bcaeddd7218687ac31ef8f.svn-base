	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZTYYSTZ");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZTYYSTZ", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZTYYSTZ");
				var url = $("#SHTZTYYSTZ").attr("action");
...
<body>
	<form id="SHTZTYYSTZ" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eio-word-content">	
					<input
						...
						项目的竣工验收工作已完成，现转给你单位。兹定于
						<input type="text"
						class="eui-ipt laydate-icon" data-validation-engine="validate[required]" style="width:260px;height: 36px;" name="VB_CGLHY_TIME_ST" maxlength="10" readonly="readonly" onclick="laydate({istime: true, format: 'YYYY年MM月DD日  hh时mm分'})"/>
						在<input
						type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:273px"  name="VB_YSADDR_ST" maxlength="200"  />进行联合竣工验收，届时请派人参加，缺席视为无不同意见。
				</div>
