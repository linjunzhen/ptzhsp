...
<script type="text/javascript">
	...
	$(function(){
     ...
     //查询
     function jmInsuSearch(){
            var validateResult =$("#jmInsuForm").validationEngine("validate");
            if(!validateResult){
                return false;
            }
            var count = 0;
            //$("#bdcygdacxGrid").datagrid('clearChecked');
            AppUtil.gridDoSearch('jmInsuToolbar','jmInsuGrid');
            $('#jmInsuGrid').datagrid({
                onLoadSuccess:function(){
                    //确保初始化后只执行一次
                    if(count == 0){
                        var rows = $("#jmInsuGrid").datagrid("getRows");
                        if(rows.length==0){
                            parent.art.dialog({
		 ...
		 * 删除列表记录
		 */
		function deletejmInsuGridRecord() {
		 ...
		 * 编辑列表记录
		 */
		function editjmInsuGridRecord() {
			var entityId = AppUtil.getEditDataGridRecord("jmInsuGrid");
			if (entityId) {
...
<div class="easyui-layout" fit="true">
	<div region="center">
		<!-- =========================查询面板开始========================= -->
		<div id="jmInsuToolbar">
			<!--====================开始编写隐藏域============== -->
			<!--====================结束编写隐藏域============== -->
			
			<form action="#" name="jmInsuForm" id="jmInsuForm">
					...
					<tbody>
						<tr style="height:28px;">
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="jmInsuSearch();" />
								<input type="button" value="重置" class="eve-button"
								onclick="AppUtil.gridSearchReset('jmInsuForm')" /></td>
						</tr>
					</tbody>
				</table>
			</form>
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="false" striped="true"
			id="jmInsuGrid" fitcolumns="true" toolbar="#jmInsuToolbar"
			method="post" checkonselect="false" nowrap="false"
			selectoncheck="false" fit="true" border="false"
			url="ybCommonController.do?jmInsuGrid">
			<thead>
				<tr>
					...
					<th data-options="field:'aaa082',align:'left',formatter:formatYhzhytlb" width="10%">银行账号用途类别</th>
					<th data-options="field:'aaa149',align:'left',formatter:formatHyfxlb" width="10%">行业风险类别</th>
					<th data-options="field:'aab001',align:'left'" width="10%">单位ID</th>
					<th data-options="field:'aab002',align:'left'" width="10%">社会保险登记证编码</th>
					<th data-options="field:'aab004',align:'left'" width="10%">单位名称</th>
					<th data-options="field:'aab006',align:'left',formatter:formatGsdjzzzl" width="10%">工商登记执照种类</th>
					<th data-options="field:'aab007',align:'left'" width="10%" >工商登记执照号码</th>
					
					<th data-options="field:'aab008',align:'left'" width="10%">工商登记发照日期</th>
					<th data-options="field:'aab009',align:'left'" width="12%">工商登记有效期限（年）</th>
					<th data-options="field:'aab019',align:'left',formatter:formatDwlx" width="10%">单位类型</th>
					
					<th data-options="field:'aab020',align:'left',formatter:formatJjlx" width="10%">经济类型</th>
					<th data-options="field:'aab021',align:'left',formatter:formatLsgx" width="10%">隶属关系</th>
					<th data-options="field:'aab022',align:'left',formatter:formatHydm" width="10%">行业代码</th>
					
					<th data-options="field:'aab023',align:'left'" width="10%">主管部门或主管机构</th>
					...
					<th data-options="field:'aab999',align:'left'" width="10%">单位管理码</th>
					...
					<th data-options="field:'aac011',align:'left',formatter:formatWhcd" width="10%">文化程度</th>
					...
					<th data-options="field:'aac028',align:'left',formatter:formatNmgbs" width="10%">农民工标识(工伤生育)</th>
					...
					<th data-options="field:'aae008',align:'left'" width="10%">银行编号</th>
					
					<th data-options="field:'aae009',align:'left'" width="10%">户名</th>
					<th data-options="field:'aae010',align:'left'" width="10%">银行帐号</th>
					...
					<th data-options="field:'aaz065',align:'left'" width="10%">银行ID</th>
					...
					<th data-options="field:'bac052',align:'left'" width="10%">是否补缴</th>
				</tr>
			</thead>
		</table>
		<!-- =========================表格结束==========================-->
