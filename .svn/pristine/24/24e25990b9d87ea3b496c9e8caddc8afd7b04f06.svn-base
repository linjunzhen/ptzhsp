...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("VipForm", function(form, valid) {
			if (valid) {
				//将提交按钮禁用,防止重复提交
				$("input[type='submit']").attr("disabled","disabled");
				var formData = $("#VipForm").serialize();
				var url = $("#VipForm").attr("action");
				AppUtil.ajaxProgress({
					...
					callback : function(resultJson) {
						if(resultJson.success){
							...
							parent.$("#VipGrid").datagrid("reload");
							AppUtil.closeLayer();
						}else{
							parent.art.dialog({
								content: resultJson.msg,
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="VipForm" method="post" action="callController.do?saveOrUpdateVip">
	    <div  id="VipFormDiv">
		    <%--==========隐藏域部分开始 ===========--%>
			...
			<table style="width:100%;border-collapse:collapse;"
				...
				<tr>
					<td colspan="2"><span
						...
						value="${vip.VIP_NAME}" maxlength="8"
						class="eve-input validate[required]" name="VIP_NAME" /> <font
				...
				<tr>
					<td><span style="width: 100px;float:left;text-align:right;">身份证号：</span>
						...
						value="${vip.VIP_CARDNO}" maxlength="18"
						class="eve-input validate[required,custom[vidcard]]"
						name="VIP_CARDNO" /> <font class="dddl_platform_html_requiredFlag">*</font>
					</td>
				</tr>
