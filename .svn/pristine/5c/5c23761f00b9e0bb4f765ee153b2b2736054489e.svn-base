...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/gyjsydsyqbgdj/js/gyjsydsyqbgdj.js"></script>
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/js/bdcUtil.js"></script>
<script type="text/javascript">
	$(function() {
	
		<%-- 登记缴费信息、发证、归档信息隐藏 --%>
	    $("#djjfxx_gyjsydsyqbgdj").attr("style","display:none;");
    	$("#djfzxx_gyjsydsyqbgdj").attr("style","display:none;");
    	$("#djgdxx_gyjsydsyqbgdj").attr("style","display:none;");
		...
		if (flowSubmitObj) {
			...
			//初始化表单字段权限控制
			FlowUtil.initFormFieldRightControl(currentNodeFieldRights, "T_BDCQLC_GYJSYDSYQBGDJ_FORM");			
			if (flowSubmitObj.busRecord) {
				//初始化表单字段值
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord, "T_BDCQLC_GYJSYDSYQBGDJ_FORM");
				//初始JSON格式信息
				...
				if( flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME != '开始') {				
					...
					$("#dfspb").attr("onclick","goPrintTemplate('GYJSYDSYQBGDJJSPB','3')");
					$("#sfspb").attr("onclick","goPrintTemplate('GYJSYDSYQBGDJJSPB','3')");
					//开始、受理环节之后数据均不可编辑
					if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME!="受理" || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
						$("#T_BDCQLC_GYJSYDSYQBGDJ_FORM").find("input,select,.Wdate").attr("disabled","disabled");
						//a标签按钮禁用
	                ...
	                if(BDC_OPTYPE == "1"){//缮证（权证打印）                    
	                    ...
	                    $("#qzbsmsavebtn").removeAttr("disabled");
	                  	...
	                  	$("#BDC_QZVIEW").attr("style","");
	                   	$("#BDC_QZVIEW").removeAttr("disabled");
	                    //登记缴费、发证、归档信息
	                    $("#djjfxx_gyjsydsyqbgdj").attr("style","");
    					$("#djfzxx_gyjsydsyqbgdj").attr("style","");
    					$("#djgdxx_gyjsydsyqbgdj").attr("style","");	                   	    					
    					$("#djjfxx_gyjsydsyqbgdj").find("input,select,.Wdate").removeAttr("disabled");
    					$("#djfzxx_gyjsydsyqbgdj").find("input,select,.Wdate").removeAttr("disabled");
    					$("#djgdxx_gyjsydsyqbgdj").find("input,select,.Wdate").removeAttr("disabled");	                  
...
<body>
	...
	<form id="T_BDCQLC_GYJSYDSYQBGDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/gyjsydsyqbgdj/T_ESTATE_ZYDJ_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始引入权利人信息--%>
		<jsp:include page="./bdcqlc/gyjsydsyqbgdj/bdcQlrxx.jsp" />
		<%--结束引入权利人信息--%>
		
		<%--开始引入权属来源信息--%>
		<jsp:include page="./bdcqlc/gyjsydsyqbgdj/bdcQsly.jsp" />
		<%--结束引入权属来源信息--%>
		...
		<div id="printBtn" name="printBtn" style="display:none;">
			<table cellpadding="0" cellspacing="1" class="tab_tk_pro2">
				<tr>
					<th>
						<a href="javascript:void(0);" style="float:right; margin: 2px 10px;" class="eflowbutton" id="dfspb"
							>审批表（单方）</a>
						<a href="javascript:void(0);" style="float:right; margin: 2px 0;" class="eflowbutton" id="sfspb"
							>审批表（双方）</a>
					</th>
				</tr>
		    </table>
	    ...
	    <jsp:include page="./bdcqlc/common/djauditinfo.jsp">
	        <jsp:param value="gyjsydsyqbgdj" name="domId" />
