	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GHXZYDSQSLD");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("GHXZYDSQSLD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GHXZYDSQSLD");
				var url = $("#GHXZYDSQSLD").attr("action");
...
<body>
	<form id="GHXZYDSQSLD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />规划选址及用地申请受理单
				</div>
				<br>
				<div class="eui-word-xmbh">
					项目编号：岚综实项目【<input type="text" name="VB_GCNH_ST" class="eui-ipt1" style="width:60px"/>】<input type="text" name="VB_GCBH_ST" class="eui-ipt1" style="width:30px"/>号
				</div>
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt"
						...
						<input type="checkbox" name="VB_GCYDSQ_CK" value="1" >  项目规划选址及用地申请表					
						<input type="checkbox" name="VB_GCCNS_CK" value="1" >  申请办理行政审批事项承诺书 
						<input type="checkbox" name="VB_GCFA_CK" value="1" >  项目建设方案
						...
						data-validation-engine="validate[required]"
						style="width:150px;height: 36px;" name="VB_GCCN_TIME_ST"
						maxlength="10" readonly="readonly"
						onclick="laydate({istime: false, format: 'YYYY年MM月DD日'})" />(
					14个工作日)凭此单到区行政服务中心大厅我局项目投资窗口领取《规划选址及用地意见书》。
				</div>
