 ...
 * 描述 多媒体中心附件类管理Dao实现类
 * @author Bryce Zhang
 * @created 2016-10-19 下午4:34:57
 */
@Repository("attachFileDao")
public class AttachFileDaoImpl extends BaseDaoImpl implements AttachFileDao{
    ...
    public int countByFileType(String fileType){
        StringBuffer sql = new StringBuffer("select count(*) from T_FILES_ATTACHMENT where TYPE_ID = ? ");
    ...
    public void saveMove(Map<String, Object> colValues){
        ...
        if(StringUtils.isNotEmpty(sourceFileIds) && StringUtils.isNotEmpty(targetTypeId)){
            ...
            StringBuffer sql = new StringBuffer("update T_FILES_ATTACHMENT set TYPE_ID = ? where ATTACHMENT_ID = ? ");
