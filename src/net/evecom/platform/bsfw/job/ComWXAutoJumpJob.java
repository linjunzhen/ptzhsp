 ...
 */
public class ComWXAutoJumpJob implements Job {
    /**
     * log
     */
    private static Log log = LogFactory.getLog(ComWXAutoJumpJob.class);
    /**
     * 引入service
     */
    @Resource
    ...
    public void execute(JobExecutionContext context) {
        try {
            ...
            if (listbj != null && listbj.size() > 0) {
                log.info("发现需要自动跳转环节的商事登记流程数据量为:" + listbj.size());
            ...
            log.error("执行五星流程自动跳转任务,面签自动跳转定时器报错：ComWXAutoJumpJob.execute()", e);
        }
    }
}
