 ...
 * 描述
 * @author Danto Huang
 * @created 2018年8月1日 上午10:07:35
 */
@Repository("billOfRightDao")
public class BillOfRightDaoImpl extends BaseDaoImpl implements BillOfRightDao {
     ...
     */
    public int getMaxSn() {
        StringBuffer sql = new StringBuffer("select decode(max(C_SN),null,0,max(C_SN)) FROM T_SMOGA_BILLOFRIGHTS ");
    ...
    public int getCurVersion() {
        ...
        sql.append("select decode(max(C_VERSION),null,1,max(C_VERSION)) FROM T_SMOGA_BILLOFRIGHTS ");
        return this.jdbcTemplate.queryForInt(sql.toString());
    }
     ...
     * 描述
     * @author Danto Huang
     * @created 2018年8月3日 上午10:39:35
     * @param variables
     * @return
     * @see net.evecom.platform.smoga.dao.BillOfRightDao#saveOrUpdate(java.util.Map)
     */
    public String saveOrUpdateRecord(Map<String, Object> variables){
        String rightId = variables.get("RIGHT_ID").toString();
        ...
        if(StringUtils.isNotEmpty(rightId)&&isExist(rightId,version)){
            ...
            Set<String> columns = this.getColumnNameByTableName("T_SMOGA_BILLOFRIGHTS_HIS");
            Map<String,Object> insertMap = new HashMap<String,Object>();
            rightId = UUIDGenerator.getUUID();
            insertMap.put("RIGHT_ID", rightId);
            StringBuffer sql = new StringBuffer("insert into T_SMOGA_BILLOFRIGHTS_HIS (");
    ...
    private boolean isExist(String rightId,String version){
        StringBuffer sql = new StringBuffer("select count(*) from T_SMOGA_BILLOFRIGHTS_HIS ");
        sql.append(" where RIGHT_ID=? and C_VERSION=? ");
    ...
    private void updateRecord(Map<String,Object> variables){
        StringBuffer sql = new StringBuffer("update T_SMOGA_BILLOFRIGHTS_HIS set ");
        // 获取业务表的列名称
        Set<String> columns = this.getColumnNameByTableName("T_SMOGA_BILLOFRIGHTS_HIS");
        //获取业务表大字段的列名称
        List<String> bigColumns=this.getBigFiled("T_SMOGA_BILLOFRIGHTS_HIS");
        // 设置更新目标列
        List<String> targetCols = new ArrayList<String>();
        // 定义更新的目标值
        ...
        while (it.hasNext()) {
            ...
            if (columns.contains(fieldName.toUpperCase())
                    && !fieldName.toUpperCase().equals("RIGHT_ID")&& !fieldName.toUpperCase().equals("C_VERSION")) {
        ...
        sql.deleteCharAt(sql.length() - 1);
        sql.append(" where RIGHT_ID=? and C_VERSION=? ");
        targetVals.add(variables.get("RIGHT_ID"));
