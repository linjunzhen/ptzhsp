...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="sqjg1" <c:if test="${execution.BSYHLX!='2'||takenoway=='3'}">style="display: none;"</c:if> >
	...
	<tr>
		<td class="tab_width"><c:if test = "${nodeConfig.NODE_NAME=='开始'}"><c:if test="${serviceItem.SXXZ == '行政许可'}"><font class="tab_color">*</font></c:if></c:if>统一社会信用代码：</td>
		<td><input type="text" class="tab_text <c:if test = "${nodeConfig.NODE_NAME=='开始'}"><c:if test="${serviceItem.SXXZ == '行政许可'}">validate[required]</c:if></c:if>" maxlength="18"
	...
	<tr>
		<td class="tab_width"><c:if test = "${nodeConfig.NODE_NAME=='开始'}"><c:if test="${serviceItem.SXXZ == '行政许可'}"><font class="tab_color">*</font></c:if></c:if>法人代表：</td>
		<td><input type="text" class="tab_text <c:if test = "${nodeConfig.NODE_NAME=='开始'}"><c:if test="${serviceItem.SXXZ == '行政许可'}">validate[required]</c:if></c:if>" maxlength="14"
...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="cksjry">
	<tr>
