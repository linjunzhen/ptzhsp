...
function FLOW_SubmitFun(flowSubmitObj){
	...
	//先判断表单是否验证通过
	var validateResult =$("#T_BDCQLC_BLSDQYW_FORM").validationEngine("validate");
	if(validateResult){
		...
		if (POWERPEOPLEINFO_JSON && POWERPEOPLEINFO_JSON != "" && POWERPEOPLEINFO_JSON != "[]") {
		 ...
		 if(submitMaterFileJson||submitMaterFileJson==""){
			 ...
			 //先获取表单上的所有值
			 var formData = FlowUtil.getFormEleData("T_BDCQLC_BLSDQYW_FORM");
...
function initAutoTable(flowSubmitObj){
	...
	console.log(powerpeopleinfoJson);
	...
	$.dialog.open("bdcApplyController.do?bdcDyInfoSelector&allowCount=1", {
		...
		close: function () {
			...
			if(bdcdaxxcxInfo && bdcdaxxcxInfo.length == 1){
				...
				FlowUtil.initFormFieldValue(info,"T_BDCQLC_BLSDQYW_FORM");
...
//通过申报号查询许可文件编号  exeid:申报号    exeidType：子流程业务类型(0:水力  1:电力  2:燃气  3:广电)
function queryXkfileNum(){
    ...
    $.ajaxSettings.async = false;
	$.post("bdcDyqscdjController.do?queryXkfileNum",{
		exeid:$("#EXEID").text(),
		exeidType:exeidType,
	},
	function (data) {
		if (data) {
			var rows = data.rows;
			for (let i = 0; i < rows.length; i++) {
				$('input[name="XKFILE_NUM"]').val(rows[i].XKFILE_NUM);
				$('input[name="XKFILE_NAME"]').val(rows[i].XKFILE_NAME);
			}
		}
	});
	$.ajaxSettings.async = true;
