...
<head>
	...
	<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/sbqlc/qyzgcx/js/jquery.jqprint-0.3.js"></script>
	<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/sbqlc/qyzgcx/js/qyzgcx.js"></script>
	...
	<script type="text/javascript">
		$(function(){
			...
			if(flowSubmitObj){
				...
				//初始化表单字段权限控制
				//FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_SBQLC_QYZGCX");
				//初始化表单字段值
				if(flowSubmitObj.busRecord){
					//初始化表单字段值
					FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_SBQLC_QYZGCX_FORM");
					if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES =='受理' ||
						...
						$("#T_SBQLC_QYZGCX_FORM").find("input,select,.Wdate").attr("disabled","disabled");
						//a标签按钮禁用
						$("#qyzgcx").find(".easyui-linkbutton").attr("disabled",true).css("pointer-events","none");
						//设置复选框不可选择
	    	 			$("#payDetailGrid").datagrid("hideColumn", "ck");
					}					
...
<form id="T_SBQLC_QYZGCX_FORM" method="post">
	<%--===================重要的隐藏域内容=========== --%>
	...
	<%--缴费明细信息 --%>
	<input type="hidden" name="PAYDETAILJSON" />
	...
	<%--企业职工基本养老保险缴费记录、个人权益记录查询及政策咨询--%>
	<div id="qyzgcx">
		<jsp:include page="./sbqlc/qyzgcx/totalPage.jsp">
