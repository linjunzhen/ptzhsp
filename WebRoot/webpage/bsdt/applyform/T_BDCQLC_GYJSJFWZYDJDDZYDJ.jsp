...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/gyjsjfwzydj/ddzydj/js/ddzydj.js"></script>
...
$(function(){
    ...
    if(flowSubmitObj){
        ...
        $("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
         //初始化表单字段权限控制
        FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM");
        //初始化表单字段值
        if(flowSubmitObj.busRecord){
            FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM");
            //表单内打印按钮是否可以操作
			isButtonAvailable();
			//动态切换抵押信息
			queryTypeFn();
			...
			if(isEndFlow){
				...
				$("#T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM").find("input[type='text']").attr("disabled", "disabled");
				$("#T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM").find("input[type='button']").attr("disabled", "disabled");
				$("#T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM").find("input[type='checkbox']").attr("disabled", "disabled");
				$("#T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM").find("input[type='radio']").attr("disabled", "disabled");
				$("#T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM").find("input,select").attr("disabled", "disabled");
            ...
            if(BDC_OPTYPE != null && BDC_OPTYPE !=""){
                $("#T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM input").each(function(index){
                    $(this).attr("disabled","disabled");
                });
                $("#T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM select").each(function(index){
                    $(this).attr("disabled","disabled");
                });
                //填写权证标识码
                if(BDC_OPTYPE == "1"){
            ...
            //初始化默认业务类型为'2'-多幢转移登记-对应字典类别编码为zydjlx1
            $("input[name='ZYDJ_TYPE']").val("2");
			//动态切换抵押信息
			queryTypeFn();
...
<body>
	...
	<form id="T_BDCQLC_GYJSJFWZYDJDDZYDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--结束引入公用申报对象 --%>
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/T_ESTATE_ZYDJ_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始引入权利人信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/bdcQlrxx.jsp" />
		<%--结束引入权利人信息--%>
		
		<%--开始引入权属来源信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/bdcQslyType1.jsp" />
		<%--结束引入权属来源信息--%>
		
		<%--开始引入权利限制信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/bdcQlxz.jsp" />
		<%--结束引入权利限制信息--%>
		
		<%--开始引入抵押情况信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/T_ESTATE_ZYDJ_OBLIGEEINFO.jsp" />
		<%--结束引入抵押情况信息--%>
		...
		<%--开始引入房屋基本信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/bdcFwjbxx.jsp" />
		<%--开始引入房屋基本信息--%>
		
		<%--开始引入登记审核信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/bdcDjshxx.jsp" />
		<%--开始引入登记审核信息--%>

		<%--涉税登记信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/taxRelatedInfo.jsp"/>
		...
		<%--开始不动产询问记录基本信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/bdcXwjl.jsp" />
		<%--开始不动产询问记录基本信息--%>

		<%--开始银行询问记录基本信息--%>
		<jsp:include page="./bdcqlc/gyjsjfwzydj/ddzydj/yhXwjl.jsp" />
		<%--开始银行询问记录基本信息--%>
