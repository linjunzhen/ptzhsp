    ...
    function doSelectRows(){
		...
		for(var i = 0;i<rows.length;i++){			
			...
			if(Trim(cqzt).indexOf("查封")>-1 || Trim(cqzt).indexOf("预查封")>-1 ||Trim(cqzt).indexOf("限制")>-1 || Trim(cqzt).indexOf("无效")>-1
				...
				parent.art.dialog({
					content: "请注意，该不动产单元号（"+rows[i].BDCDYH+"）状态为"+cqzt+"，不可办理房屋租赁登记！",
					icon:"warning",
					ok: true
				});
				unaccept = true;
