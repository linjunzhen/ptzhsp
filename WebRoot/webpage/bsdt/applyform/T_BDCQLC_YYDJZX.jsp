...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/js/bdcUtil.js"></script>
 ...
 <script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/bdcqlcyydjzx/js/yydjzx.js"></script> 
...
$(function(){
	...
	if(flowSubmitObj){
		...
		$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
		 //初始化表单字段权限控制
		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_YYDJZX_FORM");
		//初始化表单字段值
		if(flowSubmitObj.busRecord){
			initAutoTable(flowSubmitObj);//初始化信息	
			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDCQLC_YYDJZX_FORM");
			if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始'){
				$("#T_BDCQLC_YYDJZX_FORM input").each(function(index){
					$(this).attr("disabled","disabled");
				});
				$("#T_BDCQLC_YYDJZX_FORM select").each(function(index){
					$(this).attr("disabled","disabled");
				});
...
// 			alert($(o).parent().parent().parent().parent().parent().parent().attr('id'));
			...
			if (GT2ICROCX.GetState() == 0){
				GT2ICROCX.ReadCard();
				$("input[name='SQR_MC']").val(GT2ICROCX.Name)
				$("input[name='SQR_ZJNO']").val(GT2ICROCX.CardNo)
			}
		} 
...
<body>
	...
	<form id="T_BDCQLC_YYDJZX_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/bdcqlcyydjzx/T_YYDJZX_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始引入预购预告信息 --%>
		<jsp:include page="./bdcqlc/bdcqlcyydjzx/T_YYDJZX_INFO.jsp" />
		<%--结束引入预购预告信息--%>
