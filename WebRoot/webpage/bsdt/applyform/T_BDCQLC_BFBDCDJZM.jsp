...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/js/bdcUtil.js"></script>
	...
	<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/js/bdcDjxx.js"></script>
 <script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/bdcqlcbfbdcdjzm/js/bfbdcdjzm.js"></script>
...
$(function(){
	...
	if(flowSubmitObj){
		...
		$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
		 //初始化表单字段权限控制
		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_BFBDCDJZM_FORM");
		//初始化表单字段值
		if(flowSubmitObj.busRecord){
			var type = $("input[type='radio'][name='TYPE'][checked='checked']").val();
			if (type == '1') {
				initPowerTable();//初始化权利信息
			}
			initAutoTable(flowSubmitObj);//初始化信息
			/*初始化缴费信息*/
			initDjfzxxTable(flowSubmitObj);
			initDjjfxxTable(flowSubmitObj);
			var BDC_OPTYPE = $("input[name='BDC_OPTYPE']").val();
			addInitBbfbdcdjzmInfo(flowSubmitObj.busRecord,flowSubmitObj.EFLOW_CURUSEROPERNODENAME,'${sessionScope.curLoginUser.fullname}','<%=dateTime%>',BDC_OPTYPE);
			isSetQlrxx(flowSubmitObj.busRecord.TYPE);
			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDCQLC_BFBDCDJZM_FORM");

			if (flowSubmitObj.busRecord.RUN_STATUS) {
				if (flowSubmitObj.busRecord.RUN_STATUS != 0) {
					/*其它环节审批表可以进行打印*/
					$("#SPBDF").attr("onclick","goPrintTemplate('BFBDCQZSHBDCDJZMSPB','3');");
					$("#SPBSF").attr("onclick","goPrintTemplate('BFBDCQZSHBDCDJZMSPB','3');");
					if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始'){
						$("#T_BDCQLC_BFBDCDJZM_FORM input").each(function(index){
							$(this).attr("disabled","disabled");
						});
						$("#T_BDCQLC_BFBDCDJZM_FORM select").each(function(index){
							$(this).attr("disabled","disabled");
						});
			...
			//补发不动产权证书和不动产登记证明（补发不动产登记证明）模块中，持证类型默认为共同持有，登记原因默认为"补发不动产权证书和不动产登记证明（补发不动产登记证明）"。
			$("select[name='TAKECARD_TYPE']").val("1");
			$('#DJYY').val("补发不动产权证书和不动产登记证明（补发不动产登记证明）");
...
<body>
	...
	<form id="T_BDCQLC_BFBDCDJZM_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<input type="hidden" name="QSLY_JSON" value="${busRecord.QSLY_JSON}" />
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/bdcqlcbfbdcdjzm/T_BFBDCDJZM_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始引入权利人信息--%>
		<jsp:include page="./bdcqlc/bdcQlr.jsp" />
		<%--结束引入权利人信息--%>
		
		<%--开始权属来源信息--%>
		<jsp:include page="./bdcqlc/bdcQsly.jsp" />
		<%--开始权属来源信息--%>
		
		<%--开始引入基本信息 --%>
		<div id="bfbdcdjzmInfo">
		</div>
		<%--结束引入基本信息--%>
