	...
	if(flowSubmitObj){
		...
		if(flowSubmitObj.busRecord){
			...
			if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始'){
				$("#T_BDC_GYJSSCDJ_FORM input").each(function(index){
					$(this).attr("disabled","disabled");
				});
				$("#T_BDC_GYJSSCDJ_FORM select").each(function(index){
					$(this).attr("disabled","disabled");
				});
...
function checkBdcdyh(){
   if($("input[name='IS_DXZPGXM']").is(':checked')){
   		$('input[name="ESTATE_NUM"]').attr("class","tab_text validate[[],custom[estateNum]]");
   		$('#bdcdyh_font').attr("style","display:none");		
   }else{
   		$('input[name="ESTATE_NUM"]').attr("class","tab_text validate[[required],custom[estateNum]]");
   		$('#bdcdyh_font').attr("style","");	
   }
}
...
<body>
	...
	<form id="T_BDC_GYJSSCDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
...
<!-- 		<%--结束引入公用申报对象 --%> -->
		...
		<%--开始引入权利信息--%>
		<jsp:include page="./estate/T_ESTATE_RIGHTINFO.jsp" />
		<%--结束引入权利信息--%>
