...
<script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/js/bdcUtil.js"></script>
 ...
 <script type="text/javascript" src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/bdcqlcygspfygdjbg/js/ygspfygdjbg.js"></script> 
...
$(function(){
	...
	if(flowSubmitObj){
		...
		$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
		 //初始化表单字段权限控制
		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_YGSPFYGDJBG_FORM");
		//初始化表单字段值
		if(flowSubmitObj.busRecord){
			initAutoTable(flowSubmitObj);//初始化信息	
			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDCQLC_YGSPFYGDJBG_FORM");

			if (flowSubmitObj.busRecord.RUN_STATUS && flowSubmitObj.busRecord.RUN_STATUS != 0) {
				/*其它环节审批表可以进行打印*/
				$("#SPBDF").attr("onclick","goPrintTemplate('YGSPFYGDJBGSPB','3');");
				$("#SPBSF").attr("onclick","goPrintTemplate('YGSPFYGDJBGSPB','3');");
			}

			if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME!='开始'){
				$("#T_BDCQLC_YGSPFYGDJBG_FORM input").each(function(index){
					$(this).attr("disabled","disabled");
				});
				$("#T_BDCQLC_YGSPFYGDJBG_FORM select").each(function(index){
					$(this).attr("disabled","disabled");
				});
			...
			//预告登记的变更（预购商品房预告登记变更）模块中，持证类型默认为共同持有，登记原因默认为"预告登记的变更（预购商品房预告登记变更）"。
			$("select[name='TAKECARD_TYPE']").val("1");
			$('#DJYY').val("预告登记的变更（预购商品房预告登记变更）");
...
<body>
	...
	<form id="T_BDCQLC_YGSPFYGDJBG_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息--%>
		<jsp:include page="./bdcqlc/bdcqlcygspfygdjbg/T_YGSPFYGDJBG_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始登记审核信息--%>
		<%--开始引入预购预告信息 --%>
		<jsp:include page="./bdcqlc/bdcqlcygspfygdjbg/T_YGSPFYGDJBG_YGYG.jsp" />
		<%--结束引入预购预告信息--%>
