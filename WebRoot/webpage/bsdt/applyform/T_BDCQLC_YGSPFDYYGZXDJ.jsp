...
<link rel="stylesheet" type="text/css"
 ...
 <script type="text/javascript" 
 	src="<%=basePath%>/webpage/bsdt/applyform/bdcqlc/bdcqlcygspfdyzxdj/js/ygspfdyygzxdj.js"></script>
...
$(function(){
	...
	if(flowSubmitObj){
		...
		$("#EXEID").append(flowSubmitObj.EFLOW_EXEID);
		 //初始化表单字段权限控制
		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BDCQLC_YGSPFDYYGZXDJ_FORM");
		//初始化表单字段值
		if(flowSubmitObj.busRecord){
			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BDCQLC_YGSPFDYYGZXDJ_FORM");

			if (flowSubmitObj.busRecord.RUN_STATUS && flowSubmitObj.busRecord.RUN_STATUS != 0) {
				/*其它环节审批表可以进行打印*/
				$("#SPBDF").attr("onclick","goPrintTemplate('YGSPFDYYGZXDJSPB','3');");
				$("#SPBSF").attr("onclick","goPrintTemplate('YGSPFDYYGZXDJSPB','3');");
			}

			if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME != '开始'){
				$("#T_BDCQLC_YGSPFDYYGZXDJ_FORM input").each(function(index){
					$(this).attr("disabled","disabled");
				});
				$("#T_BDCQLC_YGSPFDYYGZXDJ_FORM select").each(function(index){
					$(this).attr("disabled","disabled");
				});
				...
				if(flowSubmitObj.busRecord.RUN_STATUS!=0 && flowSubmitObj.EFLOW_CURUSEROPERNODENAME=='登簿'){
					$("#ygspfdyygDb").show().attr("disabled", false);
					$(".dbxxtr").show();
...
function FLOW_SubmitFun(flowSubmitObj){
	 //先判断表单是否验证通过
	 var validateResult =$("#T_BDCQLC_YGSPFDYYGZXDJ_FORM").validationEngine("validate");
	 if(validateResult){
	     ...
	     if(isAuditPass == "0"){
			 ...
			 if(submitMaterFileJson||submitMaterFileJson==""){
				 ...
				 //先获取表单上的所有值
				 var formData = FlowUtil.getFormEleData("T_BDCQLC_YGSPFDYYGZXDJ_FORM");
...
<body>
	...
	<form id="T_BDCQLC_YGSPFDYYGZXDJ_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<%--开始引入受理信息 --%>
		<jsp:include page="./bdcqlc/bdcqlcygspfdyzxdj/T_YGSPFDYYGZXDJ_ACCEPTINFO.jsp" />
		<%--结束引入受理信息--%>
		...
		<%--开始引入注销信息 --%>
		<jsp:include page="./bdcqlc/bdcqlcygspfdyzxdj/T_YGSPFDYYGZXDJ_ZXXX.jsp" />
		<%--结束引入注销信息--%>
		
		<%--开始引入注销明细 --%>
		<jsp:include page="./bdcqlc/bdcqlcygspfdyzxdj/T_YGSPFDYYGZXDJ_ZXMX.jsp" />
		<%--结束引入注销明细 --%>
