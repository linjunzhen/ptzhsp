...
<head>
	...
	* 查看单位工程信息
	**/
	function getSbgcInfo(formName,id){	
		var rowJson = $("#"+id).find("[name='ROW_JSON']").val();
		var rowxx = JSON.parse(rowJson);
		delete rowxx.SGRY;
		delete rowxx.JLRY;
		var url = "projectSgxkController.do?addSbInfo&type=0&formName="+formName+"&info="+encodeURIComponent(JSON.stringify(rowxx));
		art.dialog.open(url, {  
			title : "上部(地下室)子工程信息",
			width:"1150px",
			lock: true,
...
<body class="eui-diabody" style="margin:0px;">
	<form id="dwinfoForm" method="post">
		...
		<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20">
			<tr>
				<th><font class="syj-color2">*</font>上部(地下室)工程名称</th>
				<td colspan="3"><input type="text" class="syj-input1 validate[required]" value="${info.SINGLEPRONAME}"
					name="SINGLEPRONAME" maxlength="100" placeholder="请输入上部(地下室)单位名称"/></td>	
			</tr>	
			<tr>
		...
		<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20" style="margin-top: -1px;">		
			...
			<tr>
				...
				<td colspan="3">
					<span class="singleProMaintype_1_sbgc">
					<eve:excheckbox
					...
					<span class="singleProMaintype_2_sbgc">
					<eve:excheckbox
						dataInterface="dictionaryService.findTwoDatasForSelect"
						name="STRUCTQUALTYPE_SBGC" width="749px;" clazz="checkbox validate[required]" value="${info.STRUCTQUALTYPE_SBGC}" 
		...
		<c:if test="${!empty childrensbgcList}">
		<table cellpadding="0" cellspacing="1" class="syj-table2" id="childrenSbgcTable" style="margin-top:5px;">
			<tr>
			...
			<c:forEach items="${childrensbgcList}" var="sbzgc" varStatus="d">
				<tr class="sbgcTr" id="sbgcTr_${d.index}">
					<input type="hidden" name="ROW_JSON" value="${sbzgc.ROW_JSON}"/>
					<td style="text-align: center;">${sbzgc.SINGLEPRONAME}</td>
					<td style="text-align: center;">						
						<eve:eveselect clazz="tab_text validate[required] gclb_width" dataParams="PROTYPE"
						dataInterface="dictionaryService.findDatasForSelect" 
						defaultEmptyText="请选择工程类别" name="${d.index}SINGLEPROMAINTYPE" id="${d.index}SINGLEPROMAINTYPE" value="${sbzgc.SINGLEPROMAINTYPE_SBGC}">
						</eve:eveselect>
					</td>
					<td style="text-align: center;">${sbzgc.STRUCTQUALTYPE_SBGC}</td>
					<td style="text-align: center;">${sbzgc.ARCHHEIGHT} ${sbzgc.MUNILENGTH}</td>
					<td style="text-align: center;">${sbzgc.ARCHAREA} ${sbzgc.FLOORAREA}</td>
					<td style="text-align: center;">${sbzgc.ITEMINVEST}</td>
					<td style="text-align: center;">
					<a href="javascript:void(0);" class="eflowbutton" onclick="getSbgcInfo('sbzgc','sbgcTr_${d.index}');">查 看</a>
					</td>
				</tr>
			</c:forEach>
