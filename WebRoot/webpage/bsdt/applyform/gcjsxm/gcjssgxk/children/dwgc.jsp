...
<head>
	...
	* 查看单位工程信息
	**/
	function getDwgcInfo(formName,id){	
	...
	<form id="dwinfoForm" method="post">
		...
		<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20" style="margin-top: -1px;">		
			...
			<tr>
				...
				<td colspan="3">
					<span class="singleProMaintype_1">
					<eve:excheckbox
					...
					<span class="singleProMaintype_2">
					<eve:excheckbox
						dataInterface="dictionaryService.findTwoDatasForSelect"
						name="STRUCTQUALTYPE" width="749px;" clazz="checkbox validate[required]" value="${info.STRUCTQUALTYPE}" 
		...
		<c:if test="${!empty childrendwgcList}">
		<table cellpadding="0" cellspacing="1" class="syj-table2" id="childrenDwgcTable" style="margin-top:5px;">
			<tr>
			...
			<c:forEach items="${childrendwgcList}" var="dwzgc" varStatus="d">
				<tr class="dwgcTr" id="dwgcTr_${d.index}">
					<input type="hidden" name="ROW_JSON" value="${dwzgc.ROW_JSON}"/>
					<td style="text-align: center;">${dwzgc.SINGLEPRONAME}</td>
					<td style="text-align: center;">						
						<eve:eveselect clazz="tab_text validate[required] gclb_width" dataParams="PROTYPE"
						dataInterface="dictionaryService.findDatasForSelect" 
						defaultEmptyText="请选择工程类别" name="${d.index}SINGLEPROMAINTYPE" id="${d.index}SINGLEPROMAINTYPE" value="${dwzgc.SINGLEPROMAINTYPE}">
						</eve:eveselect>
					</td>
					<td style="text-align: center;">${dwzgc.STRUCTQUALTYPE}</td>
					<td style="text-align: center;">${dwzgc.ARCHHEIGHT} ${dwzgc.MUNILENGTH}</td>
					<td style="text-align: center;">${dwzgc.ARCHAREA} ${dwzgc.FLOORAREA}</td>
					<td style="text-align: center;">${dwzgc.ITEMINVEST}</td>
					<td style="text-align: center;">
					<a href="javascript:void(0);" class="eflowbutton" onclick="getDwgcInfo('dwgc','dwgcTr_${d.index}');">查 看</a>
					</td>
				</tr>
			</c:forEach>
