...
<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="dwgcTable">
	<tr>
	...
	<c:forEach items="${dwgcList}" var="dwgc" varStatus="s">
		<tr class="dwgcTr" id="dwgcTr_${s.index}">
			<input type="hidden" name="ROW_JSON" value="${dwgc.ROW_JSON}"/>
			<td style="text-align: center;">${dwgc.SINGLEPRONAME}</td>
			<td style="text-align: center;">
				...
				defaultEmptyText="请选择工程类别" name="SINGLEPROMAINTYPE" id="SINGLEPROMAINTYPE" value="${dwgc.SINGLEPROMAINTYPE}">
				</eve:eveselect>
			</td>
			<td style="text-align: center;">${dwgc.STRUCTQUALTYPE}</td>
			<td style="text-align: center;">${dwgc.ARCHHEIGHT} ${dwgc.MUNILENGTH}</td>
			<td style="text-align: center;">${dwgc.ARCHAREA} ${dwgc.FLOORAREA}</td>
			<td style="text-align: center;">${dwgc.ITEMINVEST}</td>
			<td style="text-align: center;">
			<a href="javascript:void(0);" class="eflowbutton" onclick="getDwgcInfo('dwgc','${s.index}');">查 看</a>
			</td>
		</tr>
	</c:forEach>
</table>
