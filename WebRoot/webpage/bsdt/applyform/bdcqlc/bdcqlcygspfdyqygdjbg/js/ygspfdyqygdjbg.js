...
function FLOW_SubmitFun(flowSubmitObj){	
	 //先判断表单是否验证通过
	 var validateResult =$("#T_BDCQLC_YGSPFDYQYGDJBG_FORM").validationEngine("validate");
	 if(validateResult){
		getQslyInfoJson();
	     ...
	     if(isAuditPass == "-1"){
			 ...
			 if(submitMaterFileJson||submitMaterFileJson==""){
				 ...
				 //先获取表单上的所有值
				 var formData = FlowUtil.getFormEleData("T_BDCQLC_YGSPFDYQYGDJBG_FORM");
		...
		close: function () {
			...
			if(bdcdydacxInfo && bdcdydacxInfo.length == 1){
				var dyinfo = {
					...
					"YWR_MC":bdcdydacxInfo[0].YWR,
					"YWR_ZJLX":bdcdydacxInfo[0].YWRZJZL,
					"YWR_ZJNO":bdcdydacxInfo[0].YWRZJH,
				...
				if(result.isPass){
					...
					FlowUtil.initFormFieldValue(dyinfo,"T_BDCQLC_YGSPFDYQYGDJBG_FORM");		
...
function bdcYgspfdyqygdjbgBooking(){
...
function bdcYgspfdyqygdjbgDJZMprint(typeId) {
	...
	if(DY_BDCQZBSM){
		...
		dataStr +="&BDCQZH="+$("input[name='DJXX_CQZH']").val();
		dataStr +="&EFLOW_EXEID="+$("#EXEID").text();
