    ...
    function saveDjshOpinion() {
		...
		if(flowSubmitObj){				
			...
			if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '预登簿' || flowSubmitObj.EFLOW_CURUSEROPERNODENAME == '登簿') {
				params.DJSHXX_HZYJ = $("textarea[name='DJSHXX_HZYJ']").val();
				isok = true;
			}
			if(!isok){					
				art.dialog({
			...
			if (flowSubmitObj) {
				AppUtil.ajaxProgress({
					url: "bdcQlcSffzInfoController.do?saveBdcscdjshxxOpinion",
					params: params,
					callback: function (data) {
						if (data) {
							if (data.success) {
								$("input[name='DJSHXX_SHR']").val(data.DJSHXX_SHR);
								$("input[name='DJSHXX_SHJSRQ']").val(data.DJSHXX_SHJSRQ);
								art.dialog({
									content: "保存成功",
									icon: "succeed",
									time: 3,
        ...
        <tr>
            <table cellpadding="0" cellspacing="1" class="tab_tk_pro2" id="djshHzOpinionInfo" style="display:none;">
                <tr id="hzyj">
                    ...
                    <td>
                        <textarea name="DJSHXX_HZYJ" cols="140" rows="10"  maxlength="2000"
                                  class="validate[required]">${busRecord.DJSHXX_HZYJ}</textarea>
                        <input type="button" class="eflowbutton" value="意见模板" onclick="cyyjmbSelector('bdcqlcOpinion_hz','DJSHXX_HZYJ');">
                    </td>
                </tr>
                <tr>
                    <td class="tab_width"><font class="tab_color">*</font>填写人：</td>
                    <td>
                        <input type="text" disabled="disabled" class="tab_text validate[required]" name="DJSHXX_SHR" value="${busRecord.DJSHXX_SHR}">
                    </td>
                </tr>
                <tr>
                    <td class="tab_width"><font class="tab_color">*</font>填写时间：</td>
                    <td>
                        <input type="text" disabled="disabled" class="tab_text validate[required]" name="DJSHXX_SHJSRQ" value="${busRecord.DJSHXX_SHJSRQ}">
                    </td>
                </tr>
            </table>
        </tr>
