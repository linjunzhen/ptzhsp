...
<script type="text/javascript">
	$(function(){
    	...
    	if(flowSubmitObj){
    		...
    		var currentNodeFieldRights = flowSubmitObj.currentNodeFieldRights;
    		 //初始化表单字段权限控制
    		FlowUtil.initFormFieldRightControl(currentNodeFieldRights,"T_BSFW_SHTZJGYS_FORM");
    		//初始化表单字段值
    		if(flowSubmitObj.busRecord){
    			FlowUtil.initFormFieldValue(flowSubmitObj.busRecord,"T_BSFW_SHTZJGYS_FORM");
    	...
    	if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME){
    			if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME=="汇总意见"){
    				...
    				$("#YWCZ_TABLE").attr("style","");
    				$("#LDQFSFTG_TR").attr("style","");
    			}else if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME=="填写公示结果"){
    				$("#YWCZ_TABLE").attr("style","");
    				$("#LDQFSFTG_TR").attr("style","");
    				$("#GSSFTG_TR2").attr("style","");
    				$("#GSSFTG_TR3").attr("style","");
    			}else if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "拟批复联合竣工验收申请"
                        ...
                        || flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "批复联合竣工验收申请") {
                    $("#YWCZ_TABLE").attr("style", "");
                    $("#GSSFTG_TR2").attr("style", "");
    				$("#GSSFTG_TR3").attr("style","");
					if('${busRecord.GSJG}'==-1){
					...
					if(flowSubmitObj.EFLOW_CURUSEROPERNODENAME=="批复联合竣工验收申请"){
	    				$("#PF_TR").attr("style","");
	    			}
                    //$("#NHZ_TR").attr("style", "");
                }
				if (flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "填写公示材料"
						...
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "拟批复联合竣工验收申请"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区行政审批局项目投资处领导审批"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区审批局领导审签"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "区管委会领导签发"
						|| flowSubmitObj.EFLOW_CURUSEROPERNODENAME == "批复联合竣工验收申请") {
					...
					$("#GS_TABLE :input[type='text']")
							.each(function(i) {
								if (this.name == "ZTZ") {
									...
									$("input[name='" + this.name + "']")
										.attr("class","tab_text validate[required,custom[fixOrMobilePhone]]");
	...
	function FLOW_SubmitFun(flowSubmitObj){
		 //先判断表单是否验证通过
		 var validateResult =$("#T_BSFW_SHTZJGYS_FORM").validationEngine("validate");
		 if(validateResult){
			 ...
			 if(submitMaterFileJson||submitMaterFileJson==""){
				 ...
				 //先获取表单上的所有值
				 var formData = FlowUtil.getFormEleData("T_BSFW_SHTZJGYS_FORM");
...
<body>
   ...
   <form id="T_BSFW_SHTZJGYS_FORM" method="post" >
	...
	<table cellpadding="0" cellspacing="1"  class="tab_tk_pro">
		<tr>
			<td class="z_tab_width">材料备注：</td>
			<td><textarea rows="6" cols="140" style='width:98%;' name="CLBZ" maxlength="500"></textarea></td>
		</tr>
	</table>
	<%--开始引入公用申报对象--%>
	...
	<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="GS_TABLE" style="display: none;">
		...
		<tr>
			...
			<td colspan="3"><textarea rows="10" cols="140"  maxlength="1000"  name="JSGMJZYNR"></textarea> </td>
		</tr>
		<tr>
			<td class="z_tab_width"><font class="tab_color">*</font> 公示起始时间：</td>
			<td><input type="text" name="GSKSSJ"  id="_GSKSSJ" style="width:190px;height:24px;line-height: 24px;" readonly="readonly"
						class="laydate-icon" /></td>
			<td class="z_tab_width"><font class="tab_color">*</font> 公示结束时间：</td>
			<td><input type="text" name="GSJSSJ" id="_GSJSSJ" style="width:190px; height:24px;line-height: 24px;" readonly="readonly"
						class="laydate-icon" /></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><input type="text" name="EMAIL" maxlength="26"
				/></td>
			<td class="z_tab_width"><font class="tab_color">*</font> 邮政编码：</td>
			<td><input type="text" name="POSTCODE" maxlength="8"
				/></td>
		</tr>
		<tr>
	...
	<table cellpadding="0" cellspacing="1" class="tab_tk_pro" id="YWCZ_TABLE" style="display: none;">
		...
		<tr id="SHSFXYXT_TR" style="display: none;">
		...
		<tr id="SHSFXTYZ_TR2" style="display: none;">
		...
		<tr id="GSSFTG_TR2" style="display: none;">
			...
			<td><eve:radiogroup typecode="GSJG" width="130px"
					fieldname="GSJG" defaultvalue="1" onclick="if(this.value=='-1'){$('#GSXT_TR').show();var x=document.getElementsByName('XTSFTG');for(var i=0;i<x.length;i++){if(x[i].value==1)x[i].checked=true;}}else {$('#GSXT_TR').hide();var x=document.getElementsByName('XTSFTG');for(var i=0;i<x.length;i++){if(x[i].checked==true)x[i].checked=false;}}"></eve:radiogroup></td>
		</tr>
		...
		<tr id="GSSFTG_TR3" style="display: none;">
            <td class="tab_width"><font class="tab_color">*</font>公示情况：</td>
            <td><textarea rows="10" cols="140" maxlength="1998" name="GSJGSM"></textarea> </td>
		</tr>
