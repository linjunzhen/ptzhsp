...
<script type="text/javascript">
	...
	function addOther(EFLOW_VARS){
		...
		var SQRZJPZDZ = $("input[name$='JYXXDZ']").eq(0).val();
		//个人或者企业地址
		EFLOW_VARS.APPLY_ADDR = SQRZJPZDZ;
	...
	function submitExt(EFLOW_VARS){
		...
		var SQLX = "";
		$("[name='SQLX']:checked").each(function(i, o){
			SQLX+=","+$(o).val();  
		});
		if(SQLX.length>0){
			SQLX = SQLX.substring(1,SQLX.length);
		}			
		addOther(EFLOW_VARS);
	...
	function checkSp(){
		...
		$("#PRODUCT_FORM").find("[name$='CPXX_PZMX']").each(function(i){
			if($(this).val() == ''){
				art.dialog({
            		content: "请先完善产品信息品种明细！",
            		icon:"warning",
            		ok: true
        		});
				$('#PRODUCT_FORM_A').parent().trigger("click");
			}
		});
		$('.del-tr').css("display","table-row");
	}
	function checkSplb(){
		var isEmpty = false;
		$("#PRODUCT_FORM").find("[name$='CPXX_LBBH']").each(function(i){
			if($(this).val() == ''){
				isEmpty = true;
			}
		});
...
<body style="background: unset;">
   ...
   <form id="T_FJFDA_SPSCXK_JBXX_FROM" method="post">
		...
		<!-- 对接对应省平台的事项代码 -->
		<input type="hidden" name="FJFDA_ITEM_CODE" value="001XK00201" />
		<%--===================重要的隐藏域内容=========== --%>
