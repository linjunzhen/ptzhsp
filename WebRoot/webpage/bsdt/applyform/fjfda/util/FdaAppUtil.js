var FdaAppUtil = new Object({
	/**
	 * 重新加载地址栏信息
	 */
	reloadHrefLocation:function(){
		window.top.location.reload();
	},
	/**
	 * 设置药品注册公共提交参数值
	 */
	setMedicalRegistCommonFieldValues:function(EFLOW_VARS){
		//申请人名称
		EFLOW_VARS.SQRMC = $("input[name='SQRLXR']").val();
		//申请人手机号
		EFLOW_VARS.SQRSJH = $("input[name='SQRSJH']").val();
		//申请人类型:取值为数字“个人（办理个人业务）=0，法人（办理单位业务）=1”
		EFLOW_VARS.APPLYTYPE = "1";
		//证件类型取字典类别 DocumentType
		EFLOW_VARS.IDTYPE = "QT";
		//证件名称
		EFLOW_VARS.IDNAME = "医疗机构制剂许可证";
		//证件号码
		EFLOW_VARS.IDNUM = $("input[name='SQRYLJGZXK']").val();
		...
		var SQRZJPZDZ = $("input[name='SQRZJPZDZ']").val();
		//个人或者企业地址
		EFLOW_VARS.APPLY_ADDR = JYCSQS+JYCSXS+JYCSXZ+SQRZJPZDZ;
		//申报人短信接收者名称
		EFLOW_VARS.EFLOW_MSGRECEIVER = $("input[name='SQRLXR']").val();
		return EFLOW_VARS;
	},
	/**
	 * 导入EXCEL数据
	 */
	importExcelData:function(config){
		var leftSpanSize = 0;
		var impCode = config.impCode;
		var fileId = config.fileId;
		var gridId = config.gridId;
		var masterId = config.masterId;
		var masterName = config.masterName;
	    leftSpanSize = $("#UploadFilesTd").children("div").length;
	    if(leftSpanSize<=0){
	        alert("请上传Excel文件");
	    }else{
	        var layload = layer.load("正在导入...");
	        $.post("excelImpController.do?impExcel",{
	            fileId:fileId,
	            impCode:impCode,
	            ...
	            masterId:masterId
	              ...
	              if(resultJson.success == true){
	                  ...
	                  art.dialog({
	                      title:"导入失败",
	                      resize: true,
	                      content: "<textarea style='height:150px;width:400px;font-size:14px;line-height:15px;'>"+resultJson.msg+"</textarea>",
	                      icon:"error",
	                      ok: function(){
	                      }
	                  });
	 ...
	 * 获取构建出来的函数方法体
	 */
	getFunctionName:function(paramArray,functionName){
	...
	setComboTreeValue:function(comboTreeName,isCheck,treeObj){
		if(!isCheck){
			...
			$("#"+comboTreeName+"_TREETEXT").val(nodes[0].name);
			$("input[name='"+comboTreeName+"']").val(nodes[0].id);
		}else{
	...
	hideComboTree:function(comboTreeName){
		...
		$("body").unbind("mousedown", function(){
			...
			if (!(event.target.id == contentId || $(event.target).parents("#"+comboTreeMenuContent).length>0)) {
				FdaAppUtil.hideComboTree(comboTreeName);
			}
		});
	},
	/**
	...
	setHideControls:function(hideNames){
		...
		targetFields.each(function(){
			...
			var isContain = FdaAppUtil.isContain(namesArray,fieldName);
			if(!isContain){
	 ...
	 * 申请编辑办件
	 */
	applyEditHandleData:function(EFLOW_EXEID,EFLOW_ISQUERYDETAIL,EFLOW_CURRENTTASK_ID,gridId){
		var row = FdaAppUtil.getEditDataGridRow(gridId);
    	if (row) {
    		var TARGET_EXEID = row.EXE_ID;
    		FdaAppUtil.ajaxProgress({
    			url : "handleEditApplyController.do?isAllowStart",
    			params : {
    				TARGET_EXEID:TARGET_EXEID
    			},
    			callback : function(resultJson) {
    				if(resultJson.success){
    					var url = "handleEditApplyController/info.do?EFLOW_EXEID="+EFLOW_EXEID+"&EFLOW_ISQUERYDETAIL="+EFLOW_ISQUERYDETAIL+"&EFLOW_CURRENTTASK_ID="+EFLOW_CURRENTTASK_ID;
    		        	url+=("&TARGET_EXEID="+TARGET_EXEID);
    		        	art.dialog.open(url, {
    		    			title : "办件修改申请",
    		    			width: "100%",
    		    		    height: "100%",
	 ...
	 * 查询单位树
	 */
	searchDepartTree:function(treeId,searchConditionId,button){
	...
	initDefaultValidateConfig:function(config){
		...
		forms.each(function(){
			...
			$("#"+formId).validationEngine("attach",{ 
				 binded:(config&&config.binded==false)?false:true,
			     promptPosition:(config&&config.promptPosition)?config.promptPosition:"topLeft",
			     autoPositionUpdate:true,
			     validateNonVisibleFields:true,
		         autoHidePrompt: (config&&config.autoHidePrompt)?config.autoHidePrompt:false,           
		         autoHideDelay: "2000",       
		         fadeDuration: "0.2",   
		         maxErrorsPerField: "1",  
	...
	validateWebsiteTabForm:function(isTemplateSave){
		...
		forms.each(function(){
			...
			if(!validateResult){
				...
				$("#"+formId+"_A").html("<img src=\"webpage/website/index/images/icon2.png\">"+text);
				//return false;
			}else{
				$("#"+formId+"_A").html("<img src=\"webpage/website/index/images/icon1.png\">"+text);
	...
	submitWebSiteFlowForm:function(formId){
		//先判断表单是否验证通过
		 var validateResult =$("#"+formId).validationEngine("validate");
		 if(validateResult){
			 var submitMaterFileJson = FdaAppUtil.getSubmitMaterFileJson();
	...
	getWindowSubmitMaterFileJson:function(isTempSave){
		 ...
		 for(var index in applyMatersObj){
			...
			//获取收取方式
			var RECEIVE_METHOD = $("#RECEIVE_METHOD_"+MATER_CODE).val();
			if(!RECEIVE_METHOD){
				RECEIVE_METHOD = "1";
			}
			if(MATER_ISNEED=="1"){
				...
				}else if(RECEIVE_METHOD=="4"){
					var leftSpanSize = $("#UploadedFiles_DZZZ_"+MATER_CODE).children("p").length;
					if(leftSpanSize==0){
						isUploadMustFile = false;
						break;
					}
	...
	initNetUploadMaters:function(config){
		...
		for(var index in applyMatersObj){
			...
			var swUploadObj = FdaAppUtil.bindSwfUpload({
				file_types : MATER_TYPE,
				file_size_limit :MATER_SIZE+" MB",
				post_params : {
			           "uploadPath":config.uploadPath?config.uploadPath:"applyform",
	            ...
	            upload_success_handler:function(resultJson){
	            	if(resultJson.success==true){
	            		...
	            		spanHtml +="<a href=\"javascript:void(0);\"  onclick=\"FdaAppUtil.delUploadMater('"+fileId+"','"+attachKey+"');\" ><font color=\"red\">删除</font></a></p>"
	 ...
	 * 
	 */
	reloadExRadio:function(radioName,config){
		var radioDiv = $("#"+radioName+"_RADIO");
		var params = {};
		$.each(radioDiv.get(0).attributes, function(i, obj){
			params[obj.name] = obj.value;
		});
		params = FdaAppUtil.mergeObject(config,params);
		params.name = radioName;
		FdaAppUtil.ajaxProgress({
			url : "eveControlController/reloadExRadio.do",
			params :params,
			callback : function(resultJson) {
				var resultHtml = resultJson.resultHtml;
				radioDiv.parent().html(resultHtml);
			}
		});
	},
	/**
	 ...
	 * 
	 */
	reloadExCheckBox:function(checkBoxName,config){
		var checkBoxDiv = $("#"+checkBoxName+"_CHECKBOX");
		var params = {};
		$.each(checkBoxDiv.get(0).attributes, function(i, obj){
			params[obj.name] = obj.value;
		});
		params = FdaAppUtil.mergeObject(config,params);
		params.name = checkBoxName;
		FdaAppUtil.ajaxProgress({
			url : "eveControlController/reloadExCheckBox.do",
			params :params,
			callback : function(resultJson) {
				var resultHtml = resultJson.resultHtml;
				checkBoxDiv.parent().html(resultHtml);
			}
		});
	},
	/**
	 ...
	 * var select = $("select[name='LEAVE_TYPE']");
	   FdaAppUtil.reloadSelect(select,{dataParams:"ExpressWay"});
	 */
	reloadSelect:function(selectObj,config,callback){
		...
		FdaAppUtil.ajaxProgress({
			url : "fdaEveControlController/reloadSelect.do",
			params : {
				dataInterface:config.dataInterface?config.dataInterface:dataInterface,
				dataParams:config.dataParams?config.dataParams:dataParams,
	...
	enterSystem:function(loginFormId,isNeedValidCode){
	    ...
	    if (!$.support.leadingWhitespace) {
	       if(event.keyCode == 13){
		    	FdaAppUtil.submitLoginForm(loginFormId,isNeedValidCode);
		    }
	    }else{
	...
	submitLoginForm:function(loginFormId,isNeedValidCode){
		...
		var remember = FdaAppUtil.getCheckBoxCheckedValue("rememberPassword");
	...
	fileQueued:function(file){
		try{
			var size = FdaAppUtil.convertFileSize(file);
		} catch (ex) {
			this.debug(ex);
		}
	},
	...
	initUploadControl:function(config){
		...
		if(config.singleFileId){
			$.post("fileAttachController.do?get", {
				...
				if(fileName.length>30){
					...
					$("#"+config.singleFileDivId).append("<a href='#' style='float:left;height:28px; line-height:28px;' onclick='FdaAppUtil.downLoadFile(\""+fileId+"\");' >下载&nbsp;&nbsp;</a>");
				    $("#"+config.singleFileDivId).append("<a href='#' style='float:left;height:28px; line-height:28px;' onclick='FdaAppUtil.removeUploadFile(\""+fileId+"\",\""+config.singleFileDivId+"\",\""+singleFileFieldName+"\");' >删除</a>");
				}else{
					$("#"+config.singleFileDivId).html("<div style='float:left;margin:0px auto; height:28px; line-height:28px;'>"+fileName+"&nbsp;&nbsp;&nbsp;&nbsp;"+fileSize+"&nbsp;&nbsp;&nbsp;</div>");
					$("#"+config.singleFileDivId).append("<a href='#' style='float:left;height:28px; line-height:28px;' onclick='FdaAppUtil.downLoadFile(\""+fileId+"\");' >下载&nbsp;&nbsp;</a>");
				    $("#"+config.singleFileDivId).append("<a href='#' style='float:left;height:28px; line-height:28px;' onclick='FdaAppUtil.removeUploadFile(\""+fileId+"\",\""+config.singleFileDivId+"\",\""+singleFileFieldName+"\");' >删除</a>");
	    ...
	    if(config.uploadFileType&&config.uploadFileType=="image"){
	    	fileQueued = FdaAppUtil.fileQueued;
	    	...
	    	fileQueued = function(file) {
				try {
					var fileSize = FdaAppUtil.convertFileSize(file);
	    ...
	    if(config.uploadFileType&&config.uploadFileType=="image"){
	    	...
	    	uploadSuccess = function(file, serverData, responseReceived){
				...
				var fileId = resultJson.fileId;
				$("#"+config.singleFileDivId).append("<a href='#' style='float:left;height:28px; line-height:28px;' onclick='FdaAppUtil.downLoadFile(\""+fileId+"\");' >下载&nbsp;&nbsp;</a>");
			   $("#"+config.singleFileDivId).append("<a href='#' style='float:left;height:28px; line-height:28px;' onclick='FdaAppUtil.removeUploadFile(\""+fileId+"\",\""+config.singleFileDivId+"\",\""+config.singleFileFieldName+"\");' >删除</a>");
	    	...
	    	uploadSuccess = function(file, serverData, responseReceived){
				...
				$("#"+file.id+"_download").attr("onclick","FdaAppUtil.downLoadFile(\""+fileId+"\");")
				$("#"+file.id+"_delete").text('删除');
				$("#"+file.id+"_delete").attr("onclick","FdaAppUtil.removeUploadFile(\""+fileId+"\",null,null,\""+config.uploadTableId+"\",\""+config.uploadButtonId+"\");")
			    $("#"+file.id).progressbar({
	                 value: 100
	            });
			}
		...
		var swfu = new SWFUpload({
			...
			//加入上传队列处理完后触发 
			file_dialog_complete_handler : FdaAppUtil.fileDialogComplete,
			//在文件往服务端上传之前触发
			upload_start_handler : FdaAppUtil.uploadStart,
			//上传进度
			upload_progress_handler : uploadProgress,
			//上传失败，被终止或没成功触发
			upload_error_handler : FdaAppUtil.uploadError,
			//上传成功触发，不管是否成功接收
			upload_success_handler : uploadSuccess,
			//上传完成时触发
			upload_complete_handler : FdaAppUtil.uploadComplete,
			swfupload_loaded_handler:function(){
	...
	gridSearchReset:function(searchFormName){
		var a=$("form[name='"+searchFormName+"']").find("input[type='hidden']");  
		...
		if(version[1]){
			...
			{ 
				for (x in nameArray)
				{
					$("input[name='"+nameArray[x]+"']").val(valueArray[x]);
				}
			} 
		}
	...
	getQueryParams:function(elementId){
			...
			if(value!=null&&value!=""){
				...
				if(className=="laydate-icon"){
					if(value.indexOf(" ")==-1){
						...
						if(name.indexOf("FZRQ") != -1 ||name.indexOf("YXQ") != -1 ){
		...
		if(checkgroups){
			for(var j=0;j<checkgroups.length;j++){
				...
				if(name!=null&&name!=""){
					queryParams[name] = FdaAppUtil.getCheckBoxCheckedValue(name);
				}
			}
		}		
		return queryParams;
	...
	gridDoSearch:function(searchToolBarId,gridId){
		var queryParams = FdaAppUtil.getQueryParams(searchToolBarId);
		$("#"+gridId).datagrid('load',queryParams); 
		$("#"+gridId).datagrid('clearSelections').datagrid('clearChecked');
		if($("#"+searchToolBarId).find(".eui-toobar").height()> 94){
			$("#"+searchToolBarId).find(".eui-toobar").css("height","94px");
			$("#"+searchToolBarId).find(".eui-toobar").css("max-height","94px");
			$("#"+searchToolBarId).find(".highSearch").removeClass("on");
			$("#"+searchToolBarId).find(".highSearch").text("高级搜索");
		}
	},	
	/**
	 * easyUI grid做查询API
	...
	ajaxProgress:function(config){
		...
		/* $.post(config.url,config.params, function(responseText, status, xhr) {
		 ...
		 $.ajax({
	            type: "POST",
	            url: config.url,
	            timeout : config.timeout?config.timeout:60000, //超时时间设置，单位毫秒
	            data:config.params, 
	            cache: false,
	...
	getLoginUserInfo:function(){
		var userJsonInfo=parent.$("#loginUserInfoJson").val();
		var __curUser=$.parseJSON(userJsonInfo);
		return __curUser;
	},
	/**
	 * 判断当前用户是否拥有资源KEY
	...
	isGranted:function(resKey){
		if(!__curUser){
			__curUser = FdaAppUtil.getLoginUserInfo();
		}
	...
	initAuthorityRes:function(parentDomId){
		...
		for(var i =0;i<operateButtons.length;i++){
			...
			if(!FdaAppUtil.isGranted(resKey)){
	...
	deleteDataGridRecord:function(url,gridId,callback){
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			parent.art.dialog.confirm("您确定要删除该记录吗?", function() {
				var layload = layer.load('正在提交请求中…');
				var colName = $dataGrid.datagrid('options').idField;  
				var selectColNames = "";
				for ( var i = 0; i < rowsData.length; i++) {
		...
		$.ajaxSetup({
			...
			complete : function(XHR, TS) {
				if(XHR.responseText&&XHR.responseText=="loginrepeated"){					
					...
					alert("会话过期，请重新登录!");
				    window.top.location.href = __ctxPath+"/loginController.do?jumpToLogin";
     ...
     * @returns
     */
    convert:function(type, code) {
        var map = {};
        if (map[type] == null) {
            var map1 = {};
            $.ajax({
                url : 'dictionaryController.do?loadDicByType&typeCode=' + type,
                async : false,
                success : function(json) {
                    var data = $.parseJSON(json);
                    $.each(data, function(n, value) {
                        map1[value.DIC_CODE] = value.DIC_NAME;
                    });
                }
            });
            map[type] = map1;
            return map[type][code] == null ? '' : map[type][code];
        } else {
            return map[type][code] == null ? '' : map[type][code];
        }
    },
	...
	getStringSubFunction : function (String,endIndex,isHasSpot) {
		String = $.trim(String);
		var newString = "";
	    if(endIndex&&endIndex>0){
	    	var length = String.length;
	    	if(length>endIndex){
	    		newString = String.substr(0,endIndex);
	    		if(!(isHasSpot&&isHasSpot=='false')){
	    			newString += "...";
	    		}
	    	}else{
	    		newString = String;
	 ...
	 * 初始化select2下拉框
	 */
	initSelect2:function(selectorRule){
		var selectConfig = {
				language: "zh-CN"
		};
	 ...
	 * 设置某块区域下为只读
	 */
	setFieldReadOnlyControl:function(selectorRule){
		var targetFields = $(selectorRule).find("[name]");
		targetFields.each(function(){
			var targetField = $(this);
...
function highSearch(obj){
	$prepre = $(obj).parent().parent();
	if($(obj).hasClass("on")){
		$prepre.css("height","94px");
		$prepre.css("max-height","94px");
		$(obj).removeClass("on");
		$(obj).text("高级搜索");
	}else{
		$prepre.css("height","auto");
		$prepre.css("max-height","999px");
		$(obj).addClass("on");
		$(obj).text("收起");
	}
}
...
var t = "";
function autoHighSearch(id){
	/*if(t != ""){
		clearTimeout(t);
	}*/
	var width = $(window).width();
	t = setTimeout(function(){
		var $parentObj = $("#"+id).is("div")?$("#"+id):$("#"+id).parent();
		var queryLenth = $parentObj.find(".eui-toobar-con .eui-group").length;
		if($parentObj.children("div").get(0)){
			if($parentObj.children("div").get(0).offsetHeight == 0){
				$parentObj.find(".eui-toobar").css("top","0px");
			}else{
				$parentObj.find(".eui-toobar").css("top",$parentObj.children("div").get(0).offsetHeight+"px");
			}
		}else{
			$parentObj.find(".eui-toobar").css("top","0px");
		}
		$parentObj.find(".eui-toobar").prev().css("height",
				$parentObj.find(".eui-toobar")[0].offsetHeight+"px");
		$parentObj.find(".highSearch").hide();
		var m = (width-244)/251;
		if(m <4 && queryLenth>6){
			$parentObj.find(".highSearch").show();
		}
		if(m<5 && queryLenth>8){
			$parentObj.find(".highSearch").show();
		}
		if(m<6 && queryLenth>10){
			$parentObj.find(".highSearch").show();
		}
		if(m<7 && queryLenth>12){
			$parentObj.find(".highSearch").show();
		}
	},200);
}