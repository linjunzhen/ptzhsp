...
<script type="text/javascript"
	src="<%=basePath%>/webpage/bsdt/applyform/ybqlc/yrdwcb/js/yrdwcb.js"></script>
<script type="text/javascript">
	$(function() {					
		//地税比对结果不可编辑
		$('#DSBDJG').prop("disabled","disabled");
		//行政区划默认平潭县
	    $("select[name='XZQH']").append("<option value='350128' selected='true'>平潭县</option>");
	    //税务机构名称默认
	    $("input[name='SWJGMC']").val("平潭县地方税务局");	    	
		//初始化验证引擎的配置
		...
		if (flowSubmitObj) {
			...
			//初始化表单字段权限控制
			//FlowUtil.initFormFieldRightControl(currentNodeFieldRights, "T_YBQLC_YRDWCB_FORM");
			//初始化表单字段值
			if (flowSubmitObj.busRecord) {
				FlowUtil.initFormFieldValue(flowSubmitObj.busRecord, "T_YBQLC_YRDWCB_FORM");
				
				if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES =='审查' || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){
					 $("#T_YBQLC_YRDWCB_FORM").find("input,select").attr("disabled","disabled");				 
				}				
				if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES != '开始' || flowSubmitObj.EFLOW_EXERUNSTATUS == '2'){//除退回至开始环节外开放受理信息
		      	...
		      	if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES != '受理'){
					...
					xzConfGrid2.editor={};
					var xzConfGrid3 = $('#paramConfGrid').datagrid('getColumnOption', 'aab050');
					xzConfGrid3.editor={};
					
	    	 		//设置复选框不可选择
	    	 		$("#paramConfGrid").datagrid("hideColumn", "ck");	
	    	 		
					if(flowSubmitObj.EFLOW_CUREXERUNNINGNODENAMES =='审查'){
					 	/*  $("#pushInfo").attr("style","");//开放推送医保按钮
					 	 $('#pushYbBtn').removeAttr("disabled","disabled");	 */				 	 	
					 	 $('input[name="DWBXH"]').removeAttr("disabled","disabled");
					 	 $('input[name="DWBH"]').removeAttr("disabled","disabled");
					 }	    	 		
				}
...
<body>
	...
	<form id="T_YBQLC_YRDWCB_FORM" method="post">
		<%--===================重要的隐藏域内容=========== --%>
		...
		<jsp:include page="./ybqlc/yrdwcb/yrdwcb.jsp" >
