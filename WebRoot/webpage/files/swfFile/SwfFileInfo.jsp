...
<head>
    ...
    <script type="text/javascript">
	    $(document).ready(function(){
	    	...
	    	//初始化进度条
	    	$('#swfFileProcessBar').progressbar({value: 0});
	    	...
	    	uploader.on('fileQueued', function(file){
	    		...
	    		$('#swfFileProcessBar').progressbar({value: 0});
	    		$("#SwfFileForm").find("input[name='SWF_NAME']").val(file.name);
	    		$("#SwfFileForm").find("input[name='SWF_SUFFIX']").val(file.ext);
	    		//格式化文件大小
	    		var fileSize = WebUploader.Base.formatSize(file.size);
	    		$("#SwfFileForm").find("input[name='SWF_SIZE']").val(fileSize);
	    	});
	    	...
	    	uploader.on('uploadStart', function(file){
	    		...
	    		//编辑
	    		if('${swfFile.SWF_ID}'){
	    			uploader.options.formData.uid = '${swfFile.SWF_ID}';
	    		//新增
	    		}else{
	    	...
	    	uploader.on('uploadProgress', function(file, percentage){
	    	    ...
	    	    $('#swfFileProcessBar').progressbar({value: progress});
	    	});
	    	//当文件上传成功时触发
	    	uploader.on('uploadSuccess', function(file, response){
	    		if(response.success){
	    			...
	    			parent.$("#SwfFileViewGrid").datagrid("reload");
					AppUtil.closeLayer();
	    		}else{
	    			$('#swfFileProcessBar').progressbar({value: 0});
	    			parent.art.dialog({
	    	...
	    	AppUtil.initWindowForm("SwfFileForm", function(form, valid){
        		if(valid){
        			if(fileInQueque){
        				uploader.upload();
        			}else{
...
<body style="margin:0px;background-color: #f7f7f7;">
    <form id="SwfFileForm" method="post">
        <!--==========隐藏域部分开始 ===========-->
        <input type="hidden" name="TYPE_ID" value="${swfFile.TYPE_ID}"/>
        <!--==========隐藏域部分结束 ===========-->
			...
			<tr style="line-height:9px;">
			    <td>
			        <span style="width:110px;text-align:right;float:left;line-height:28px;">
			            <font class="dddl_platform_html_requiredFlag">*</font>flash名：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="SWF_NAME" style="width:240px;float:left;" value="${swfFile.SWF_NAME}" readonly/>
			        <div id="swfFilePickBtn" style="float:left;margin-left:3px;">选择</div>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;">
			            <font class="dddl_platform_html_requiredFlag">*</font>flash类型：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="SWF_SUFFIX" style="width:240px;float:left;" value="${swfFile.SWF_SUFFIX}" readonly/>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;">
			            <font class="dddl_platform_html_requiredFlag">*</font>flash大小：
			        </span>
			        <input class="eve-input validate[required]" type="text" name="SWF_SIZE" style="width:240px;float:left;" value="${swfFile.SWF_SIZE}" readonly/>
			    </td>
			</tr>
			<tr>
			    <td>
			        <span style="width:110px;text-align:right;float:left;">上传进度：</span>
			        <div id="swfFileProcessBar" style="width:240px;"></div>
			    </td>
			</tr>
        </table>
