function loadFactorList(processCode){
	$("#busesuperchange_addprocess").val(processCode);
	$('#BusEsuperChangeAddGrid').datagrid({  //初始化datagrid
	    ...
	    onBeforeEdit:function(index,row){
	        row.editing = true;
	        $('#BusEsuperChangeAddGrid').datagrid('refreshRow', index);
	        editcount++;
	    },
...
function editrow(index){
	$('#BusEsuperChangeAddGrid').datagrid('beginEdit', index);
}
function deleterow(index){
	$.messager.confirm('确认','是否真的删除?',function(r){
		if (r){
			$('#BusEsuperChangeAddGrid').datagrid('deleteRow', index);
		}
	});
}
function saverow(index){
	$('#BusEsuperChangeAddGrid').datagrid('endEdit', index);
}
function cancelrow(index){
	$('#BusEsuperChangeAddGrid').datagrid('cancelEdit', index);
}
function addrow(){
	if (editcount > 0){
	...
	$('#BusEsuperChangeAddGrid').datagrid('appendRow',{
		COLUMN_NAME:'',
		BUSSYS_SN:''
	});
}
function saveData(){
	var url = $("#BusEsuperChangeAddForm").attr("action");
				
	var insertRows = $('#BusEsuperChangeAddGrid').datagrid('getChanges','inserted');
	var updateRows = $('#BusEsuperChangeAddGrid').datagrid('getChanges','updated');
	var deleteRows = $('#BusEsuperChangeAddGrid').datagrid('getChanges','deleted');
	var changesRows = {
			inserted : [],
			updated : [],
			deleted : [],
	...
	var processValue = $('#busesuperchange_addprocess').val();
	if(processValue){
