...
<script type="text/javascript">
	$(function() {
		...
		if (valid) {
			...
			/**====可编辑表格数据开始======**/
			var data = $('#factorMngChangeEditGrid').datagrid('getData');
			for(var t=0;t<data.rows.length;t++){
				$('#factorMngChangeEditGrid').datagrid('endEdit', t);
			}
			var insertRows = $('#factorMngChangeEditGrid').datagrid('getChanges','inserted');
				var updateRows = $('#factorMngChangeEditGrid').datagrid('getChanges','updated');
				var deleteRows = $('#factorMngChangeEditGrid').datagrid('getChanges','deleted');
				var changesRows = {
				inserted : [],
				updated : [],
				deleted : [],
	...
	* 初始化表格
	*/
	$('#factorMngChangeEditGrid').datagrid({  //初始化datagrid
	    url:'processChange.do?factInfodatagrid',
	    idField:"CHANGE_ID",
	    rownumbers: true,
	    fit:true,
	    border:false,
	    ...
	    fitColumns:true,
	    pagination:false,
	    toolbar: "#factorMngChangeEditbar",
	    queryParams: {
		...
		columns:[[
	        ...
	        {field:'SUPER_ELEMENTS',title:'要素名称',width:160,align:'left',editor:'text'
	        	/**formatter:function(value,row,index){
                	if(row.editing){
                		var html='<input type="text" maxlength="19" value='+value+'/>*';
                    	return html;
                	} else {
                    	return value;
                	}
	    ...
	    onBeforeEdit:function(index,row){
	        row.editing = true;
	        $('#factorMngChangeEditGrid').datagrid('refreshRow', index);
	        editcount++;
	    },
	    ...
	    onClickRow:function (index, row) {
            $('#factorMngChangeEditGrid').datagrid('beginEdit', index);
        }		
	});
	AppUtil.initAuthorityRes("factorMngChangeEditbar");
});	
...
function deleterow(serialId){
	$.messager.confirm('确认','您确定要删除掉该记录吗?',function(r){
		if (r){
			var $dataGrid = $("#factorMngChangeEditGrid");
			...
			$('#factorMngChangeEditGrid').datagrid('deleteRow', rowindex);
		}
	});
}
function saverow(index){
	$('#factorMngChangeEditGrid').datagrid('endEdit', index);
}
function cancelrow(index){
	$('#factorMngChangeEditGrid').datagrid('cancelEdit', index);
}
function addrow(){
	var $dataGrid = $("#factorMngChangeEditGrid");
	var length = $dataGrid.datagrid('getData').rows.length+1;
	$('#factorMngChangeEditGrid').datagrid('appendRow',{
		CHANGE_ID:'t_'+length,
		SUPER_ELEMENTS:'',
		ANALYSIS_TYPE:'1',
		RULE_DESC:''
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="factorChangeForm" method="post" action="processChange.do?saveOrUpdateBatch">
		<!--==========隐藏域部分开始 ===========-->
		...
		<table style="width:100%;border-collapse:collapse;"
				...
				<tr style="height:350px;">
					<td colspan="1">
					<!-- =========================表格开始==========================-->
					<div id="factorMngChangeEditbar"></div>
					<table id="factorMngChangeEditGrid"></table>
					<!-- =========================表格结束==========================-->
					</td>
				</tr>
			</table>
