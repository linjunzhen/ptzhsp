...
$(function() {
	AppUtil.initWindowForm("submitAuditChangeFrom", function(form, valid) {
		if (valid) {
			var rows = $("#AUDID_UPLOADTABLE_CHANGE").datagrid("getRows");
			var fileIds = "";
			...
			$("#ATTACHMENT_CHANGE").val(fileIds);
			//将提交按钮禁用,防止重复提交
			$("input[type='submit']").attr("disabled", "disabled");
			var formData = $("#submitAuditChangeFrom").serialize();
			var url = $("#submitAuditChangeFrom").attr("action");
			...
			AppUtil.ajaxProgress({
				...
				callback : function(resultJson) {
					if (resultJson.success) {
						...
						parent.$("#BusAuditChangeGrid").datagrid("reload");
						AppUtil.closeLayer();
					} else {
						parent.art.dialog({
							content : resultJson.msg,
	...
	AppUtil.initUploadControl({
		...
		uploadButtonId : "AUDIT_UPLOAD_BTN_CHANGE",
		limit_size : "10 MB",
		uploadTableId:"AUDID_UPLOADTABLE_CHANGE",
...
function formatDel(val,row){
	if(val){
		...
		return "<a href='#' style='float:left;height:28px; line-height:28px;' onclick='AppUtil.removeUploadFile(\""+row.FILE_ID+"\",null,null,\"AUDID_UPLOADTABLE_CHANGE\",\"AUDIT_UPLOAD_BTN_CHANGE\");' >删除</a>"
	}
}
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="submitAuditChangeFrom" method="post" action="busAuditChangeController.do?submitAudit" style="height:400px">
			<!--==========隐藏域部分开始 ===========-->
			<input type="hidden" name="CURLOGIN_USERID" value="${sessionScope.curLoginUser.userId}">
			<input type="hidden" name="BUS_CODE" value="${BUS_CODE}">
			<input type="hidden" name="APPLY_ID" value="${APPLY_ID}">
			<input type="hidden" name="ATTACHMENT" id="ATTACHMENT_CHANGE" value="">
			<!--==========隐藏域部分结束 ===========-->
			<table style="width:100%;border-collapse:collapse;"
				...
				<tr>
					<td align="center" style="padding:3px 0 2px 0px">
						...
						<span style="width: 50px;float:left;text-align:left;padding-top:3px;"><a id="AUDIT_UPLOAD_BTN_CHANGE"></a></span>
						<span style="float:left">(请上传word文档)</span>
					</td>
				</tr>	
			
			</table>
			<table id="AUDID_UPLOADTABLE_CHANGE" class="easyui-datagrid" rownumbers="true" pagination="false" fitColumns="false"
			method="post" idField="SWF_FILEID" checkOnSelect="false"  style="width:300px;htight:200px"
			...
			<thead>
				...
				<th data-options="field:'fileprogress'" width="150">进度</th>
				<th data-options="field:'FILE_DELETE'" width="100">操作</th>
				</tr>
				</thead>
			</table>
