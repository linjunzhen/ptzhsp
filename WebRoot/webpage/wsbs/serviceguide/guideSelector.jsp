...
<script type="text/javascript">
	...
	function loadAutoCompleteDatas() {
		$.post("serviceGuideController.do?load", {
			...
			$("#AutoCompleteInput").autocomplete(
					resultJson,
					{
						...
						formatItem : function(row, i, max) {
							//下拉框显示
							return "<div>" + row.PROJECT_NAME+"</div>";
						},
						formatMatch : function(row) {
							//查询条件
							return row.PROJECT_NAME+row.PINYIN;
						},
					...
					function(event, data, formatted) {
						...
						if ($("#selectedRecords #" + data.GUIDE_ID).text() == ""){
							...
							if((selectedLength>allowCount)&&allowCount!=0){
								...
								$("#selectedRecords").append(
										"<option id='"+data.GUIDE_ID+"' value='"+data.GUIDE_ID+"'>"
	...
	function loadDatas(params) {
		$("#selectRecords").html("");
		$.post("serviceGuideController.do?load", params, function(responseText,
				status, xhr) {
			var resultJson = $.parseJSON(responseText);
			for ( var index in resultJson) {
				var record = resultJson[index];
				if ($("#" + record.GUIDE_ID).text() == "") {
					$("#selectRecords").append(
							"<option id='"+record.GUIDE_ID+"' value='"+record.GUIDE_ID+"' >"
									+ record.PROJECT_NAME + "</option>");
				}
	...
	function choiceSelectRecords(){
		...
		if(selectedRecords.length>=1){
			...
			for(var i = 0;i<selectedRecords.length;i++){
				...
				guideIds+=selectedRecords[i].value;
				guideNames+=selectedRecords[i].text;
			}
			art.dialog.data("selectGuideInfo", {
				guideIds:guideIds,
	...
	function loadSelectedRecords(){
		var guideIds = $("input[name='guideIds']").val().split(",");
		var guideNames = $("input[name='guideNames']").val().split(",");
		for(var i =0;i<guideIds.length;i++){
			$("#selectedRecords").append(
					"<option id='"+guideIds[i]+"' value='"+guideIds[i]+"' >"
							+ guideNames[i] + "</option>");
		}
	}
	...
	$(function() {
		var allowCount = $("input[name='allowCount']").val();
		var guideIds = $("input[name='guideIds']").val();
		if(guideIds){
			loadSelectedRecords();
		}
		loadDatas();
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="SelectorForm" method="post"
		...
		<input type="hidden" name="guideIds" value="${guideIds}"> 
		<input type="hidden" name="guideNames" value="${guideNames}"> 
