...
<script type="text/javascript">
        ...
        if(yhmc){
			...
			function sbzylctPage(itemList){
				console.log(1);
				$("#sbzylctList").html("");
				var newhtml = "<table>";
				$.each( itemList, function(index, node){
					newhtml +='<tr>';
					newhtml +='<td width="60%"><a title="'+node.ITEMTITLE+'" href="<%=path%>/contentController/view.do?contentId='+node.TID+'">'+node.ITEMTITLE +'</a></td>';
					newhtml +='<td width="40%">'+node.ITEMRELDATE +'</td>';
					newhtml +='</tr>';
				});
				newhtml += "</table>";
				$("#sbzylctList").html(newhtml);
			}
		</script>
</head>
<body style="background: #f0f0f0;">
	<%--开始编写头部文件 --%>
	<jsp:include page="/webpage/website/newproject/head.jsp?currentNav=sbzy" />
	<%--结束编写头部文件 --%>
	<div class="eui-main">
		<div class="eui-content">
			...
			<div class="eui-publicity">
				<div class="eui-publicity-right">
					...
					<div class="eui-policy-table" id="sbzylctList"></div>
					<div class="page">
						<jsp:include page="../website/common/page.jsp">
							<jsp:param
								value="${path}/govIvestController/sbzylctInfo.do?moduleId=${moduleId }"
								name="pageURL" />
							<jsp:param value="sbzylctPage" name="callpage" />
