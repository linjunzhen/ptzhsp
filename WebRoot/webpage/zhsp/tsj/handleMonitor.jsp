...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteHandleMonitorTsjRecord() {
		AppUtil.deleteDataGridRecord("executionController.do?multiDel",
				"handleMonitorTsjGrid");
	};
	
	/**
	* 终结流程实例
	*/
	function endHandleMonitorTsjRecord(){
	...
	*/
	function changeFlowBranch(){
		var entityId = AppUtil.getEditDataGridRecord("handleMonitorTsjGrid");
		if (entityId) {
	...
	$(document).ready(function() {
		var handleMonitorTsjStart = {
			elem : "#handleMonitorTsjT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var handleMonitorTsjEnd = {
			elem : "#handleMonitorTsjT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
		...
		laydate(handleMonitorTsjStart);
		laydate(handleMonitorTsjEnd);
	});
	  
  	/*序号列宽度自适应-----开始-----*/
	function fixRownumber(){
		var grid = $('#handleMonitorTsjGrid');  
...
<div class="easyui-layout" fit="true">
	<div region="center">
		<!-- =========================查询面板开始========================= -->
		<div id="handleMonitorTsjToolbar">
			<!--====================开始编写隐藏域============== -->
			<!--====================结束编写隐藏域============== -->
			<!-- <div class="right-con">
					...
					<div class="e-toolbar-ct">
						<div class="e-toolbar-overflow">
						   <a href="#"
								...
								onclick="endHandleMonitorTsjRecord();">终结流程实例</a> 
						   <a href="#"
								class="easyui-linkbutton" 
						   ...
						   <a href="#"
								...
								onclick="deleteHandleMonitorTsjRecord();">删除流程实例</a>
						</div>
					</div>
				</div>
			</div> -->
			<form action="#" name="HandleMonitorTsjForm">
...
url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=FlowRunStatus',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 128,panelHeight: 'auto' " />
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;"><input type="text"
								...
								id="handleMonitorTsjT.CREATE_TIME_BEGIN" name="Q_T.CREATE_TIME_&gt;=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="handleMonitorTsjT.CREATE_TIME_END" name="Q_T.CREATE_TIME_&lt;=" /></td>
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('handleMonitorTsjToolbar','handleMonitorTsjGrid')" />
								<input type="button" value="重置" class="eve-button"
								onclick="AppUtil.gridSearchReset('HandleMonitorTsjForm')" /></td>
						</tr>
					</tbody>
				</table>
			</form>
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="handleMonitorTsjGrid" fitcolumns="true" toolbar="#handleMonitorTsjToolbar"
			method="post" idfield="EXE_ID" checkonselect="false" nowrap="false"
			selectoncheck="false" fit="true" border="false"
			url="serviceHandleController.do?handleMonitor&Q_T.RUN_STATUS_NEQ=0&Q_TWS.SXLX_EQ=3">
			<thead>
				<tr>
