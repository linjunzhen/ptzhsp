...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteHandledByMePtjGridRecord() {
	...
	function revokeFlowExe() {
		var exeId = AppUtil.getEditDataGridRecord("HandledByMePtjGrid");
		if (exeId) {
			art.dialog.confirm("您确定要追回该流程吗?", function() {
				...
				$.post("executionController.do?revokeFlow",{
						...
						if(resultJson.success == true){
							...
							$("#HandledByMePtjGrid").datagrid('reload');
	...
	$(document).ready(function() {
		var handleByMePtjStart = {
			elem : "#handleByMePtjT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var handleByMePtjEnd = {
			elem : "#handleByMePtjT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
		...
		laydate(handleByMePtjStart);
		laydate(handleByMePtjEnd);

		AppUtil.initAuthorityRes("HandledByMePtjToolbar");

	});
	
	/*序号列宽度自适应-----开始-----*/
	function fixRownumber(){
		var grid = $('#HandledByMePtjGrid');  
...
<div class="easyui-layout" fit="true">
	<div region="center">
		<!-- =========================查询面板开始========================= -->
		<div id="HandledByMePtjToolbar">
			<!--====================开始编写隐藏域============== -->
			<!--====================结束编写隐藏域============== -->
			<form action="#" name="HandledByMePtjForm">
...
url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=FlowRunStatus',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 128,panelHeight: 'auto' " />
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;"><input type="text"
								...
								id="handleByMePtjT.CREATE_TIME_BEGIN" name="Q_T.CREATE_TIME_&gt;=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="handleByMePtjT.CREATE_TIME_END" name="Q_T.CREATE_TIME_&lt;=" /></td>
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('HandledByMePtjToolbar','HandledByMePtjGrid')" />
								<input type="button" value="重置" class="eve-button"
								onclick="AppUtil.gridSearchReset('HandledByMePtjForm')" /></td>
						</tr>
					</tbody>
				</table>
			</form>
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="HandledByMePtjGrid" fitcolumns="true" toolbar="#HandledByMePtjToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"  nowrap="false"
			selectoncheck="false" fit="true" border="false"
			url="serviceHandleController.do?handledByMe&Q_TWS.SXLX_EQ=2">
			<thead>
				<tr>
