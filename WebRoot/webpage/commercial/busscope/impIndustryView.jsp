	...
	<script type="text/javascript">
		...
		$(function() {
			AppUtil.initWindowForm("uploadInfoForm", function(form, valid) {
				if (valid) {
					var fileId=$("input[name='APPLY_PATH']").val();
					if(fileId==""||fileId==undefined){
						parent.art.dialog({
							content : "请先上传行业分类excel",
							icon : "succeed",
							time : 3,
							ok : true
						});
...
<form id="uploadInfoForm" method="post"
	  action="busScopeController/importXlsData.do">
	<div id="uploadInfoFormDiv" data-options="region:'center',split:true,border:false">
		<!--==========隐藏域部分开始 ===========-->
		<input type="hidden" name="APPLY_PATH" value="" >
		<!--==========隐藏域部分结束 ===========-->
		<table style="width:100%;border-collapse:collapse;"
			   class="dddl-contentBorder dddl_table">
			<tr>
				<td><span style="width: 120px;float:left;text-align:right;">行业分类excel：</span>
