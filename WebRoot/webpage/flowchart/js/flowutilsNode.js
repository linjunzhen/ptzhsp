...
function init(diagramName,isReadOnly,scale,paleteName,toolbar,toolIds,hasRightMenu,clkCallbak,stateColor,linkwidth,highkey,viewName){
    ...
    //myDiagram.fixedBounds.height=720;
 ...
 // when the document is modified, add a "*" to the title and enable the "Save" button
    ...
    var statetemple=
      gojs(go.Node, "Spot", nodeStyle(),
        ...
        {
            //click: function(e, obj) {alert("Clicked on " + obj.part.data.key); },
    	  	click: function(e, obj) {
	  			var part = obj.part;
	  			if(part instanceof go.Node) {
	  				var node = part;
            ...
            selectionChanged: function(part) {
                //var old = part.elt(0);
    	  		if (part.isSelected) {
	  				...
	  				shape.fill="red";
	  				txtNode.font="bold 12pt Helvetica, Arial, sans-serif";
    	  		}else{
    	  			var txtNode=part.findObject("txtNode");
    ...
    myDiagram.commandHandler.deleteSelection = function() {
        ...
        if(items.count > 0) {  // if there are any selected items, delete them
        	...
        	if(flg){
        		...
        		/**myDiagram.selection.each(function(p) { 
        			//myDiagram.model.removeNodeData(p.data);
        			var nodedata = p.data;
        	        myDiagram.remove(p);
...
function findAllSelectedItems() {
          var items = [];
          var selnode=myDiagram.selection;
...
function diagramCenter(){
	...
	var nodeArray=myDiagram.model.nodeDataArray;
  	...
  	var locsc=nodeArray[0].loc;
	var arrayc=locsc.split(" ");
	var xc=arrayc[0];
  	var maxx=0;
	var minx=xc;
   	...
   	alert(myDiagram.viewportBounds.x);
   	var middlex=(parseInt(maxx)+parseInt(minx))/2;
   	//var b = myDiagram.computePartsBounds(myDiagram.nodes);
   	var mc=myDiagram.computeBounds();
   	//alert(JSON.stringify(q));
   	//alert(maxx+";"+minx+";"+middlex);
   	//alert(mc.width);
   	//alert(myDiagram.div.offsetLeft);
   	//alert(mc.width);
   	var width=myDiagram.div.style.width;
   	var divmiddle=parseInt(width)/2;
   	var cha=divmiddle-middlex;
   	//alert(divmiddle+";;;"+middlex+";;;"+cha);
   	//myDiagram.initialPosition.x=cha+100;
   	//alert(divmiddle+";"+cha);
   	//myDiagram.moveParts(myDiagram.nodes, go.Point.parse(""+cha+" "+0+""), false);
   	//alert(JSON.stringify(b.x));
   	//alert(mc.right);
}
/**
 * 鼠标事件
...
function cancelSelected(){
	...
	for(var i=0;i<nodeArray.length;i++){
		...
		if(cat!="end"&&cat!="start"&&cat!="Comment"){
			...
			model.setDataProperty(nodeArray[i], "isSelected",false);
 ...
 * @return
 */
function changeNodesColor(ids,color){
	var nodeArray=myDiagram.model.nodeDataArray;
	var model=myDiagram.model;
	for(var j=0;j<ids.length;j++){
		for(var i=0;i<nodeArray.length;i++){
			if(nodeArray[i].id==ids[j]){
				model.startTransaction("updatecolor");
		  		model.setDataProperty(nodeArray[i], "color", color);
		  		model.commitTransaction("updatecolor");
		  		break;
