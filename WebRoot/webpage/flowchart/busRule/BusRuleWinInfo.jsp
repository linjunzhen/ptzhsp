...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("BusRuleWinForm", function(form, valid) {
			if (valid) {
				//将提交按钮禁用,防止重复提交
				$("input[type='submit']").attr("disabled", "disabled");
				var formData = $("#BusRuleWinForm").serialize();
				var url = $("#BusRuleWinForm").attr("action");
				AppUtil.ajaxProgress({
					...
					callback : function(resultJson) {
						if (resultJson.success) {
							...
							window.parent.$("#BusRuleAddGrid").datagrid("reload");
							window.close();
						} else {
							parent.art.dialog({
								content: resultJson.msg,
	...
	function getExp(){
		...
		if($temp_2 == ">"||$temp_2=="<"||$temp_2=="<>"){
			if($temp_3 == ""){
				...
				$("#BusRuleWinForm input[name='JUDGE_CONDITIONS']").val(value);
				$("#BusRuleWinForm textarea[name='JUDGE_DESC']").val(texts);
				return;
			}
		}
	...
	function toAndExp(){
		...
		var $temp_4t = $("#Esuper_JudgeB").combobox('getText');
		var $temp_5t = $("#Esuper_TempC").combobox('getText');
		var oldval = $("#BusRuleWinForm input[name='JUDGE_CONDITIONS']").val();
		var oldtext = $("#BusRuleWinForm textarea[name='JUDGE_DESC']").val();
		// 拼凑([field_1]+[field_2])>[field_3]
 		var value = '';	var texts ='';
		if($temp_1==""||$temp_1.size==0){
		   alert("规则变量1不能为空！");
		...
		if($temp_2 == ">"||$temp_2=="<"||$temp_2=="<>"){
			if($temp_3 == ""){
		    	...
		    	value = '['+$temp_1+'] '+$temp_2+'['+$temp_3+']';
		    	texts = '['+$temp_1t+'] '+$temp_2t+'['+$temp_3t+']';
				$("#BusRuleWinForm input[name='JUDGE_CONDITIONS']").val(oldval+' and '+value);
				$("#BusRuleWinForm textarea[name='JUDGE_DESC']").val(oldtext+' 与 '+texts);
				return;
			}
		}
		//当temp_2选择"非空"时 且temp_3，temp_4，temp_5为空时，
	...
	function toOrExp(){
		...
		if($temp_2 == ">"||$temp_2=="<"||$temp_2=="<>"){
			if($temp_3 == ""){
		    	...
		    	value = '['+$temp_1+'] '+$temp_2+'['+$temp_3+']';
		    	texts = '['+$temp_1t+'] '+$temp_2t+'['+$temp_3t+']';
				$("#BusRuleWinForm input[name='JUDGE_CONDITIONS']").val(oldval+' or '+value);
				$("#BusRuleWinForm textarea[name='JUDGE_DESC']").val(oldtext+' 或 '+texts);
				return;
			}
		}
		//当temp_2选择"非空"时 且temp_3，temp_4，temp_5为空时，
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="BusRuleWinForm" method="post"
