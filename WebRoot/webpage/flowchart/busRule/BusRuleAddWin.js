function loadFactorList(processCode){
	$("#busrule_addprocess").val(processCode);
	$('#BusRuleAddGrid').datagrid({  //初始化datagrid
...
function addrow(){
	var entityId = $("#busrule_addprocess").val();
	if (entityId) {
		var url="busRuleController.do?winInfo&entityId=" + entityId; 
	    var name='监察规则信息';                        
	    var iWidth=650;                         
	    var iHeight=600;                       
...
function editrow(){
	var entityId = AppUtil.getEditDataGridRecord("BusRuleAddGrid");
	if (entityId) {
		var url="busRuleController.do?winEditInfo&entityId=" + entityId; 
	    var name='监察规则信息';                        
	    var iWidth=650;                         
	    var iHeight=600;                       
	    var iTop = (window.screen.availHeight-30-iHeight)/2;
...
function delrow(){
	AppUtil.deleteDataGridRecord("busRuleController.do?muitDel","BusRuleAddGrid");
}