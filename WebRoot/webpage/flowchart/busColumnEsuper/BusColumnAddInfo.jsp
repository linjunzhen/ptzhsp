...
<script type="text/javascript">
	$(function() {
		AppUtil.initWindowForm("BusColumnAddForm", function(form, valid) {
			if (valid) {
				//将提交按钮禁用,防止重复提交
				$("input[type='submit']").attr("disabled", "disabled");
				var formData = $("#BusColumnAddForm").serialize();
				...
				AppUtil.ajaxProgress({
					...
					callback : function(resultJson) {
						if (resultJson.success) {
							...
							window.open("busColumnEsuperController.do?toAddColumn&entityId=" + busCode,"addColumnWin");
							AppUtil.closeLayer();
						} else {
							parent.art.dialog({
								content: resultJson.msg,
	...
	function toAddColumnWindow(){
		var unitCode = $("#BusColumnAddForm input[name='BUSSYS_UNITCODE']").val();
		var platCode = $("#BusColumnAddForm input[name='BUSSYS_PLATCODE']").val();
		var busCode = $("#BusColumnAddForm input[name='BUS_CODE']").val();
		if(!unitCode){
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="BusColumnAddForm" method="post"
		action="busColumnEsuperController.do?checkColumn">
		<!--==========隐藏域部分开始 ===========-->
		<!--==========隐藏域部分结束 ===========-->
		<table style="width:100%;border-collapse:collapse;"
			...
			<tr>
				<td><span style="width: 150px;float:left;text-align:right;">业务单位：</span>
					<input class="easyui-combobox validate[required]" style="width:250px;" name="BUSSYS_UNITCODE" 
			...
			<tr>
				<td><span style="width: 150px;float:left;text-align:right;">业务专项：</span>
					<input class="easyui-combobox validate[required]" style="width:250px;" name="BUS_CODE" id="PROCESSCODECombox"  
