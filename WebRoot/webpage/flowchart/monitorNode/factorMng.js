					...
					for (var i = 0; i < data.length; i++){
						var desc=data[i].RULE_DESC==null?"":data[i].RULE_DESC;
						 html+='<tr>'
						 		...
						 		+desc
						 		//+' <a href="javascript:void(0);" class="easyui-linkbutton" onclick="editFactorGridRecord(\''+data[i].RULE_ID+'\');">修改</a>'
						 		//+' <a href="javascript:void(0);" class="easyui-linkbutton"'
						 		//+' onclick="delFactorInfo(\''+data[i].RULE_ID+'\',\''+data[i].PROCESS_CODE+'\');">删除</a>'
						 		+'</td></tr>';
						 num++;
					} 
			...
			success : function(data) {
				...
				if(data.length>0){
					...
					for (var i = 0; i < data.length; i++){
						var desc=data[i].RULE_DESC==null?"":data[i].RULE_DESC
						 html+='<tr>'
						 		+'<td style="width: 36px;align:center;" class="tdOver">'+num+'</td>'
						 		+'<td style="width:110px;overflow: hidden;text-overflow:ellipsis;" class="tdOver">'
						 		+data[i].SUPER_ELEMENTS+'</td><td style="width:120px;" class="tdOver">'
						 		+data[i].DIC_NAME
						 		+'</td>'
						 		+'<td style="width:46px;" class="tdOver">'
						 		+desc
						 		//+' <a href="javascript:void(0);" class="easyui-linkbutton" onclick="editFactorGridRecord(\''+data[i].RULE_ID+'\');">修改</a>'
						 		//+' <a href="javascript:void(0);" class="easyui-linkbutton"'
						 		//+' onclick="delFactorInfo(\''+data[i].RULE_ID+'\',\''+data[i].PROCESS_CODE+'\');">删除</a>'
...
function backDraw(){
	 $.ajax({
		...
		success : function(data) {
			if(data== "success"){
				//alert("返回绘图成功!");
				$.messager.alert("系统提示", "返回绘图成功", "info",function(){
					window.opener.reloadNodeList();
					window.close();
				});
			}else{
				$.messager.alert("系统提示", "返回绘图失败", "info");
			}
		},
		error:function(msg){
...
function addRow(){
	var factorTbl=$("#factorTable");
	var newTr = factorTbl.insertRow();//添加一行
	var newTd0 = newTr.insertCell();//添加两列
	var newTd1 = newTr.insertCell();
	var newTd2 = newTr.insertCell();
	//设置列内容和属性
	newTd0.innerHTML = '<span style="width:80px;>'+''+'</span>';
	newTd1.innerHTML = '<span style="width:150px;>'+''+'</span>';
	newTd2.innerHTML = '<span style="width:150px;>'+''+'</span>';
