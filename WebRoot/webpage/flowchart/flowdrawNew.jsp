...
function loadSvg(evt) {
	...
	//alert(node.getElementsByTagName("polygon").length);
	//findnode(node);
}
...
function mouseOver(code,flag){
	var src=everoot+"/webpage/flowchart/images/";
	if(flag=='1'){
		src+="arrow_start_blue.png";
	}else{
		src+="arrow_go_blue.png";
	}
	//alert($("#img_"+code).attr("src"));
	document.getElementById("img_"+code).src=src;
}
function mouseOut(code,flag){
	var src=everoot+"/webpage/flowchart/images/";
	if(flag=='1'){
		src+="arrow_start.png";
	}else{
	...
	<div data-options="region:'north'," title="${busInfo.busName}" split="false" border="false"
		 ...
		 <style>
    		...
    		<div class="left_float">
            	<ul>
                	<li><a href="javascript:void(0);" onclick="tempSaveFlow();" id="tempSave" ><p><img src="<%=path%>/webpage/flowchart/images/tempSave.png"></p>保存</a></li>
            ...
            <c:if test="${empty tacheInfoList}">暂无数据</c:if>
            	<svg id="flowElement" width="860px" height="76px" onload="loadSvg(evt);">
				<c:forEach items="${tacheInfoList}" var="tacheInfo" varStatus="status">
					<c:choose>
						<c:when test="${status.index==0}">
							<polygon id="poly_" points="10,10 10,50 130,50 150,30 130,10 " fill="gray" onclick="clkPloy(evt,'${tacheInfo.tacheCode}')"
								style="stroke:#000000;stroke-width:1">
							</polygon>
						</c:when>
						<c:otherwise>
							<polygon id="poly2" 
							points="${status.index*130+10},10 ${status.index*130+30},30 ${status.index*130+10},50 ${status.index*130+130},50 ${status.index*130+150},30 ${status.index*130+130},10 "
							 fill="gray" onclick="clkPloy(evt)"
								style="stroke:#000000;stroke-width:1">
							</polygon>
						</c:otherwise>
					</c:choose>	
