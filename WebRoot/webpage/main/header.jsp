...
<div class="eui-header">
            ...
            <dl class="eui-nav">
            	<c:forEach var="topChildMenu" items="${topChildMenuList}">
					<c:if test="${topChildMenu.ishide==null}">
                		<dt class="dropdown"  ><a class="dropdown-toggle" data-toggle="dropdown" onclick="changeLeft('${topChildMenu.RES_KEY}','${topChildMenu.RES_NAME}','${topChildMenu.SUBSYSTEM_CODE}','${topChildMenu.EXTERNAL_URL}')"  href="javascript:void(0);" role="button" aria-haspopup="true" aria-expanded="false" title="${topChildMenu.RES_NAME}" id="${topChildMenu.RES_KEY}"><%-- <img src="webpage/system/login/images/${topChildMenu.RES_KEY}.png">  --%><span>${topChildMenu.RES_NAME}</span><!-- <label></label> --></a>
            ...
            <div class="eui-system">
                <ul>
                	<li><a style="background: none;">技术支持:23162985/13328297005</a></li>
                     ...
                     ${sessionScope.curLoginUser.fullname}</a>
                    	<div class="eui-syscenter  dropdown-menu" >
                        	...
                        	<ul>
	                            ...
	                            <a href="javascript:showModifyPassWin();"><li><i class="eicon-edit"></i> 修改密码</li></a>
	                            <a href="javascript:showHeadPortrait();"><li><i class="eicon-photo"></i> 头像设置</li></a>
	                            <a href="javascript:showExitWin();"><li><i class="eicon-power-off"></i> 退出用户</li></a>
...
<script type="text/javascript">
	...
	//窗口改变大小事件
		...
		}else if ('${sessionScope.curLoginUser.isUniqueBind}' == "-1") {
			$.dialog.open("sysUserController.do?showCardAndMobile&isForce=true", {
				title : "必须绑定本人手机号和身份证",
				width : "450px",
				esc: false,//取消esc键  
				lock : true, 
	...
	function setLeftList(itemList){
			...
			if(itemList[i].ishide==null){
				...
				if(itemList[i].hasChild){
					...
					for(var j=0; j<itemList[i].secondMenuList.length; j++){
					   if(itemList[i].secondMenuList[j].ishide==null){
		...
		changeCss();
		$("div.jquery-accordion-menu li:first").trigger("click");
		conculateLeft(leftCount);
	}
	
	function conculateLeft(lilen){
		var totalH = $('.eui-content').height();
		//var lilen = $('#jquery-accordion-menu li').size();
		var ulH = totalH - lilen * 40;
		$('.jquery-accordion-submenu').css('max-height',ulH);
		$('.jquery-accordion-submenu').addClass("sc");
		$(".sc").perfectScrollbar();
	}
