	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			...
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZHZZHPSHZH");
    		}
    	}
		...
		AppUtil.initWindowForm("SHTZHZZHPSHZH", function(form, valid) {
			if (valid) {
				getOpinion();
				getExpert();
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZHZZHPSHZH");
				var url = $("#SHTZHZZHPSHZH").attr("action");
...
<body>
	<form id="SHTZHZZHPSHZH" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eui-word-title">
					关于<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目<br>《项目申请报告+》（送审版）综合<br>（专家）评审意见回执函
				</div>
				...
				<div class="eio-word-content">
					《平潭综合实验区投资项目〈项目申请报告+〉（送审版）综合(专家)评审联系单》（项目编号：岚综实项目〔<input type="text" class="eui-ipt"
					...
					value="${SQData.KXXBGXMBHH}" />号）收悉。<br>
					   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;经对<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" /> 《项目申请报告+》（送审版）综合(专家)评审涉及的<input type="text" class="eui-ipt"
