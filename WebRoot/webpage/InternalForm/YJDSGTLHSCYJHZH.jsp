	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"YJDSGTLHSCYJHZH");
    		}
    	}
		//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("YJDSGTLHSCYJHZH", function(form, valid) {
			if (valid) {
				getOpinion();
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("YJDSGTLHSCYJHZH");
				var url = $("#YJDSGTLHSCYJHZH").attr("action");
...
<body>
	<form id="YJDSGTLHSCYJHZH" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				...
				<div class="eui-word-title">
					关于<input type="text" class="eui-ipt" data-validation-engine="validate[required]" readonly="readonly" style="width:273px"  name="VB_GC_NAME_ST" maxlength="200" value="${SQData.blxmmc}" />项目《一阶段施工图》（修编版）
					联合审查意见回执函
