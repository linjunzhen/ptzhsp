	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GCKXXYJBGPSJYYJQSD");
    		}
    	}
		...
		AppUtil.initWindowForm("GCKXXYJBGPSJYYJQSD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GCKXXYJBGPSJYYJQSD");
				var url = $("#GCKXXYJBGPSJYYJQSD").attr("action");
...
<body>
	<form id="GCKXXYJBGPSJYYJQSD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《工程可行性研究报告+》（送审版）<br />综合(专家)评审建议意见签收单
				</div>
				<br>
				...
				<div class="eui-word-name">
					<input type="text" class="eui-ipt"
						...
						name="VB_KXXBGXMSQRY_ST" readonly="readonly" maxlength="50"
						value="${SQData.xmsqry}" />：
				</div>
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]" style="width:135px"
						readonly="readonly" name="VB_KXXBGSQSJ_ST" maxlength="30"
						value="${SQData.xmsqsj}" />提请的<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]" style="width:200px;"
						name="VB_KXXBGXMMC_ST" readonly="readonly"
						value="${SQData.blxmmc}" />项目《工程可行性研究报告+》（送审版）综合(专家)评审已完成，现将项目综合(专家)评审会建议意见汇总表转交给你单位，请及时组织修编，并申请修编版批复。
				</div>	
				<br>
