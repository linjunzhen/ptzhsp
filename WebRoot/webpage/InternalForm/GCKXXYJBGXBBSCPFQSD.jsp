	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GCKXXYJBGXBBSCPFQSD");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("GCKXXYJBGXBBSCPFQSD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GCKXXYJBGXBBSCPFQSD");
				var url = $("#GCKXXYJBGXBBSCPFQSD").attr("action");
...
<body>
	<form id="GCKXXYJBGXBBSCPFQSD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目《工程可行性研究报告+(修编版)综合审查意见书》签收单
				</div>
				<br>
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt"	data-validation-engine="validate[required]" style="width:135px"	readonly="readonly" name="VB_KXXBGXMSQSJ_ST"
					  maxlength="30" value="${SQData.xmsqsj}" />提请的<input type="text" class="eui-ipt"	data-validation-engine="validate[required]" style="width:200px;"
						name="VB_KXXBGSUBJECT_ST" readonly="readonly"	value="${SQData.blxmmc}" />项目《工程可行性研究报告+》（修编版）联合审查已完成，现将项目（
						<input type="checkbox" name="VB_KXXBGXBBSCYJS_CK" value="1" >  《工程可行性研究报告+（修编版）综合审查意见书》 
						<input type="checkbox" name="VB_KXXBGLHSCHZB_CK" value="1" >  《<项目可研报告+>联合审查情况汇总表》）交给你单位。。
				</div>
