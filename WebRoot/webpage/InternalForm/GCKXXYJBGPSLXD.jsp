	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GCKXXYJBGPSLXD");
    		}
    	}
		...
		AppUtil.initWindowForm("GCKXXYJBGPSLXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GCKXXYJBGPSLXD");
				var url = $("#GCKXXYJBGPSLXD").attr("action");
...
<body>
	<form id="GCKXXYJBGPSLXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《工程可行性研究报告+》（送审版）<br />综合(专家)评审联系单
				</div>
				<br>
				<div class="eui-word-xmbh">
					项目编号：岚综实项目〔<input type="text"  data-validation-engine="validate[required]" name="VB_KXXBGXMBHN_ST" class="eui-ipt1" style="width:60px"/>〕<input type="text"  data-validation-engine="validate[required]" name="VB_KXXBGXMBHH_ST"  class="eui-ipt1" style="width:40px"/>号
				</div>
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目的《工程可行性研究报告+》（送审版）已完成，现转给你单位。请于<input
						...
						onclick="laydate({istime: false, format: 'YYYY年MM月DD日'})" />前将相关评审建议意见反馈我局，超期视为默认同意。
				</div>
				...
				<div class="eui-word-notes clearfix">
					<label>附件：</label><span>
					1.《工程可行性研究报告+》（送审版）综合（专家）评审申请表；<br />
					2.《工程可行性研究报告+》（送审版）
					</span>
				</div>
