	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZHZPFFW");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZHZPFFW", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZHZPFFW");
				var url = $("#SHTZHZPFFW").attr("action");
...
<body>
	<form id="SHTZHZPFFW" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-xmbh">
					岚综实项目审批〔<input type="text" name="VB_BHN_ST" data-validation-engine="validate[required]" class="eui-ipt1" style="width:60px"/>〕<input type="text" name="VB_BHH_ST"  data-validation-engine="validate[required]" class="eui-ipt1" style="width:40px"/>号
				</div>
				...
				<div class="eio-word-content">
					...
					<input type="text" class="eui-ipt1 laydate-icon" data-validation-engine="validate[required]"
						...
						onclick="laydate({istime: false, format: 'YYYY年MM月DD日'})" />受理了你公司提出的项目《申请报告+》申请，按照《行政许可法》第二十五条和第二十六条的规定，经审查，集中公示后，决定如下。
				<br>
					&nbsp;&nbsp;一、关于工程可行性的意见：<br>
					&nbsp;&nbsp;<textarea rows="6" 
