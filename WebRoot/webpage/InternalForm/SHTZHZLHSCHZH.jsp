	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			...
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZHZLHSCHZH");
    		}
    	}
		...
		AppUtil.initWindowForm("SHTZHZLHSCHZH", function(form, valid) {
			if (valid) {
				getOpinion();
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZHZLHSCHZH");
				var url = $("#SHTZHZLHSCHZH").attr("action");
...
<body>
	<form id="SHTZHZLHSCHZH" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eui-word-title">
					关于<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目《项目申请报告+》（修编版）联合审查意见回执函
				</div>
				...
				<div class="eio-word-content">
					你局关于《平潭综合实验区投资项目〈项目申请报告+〉修编版联合审查联系单》（项目编号：岚综实项目〔<input type="text" class="eui-ipt"
					...
					value="${SQData.KXXBGXMBHH}" />号）收悉。<br>
					 &nbsp;&nbsp;&nbsp;&nbsp; 经对<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" /> 项目《项目申请报告+》（修编版）联合审查涉及的<input type="text" class="eui-ipt"
