	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZBALHSCLXD");
    		}
    	}
		//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZBALHSCLXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZBALHSCLXD");
				var url = $("#SHTZBALHSCLXD").attr("action");
...
<body>
	<form id="SHTZBALHSCLXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br> 《项目备案请示+》（修编版）联合审查联系单
				</div>
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt"
						...
						name="VB_GCXM_NAME_ST" maxlength="200" value="${SQData.blxmmc}" />项目的《项目备案请示+》（修编版）已完成，现转给你单位。
					请于<input type="text" class="eui-ipt1 laydate-icon"
						data-validation-engine="validate[required]"
						style="width:150px;height: 36px;" name="VB_GCXM_TIME_ST"
						maxlength="10" readonly="readonly"
						onclick="laydate({istime: false, format: 'YYYY年MM月DD日'})" />前（ 5个工作日）将项目相关的审查意见（
						 <input type="checkbox" name="VB_GKSC_CK" value="1">  工可审查意见
						 <input type="checkbox" name="VB_HPSC_CK" value="1">  环评审查意见
					 <input type="checkbox" name="VB_SBSC_CK" value="1">  水保方案审查意见
					 <input type="checkbox" name="VB_JNSC_CK" value="1">  节能评估报告审查意见
					<input type="checkbox" name="VB_QT_CK" value="1">  其他 <input type="text" class="eui-ipt"
						style="width:60px"
						name="VB_QTDETAIL_ST" />）提交我局。
					超期不反馈意见的，视为默认同意。在综合(专家)评审阶段未提出的意见，在本审查阶段提出的，如无充足理由，不予采纳。
				</div>
				...
				<div class="eui-word-notes clearfix">
					<label>附件：</label><span> 1. 《项目备案请示+》（修编版）联合审查申请表；<br />
						2. <input type="text" class="eui-ipt"
						data-validation-engine="validate[required]"
						style="width:100px;font-size: 14px;"
						maxlength="200" value="${SQData.blxmmc}" />《项目备案请示+》（修编版）；<br />
