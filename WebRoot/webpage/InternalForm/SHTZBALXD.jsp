	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZBALXD");
    		}
    	}
		...
		AppUtil.initWindowForm("SHTZBALXD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZBALXD");
				var url = $("#SHTZBALXD").attr("action");
...
<body>
	<form id="SHTZBALXD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《项目备案请示+》（送审版）综合（专家）评审联系单
				</div>
				<br>
				...
				<div class="eio-word-content">
					<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目的《项目备案请示+》（送审版）已完成，现转给你单位。请于<input
