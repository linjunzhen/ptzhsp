	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			...
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GCKXXYJGBYJHZH");
    		}
    	}
		...
		AppUtil.initWindowForm("GCKXXYJGBYJHZH", function(form, valid) {
			if (valid) {
				getOpinion();
				getExpert();
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GCKXXYJGBYJHZH");
				var url = $("#GCKXXYJGBYJHZH").attr("action");
	...
	function getExpert(){
		...
		if($("input[name='VB_HAVEEXPERT_ST']:checked").val()==1){
			...
			for(var i=1;i<=expertSn;i++){
				...
				expertInfo +="（"+i+"）"+experts+"，主要法律依据及理由：";
				expertInfo += reasons;
			}
		}
...
<body>
	<form id="GCKXXYJGBYJHZH" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		<jsp:include page="commonform.jsp" />
		<!-- ==============结束引入通用的隐藏界面============= -->
		<input type="hidden" name="VB_GCKXXYJBGOPTION_ST" id="gcopinion">
		<input type="hidden" name="VB_GCKXXYJBGOPTION_SN_ST" id="gcopinionsn">
		<input type="hidden" name="VB_GCKXXYJBGEXPERT_ST" id="gcexpert">
		<input type="hidden" name="VB_GCKXXYJBGEXPERT_SN_ST" id="gcexpertsn">
		<div class="eui-word">
			<div class="eui-word-top"></div>
			<div class="eui-word-con">
				<div class="eui-word-xmbh">
					<input type="text" placeholder="请输入部门函编号" name="VB_GCKXXYJBGHZHBH_ST" class="eui-ipt1" style="width:150px"/>
				</div>
				
				<div class="eui-word-title">
					关于<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]" readonly="readonly"
						style="width:150px" name="VB_GCKXXYJBGSUBJECT_ST" maxlength="200"
						value="${SQData.blxmmc}" />项目<br>《工程可行性研究报告+》（送审版）综合<br>（专家）评审意见回执函
				</div>
				...
				<div class="eio-word-content">
					《平潭综合实验区投资项目〈工程可行性研究报告+〉（送审版）综合(专家)评审联系单》（项目编号：岚综实项目〔<input type="text" class="eui-ipt"
					data-validation-engine="validate[required]" style="width:60px" name="VB_GCKXXYJBGBHN_ST" maxlength="50"
					value="${SQData.KXXBGXMBHN}" />〕<input type="text" class="eui-ipt"
					data-validation-engine="validate[required]" style="width:35px" name="VB_GCKXXYJBGBHH_ST" maxlength="50"
					value="${SQData.KXXBGXMBHH}" />号）收悉。<br>
					   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;经对<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]" readonly="readonly"
						style="width:160px" maxlength="200" name="VB_GCKXXYJBGSUBJECT_ST"
						value="${SQData.blxmmc}" /> 项目《工程可行性研究报告+》（送审版）综合(专家)评审涉及的<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]"
						style="width:160px" name="VB_GCKXXYJBGSX_ST" maxlength="200" />审批事项进行认真审查，意见如下:<br>
					...
					<div id="expertDivs" style="">
						<div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;专家信息：<br>
						  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<textarea id="expert1" rows="3" data-validation-engine="validate[required]"
							style="width: 95%;font-size:21px; vertical-align:top; overflow: hidden;line-height: 36px;"
							onpropertychange="this.style.height=this.scrollHeight + 'px'"
							oninput="this.style.height=this.scrollHeight + 'px'"
