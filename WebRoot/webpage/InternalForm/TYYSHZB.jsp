	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"TYYSHZB");
    		}
    	}
    	//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("TYYSHZB", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("TYYSHZB");
				var url = $("#TYYSHZB").attr("action");
...
<body>
	<form id="TYYSHZB" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					<input type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:273px" 
					 name="VB_CGLSUBJECT_ST" maxlength="200" value="${SQData.blxmmc}" />项目<br />联合竣工验收意见汇总表（一）
				</div>
				<br>
