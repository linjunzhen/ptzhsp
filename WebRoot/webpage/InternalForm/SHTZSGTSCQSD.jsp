	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZSGTSCQSD");
    		}
    	}
		...
		AppUtil.initWindowForm("SHTZSGTSCQSD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZSGTSCQSD");
				var url = $("#SHTZSGTSCQSD").attr("action");
...
<body>
	<form id="SHTZSGTSCQSD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《施工图》（送审稿）审查意见签收单
				</div>
				<br>
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt"
						...
						value="${SQData.blxmmc}" />项目《施工图》联合审查已完成，现将项目《施工图》（送审稿）审查意见交给你单位。
				</div>	
				<br>
