	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			...
    			WordUtil.initFormFieldValue(dataObj.variableMap,"GCKXXYJBGXBBBMYJHZH");
    		}
    	}
		...
		AppUtil.initWindowForm("GCKXXYJBGXBBBMYJHZH", function(form, valid) {
			if (valid) {
				getOpinion();
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("GCKXXYJBGXBBBMYJHZH");
				var url = $("#GCKXXYJBGXBBBMYJHZH").attr("action");
...
<body>
	<form id="GCKXXYJBGXBBBMYJHZH" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
			...
			<div class="eui-word-con">
				...
				<div class="eui-word-title">
					关于<input type="text" class="eui-ipt"
						...
						style="width:130px" name="VB_GCKXXYJBGSUBJECT_ST" maxlength="200"
						value="${SQData.blxmmc}" />项目《可行性研究报告+》（修编版）联合审查意见回执函
				</div>
				...
				<div class="eio-word-content">
					你局关于《平潭综合实验区投资项目〈工程可行性研究报告+〉修编版联合审查联系单》（项目编号：岚综实项目〔<input type="text" class="eui-ipt"
					data-validation-engine="validate[required]" style="width:70px" name="VB_GCKXXYJBGBHN_ST" maxlength="50"
					value="${SQData.KXXBGXMBHN}" />〕<input type="text" class="eui-ipt"
					data-validation-engine="validate[required]" style="width:30px" name="VB_GCKXXYJBGBHH_ST" maxlength="50"
					value="${SQData.KXXBGXMBHH}" />号）收悉。<br>
					     经对<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]" readonly="readonly"
						style="width:200px" maxlength="200" name="VB_GCKXXYJBGSUBJECT_ST"
						value="${SQData.blxmmc}" /> 项目《工程可行性研究报告+》（修编版）联合审查涉及的<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]"
						style="width:200px" name="VB_GCKXXYJBGSX_ST" maxlength="200" />事项进行认真审查，意见如下:<br>
				...
				<div class="eui-word-time">
					<input type="text" class="eui-ipt1 laydate-icon"
						data-validation-engine="validate[required]"
						style="width:150px;height: 36px;" name="VB_GCKXXYJBGDQ_TIME_ST"
