	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZBASLD");
		...
		AppUtil.initWindowForm("SHTZBASLD", function(form, valid) {
			if (valid) {
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZBASLD");
				var url = $("#SHTZBASLD").attr("action");
...
<body>
	<form id="SHTZBASLD" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				<div class="eui-word-title">
					平潭综合实验区投资项目<br />《项目备案请示+》（送审版）<br />综合(专家)评审受理单
				</div>
				...
				<div class="eio-word-content">
					你单位于<input type="text" class="eui-ipt"	data-validation-engine="validate[required]" style="width:135px"	readonly="readonly" name="VB_XMSQSJ_ST"
					  maxlength="30" value="${SQData.xmsqsj}" />提请的<input type="text" class="eui-ipt"	data-validation-engine="validate[required]" style="width:200px;"
						name="VB_SUBJECT_ST" readonly="readonly"	value="${SQData.blxmmc}" />项目《项目备案请示+》（送审版）综合(专家)评审申请材料（
						<input type="checkbox" name="VB_PSSQB_CK" value="1" >  《项目备案请示+》（送审版）综合（专家）评审申请表
						<input type="checkbox" name="VB_CNS_CK" value="1" >  申请办理行政审批事项承诺书
						<input type="checkbox" name="VB_SSB_CK" value="1" >  《项目备案请示+》（送审版）
					 ...
					 11个工作日)凭此单到区行政服务中心大厅我局项目投资窗口领取《项目备案请示+》（送审版）综合(专家)评审建议意见汇总表》。
				</div>
