	...
	$(function(){
		...
		if(dataObj){
    		...
    		if(dataObj.variableMap){
    			WordUtil.initFormFieldValue(dataObj.variableMap,"SHTZSGTLHSCYJHZH");
    		}
    	}
		//初始化input时宽度自适应
    	$("input").each(function(){
		...
		AppUtil.initWindowForm("SHTZSGTLHSCYJHZH", function(form, valid) {
			if (valid) {
				getOpinion();
				$("input[type='submit']").attr("disabled","disabled");
				var formData = WordUtil.getFormEleData("SHTZSGTLHSCYJHZH");
				var url = $("#SHTZSGTLHSCYJHZH").attr("action");
...
<body>
	<form id="SHTZSGTLHSCYJHZH" method="post" action="variableController.do?saveHtmlData">
		<!-- ==============开始引入通用的隐藏界面============= -->
		...
		<div class="eui-word">
			...
			<div class="eui-word-con">
				...
				<div class="eui-word-title">
					关于<input type="text" class="eui-ipt" data-validation-engine="validate[required]" readonly="readonly" style="width:273px"  name="VB_GC_NAME_ST" maxlength="200" value="${SQData.blxmmc}" />项目《施工图》（修编版）
					联合审查意见回执函
				...
				<div class="eio-word-content">
					你局关于《平潭综合实验区投资项目&lt;施工图&gt;（修编版）审查联系单》（项目编号：岚综实项目〔<input type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:60px"  name="VB_XMNH_ST" maxlength="4" />〕<input type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:30px"  name="VB_XMBH_ST" maxlength="4" /> 号）收悉。经对<input type="text" class="eui-ipt" data-validation-engine="validate[required]" style="width:273px" readonly="readonly"  name="VB_GCXM_NAME_ST" maxlength="200" value="${SQData.blxmmc}" />
					项目《施工图》联合审查涉及的<input type="text" class="eui-ipt"
						data-validation-engine="validate[required]"
						style="width:200px" name="VB_SX_ST" maxlength="200" />事项进行认真研究：<br>
