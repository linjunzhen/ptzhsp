...
<script type="text/javascript">
	...
	function restartFlowTaskExplain(){
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			if(!isAllowStart){
				...
				AppUtil.operateDataGridRecord("flowTaskController.do?reStart",{selectTaskIds:selectTaskIds},"BdcNeedMeHandleGrid");
			}else{
	 ...
	 * 编辑流程实例列表记录
	 */
	function editBdcNeedMeHandleGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("BdcNeedMeHandleGrid");
		if (entityId) {
	...
	function formatTaskStatus(val,row){
		  if(val=="1"){
			  ...
			  return "<font color='#0368ff'><b>已审核</b></font>";
		  }else if(val=="3"){
	...
	$(document).ready(function() {
		var start1 = {
			elem : "#BdcNeedMeHandleT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var end1 = {
			elem : "#BdcNeedMeHandleT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
	...
	function showBdcDwslBjxxGridRecord() {
		var rows = $("#BdcNeedMeHandleGrid").datagrid("getChecked");
        if(rows.length==0){
            art.dialog({
                content: "请至少选择一条记录!",
                icon:"warning",
						...
						<div class="e-toolbar-overflow">
						    <div style="color:#005588;">
								<img src="plug-in/easyui-1.4/themes/icons/script.png"
									style="position: relative; top:7px; float:left;" />&nbsp;待受理列表
								<a href="#"
                                class="easyui-linkbutton" reskey="SHOW_Bjxx"
                                iconcls="eicon-eye" plain="true"
                                onclick="showBdcDwslBjxxGridRecord();">查看补件要求</a>
<%--                                <a href="#"--%>
...
<%--                                onclick="restartFlowTask();">重启流程任务</a>--%>
					...
					<tbody>
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;padding-left:4px;"><input type="text"
								...
								id="BdcNeedMeHandleT.CREATE_TIME_BEGIN" name="Q_E.CREATE_TIME_>=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;padding-left:4px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="BdcNeedMeHandleT.CREATE_TIME_END" name="Q_E.CREATE_TIME_<=" /></td>
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('BdcNeedMeHandleToolbar','BdcNeedMeHandleGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="BdcNeedMeHandleGrid" fitcolumns="false" toolbar="#BdcNeedMeHandleToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="bdcExecutionController.do?bdcNeedMeHandle&Q_T.TASK_NODENAME_LIKE=%E5%8F%97%E7%90%86&isHaveHandup=true">
			<thead>
				<tr>
					<th field="ck" checkbox="true"></th>
					<th data-options="field:'TASK_ID',hidden:true">TASK_ID</th>
					<th data-options="field:'TASK_STATUS',align:'left'" width="10%" formatter="formatTaskStatus">任务状态</th>
