...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteMyApplyExecGridRecord() {
	 ...
	 * 编辑流程实例列表记录
	 */
	function editMyApplyExecGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("MyApplyExecGrid");
		if (entityId) {
	...
	function ckhz() {
        var entityId = AppUtil.getEditDataGridRecord("MyApplyExecGrid");
        if (entityId) {
        	$.dialog.open("webSiteController/hzxx.do?&exeId="+entityId, {
                title : "查看回执信息",
                width : "800px",
                height : "400px",
                lock : true,
	...
	function deleteDraft(){
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			for ( var i = 0; i < rowsData.length; i++) {
				var RUN_STATUS = rowsData[i].RUN_STATUS;
				if(RUN_STATUS!=0){
					art.dialog({
						content: "只能选择草稿进行删除!",
						icon:"warning",
	...
	function getBackMyApply(){
		var exeId = AppUtil.getEditDataGridRecord("MyApplyExecGrid");
		if (exeId) {
			art.dialog.confirm("您确定要撤回该流程吗?", function() {
				...
				$.post("executionController.do?getBackMyApply",{
						...
						if(resultJson.success == true){
							...
							$("#MyApplyExecGrid").datagrid('reload');
						...
						<div class="e-toolbar-overflow">
						    <div style="color:#005588;">
								<img src="plug-in/easyui-1.4/themes/icons/script.png"
									style="position: relative; top:7px; float:left;" />&nbsp;我的申请列表
								<a href="#"
								class="easyui-linkbutton" 
								iconCls="eicon-trash-o" plain="true"
								onclick="deleteDraft();">删除草稿</a>
								<a href="#"
                                class="easyui-linkbutton" 
                                iconCls="eicon-eye" plain="true"
                                onclick="ckhz();">查看回执单</a>
<%--								<a href="#"--%>
<%--                                class="easyui-linkbutton" --%>
<%--                                iconCls="eicon-rotate-left" plain="true"--%>
<%--                                onclick="getBackMyApply();">撤回流程</a>--%>
							</div>
						</div>
					</div>
				</div>
...
url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=FlowRunStatus',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 128,panelHeight: 'auto' " />
						...
						<tr style="height:28px;">
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('MyApplyExecToolbar','MyApplyExecGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="MyApplyExecGrid" fitcolumns="true" toolbar="#MyApplyExecToolbar"
			method="post" idfield="EXE_ID" checkonselect="false" nowrap="false"
			selectoncheck="false" fit="true" border="false"
			url="executionController.do?datagrid">
			<thead>
				<tr>
