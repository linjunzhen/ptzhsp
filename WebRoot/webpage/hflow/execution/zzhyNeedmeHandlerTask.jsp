...
<script type="text/javascript">
	 ...
	 * 删除流程实例列表记录
	 */
	function deleteZzhyNeedMeHandleGridRecord() {
		AppUtil.deleteDataGridRecord("executionController.do?multiDel",
				"ZzhyNeedMeHandleGrid");
	};
	function restartFlowTaskExplain(){
		var $dataGrid = $("#ZzhyNeedMeHandleGrid");
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			if(!isAllowStart){
				...
				AppUtil.operateDataGridRecord("flowTaskController.do?reStart",{selectTaskIds:selectTaskIds},"ZzhyNeedMeHandleGrid");
			}else{
	 ...
	 * 编辑流程实例列表记录
	 */
	function editZzhyNeedMeHandleGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("ZzhyNeedMeHandleGrid");
		if (entityId) {
	...
	function formatSign(val,row){
	    if(val=='1'){
	        return "<font color='blue'><b>商事面签流程</b></font>";
		}else{
            return "<font color='green'><b>线下流程</b></font>";
		}
	}
	function formatSubject(val,row){
		...
		var url=""
		if(taskStatus!="1"){
			url ="executionController.do?goDetail&exeId="+exeId;
		}else{
			url ="executionController.do?goHandle&exeId="+exeId+"&taskId="+taskId;
		}
		href += "javascript:void(0)' onclick=\"toPostUrl('"+url+"');\"";
		href += "' target='_top'><span style='text-decoration:underline;color:#0368ff;'>"+val+"</span></a>";
	    return href;
	}
	...
	$(document).ready(function() {
		var start1 = {
			elem : "#ZzhyNeedMeHandleT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var end1 = {
			elem : "#ZzhyNeedMeHandleT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
	...
	function showBjxxGridRecord() {
		var rows = $("#ZzhyNeedMeHandleGrid").datagrid("getChecked");
        if(rows.length==0){
            art.dialog({
                content: "请至少选择一条记录!",
                icon:"warning",
...
<div class="easyui-layout eui-jh-box" fit="true">
	<div region="center">
		<!-- =========================查询面板开始========================= -->
		<div id="ZzhyNeedMeHandleToolbar">
			<!--====================开始编写隐藏域============== -->
			<!--====================结束编写隐藏域============== -->
			<div class="right-con">
...
<%--                                onclick="restartFlowTask();">重启流程任务</a>--%>
				...
				<table class="dddl-contentBorder dddl_table"
					...
					<tbody>
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;padding-left:4px;"><input type="text"
								...
								id="ZzhyNeedMeHandleT.CREATE_TIME_BEGIN" name="Q_E.CREATE_TIME_>=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;padding-left:4px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="ZzhyNeedMeHandleT.CREATE_TIME_END" name="Q_E.CREATE_TIME_<=" /></td>
							<td style="width:68px;text-align:right;">申请人</td>
							<td style="width:135px;"><input class="eve-input"
								type="text" maxlength="20" style="width:128px;"
								name="Q_E.SQRMC_LIKE" /></td>
							<td style="width:80px;">是否全程网办:</td>
							<td style="width:135px;"> <input class="easyui-combobox" type="text" maxlength="20" style="width:78px;" name="Q_E.ISNEEDSIGN_="  data-options="
					url:'dictionaryController.do?load&amp;defaultEmpty=true&amp;typeCode=YesOrNo',editable:false,method: 'post',valueField:'DIC_CODE',textField:'DIC_NAME',panelWidth: 74,panelHeight: 'auto' " />
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('ZzhyNeedMeHandleToolbar','ZzhyNeedMeHandleGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="ZzhyNeedMeHandleGrid" fitcolumns="false" toolbar="#ZzhyNeedMeHandleToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="flowTaskController.do?zzhyNeedMeHandle&Q_T.TASK_NODENAME_EQ=%e7%aa%97%e5%8f%a3%e5%8a%9e%e7%90%86&isHaveHandup=true">
			<thead>
				<tr>
					...
					<th data-options="field:'ISNEEDSIGN',align:'left'" width="10%" formatter="formatSign">是否面签</th>
