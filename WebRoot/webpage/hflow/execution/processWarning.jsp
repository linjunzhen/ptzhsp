...
<script type="text/javascript">
	...
	function restartFlowTaskExplain(){
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			if(!isAllowStart){
				...
				AppUtil.operateDataGridRecord("flowTaskController.do?reStart",{selectTaskIds:selectTaskIds},"ProcessWarningGrid");
			}else{
	 ...
	 * 编辑流程实例列表记录
	 */
	function editProcessWarningGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("ProcessWarningGrid");
		if (entityId) {
	...
	function toPostUrl(url){
		...
		$("#ProcessWarningGrid").append(ssoForm);
		ssoForm.submit();		
	}
	...
	$(document).ready(function() {
		var start1 = {
			elem : "#ProcessWarningT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var end1 = {
			elem : "#ProcessWarningT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
	...
	function showBjxxGridRecord() {
		var rows = $("#ProcessWarningGrid").datagrid("getChecked");
        if(rows.length==0){
            art.dialog({
                content: "请至少选择一条记录!",
                icon:"warning",
...
<div class="easyui-layout eui-jh-box" fit="true">
	<div region="center">
						...
						<div class="e-toolbar-overflow">
						    <div style="color:#005588;">
								<img src="plug-in/easyui-1.4/themes/icons/script.png"
									style="position: relative; top:7px; float:left;" />&nbsp;办件预警列表
							</div>
					...
					<tbody>
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;padding-left:4px;"><input type="text"
								...
								id="ProcessWarningT.CREATE_TIME_BEGIN" name="Q_E.CREATE_TIME_>=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;padding-left:4px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="ProcessWarningT.CREATE_TIME_END" name="Q_E.CREATE_TIME_<=" /></td>
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('ProcessWarningToolbar','ProcessWarningGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="false" striped="true"
			id="ProcessWarningGrid" fitcolumns="false" toolbar="#ProcessWarningToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="flowTaskController.do?processWarningdata">
			<thead>
				<tr>
