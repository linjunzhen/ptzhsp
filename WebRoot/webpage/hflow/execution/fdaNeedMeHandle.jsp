...
<script type="text/javascript">
	...
	function restartFlowTaskExplain(){
		...
		if (!(rowsData != null && typeof (rowsData) != 'undefined' && rowsData.length != 0)) {
			...
			if(!isAllowStart){
				...
				AppUtil.operateDataGridRecord("flowTaskController.do?reStart",{selectTaskIds:selectTaskIds},"fdaNeedMeHandleGrid");
			}else{
	 ...
	 * 编辑流程实例列表记录
	 */
	function editfdaNeedMeHandleGridRecord() {
		var entityId = AppUtil.getEditDataGridRecord("fdaNeedMeHandleGrid");
		if (entityId) {
	...
	$(document).ready(function() {
		var start1 = {
			elem : "#fdaNeedMeHandleT.CREATE_TIME_BEGIN",
			format : "YYYY-MM-DD",
			istime : false,
		...
		var end1 = {
			elem : "#fdaNeedMeHandleT.CREATE_TIME_END",
			format : "YYYY-MM-DD",
			istime : false,
			choose : function(datas) {
				var beginTime = $("input[name='Q_T.CREATE_TIME_>=']").val();
	...
	function showBjxxGridRecord() {
		var rows = $("#fdaNeedMeHandleGrid").datagrid("getChecked");
        if(rows.length==0){
            art.dialog({
                content: "请至少选择一条记录!",
                icon:"warning",
...
<%--                                onclick="restartFlowTask();">重启流程任务</a>--%>
					...
					<tbody>
						...
						<tr style="height:28px;">
							...
							<td style="width:135px;padding-left:4px;"><input type="text"
								...
								id="fdaNeedMeHandleT.CREATE_TIME_BEGIN" name="Q_E.CREATE_TIME_>=" /></td>
							<td style="width:68px;text-align:right;">申请日期至</td>
							<td style="width:135px;padding-left:4px;"><input type="text"
								style="width:108px;float:left;" class="laydate-icon"
								id="fdaNeedMeHandleT.CREATE_TIME_END" name="Q_E.CREATE_TIME_<=" /></td>
							...
							<td colspan="2"><input type="button" value="查询"
								class="eve-button"
								onclick="AppUtil.gridDoSearch('fdaNeedMeHandleToolbar','fdaNeedMeHandleGrid')" />
		...
		<table class="easyui-datagrid" rownumbers="true" pagination="true" striped="true"
			id="fdaNeedMeHandleGrid" fitcolumns="false" toolbar="#fdaNeedMeHandleToolbar"
			method="post" idfield="EXE_ID" checkonselect="false"
			selectoncheck="false" fit="true" border="false" nowrap="false"
			url="flowTaskController.do?fdaNeedMeHandle&Q_T.TASK_NODENAME_EQ=受理&isHaveHandup=true">
			<thead>
				<tr>
