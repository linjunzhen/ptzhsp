...
function callpage(itemList){
	...
	$.each( itemList, function(index, node){
		...
		newhtml +="<a href=\"javascript:tsxx('"+node.COMPLAIN_ID+"')\">";
		newhtml +=node.COMPLAIN_TITLE
		if(node.REPLY_STATUS==1){
	...
	newhtml += "</ul>";
	$("#wdtslist").html(newhtml);
}
function tsxx(entityId){
	$.dialog.open("complainController/yhzxInfo.do?entityId="+entityId, {
		title : "投诉信息",
		width : "800px",
		height : "510px",
		lock : true,
    ...
    <div class="container lbpadding20">
    	...
    	<div class="nmain clearfix">
        	...
        	<jsp:include page="./yhmenu.jsp" >
        	    <jsp:param value="wdts" name="menuKey"/>
        	</jsp:include>
        	<%--结束引入用户中心的左侧菜单 --%>
        	
            <div class="nmainR">
            	<div class="nmainRtitle" style="margin-top:0px;"><h3>我的投诉</h3></div>
				<!--列表DIV-->
                <div class="userClist" id="wdtslist">                   
                </div>
                <div>
					<%--开始引入分页JSP --%>
					<jsp:include page="../common/page.jsp" >
					<jsp:param value="complainController.do?pagelist" name="pageURL"/>
