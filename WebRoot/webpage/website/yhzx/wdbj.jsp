...
<head>
	    ...
	    for(var i=0; i<itemList.length; i++){
            ...
            if(itemList[i].RUN_STATUS==1){//正在办理
                if(itemList[i].TASK_STATUS=='1'){//正常流程流转待办任务===OK
                    ...
                    +itemList[i].ITEM_CODE+"&exeId="+itemList[i].EXE_ID+"&taskId="+itemList[i].TASK_ID+"\"  class=\"userbtn3\">办理</a>";
                    ...
                    if(CUR_STEPNAMES == '窗口办理'||CUR_STEPNAMES == '智能审批'){
    ...
    //					itemList[i].ITEM_CODE+"');\"  class=\"userbtn3\">申请撤办</a>";
                    ...
                    var ISNEEDSIGN = itemList[i].ISNEEDSIGN;
                     //面签信息展示
                     if((CUR_STEPNAMES == '身份认证'||CUR_STEPNAMES == '秒批身份认证')&&ISNEEDSIGN=='1'){
                    ...
                    if((CUR_STEPNAMES == '身份认证'||CUR_STEPNAMES == '秒批身份认证')&&ISNEEDSIGN=='1'&&ISFACESIGN!='1'){
	...
	**/
	...
	function clxz(exeId,itemCode){
		if(itemCode=='201605170002XK00002L'){			
			$.dialog.open("webSiteController/clxz.do?itemCode="+itemCode+"&exeId="+exeId+"&clxzlx=bg", {
				title : "材料下载",
				width : "1000px",
				height : "400px",
				lock : true,
    ...
    <div class="container lbpadding20">
    	...
    	<div class="nmain clearfix">
        	...
        	<jsp:include page="./yhmenu.jsp" >
        	    <jsp:param value="wdbj" name="menuKey"/>
        	</jsp:include>
        	<%--结束引入用户中心的左侧菜单 --%>
