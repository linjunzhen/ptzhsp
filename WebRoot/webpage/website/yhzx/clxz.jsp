...
<body style="background: none; min-width:800px;">
	<table cellpadding="0" cellspacing="1" class="syj-table2 tmargin20">
		<tr>
		...
		||fn:contains(applyMater.MATER_FILTER,busRecord.COMPANY_SETNATURE)}">
			...
			||(applyMater.MATER_NAME=='股东决定' && fn:length(holderList)==1)
			...
			<c:if test="${applyMater.MATER_NAME!='股东决定' &&applyMater.MATER_NAME!='一人股东决定' && applyMater.MATER_NAME!='股东会决议' && applyMater.MATER_NAME!='董事会决议'}">
				<tr <c:if test="${applyMater.MATER_PARENTCODE!=null&&applyMater.MATER_PARENTCODE!=''}"> class="${applyMater.MATER_PARENTCODE}"</c:if> >				
					<td style="width:400px">
						${applyMater.MATER_NAME}
						<c:if test="${applyMater.MATER_NAME=='福建省市场主体住所(经营场所)申报承诺表'}">
		...
		<c:if test="${busRecord.IS_ACCOUNT_OPEN==1 }">
			<tr>
				<td style="width:400px">
					单位银行结算账户申请书			
				</td>
