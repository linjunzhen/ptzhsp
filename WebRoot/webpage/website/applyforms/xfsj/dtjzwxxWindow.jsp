...
<head>
	...
	<script type="text/javascript">
		$(function() {
		    floorCountChange();
			AppUtil.initWindowForm("DtjzwForm", function(form, valid) {
				if (valid) {
					//将提交按钮禁用,防止重复提交
					$("input[type='submit']").attr("disabled","disabled");
					var formData = $("#DtjzwForm").serialize();
					var url = $("#DtjzwForm").attr("action");
					AppUtil.ajaxProgress({
						...
						callback : function(resultJson) {
							if(resultJson.success){
								...
								parent.$("#dtjzwxxGrid").datagrid("reload");
								AppUtil.closeLayer();
							}else{
								parent.art.dialog({
									content: resultJson.msg,
	...
	//建筑地下层数改变事件		
		function floorCountChange(){
			var floorCount = $("input[name='BOTTOM_FLOOR_COUNT']").val();
			if(floorCount=="0"){
				$("#underground_tr").hide();
			}else{
				$("#underground_tr").show();
...
<body style="margin:0px;background-color: #f7f7f7;">
	<form id="DtjzwForm" method="post" action="xfDesignController.do?saveOrUpdateDtjzw">
	    <div>
	     	<!--==========隐藏域部分开始 ===========-->
	    	...
	    	<!--==========隐藏域部分结束 ===========-->
		    <table style="width:100%;border-collapse:collapse;"class="dddl-contentBorder dddl_table tab_tk_pro1">
				...
				<tr style="height:40px;">
					...
					<td style="width:70%">
						&nbsp;<eve:eveselect clazz="tab_text w280 validate[required]" dataParams="TBFCPRJSTRUCTURETYPEDIC"
							dataInterface="dictionaryService.findDatasForSelect"
							defaultEmptyText="选择结构类型" name="FC_STRUCTURE_TYPE_NUM"
				...
				<tr style="height:40px;">
					...
					<td style="width:70%">
						&nbsp;<eve:eveselect clazz="tab_text validate[required]" dataParams="TBREFLEVELDIC"
							dataInterface="dictionaryService.findDatasForSelect"
							defaultEmptyText="选择建筑耐火等级数" name="REFRACTORY_LEVEL_NUM"
				...
				<tr style="height:40px;">
					...
					<td>
						<input type="text" style="width:579px;float:left;" class="eve-input validate[required]" 
							value="${dtjzwxxInfo.BOTTOM_FLOOR_COUNT}" name="BOTTOM_FLOOR_COUNT" onchange = "floorCountChange();"/>
					</td>
				</tr>
				<tr style="height:40px;display:none;" id="underground_tr">
					<td class="center">
						<font class="dddl_platform_html_requiredFlag">*</font>建筑地下耐火等级
					</td>
					<td style="width:70%">
						&nbsp;<eve:eveselect clazz="tab_text validate[required]" dataParams="UNDERGROUNDLEVELDIC"
							dataInterface="dictionaryService.findDatasForSelect"
							defaultEmptyText="选择建筑地下耐火等级数" name="UNDERGROUND_FIRE_LEVEL"
							value="${dtjzwxxInfo.UNDERGROUND_FIRE_LEVEL}">
						</eve:eveselect>
					</td>
				</tr>
