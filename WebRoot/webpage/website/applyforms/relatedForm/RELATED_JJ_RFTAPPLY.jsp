...
<body>
							...
							<table cellpadding="0" cellspacing="0" class="syj-table2">
								...
								<tr>
									<td>
										<eve:excheckbox
											dataInterface="dictionaryService.findDatasForSelect"
											name="CURR_CONTENT" width="650px;" clazz="checkbox "
						...
						<div id="purchaseDiv">
							<div style="position:relative;">
								...
								<table class="easyui-datagrid" toolbar="#purchaseToolbar" id="purchaseGrid" rownumbers="true" border="true" fitColumns="true"
									...
									<thead>
										<tr>
											<th data-options="field:'BRAND_MODEL',align:'left',editor:{type:'validatebox',options:{required:true,validType:'length[1,32]'}}"
												width="30%">厂牌型号</th>
											<th data-options="field:'AMOUNT',align:'left',editor:{type:'numberbox',options:{required:true,min:0,max:99999}}"
												width="10%">数量</th>
											<th data-options="field:'DEAD_WEIGHT',align:'left',editor:{type:'numberbox',options:{required:true,min:0,max:99999,precision:2}}"
												width="10%">载重质量（吨）</th>
											<th data-options="field:'TECH_LEVEL',align:'left',editor:{type:'validatebox',options:{required:true,validType:'length[1,32]'}}"
												width="20%">车辆技术等级</th>
											<th data-options="field:'OUT_SIZE',align:'left',editor:{type:'validatebox',options:{required:true,validType:'length[1,64]'}}"
												width="29%">外廓尺寸（毫米）</th>
										</tr>
									</thead>
								</table>
						...
						<div id="currentDiv">
							<div style="position:relative;">
								...
								<table class="easyui-datagrid" toolbar="#currentToolbar" id="currentGrid" rownumbers="true" border="true" fitColumns="true"
									...
									<thead>
										<tr>
											<th data-options="field:'TRAN_CERT',align:'left',editor:{type:'validatebox',options:{required:true,validType:'length[1,32]'}}"
												width="20%">道路运输证号</th>
	...
	<script type="text/javascript">
	    ...
	    function purchaseSubmit(){
	    	if(!confirmPurchase()){
	            alert("“已购置货物运输车辆情况”信息不完整。请核对并填写后再提交。")
	            return false;
	        }
	    ...
	    function purchasePlanSubmit(){
	    	if(!confirmPurchasePlan()){
	            alert("“拟购置货物运输车辆情况”信息不完整。请核对并填写后再提交。")
	            return false;
	        }
	    ...
	    function currentSubmit(){
	    	if(!confirmCurrent()){
	            alert("“现有运输车辆情况”信息不完整。请核对并填写后再提交。")
	            return false;
	        }
