...
<head>
	...
	$(function() {
		...
		AppUtil.initWindowForm("dwinfoForm", function(form, valid) {
			if (valid) {
				var isValid = $("#dwinfoForm").form('validate');
				if(isValid){
					//将提交按钮禁用,防止重复提交
		...
		setSingleProMaintype("${info.SINGLEPROMAINTYPE}","");
		setGreenbuidinglevel("${info.ISGREENBUILDING_DWGC}","");
		var dicDesc = 1;
		...
		$("[name$='STRUCTQUALTYPE'][type='checkbox']").each(function() {
...
function onlyNumber6(obj){
	...
	function setDwgcInfo(index){
		var STRUCTUPFLOORAREA_DWGC = $("#dwinfoForm [name$='STRUCTUPFLOORAREA_DWGC']").val();//±0.000以上面积
		var STRUCTDWFLOORAREA_DWGC = $("#dwinfoForm [name$='STRUCTDWFLOORAREA_DWGC']").val();//±0.000以下面积
		var ARCHAREA = Number(STRUCTUPFLOORAREA_DWGC)+Number(STRUCTDWFLOORAREA_DWGC);	
		if(Number(ARCHAREA)>0){	
			$("#dwinfoForm [name$='ARCHAREA']").val(ARCHAREA.toFixed(3));
		}else{
			$("#dwinfoForm [name$='ARCHAREA']").val('');
		}
	}
	</script>
</head>
...
<body class="eui-diabody" style="margin:0px;">
	<form id="dwinfoForm" method="post" style="margin-bottom: 30px;margin-top: 30px;">
		...
		<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20">
			...
			<tr>
				...
				<td colspan="3">						
					<input name="SINGLEPROMAINTYPE" id="singlepromaintype" class="easyui-combobox"
						data-options="
							...
							onSelect:function(rec){								
							   ...
							   $('#singleprotype').combobox('setValue',''); 
							   $('#singleprosubtype').combobox('setValue',''); 
								setSingleProMaintype(rec.DIC_CODE,'');
							}
					" value="${info.SINGLEPROMAINTYPE}"/>				
					<input name="SINGLEPROTYPE" id="singleprotype" class="easyui-combobox"
						data-options="
							url:'',
							method:'post',
							valueField:'VALUE',
					...
					<input name="SINGLEPROSUBTYPE" id="singleprosubtype" class="easyui-combobox" 
						data-options="
							url:'',
							method:'post',
							valueField:'VALUE',
		...
		<table cellpadding="0" cellspacing="0" class="syj-table2">		
			...
			<tr class="singleProMaintype_2">	
				...
				<td>
					...
					name="MUNILENGTH" maxlength="16" placeholder="请输入市政长度" value="${info.MUNILENGTH}" onblur="onlyNumber3(this);setDwgcInfo('');" onkeyup="onlyNumber3(this);"/>M
				</td>
				<th>路桥面积</th>
				<td>
			...
			<tr>
				...
				<td colspan="3">
					<eve:radiogroup typecode="YesOrNo" width="130px" value="${info.ISSUPERHIGHTBUILDING}" 
			...
			<tr class="singleProMaintype_1">
				...
				<td>
				  <input type="text" class="syj-input1 validate[required],custom[numberplus]" name="STRUCTUPFLOORNUM"  maxlength="16"  placeholder="请输入±0.000以上层数" value="${info.STRUCTUPFLOORNUM}" onblur="onlyNumber3(this);setDwgcInfo('');" onkeyup="onlyNumber3(this);"/>
				</td>
				<th><span class="syj-color2">*</span> ±0.000以下层数</th>
				<td>
				  <input type="text" class="syj-input1 validate[required],custom[numberplus]" name="STRUCTDWFLOORNUM"  maxlength="16"  placeholder="请输入±0.000以下层数" value="${info.STRUCTDWFLOORNUM}" onblur="onlyNumber3(this);setDwgcInfo('');" onkeyup="onlyNumber3(this);"/>
				</td>
			</tr>
			<tr class="singleProMaintype_1">
				<th><span class="syj-color2">*</span> ±0.000以上面积</th>
				<td>
				  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="STRUCTUPFLOORAREA_DWGC"  maxlength="16" placeholder="请输入±0.000以上面积" value="${info.STRUCTUPFLOORAREA_DWGC}" onblur="onlyNumber2(this);setDwgcInfo('');" onkeyup="onlyNumber2(this);"/>M<sup style="vertical-align: super;font-size: smaller;">2</sup>
				</td>
				<th><span class="syj-color2">*</span> ±0.000以下面积</th>
				<td>
				  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="STRUCTDWFLOORAREA_DWGC"  maxlength="16"  placeholder="请输入±0.000以下面积" value="${info.STRUCTDWFLOORAREA_DWGC}" onblur="onlyNumber2(this);setDwgcInfo('');" onkeyup="onlyNumber2(this);"/>M<sup style="vertical-align: super;font-size: smaller;">2</sup>
				</td>
			</tr>
			<tr>
				<th>基坑支护方法</th>
			...
			<tr>
				...
				<td colspan="3">
				  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="ITEMINVEST"  maxlength="16"  placeholder="请输入工程总造价（万元）" value="${info.ITEMINVEST}" onblur="onlyNumber6(this);setDwgcInfo('');" onkeyup="onlyNumber6(this);"/><div style="color:red;"> 指的是本单位工程的总造价，桩基工程先行办理施工许可时也要填写整个单位工程（含上部工程）的总造价。</div>
				</td>
			</tr>
			<tr>
				<th><span class="syj-color2">*</span> 是否为减隔震建筑	</th>
				<td>
					<eve:radiogroup typecode="YesOrNo" width="130px" value="${info.ISSHOCKISOLATIONBUILDING}" 
					fieldname="ISSHOCKISOLATIONBUILDING" ></eve:radiogroup>
				</td>
				<th><span class="syj-color2">*</span> 是否为装配式建筑	</th>
				<td>
					<eve:radiogroup typecode="YesOrNo" width="130px" value="${info.ISEQUIPPEDARCHITECTURE}" 
			...
			<tr>
				...
				<td>
					...
					fieldname="ISGREENBUILDING_DWGC"  value="${info.ISGREENBUILDING_DWGC}" ></eve:radiogroup>
				</td>
