...
<head>
	...
	$(function() {
		...
		AppUtil.initWindowForm("sbinfoForm", function(form, valid) {
			if (valid) {
				var isValid = $("#sbinfoForm").form('validate');
				if(isValid){
					//将提交按钮禁用,防止重复提交
					$("input[type='submit']").attr("disabled","disabled");
					var info = {};
					$("#sbinfoForm").find("*[name]").each(function(){
		...
		setSbgcSingleProMaintype("${info.SINGLEPROMAINTYPE}","");
		setSbgcGreenbuidinglevel("${info.ISGREENBUILDING_SBGC}","");
		var dicDesc = 1;
		...
		$('#singleprosubtype_sbgc').combobox({
		...
		$("[name$='STRUCTQUALTYPE_SBGC'][type='checkbox']").each(function() {
...
function onlyNumber6(obj){
	...
	function setSbgcInfo(index){
		var STRUCTUPFLOORAREA_DWGC = $("#sbinfoForm [name$='STRUCTUPFLOORAREA_DWGC']").val();//±0.000以上面积
		var STRUCTDWFLOORAREA_DWGC = $("#sbinfoForm [name$='STRUCTDWFLOORAREA_DWGC']").val();//±0.000以下面积
		var ARCHAREA = Number(STRUCTUPFLOORAREA_DWGC)+Number(STRUCTDWFLOORAREA_DWGC);	
		if(Number(ARCHAREA)>0){	
			$("#sbinfoForm [name$='ARCHAREA']").val(ARCHAREA.toFixed(3));
		}else{
			$("#sbinfoForm [name$='ARCHAREA']").val('');
		}
	}
	</script>
</head>
...
<body class="eui-diabody" style="margin:0px;">
	<form id="sbinfoForm" method="post" style="margin-bottom: 30px;margin-top: 30px;">
		...
		<table cellpadding="0" cellspacing="0" class="syj-table2 tmargin20">
			<tr>
				...
				<td colspan="3"><input type="text" class="syj-input1 validate[required] w838" value="${info.SINGLEPRONAME}"
			...
			<tr>
				...
				<td colspan="3">						
					<input name="SINGLEPROMAINTYPE" id="singlepromaintype_sbgc" class="easyui-combobox"
						data-options="
							...
							onSelect:function(rec){								
							   ...
							   $('#singleprotype_sbgc').combobox('setValue',''); 
							   $('#singleprosubtype_sbgc').combobox('setValue',''); 
								setSbgcSingleProMaintype(rec.DIC_CODE,'');
							}
					" value="${info.SINGLEPROMAINTYPE}"/>				
					<input name="SINGLEPROTYPE" id="singleprotype_sbgc" class="easyui-combobox" 
						data-options="
							url:'',
							method:'post',
							valueField:'VALUE',
					...
					<input name="SINGLEPROSUBTYPE" id="singleprosubtype_sbgc" class="easyui-combobox" 
						data-options="
							url:'',
							method:'post',
							valueField:'VALUE',
		...
		<table cellpadding="0" cellspacing="0" class="syj-table2 ">
			...
			<tr>
				...
				<td colspan="3">
					<eve:radiogroup typecode="YesOrNo" width="130px" value="${info.ISSUPERHIGHTBUILDING_SBGC}" 
					fieldname="ISSUPERHIGHTBUILDING_SBGC"></eve:radiogroup>
				</td>
			</tr>	
            <tr class="singleProMaintype_1_sbgc">
                <th><span class="syj-color2">*</span> ±0.000以上层数</th>
                <td>
                  <input type="text" class="syj-input1 validate[required],custom[numberplus]" name="STRUCTUPFLOORNUM"  maxlength="16"  placeholder="请输入±0.000以上层数" value="${info.STRUCTUPFLOORNUM}" onblur="onlyNumber3(this);setSbgcInfo('');" onkeyup="onlyNumber3(this);"/>
                </td>
                <th><span class="syj-color2">*</span> ±0.000以下层数</th>
                <td>
                  <input type="text" class="syj-input1 validate[required],custom[numberplus]" name="STRUCTDWFLOORNUM"  maxlength="16"  placeholder="请输入±0.000以下层数" value="${info.STRUCTDWFLOORNUM}" onblur="onlyNumber3(this);setSbgcInfo('');" onkeyup="onlyNumber3(this);"/>
                </td>
            </tr>
            <tr class="singleProMaintype_1_sbgc">
                <th><span class="syj-color2">*</span> ±0.000以上面积</th>
                <td>
                  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="STRUCTUPFLOORAREA_DWGC"  maxlength="16" placeholder="请输入±0.000以上面积" value="${info.STRUCTUPFLOORAREA_DWGC}"    onblur="onlyNumber2(this);setSbgcInfo('');" onkeyup="onlyNumber2(this);"/>M<sup style="vertical-align: super;font-size: smaller;">2</sup>
                </td>
                <th><span class="syj-color2">*</span> ±0.000以下面积</th>
                <td>
                  <input type="text" class="syj-input1 validate[required],custom[JustNumber]" name="STRUCTDWFLOORAREA_DWGC"  maxlength="16"  placeholder="请输入±0.000以下面积" value="${info.STRUCTDWFLOORAREA_DWGC}"   onblur="onlyNumber2(this);setSbgcInfo('');" onkeyup="onlyNumber2(this);"/>M<sup style="vertical-align: super;font-size: smaller;">2</sup>
                </td>
            </tr>
			<tr>
				<th>基坑支护方法</th>
			...
			<tr>
				...
				<td>
					<eve:radiogroup typecode="YesOrNo" width="130px" value="${info.ISSHOCKISOLATIONBUILDING_SBGC}" 
					fieldname="ISSHOCKISOLATIONBUILDING_SBGC" ></eve:radiogroup>
				</td>
				<th><span class="syj-color2">*</span> 是否为装配式建筑	</th>
				<td>
					<eve:radiogroup typecode="YesOrNo" width="130px" value="${info.ISEQUIPPEDARCHITECTURE_SBGC}" 
					fieldname="ISEQUIPPEDARCHITECTURE_SBGC"></eve:radiogroup>
				</td>
			</tr>	
			<tr>
				...
				<td>
					...
					fieldname="ISGREENBUILDING_SBGC"  value="${info.ISGREENBUILDING_SBGC}" ></eve:radiogroup>
				</td>
			...
			<tr>
				...
				<td colspan="3">
					...
					<eve:excheckbox
						...
						name="STRUCTQUALTYPE_SBGC" width="749px;" clazz="checkbox validate[required]" value="${info.STRUCTQUALTYPE_SBGC}" 
			...
			<tr>
				...
				<td colspan="3">
					<eve:excheckbox
						...
						name="BASICQUALTYPE_SBGC" width="749px;" clazz="checkbox validate[required]" value="${info.BASICQUALTYPE_SBGC}"  
			...
			<tr>
				...
				<td colspan="3">
					<eve:excheckbox
						dataInterface="dictionaryService.findDatasForSelect" value="${info.REBELQUAKELEVEL_SBGC}"  
						name="REBELQUAKELEVEL_SBGC" width="749px;" clazz="checkbox validate[required]"
