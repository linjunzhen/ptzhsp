...
<div style="position:relative;">
	<table cellpadding="0" cellspacing="0" class="syj-table2 " id="queryArea">
	<!-- 	<tr>
			 <th>统一社会信用代码：</th>
			<td colspan="2">
		...
		<tr>
			<th>企业名称：</th>
			<td colspan="2">
				<input type="text" class="syj-input1" style="width: 680px;" 
					   name="COMPANY_NAME_QUERY" maxlength="64" placeholder="请输入企业名称全称" />
			</td>
			<td>	
				<span class="btnOneP" onclick="queryCreditCompanyInfo()" style="cursor:pointer;height: 40px;line-height:40px;margin: 0;background: #1658a0;">公司查询</span>
			</td>
		</tr>

	</table>
	<table cellpadding="0" cellspacing="0" class="syj-table2 syj-table3" style="margin-top: 10px;" id="changeOptions">							
		<tr>
...
<script type="text/javascript">
	...
	function queryCreditCompanyInfo(){
		...
		var companyName = $("input[name='COMPANY_NAME_QUERY']").val();
			...
			$.post("enterpriseChangeController/queryCreditEnterprise.do",{
				...
				if(obj.success){
					...
					if(data.baseInfo){
						...
						$("#content_01").html(data.baseInfo.ENTNAME);
						$("#content_02").html(data.baseInfo.DOM);
						$("#content_03").html(data.fddbr.NAME);
						$("#content_04").html(data.baseInfo.ENTTYPE_CN);	
						if(data.baseInfo.REGCAP==null || data.baseInfo.REGCAP ==undefined || data.baseInfo.REGCAP==""){
							$("#content_05").html("");
						}else{
							$("#content_05").html(data.baseInfo.REGCAP+" 万元");	
						}				
						if(data.baseInfo.OPTO!=undefined){
						    if(data.baseInfo.OPFROM!=undefined){
						    	$("#content_06").html((data.baseInfo.OPFROM).substring(0,10)+" 至  "+(data.baseInfo.OPTO).substring(0,10));
						    }
						}else{
							 if(data.baseInfo.OPFROM!=undefined){
							 	$("#content_06").html((data.baseInfo.OPFROM).substring(0,10)+" 至  长期 ");
							 }
						}
						$("#content_08").html(data.baseInfo.OPSCOPE.replace(/\s/g,""));
						...
						for(var i=0;i,i<investors.length;i++){
						    ...
						    html_07+=investors[i].INV+"<br>";
							
							html_09+=investors[i].INV+"<br>";
						}
						...
						for(var i=0;i<members.length;i++){
							if(members[i].POSITION_CN.indexOf("董事")>=0){
								director += members[i].POSITION_CN + "：" + members[i].NAME+"<br>";
							}else if(members[i].POSITION_CN.indexOf("监事")>=0){
								supervisor += members[i].POSITION_CN + "：" + members[i].NAME+"<br>";
								jsnum++;
							}
							
							if(members[i].POSITION_CN.indexOf("经理")>=0){
								manager += members[i].POSITION_CN + "：" + members[i].NAME+"<br>";
							}
						}
		...
		$.post("enterpriseChangeController/findChangeOption.do",{
			...
			if('${busRecord.CHANGE_ID}'){
				...
				for(var i=0;i<holers.length;i++){
					...
					dsxxOptionHtml += "<option value='"+i+"SHAREHOLDER'>"+"原股东-"+holers[i].SHAREHOLDER_NAME+"</option>";
				...
				<c:forEach items="${directorList}" var="director">
					...
					dsxxOptionHtml += "<option value='${director.DIRECTOR_NAME_OLD}'>原董事-${director.DIRECTOR_NAME_OLD}</option>";
					$("select[name$='setDsInfo']").html(dsxxOptionHtml);
				</c:forEach>
				...
				<c:forEach items="${holderChangeList}" var="holder">
					...
					dsxxOptionHtml += "<option value='new"+i+"SHAREHOLDER'>"+"新股东-${holder.SHAREHOLDER_NAME}</option>";
			    	i++;
			    </c:forEach>
