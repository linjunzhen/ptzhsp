	...
	$(function(){
		...
		if(EFLOW_FLOWOBJ){
    		...
    		if(eflowObj.busRecord){
    			FlowUtil.initFormFieldValue(eflowObj.busRecord,"T_BSFW_CYJB_FORM");
					...
					<table cellpadding="0" cellspacing="0" class="bstable1" style="table-layout: fixed;margin-top:-1px;">
						...
						<tr>
							...
							<td colspan="3"><input type="text" class="tab_text validate[required]"
								style="width: 660px" value="${sessionScope.curLoginMember.YHMC}-${serviceItem.ITEM_NAME}"
								name="SBMC" maxlength="120"/>

								<span class="bscolor1"><strong style="font-weight: 400;">友情提醒：请规范填写“申报对象信息”</strong></span></td>
						</tr>
						<tr>
							<th> 申报状态</th>
							<td colspan="3" style="line-height: 0;">
								　<span style="margin-right: 15px;">
									<input disabled="disabled" type="checkbox" <c:if test="${EFLOW_FLOWEXE.RUN_STATUS==null||EFLOW_FLOWEXE.CUR_STEPNAMES=='开始'}">checked="checked"</c:if> /> 待提交</span>
								　<span style="margin-right: 15px;">
									<input disabled="disabled" type="checkbox" <c:if test="${EFLOW_FLOWEXE.RUN_STATUS=='1'&&EFLOW_FLOWEXE.CUR_STEPNAMES!='开始'}">checked="checked"</c:if> /> 审核阶段</span>
								　<span style="margin-right: 15px;">
									<input disabled="disabled" type="checkbox" <c:if test="${EFLOW_FLOWEXE.RUN_STATUS!=null&&EFLOW_FLOWEXE.RUN_STATUS!='1'}">checked="checked"</c:if> /> 审核完成</span>
							</td>
						</tr>
					</table>
				</div>
...
<%--			<c:if test="${'1'=='1'}"></c:if>--%>
				...
				<td colspan="3">					
	                ...
	                " value="" />              					
						...
						<tr id="mataddr" style="display: none;">
							...
							<td colspan="3">
								<input type="text" maxlength="126" class="tab_text" style="width: 838px;" name="MAT_SEND_ADDR" id="mat_send_addr"
						...
						<tr id="mataddrremarks" style="display: none;">
							...
							<td colspan="2">
								<input type="text" maxlength="126" class="tab_text" style="width: 838px;" name="MAT_SEND_REMARKS" id="mat_send_remarks"
				...
				<div class="bsboxC">
					<table cellpadding="0" cellspacing="0" class="bstable1">
						<tr>
							...
							<td colspan="2">
								　　<input type="radio" name="FINISH_GETTYPE" value="02" disabled="disabled" <c:if test="${busRecord.FINISH_GETTYPE=='02' }">checked="checked"</c:if> />快递送达
							</td>
						</tr>
				...
				<td colspan="3">					
	                ...
	                " value="" />              					
						...
						<tr id="addr" style="display: none;">
							<th><span class="bscolor1">*</span> 详细地址：</th>
							<td colspan="3">
								<input type="text" maxlength="126" class="tab_text" style="width: 838px;" name="RESULT_SEND_ADDR" id="result_send_addr"
						...
						<tr id="addrremarks" style="display: none;">
							<th><span class="bscolor1">*</span> 邮递备注：</th>
							<td colspan="2">
								<input type="text" maxlength="126" class="tab_text" style="width: 838px;" name="RESULT_SEND_REMARKS" id="result_send_remarks"
			...
			<div class="bsbox clearfix">
				<div class="bsboxT">
					<ul>
						<li class="on" style="background:none"><span>所需材料</span><font color="red">（上传图片时建议上传200dpi的图片。）</font></li>
					</ul>
				</div>
		...
		<jsp:include page="./submitdiv.jsp" >
		     <jsp:param value="submitForOK('T_BSFW_CYJB_FORM');" name="submitFn"/>
		     <jsp:param value="AppUtil.tempSaveWebSiteFlowForm('T_BSFW_CYJB_FORM');" name="tempSaveFn"/>
		</jsp:include>
		<%--结束引入提交DIV界面 --%>
