$(function() {
	...
	if(companySetNature=='02'){//设董事会、监事设立 
		$("#addGdxx").show();
		$("#addDsxx").show();
	}else if(companySetNature=='07'){//设执行董事设立
        $("#addGdxx").show();
        $("#addDsxx").show();
    }else if(companySetNature=='03'){//设执行董事设立
		$("#addGdxx").show();
	}else if(companySetNature=='04'){//设董事会监事会设立
		$("#addGdxx").show();
		$("#addDsxx").show();
		$("#addJsxx").show();
	...
	$("input[name='COMPANY_NAME']").bind('input propertychange', function() {
		...
		var istax = $("input[name='ISTAX']").val();
		if(istax=='1'){
			$("input[name$='ACCOUNT_NAME']").val($(this).val());
		}
	}); 
...
function initJobInfo(){
	var COMPANY_TYPECODE= $("input[name='COMPANY_TYPECODE']").val();
	var COMPANY_SETTYPE= $("input[name='COMPANY_SETTYPE']").val();
	var COMPANY_SETNATURE= $("input[name='COMPANY_SETNATURE']").val();//公司设立性质编码
	if(COMPANY_TYPECODE=='zrrdz' && COMPANY_SETTYPE=='yxzrgssl' && COMPANY_SETNATURE=='01'){//有限责任公司（自然人独资）  一人有限公司
	...
	} else if(COMPANY_TYPECODE=='zrrdz' && COMPANY_SETTYPE=='yxzrgssl' && COMPANY_SETNATURE=='07'){//有限责任公司（自然人独资）  一人有限公司设董事会、监事设立
		...
		$("[name$='MANAGER_GENERATION_MODE']").val("03");
		$("[name$='MANAGER_GENERATION_MODE']").attr("disabled",true);		
	} else if(COMPANY_TYPECODE=='frdz' && COMPANY_SETTYPE=='yxzrgssl' && COMPANY_SETNATURE=='01'){//有限责任公司（法人独资）  一人有限公司
		$("#gdxxDiv").children("div").each(function(i){
		...
		$("[name$='MANAGER_GENERATION_MODE']").val("03");
		$("[name$='MANAGER_GENERATION_MODE']").attr("disabled",true);		
	} else if(COMPANY_TYPECODE=='frdz' && COMPANY_SETTYPE=='yxzrgssl' && COMPANY_SETNATURE=='07'){//有限责任公司（法人独资）  一人有限公司设董事会、监事设立
		$("#gdxxDiv").children("div").each(function(i){
			$(this).find("[name$='SHAREHOLDER_TYPE']").find("option[value='zrr']").remove();
		});		
		$("[name$='DIRECTOR_GENERATION_MODE']").attr("disabled",true);		
		...
		$("[name$='MANAGER_GENERATION_MODE']").val("03");
		$("[name$='MANAGER_GENERATION_MODE']").attr("disabled",true);		
	} else if(COMPANY_TYPECODE=='zrrkg' && COMPANY_SETTYPE=='yxzrgssl'){//有限责任公司（自然人投资或控股）
		$("#gdxxDiv").children("div").each(function(i){
			$(this).find("[name$='SHAREHOLDER_TYPE']").val("zrr");
			$(this).find("[name$='SHAREHOLDER_TYPE']").attr("disabled",true);
			var LEGAL_PERSON = $(this).find("[name$='LEGAL_PERSON']").attr("name");
		...
		//}
	} else if(COMPANY_TYPECODE=='qtyxzrgs' && COMPANY_SETTYPE=='yxzrgssl'){//其他有限责任公司
		$("[name$='DIRECTOR_GENERATION_MODE']").val("02");
		$("[name$='DIRECTOR_GENERATION_MODE']").attr("disabled",true);		
		$("[name$='SUPERVISOR_GENERATION_MODE']").val("02");
		$("[name$='SUPERVISOR_GENERATION_MODE']").attr("disabled",true);		
...
function setBankInfo(Value){
	...
	$("#businessScope").val($("[name='INVEST_INDUSTRY']").val());
...
function FLOW_SubmitFun() {
	...
	var companySetNature= $("input[name='COMPANY_SETNATURE']").val();//公司设立性质编码
	var COMPANY_TYPECODE= $("input[name='COMPANY_TYPECODE']").val();//公司类型
	if(COMPANY_TYPECODE=='zrrkg'&& companySetNature=='04'){//有限责任公司（自然人投资或控股）
		if($("#gdxxDiv").children("div").length<2){//股东不能少于2个			
			art.dialog({
				content : "股东不能少于2个！",
				icon : "warning",
				ok : true
	...
	} else if(COMPANY_TYPECODE=='qtyxzrgs' && companySetNature!='03'){//其他有限责任公司
		if($("#gdxxDiv").children("div").length<2){//股东不能少于2个			
			art.dialog({
				content : "股东不能少于2个！",
				icon : "warning",
		...
		var gdxxFlag = false;
		$("#gdxxDiv").children("div").each(function(i){
			var SHAREHOLDER_TYPE = $(this).find("[name$='SHAREHOLDER_TYPE']").val();
...
// var SSSBLX = $("input[name='SSSBLX']").val();
// var companyTypeCode= $("input[name='COMPANY_TYPECODE']").val();//公司类型编码
// if(!(SSSBLX=='1'&&companyTypeCode=='qtyxzrgs'&&val=='qtjg')){
// }
...
function addGdxxDiv(){
	...
	var sssblx=$("input[name='SSSBLX']").val();
	$.post("domesticControllerController/refreshGdxxDiv.do?companyTypeCode="+companyTypeCode+"&sssblx="+sssblx,{
	}, function(responseText, status, xhr) {
        ...
        $("select[name='setHolderInfo']").append("<option value='"+id+"' name='selectholder"+id+"'>"+"股东信息"+holder+"</option>");
        holder=holder+1;
        }
        if(sssblx=='1'){
			$("[name$='INVESTMENT_CASH']").addClass(" custom[numMustBiggerZero]");
		}
...
function addGdxxDivMp(deleKey){
	var companyTypeCode=$("input[name='COMPANY_TYPECODE']").val();
	var sssblx=$("input[name='SSSBLX']").val();
	$.post("domesticControllerController/refreshGdxxDivMp.do?companyTypeCode="+companyTypeCode+"&sssblx="+sssblx+"&deleKey="+deleKey,{
	}, function(responseText, status, xhr) {
		$("#gdxxDiv").append(responseText);
		var CURRENCY = $("[name='CURRENCY']").val();
		var reg= /="[a-z0-9]*SHAREHOLDER_NAME/gi;
...
function addInitGdxxDiv(holder,i){
	$.post("domesticControllerController/refreshGdxxDiv.do",{
	}, function(responseText, status, xhr) {
		$("#gdxxDiv").append(responseText);	
...
function addDsxxDiv(){
	...
	$.post("domesticControllerController/refreshDsxxDiv.do",{
		ssdjzw:ssdjzw
	}, function(responseText, status, xhr) {
 ...
 * 添加董事信息
 */
function addDsxxDivMp(deleKey){
	var companySetNature= $("input[name='COMPANY_SETNATURE']").val();//公司类型编码
	var ssdjzw = '04'
	if(companySetNature=='02'||companySetNature=='04'||companySetNature=='07'){
		ssdjzw = '01';
	}
	$.post("domesticControllerController/refreshDsxxDivMp.do?deleKey="+deleKey,{
		ssdjzw:ssdjzw
	}, function(responseText, status, xhr) {
		$("#dsxxDiv").append(responseText);
		$("#dsxxDiv div").eq($("#dsxxDiv div").length-1).find("[name$='DIRECTOR_APPOINTOR']").val($("[name='DIRECTOR_APPOINTOR']").val());
...
function addJsxxDiv(deleKey){
	$.post("domesticControllerController/refreshJsxxDiv.do?deleKey="+deleKey,{
	}, function(responseText, status, xhr) {
		$("#jsxxDiv").append(responseText);
		$("[name$='SUPERVISOR_APPOINTOR']").val($("[name='SUPERVISOR_APPOINTOR']").val());
		$("[name$='SUPERVISOR_GENERATION_MODE']").val($("[name='SUPERVISOR_GENERATION_MODE']").val());
		$("[name$='SUPERVISOR_GENERATION_MODE']").attr("disabled",true);
		setDefaultPersonInfo();
	});
}
 ...
 */
function addJlxxDiv(){
	$.post("domesticControllerController/refreshJlxxDiv.do",{
	}, function(responseText, status, xhr) {
...
function setLegalValue(val){
	...
	$("#jsxxDiv div").each(function(i){
		...
		if(SUPERVISOR_JOB==val){			
            ...
            $("input[name='LEGAL_MOBILE']").val(SUPERVISOR_PHONE);
            $("[name='LEGAL_PRODUCEMODE']").val(SUPERVISOR_GENERATION_MODE);
			isok = true;
			return;
		}
	});
