...
<div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
  <div class="layui-tab-content">
	<e:for filterid="${info.TYPE_ID}" end="100" var="efor" dsid="1004">
				...
				<script type="text/javascript">
					$(function(){
						$.post("flowConfigDistributeController/applyItemMaterList.do",{
							...
							isWebsite:'2'
							//materType:'generalityMater'
						}, function(responseText, status, xhr) {
							if($("#${efor1.S_ITEM_CODE}_MATER").length==0){
								$("#${efor.STAGE_ID}generalityMaterDiv").prepend(responseText);
								$("#${efor.STAGE_ID}generalityMaterDiv").find('select').prop("disabled", "disabled");
							}
						});
					});
				</script>
		...
		<div id="${efor.STAGE_ID}generalityMaterDiv">
		</div>
	</div>
	</e:for>
  </div>
