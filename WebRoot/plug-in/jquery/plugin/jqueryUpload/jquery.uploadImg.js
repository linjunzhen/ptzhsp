...
(function($) {
	...
	var methods = {
		"init": function(options) {
			var settings = $.extend({
				"url": "/file-upload?path=myfile&busTableRecordId=&userId=-1",
				"swf": "swfupload/swfupload.swf",
				"isBrowse": false, // 控件展现成上传形式还是查看形式，true 上传形式 false 查看形式
				"fileSize": "20MB", // 限制文件大小，单位KB,MB,GB
				"fileTypes": "*.*", // 默认所有文件
			...
			return this.each(function(idx, o) {
				...
				function _drawComponent() {
					$bindedDom = $("<span></span>").append("<input type='button' value='上传外部图片' />");
					$this.before($bindedDom);
					$this.css("display", "none");
				}
				
				function _bindSwfupload(thiz) {
					var swfupload = new SWFUpload({
						"upload_url" : settings.url + "&busTableName=" + settings.tableName,
						// 上传文件设置
		...
		"uploadSuccessHandler" : function(file, serverData, response) {
			try {
				...
				if(_resp && !_resp.success){
					alert(_resp.message);
					//获取当前有行
					var row = $img.datagrid("getSelected");
					var rowIndex = $img.datagrid('getRowIndex', row);
					//删除错误的列
					$img.datagrid('deleteRow', rowIndex);
					//删除后重新加载下
					$img.datagrid('reload');
				}
				/**********************************************/
				if (file.type.toLowerCase() == ".jpeg" || file.type.toLowerCase() == ".jpg" || file.type.toLowerCase() == ".png" || file.type.toLowerCase() == ".gif" || file.type.toLowerCase() == ".bmp") {
...
//						fadeIn(newImg, 0);
//					};
					var fn = $(this).data("upload").onUploaded;
//					newImg.src = resp.fullFilePath;
